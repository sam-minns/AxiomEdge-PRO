# End_To_End_Advanced_ML_Trading_Framework_PRO_V130_Report_Nicknames
#
# V130 UPDATE (Reporting Nicknames):
# 1. NEW - STRATEGY NICKNAMES: The framework now assigns a unique, AI-generated
#    codename (e.g., 'Apollo', 'Vortex') to each script version for cleaner
#    and more readable performance reports.
# 2. NEW - NICKNAME LEDGER: A new 'nickname_ledger.json' file is used to
#    persistently store the mapping between script versions and their nicknames.
# 3. NEW - REPORT TRACEABILITY: Reports now include a mapping section to show
#    which nickname corresponds to which full script version.
#
# V129 UPDATE (Critical Method & Logic Fixes):
# 1. FIX - ATTRIBUTEERROR: Restored the 'analyze_cycle_and_suggest_changes' method.
# 2. FIX - CHAMPION REPORTING LOGIC: Corrected the champion update sequence.
...
class ConfigModel(BaseModel):
    ...
    def __init__(self, **data: Any):
        ...
        # Use nickname for folder name if available, otherwise use the report label
        folder_name = self.nickname.replace(" ", "_") or self.REPORT_LABEL
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict, nickname_ledger: Dict):
    ...
    current_config['nickname'] = nickname_ledger.get(current_config['REPORT_LABEL'], current_config['REPORT_LABEL'])
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V130_Report_Nicknames",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V130_Report_Nicknames.py