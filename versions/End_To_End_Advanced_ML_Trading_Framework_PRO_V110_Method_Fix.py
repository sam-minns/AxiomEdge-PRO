# End_To_End_Advanced_ML_Trading_Framework_PRO_V110_Method_Fix
#
# V110 UPDATE (Critical Bug Fix):
# 1. CRITICAL FIX: Corrected an `AttributeError` that occurred at startup. The
#    `get_pre_flight_config` method was being called but was never defined within
#    the `GeminiAnalyzer` class. The method has now been implemented, restoring
#    the "Pre-Flight Check" and "Champion" functionalities.
#
# V109 UPDATE (Dynamic Strategy Dispatch):
# 1. Re-architected the framework to use a "Strategy Playbook" and a dynamic,
#    per-cycle strategy selection mechanism driven by a "Market Forecaster" AI.
...
class GeminiAnalyzer:
    ...
    # NEW (V110): Implemented the missing Pre-Flight Check method
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, fallback_config: Dict) -> Dict:
        ...
        prompt = (
            ...
            "1.  **Review the Champion**: Note which strategy the current champion used.\n"
            "2.  **Review History**: See if other strategies have ever outperformed or shown more stability.\n"
            "3.  **Select a Strategy**: Choose the `strategy_name` from the playbook that you believe has the highest potential for this new run.\n\n"
        ...
        if suggestions and "strategy_name" in suggestions:
            ...
            logger.info(f"  - Pre-Flight Check complete. AI chose strategy '{suggestions['strategy_name']}' with params: {suggestions}")
            return final_config
        else:
            logger.warning("  - Pre-Flight Check failed to select a strategy. Using default.")
            return fallback_config

    def choose_strategy_for_cycle(self, train_data_summary: Dict, playbook: Dict, fallback_strategy: str) -> str:
        if not self.api_key_valid:
            logger.warning("No API key. Skipping strategy selection, using fallback.")
            return fallback_strategy
        
        logger.info("  - AI Foreman: Analyzing market conditions to select best strategy for this cycle...")
        prompt = (
            "You are a Market Regime Classifier. Your task is to analyze the summary statistics of the upcoming training period and select the most appropriate strategy from the provided `strategy_playbook`. "
            "Your goal is to match the strategy to the market conditions.\n\n"
            "- If `avg_adx` is high (>25) and `volatility_rank` is high, the market is likely **trending**. `TrendFollower` is a good choice.\n"
            "- If `avg_adx` is low (<20) and `rsi_std` is high, the market is likely **ranging/reverting**. `MeanReversion` is a good choice.\n"
            "- If `volatility_rank` is very high, consider `VolatilityBreakout`.\n\n"
            "Respond ONLY with a valid JSON object with a single key, `strategy_name`, containing the name of your chosen strategy from the playbook.\n\n"
            f"STRATEGY PLAYBOOK (Your options):\n{json.dumps(playbook, indent=2)}\n\n"
            f"MARKET CONDITIONS FOR UPCOMING CYCLE:\n{json.dumps(train_data_summary, indent=2)}"
        )
        response_text = self._call_gemini(prompt)
        suggestions = self._extract_json_from_response(response_text)
        
        strategy = suggestions.get("strategy_name")
        if strategy in playbook:
            logger.info(f"  - AI Foreman selected strategy: {strategy}")
            return strategy
        else:
            logger.warning(f"  - AI failed to select a valid strategy. Using fallback: {fallback_strategy}")
            return fallback_strategy
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        ...
        prompt = (
            "You are an expert trading model analyst. Your primary goal is to tune the parameters of a **pre-selected master strategy** to adapt to changing market conditions within a walk-forward run. The master strategy has already been chosen; your job is to refine it.\n\n"
...
# ... (The rest of the script is identical to V109) ...
...
class FeatureEngineer:
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        all_features = ModelTrainer.TREND_FEATURES + ModelTrainer.REVERSAL_FEATURES + ModelTrainer.VOLATILITY_FEATURES
...
class ModelTrainer:
    ...
    BASE_FEATURES = list(set(TREND_FEATURES + REVERSAL_FEATURES + VOLATILITY_FEATURES + SESSION_FEATURES))
...
def main():
    # FIX (V110): Initialize start_time here
    start_time = datetime.now()
    run_timestamp_str = start_time.strftime("%Y%m%d-%H%M%S")
    
    strategy_playbook = {
        "TrendFollower": {
            "description": "Aims to catch long trends, using HTF context and trend strength.",
            "features": list(set(ModelTrainer.TREND_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (150, 250), "dd_range": (0.25, 0.40)
        },
        "MeanReversion": {
            "description": "Aims for short-term reversals, using oscillators.",
            "features": list(set(ModelTrainer.REVERSAL_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (40, 80), "dd_range": (0.15, 0.25)
        },
        "VolatilityBreakout": {
            "description": "Trades breakouts during high volatility sessions.",
            "features": list(set(ModelTrainer.VOLATILITY_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (60, 120), "dd_range": (0.20, 0.35)
        }
    }
    
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V110_Method_Fix",
        "run_timestamp": run_timestamp_str, "strategy_name": "TrendFollower",
        ...
        "selected_features": strategy_playbook["TrendFollower"]["features"]
    }
    ...
    ai_config_suggestion = gemini_analyzer.get_pre_flight_config(framework_history, strategy_playbook, fallback_config)
    ...
    # The pre-flight check has already selected the master strategy for the whole run
    logger.info(f"-> MASTER STRATEGY FOR THIS RUN: {initial_config.strategy_name}")
    ...
    for i,period_start_date in enumerate(retraining_dates):
        # The master strategy is fixed for the run, but we can still tune its params
        ...
        current_strategy_features = strategy_playbook.get(config.strategy_name, {}).get("features", [])
        suggested_params = gemini_analyzer.analyze_cycle_and_suggest_changes(in_run_historical_cycles, current_strategy_features)
    ...
    run_summary = {
        ...
        "strategy_name": initial_config.strategy_name,
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V110_Method_Fix.py