# End_To_End_Advanced_ML_Trading_Framework_PRO_V54_KeyError_FIX
#
# V54 Update:
# 1. Fixed a KeyError for the 'market_regime' column.
# 2. Updated the label generation logic to use the new 'adx_trending' state-based
#    feature instead of the old, removed 'market_regime' feature.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V54_KeyError_FIX",
...
class FeatureEngineer:
    ...
    def _label_group(self, group: pd.DataFrame, sl_atr: float, tp_atr: float, lookahead: int) -> pd.DataFrame:
        ...
        # FIX: Use 'adx_trending' instead of the removed 'market_regime' column
        outcomes, prices, lows, highs, atr, regime = np.zeros(len(group)), group['Close'].values, group['Low'].values, group['High'].values, group['ATR'].values, group['adx_trending'].values
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V53_State_Features.py