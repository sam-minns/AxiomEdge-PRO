# End_To_End_Advanced_ML_Trading_Framework_PRO_V189_ForcedSimplicity.py
#
...
#     based on user analysis. The AI is now explicitly forbidden from making minor
#     tweaks after a `TRAINING_FAILURE`.
# 2.  ADDED: The AI is now forced to choose one of two distinct recovery paths:
#     a) Propose a completely different and drastically smaller feature set (4-6 features).
#     b) Drastically simplify the model's complexity (reducing `max_depth`, `n_estimators`).
# 3.  ENHANCED: The initial setup prompt and standard cycle review prompts now
#     have a stronger bias towards simplicity, encouraging the selection of smaller,
#     more targeted feature sets from the start. This is designed to break the
#     "more is better" feature trap.
# 4.  ENHANCED: Prompts for strategies on "probation" now more aggressively
#     prioritize risk reduction and model stability over performance optimization.
# 5.  ADDED: AI now suggests the number of `OPTUNA_TRIALS` for each run, with sanitization guardrails.
#
# V188 UPDATE (Micro-Dynamic Feature Suite):
# 1.  ADDED: Indicator Dynamics, Relative Performance, Candle Microstructure Analysis.
# 2.  ENHANCED: The initial AI prompt was updated to mention these new feature categories.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, cycle_status: str) -> Dict:
        ...
        base_prompt_intro = "You are an expert trading model analyst. Your primary goal is to create a STABLE and PROFITABLE strategy by making intelligent, decisive changes."
        ...
        if cycle_status == "TRAINING_FAILURE":
            # V189 - New, more forceful prompt for training failures.
            prompt_details = (
                ...
                "**YOUR TASK: Choose ONE of the following two paths. Do NOT combine them.**\n\n"
                "**PATH A: DRASTICALLY CHANGE THE FEATURE SET**\n"
                "   - Propose a **completely different and much smaller feature set.**\n"
                "   - The current features are likely the problem. Select a new, targeted list of just **4-6 features** from the `AVAILABLE FEATURES` list that you believe are most relevant to the strategy's goal.\n"
                f"   - {context_feature_mandate}\n"
                "   - Do NOT suggest changes to any other parameters if you choose this path.\n\n"
                "**OR**\n\n"
                ...
                "   - Keep the existing `selected_features` the same.\n"
                "   - Suggest much **simpler hyperparameter search spaces** by proposing new values for `OPTUNA_TRIALS` (e.g., reduce by 50%), and/or drastically reducing `max_depth` and `n_estimators` in the next search (not a direct param, but understood for your hyperparameter selection logic).\n"
                "   - You can also suggest a larger `LOOKAHEAD_CANDLES` value if you think the labeling is the issue.\n\n"
                "**Explain your choice** briefly in an `analysis_notes` key."
            )
        elif cycle_status == "PROBATION":
            # V189 - Enhanced probation prompt.
            prompt_details = (
            ...
            # V189 - Enhanced standard prompt for simplicity.
            prompt_details = (
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        # V189 - Updated feature selection guidance for intervention
        feature_selection_guidance = (
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        # V189 - Allow AI to suggest simpler search space implicitly
...
def _sanitize_ai_suggestions(suggestions: Dict[str, Any]) -> Dict[str, Any]:
    ...
    bounds = {
        ...
        'OPTUNA_TRIALS': (25, 200), # Clamp AI suggestions between 25 and 200 trials
        # ---
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        # --- FIXED CODE ---
        # This block validates the frequency string before calling to_timedelta
        is_short_cycle = False
        try:
            # Check for common invalid patterns before calling to_timedelta
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V189_ForcedSimplicity",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V189_FIXED.py