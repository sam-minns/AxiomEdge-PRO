# End_To_End_Advanced_ML_Trading_Framework_PRO_V207_Linux.py
#
# V207 UPDATE (Robustness & Realism Refactoring):
#   1. FIXED (True Cross-Validation): The Optuna hyperparameter optimization now uses 5-fold StratifiedKFold
#      cross-validation instead of a single train-validation split. This provides a much more robust
#      evaluation of model parameters, reducing the risk of overfitting to a lucky validation set.
#   2. ADDED (Explicit Complexity Penalty): The Optuna objective function now includes a direct, tunable
#      penalty for model complexity (based on `n_estimators` and `max_depth`). This explicitly punishes
#      overly complex models, forcing the AI to favor simpler, more generalizable solutions.
#   3. ADDED (Commission Modeling): A `COMMISSION_PER_LOT` parameter has been added to the configuration
#      and is now factored into the PNL calculation for every trade, better simulating ECN broker costs.
#   4. ADDED (Exit Slippage Simulation): The backtester now simulates random, volatility-based slippage
#      on Stop Loss exits, creating more conservative and realistic results by modeling potentially
#      worse-than-expected fills on losing trades.
#   5. CORRECTED (Changelog Accuracy): Fixed major discrepancies where V206 claimed K-Fold validation and
#      a complexity penalty were implemented when they were not. The framework now correctly implements these features.
#
...
#      trade execution (candle N), realistically modeling the delay in live environments.
#   2. ADDED (Dynamic Spreads): The cost model now uses a symbol-specific dictionary of typical
#      spreads. Spreads automatically widen during high-volatility periods.
#   3. ADDED (Variable Slippage): Slippage is now a randomized, volatility-scaled factor, modeling
#      the unpredictable nature of execution fills during different market conditions.
...
#   5. IMPROVED (Optimization Reliability): The hyperparameter optimization process now uses 3-fold
#      cross-validation (KFold) for more reliable and generalized model parameter selection.
#   6. FIXED (Robust Imports): The script now gracefully handles the absence of the `optuna-integration`
#      package, preventing crashes.
#
# --- SCRIPT VERSION ---
VERSION = "207"
# ---------------------
...
class ConfigModel(BaseModel):
    ...
    # V207 UPDATE: Added commission to model ECN broker costs.
    COMMISSION_PER_LOT: confloat(ge=0.0) = 3.5 # Per side commission (e.g., $3.5 per 100k lot)
...
def initialize_playbook(playbook_path: str) -> Dict:
    DEFAULT_PLAYBOOK = {
        ...
        "ElliotWaveMomentum": {
            ...
            "lookahead_range": [80, 200],
            "dd_range": [0.20, 0.35],
            "ideal_regime": ["Strong Trending"],
            "ideal_macro_env": ["Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Indices", "Forex Majors"]
        },
        "ElliotWaveFibonacci": {
        ...
        "FvgLiquidityGrab": {
            ...
            "ideal_regime": ["Any"],
            "ideal_macro_env": ["Any"],
            "asset_class_suitability": ["Forex Majors", "Indices", "Crypto"]
        },
        "EngulfingVolumeConfirmation": {
            ...
            "lookahead_range": [20, 70],
            "dd_range": [0.10, 0.25],
            "ideal_regime": ["Any"],
            "ideal_macro_env": ["Any"],
            "asset_class_suitability": ["Equities", "Forex Majors", "Indices"]
        },
        ...
        "SeasonalCycleRSI": {
            ...
            "lookahead_range": [28, 84],
            "dd_range": [0.20, 0.40],
            "ideal_regime": ["Any"],
            "ideal_macro_env": ["Any"],
            "asset_class_suitability": ["Indices", "Commodities"]
        },
        "SeasonalCycleVolatility": {
            ...
            "dd_range": [0.25, 0.45],
            "ideal_regime": ["High Volatility", "Ranging"],
            "ideal_macro_env": ["Event-Driven"],
            "asset_class_suitability": ["Any"]
        },
        ...
        "DivergenceConfluenceReversal": {
            ...
            "ideal_regime": ["Any"],
            "ideal_macro_env": ["Neutral", "Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Forex Majors"]
        },
        "DynamicSR_BreakoutRSI": {
            ...
            "lookahead_range": [50, 140],
            "dd_range": [0.20, 0.35],
            "ideal_regime": ["Strong Trending", "High Volatility"],
            "ideal_macro_env": ["Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Equities", "Indices", "Commodities"]
        },
        ...
        "RSI_Stochastic_Confluence": {
            ...
            "ideal_regime": ["Ranging", "Low Volatility"],
            "ideal_macro_env": ["Neutral"],
            "asset_class_suitability": ["Forex Crosses", "Indices"]
        },
        "OscillatorMomentum": {
        ...
        "MultiStructureBreakout": {
            ...
            "features": ["bos_up_signal", "bos_down_signal", "volume", "ATR", "DAILY_ctx_Trend"],
            "lookahead_range": [60, 150],
            "dd_range": [0.20, 0.35],
            "ideal_regime": ["Strong Trending"],
            "ideal_macro_env": ["Risk-On", "Risk-Off"],
        ...
        "CHoCH_Orderblock_Entry": {
            "description": "[HYBRID/CHOCH] This strategy identifies a Change of Character (CHoCH) and then waits for price to pull back to the Fair Value Gap that initiated the CHoCH move. This provides a high-probability, low-risk entry for the new, emerging trend.",
            "features": ["choch_up_signal", "choch_down_signal", "fvg_bullish_exists", "fvg_bearish_exists", "volume", "DAILY_ctx_Trend"],
            "lookahead_range": [50, 130],
            "dd_range": [0.15, 0.25],
            "ideal_regime": ["Any"],
            "ideal_macro_env": ["Any"],
...
def main():
    ...
    fallback_config={
        ...
        "COMMISSION_PER_LOT": 3.5, # V207: Added default commission
        "CONFIDENCE_TIERS": {
