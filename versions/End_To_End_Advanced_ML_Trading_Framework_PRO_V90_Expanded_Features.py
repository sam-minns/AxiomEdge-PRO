# End_To_End_Advanced_ML_Trading_Framework_PRO_V90_Expanded_Features
#
# V90 Update:
...
# 3. This update directly incorporates the user-requested list of features,
#    ensuring the framework can test a wide array of market dynamics.
...
class ModelTrainer:
    ...
    def _generate_shap_summary(self,model:xgb.XGBClassifier,X_scaled:np.ndarray,feature_names:pd.Index):
        ...
        try:
            ...
            # For multiclass, shap_explanation.values is a list of arrays (one for each class)
            if isinstance(shap_explanation.values, list):
                # Take the absolute mean across all classes for overall importance
                shap_values = np.abs(np.array(shap_explanation.values)).mean(0)
            else:
                shap_values = shap_explanation.values

            per_class_importance=np.abs(shap_values).mean(0)
            overall_importance=per_class_importance
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(),
        "REPORT_LABEL": "ML_Framework_V90_Expanded_Features",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V90_Expanded_Features.py