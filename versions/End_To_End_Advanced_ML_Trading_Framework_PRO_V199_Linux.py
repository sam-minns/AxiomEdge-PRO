# End_To_End_Advanced_ML_Trading_Framework_PRO_V199_Linux.py
#
# V199 UPDATE (Comprehensive AI Context & Analysis):
#   1. ADDED (Trade Excursion Analysis - MAE/MFE): The Backtester now calculates the Maximum Adverse Excursion (MAE) and Maximum Favorable Excursion (MFE) for every trade. This data is passed to the AI, allowing it to analyze not just if a trade won or lost, but *how*. It can now distinguish between poor entries (high MAE) and poor exits (high MFE on losing trades).
#   2. ADDED (Feature Importance History - SHAP Drift): The framework now tracks the history of SHAP feature importance across all cycles. The AI is prompted to analyze this for "feature drift," identifying when a core feature's predictive power collapses, which is a strong indicator of a regime change.
#   3. ADDED (Hyperparameter History - Optuna Trials): A summary of the best and worst performing hyperparameter sets from all Optuna trials across the entire run is now passed to the AI. This gives the AI long-term memory of which parameter combinations are historically effective or toxic for the current strategy.
#   4. ENHANCED (AI Analysis & Prompts): The core AI analysis prompt (`analyze_cycle_and_suggest_changes`) has been completely overhauled to leverage all the new data points (MAE/MFE, SHAP Drift, Optuna History, and Model Quality Scores), instructing the AI to perform a much more sophisticated, multi-faceted analysis before suggesting changes.
#   5. ENHANCED (Training Failure Logic): The logic for repeated training failures within a cycle is now more robust. After exceeding the max retries, the framework now triggers a strategic intervention, forcing the AI to consider that the strategy itself (not just the parameters) may be unsuitable for the current market data.
#
...
#   2. ENHANCED (AI Analysis Prompt): The core AI analysis prompt (`analyze_cycle_and_suggest_changes`) has been significantly upgraded. It now explicitly instructs the AI to analyze the relationship between the `BestObjectiveScore` and the `PNL`.
#
# V197 UPDATE (Profitability & Robustness Enhancements):
#   1. ENHANCED (Optimization Objective): Overhauled the `custom_objective` function in the hyperparameter optimizer. It now directly optimizes for a risk-adjusted return (a Sharpe Ratio proxy) across both validation and stability datasets.
#   2. ADDED (Champion Guardrail): Implemented a crucial check in the `save_run_to_memory` function. A run can now only become the 'Overall' or 'Regime' champion if it has a non-negative MAR Ratio AND executes a minimum number of trades (default 10).
#   3. REFINED (Default Configuration): Adjusted the default `fallback_config` to be more robust. The `LABEL_MIN_RETURN_PCT` was increased, and `CONFIDENCE_TIERS` were made more conservative.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(
        ...
        # --- V199: Comprehensive Multi-faceted Analysis Prompt ---
        analysis_points = [
            "--- COMPREHENSIVE PERFORMANCE ANALYSIS ---",
            "**1. Model Quality vs. PNL:** Analyze the `BestObjectiveScore` (model quality) against the final `PNL` for each cycle. Is there a correlation? A low score followed by a high PNL is a sign of LUCK, not skill. Be cautious and prioritize stability over chasing lucky wins.",
            "**2. Feature Drift (SHAP History):** Analyze the `shap_importance_history`. Are core features losing predictive power over time? Has a feature's importance collapsed recently? This signals a regime change and may require a new feature set.",
            "**3. Granular Exit Analysis (MAE/MFE):** Look at the `trade_summary` within the cycle results. High MAE (Maximum Adverse Excursion) on losing trades suggests poor entries. High MFE (Maximum Favorable Excursion) on losing trades suggests take-profit levels are too far away. Your suggestions should address this.",
            "**4. Hyperparameter Learning:** Review the `optuna_trial_history`. Are there parameter values (e.g., `max_depth` > 8) that consistently lead to failed models? Learn from this history to avoid repeating mistakes."
        ]
        ...
        if cycle_status == "TRAINING_FAILURE":
            task_guidance = (
             ...
             task_guidance = (
                ...
                "The previous cycle hit its drawdown limit. Your **only goal** is to **REDUCE RISK**. Profitability is secondary. You MUST suggest changes that lower risk, like reducing `MAX_DD_PER_CYCLE`, reducing `MAX_CONCURRENT_TRADES`, or switching to a lower `RISK_PROFILE`. Justify your choice using the analysis points."
             )
        else: # Standard cycle
            task_guidance = (
                "**STANDARD CYCLE REVIEW**\n"
                "Your task is to synthesize all four analysis points into a coherent set of changes. Propose a new configuration that improves robustness and profitability. For example, if SHAP shows a feature is losing importance AND MAE/MFE suggests entries are poor, a feature set change is warranted."
            )

        # Sanitize Optuna trials to keep prompt size manageable
        if len(all_optuna_trials) > 50:
            all_optuna_trials = sorted(all_optuna_trials, key=lambda x: x.get('value', -99), reverse=True)
            optuna_summary = {"best_5_trials": all_optuna_trials[:5], "worst_5_trials": all_optuna_trials[-5:]}
        else:
            optuna_summary = {"all_trials": all_optuna_trials}
        ...
        for i in range(1, len(candles)):
            ...
            if symbol in open_positions:
                ...
                if exit_price:
                    ...
                    # V199: Finalize MAE/MFE calculation on trade close
            ...
            if not circuit_breaker_tripped and symbol not in open_positions and len(open_positions) < max_concurrent_trades:
                ...
                if direction != 0:
                    ...
                    open_positions[symbol]={
                        ...
                        'mfe_price': entry_price, 'mae_price': entry_price # V199: Init MAE/MFE tracking prices
                    }
            day_peak_equity = max(day_peak_equity, equity)
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    while cycle_num < len(retraining_dates):
        ...
        if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
            ...
            # V199: Escalate to strategic intervention instead of just falling forward
            ...
            if intervention_suggestion:
                ...
                cycle_retry_count = 0
                continue # Retry the cycle with the new strategy
            else:
            ...
            continue # Retry cycle with new params
        
        cycle_pnl = equity_curve.iloc[-1] - last_equity if not equity_curve.empty else 0
        
        # V199: Create a trade summary with MAE/MFE for the cycle history
        trade_summary = {}
        if not trades.empty:
        ...
        cycle_result = {"StartDate": period_start_date.date().isoformat(), "EndDate": test_end.date().isoformat(), "NumTrades": len(trades), "PNL": round(cycle_pnl, 2), "Status": "Circuit Breaker" if breaker_tripped else cycle_status_msg, "BestObjectiveScore": round(best_objective_score, 4), "trade_summary": trade_summary}
        ...
        # AI call for the next cycle's parameters
