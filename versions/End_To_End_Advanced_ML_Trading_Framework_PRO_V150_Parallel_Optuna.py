# End_To_End_Advanced_ML_Trading_Framework_PRO_V150_Parallel_Optuna.py
#
...
#
# V149 UPDATE (Counter):
# 1. REMOVED - TQDM Progress Bar: The `tqdm` library has been removed.
# 2. ADDED - Simple Text Counter: Replaced the visual progress bar with a
#    simple text log for each cycle (e.g., "--- Starting Cycle [1/13] ---"),
#    simplifying the console output.
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        try:
            ...
            study.optimize(custom_objective,n_trials=self.config.OPTUNA_TRIALS,timeout=3600, n_jobs=-1) # <--- MODIFIED: Added n_jobs=-1 for parallel execution
            return study
        except Exception as e:
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V150_Parallel_Optuna",
