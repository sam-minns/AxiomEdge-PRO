# End_To_End_Advanced_ML_Trading_Framework_PRO_V177_Smart_Partial_TP.py
#
# V177 UPDATE (Intelligent & Adaptive Risk Management):
# 1. IMPLEMENTED: AI-Managed Partial TP Switch. A new master switch,
#    `USE_PARTIAL_PROFIT` (boolean), is now controlled by the AI. The default
#    is `false`, forcing the AI to make a conscious decision to activate it.
# 2. IMPLEMENTED: The entire partial TP logic in the Backtester is now
#    conditional on `USE_PARTIAL_PROFIT` being true for the cycle.
# 3. ENHANCED: AI prompts were upgraded to guide the AI to enable partial TPs
#    only when it detects a pattern of winning trades reversing before their
#    full target, otherwise keeping it off to maximize profit.
# 4. FIXED: Corrected a typo in the PerformanceAnalyzer that affected the
#    calculation of `payoff_ratio`, ensuring metrics are more accurate.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, daily_dd_report: Dict, mc_results: Dict) -> Dict:
        ...
        # V177: Smart Partial TP guidance
        partial_tp_guidance = (
            ...
            "- If you set it to `true`, you MUST also provide `PARTIAL_PROFIT_TRIGGER_R` and `PARTIAL_PROFIT_TAKE_PCT`."
        )

        base_suggestions = (
            "- `MAX_DD_PER_CYCLE`, `RETRAINING_FREQUENCY` (string with unit), `MAX_CONCURRENT_TRADES`.\n"
            "- `selected_features`: **MUST be a NON-EMPTY list**.\n"
            f"{partial_tp_guidance}"
        )

        # Base prompt for standard operation
        prompt_details = (
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration for this new strategy. Start with conservative values: `RETRAINING_FREQUENCY`: '90D', `MAX_DD_PER_CYCLE`: 0.15, `MAX_CONCURRENT_TRADES`: 2, and **`USE_PARTIAL_PROFIT`: false**. **Ensure `selected_features` is a NON-EMPTY list.**\n\n"
...
class ConfigModel(BaseModel):
    ...
    USE_PARTIAL_PROFIT: bool = False # V177 Master Switch
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, is_meta_model: bool) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        for candle in candles:
            ...
            for symbol in list(open_positions.keys()):
                ...
                # V177: Conditional Partial Profit Logic
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V177_Smart_Partial_TP",
