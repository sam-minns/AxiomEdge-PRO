# End_To_End_Advanced_ML_Trading_Framework_PRO_V106_Report_Compare
#
# V106 UPDATE (Enhanced Reporting & File Management):
# 1. NEW - COMPARATIVE REPORTING: The final report now includes a new "PERFORMANCE vs. HISTORY"
#    section at the top. This provides a concise, at-a-glance comparison of the
#    Current Run vs. the Previous Run vs. the All-Time Champion, focusing on key
#    risk-adjusted metrics like the MAR Ratio.
# 2. NEW - TIMESTAMPED RESULTS: All output files (reports, plots, logs) for each run
#    are now appended with a `YYYYMMDD-HHMMSS` timestamp. This prevents overwriting
#    and creates a clean, chronological record of every script execution.
#
# V105 UPDATE (Champion Model & Evolving Baseline):
# 1. Implemented a persistent "champion" configuration in the memory file to
#    track the best-performing run based on the Calmar Ratio.
# 2. Enhanced the Pre-Flight AI check to use this champion config as a baseline.
...
class GeminiAnalyzer:
    def __init__(self):
        api_key = os.getenv("GEMINI_API_KEY", "AIzaSyAxoYnuzd4d5EbjeKS8bJoXtq_0AW-WVdM")
        if not api_key or "YOUR" in api_key or "PASTE" in api_key:
            logger.warning("!CRITICAL! GEMINI_API_KEY not found or is a placeholder. AI analysis will be skipped.")
    ...
    def get_pre_flight_config(self, memory: Dict, fallback_config: Dict) -> Dict:
        ...
        prompt = (
            "You are a lead trading strategist reviewing the performance of all previous trading model versions. "
            "Your task is to analyze the entire run history provided below, including the current 'champion_config' which represents the best risk-adjusted performance seen to date. Your primary goal is to propose a new starting configuration that has the highest probability of outperforming the current champion on a **risk-adjusted basis (higher Calmar Ratio)**.\n\n"
            "Analyze the following:\n"
            "- **The Champion**: What features and parameters did the champion model use? Why do you think it performed well?\n"
            "- **Historical Failures**: Look at the `historical_runs`. Which configurations consistently led to 'Circuit Breaker' events or high drawdowns? Are there features that seem to introduce instability?\n"
            "- **Evolution**: How has performance changed over time? Did a change in V102 lead to a better outcome than V101?\n\n"
            "Based on this deep historical analysis, recommend the **optimal starting configuration** for the new script version. Your goal is to choose the most promising starting point based on all past evidence to maximize the chance of a stable, profitable outcome.\n\n"
            "Respond ONLY with a valid JSON object containing the following keys to configure the next run: `LOOKAHEAD_CANDLES`, `MAX_DD_PER_CYCLE`, `OPTUNA_TRIALS`, and `selected_features` (a list of strings).\n\n"
            f"FRAMEWORK MEMORY (INCLUDES CURRENT CHAMPION AND ALL PREVIOUS RUNS):\n{json.dumps(self._sanitize_dict(memory), indent=2)}"
        )
        ...
        if suggestions:
            ...
            logger.info(f"  - Pre-Flight Check complete. AI recommends starting with: {suggestions}")
            return final_config
        else:
            logger.warning("  - Pre-Flight Check failed to generate a valid config. Using default.")
            return fallback_config

    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        if not self.api_key_valid:
            logger.warning("  - Skipping AI cycle analysis: No valid Gemini API key provided.")
            return {}
        ...
        prompt = (
            ...
            "In addition, actively search for overlooked strengths, rare success events, or positive anomalies—these may reveal hidden opportunities or promising directions.\n\n"
            "**CRITICAL ANALYSIS**: If a cycle's `status` is 'Circuit Breaker', you MUST analyze the `breaker_context`. This tells you HOW the failure occurred. "
            "- Was it a 'death by a thousand cuts' (many small losses)? This suggests the model's edge is weak and the confidence threshold might be too low. "
            "- Was it a few catastrophic losses? This suggests the risk parameters (SL multiplier, etc.) are wrong for the market's volatility. "
            "Use this forensic data to make highly targeted suggestions.\n\n"
            ...
            "3.  **Feature Selection**: Select a SUBSET of features from the `available_features` list. Be creative. A simpler model may be more robust, but don't ignore features that show hidden potential.\n\n"
...
class ConfigModel(BaseModel):
    ...
    LOG_FILE_PATH: str = ""; MEMORY_FILE_PATH: str = ""
    ...
    run_timestamp: str # NEW (V106)
    def __init__(self, **data: Any):
        super().__init__(**data)
        # NEW (V106): Append timestamp to file paths for unique run artifacts
        run_id = f"{self.REPORT_LABEL}_{self.run_timestamp}"
        ...
        self.MEMORY_FILE_PATH=os.path.join(self.BASE_PATH,"Results","framework_memory.json")

# ... (The rest of the script is largely the same, but with changes in PerformanceAnalyzer and main)
...
class PerformanceAnalyzer:
    ...
    def _get_comparison_block(self, metrics: Dict, memory: Dict) -> str:
        # Safely get the champion and previous run data
        champion = memory.get('champion_config', {})
        historical_runs = memory.get('historical_runs', [])
        previous_run = historical_runs[-2] if len(historical_runs) > 1 else {}

        # Helper to format data or return N/A
        def get_data(source: Dict, key: str, is_percent: bool = False, is_int: bool = False):
            val = source.get("final_metrics", {}).get(key)
            if val is None: return "N/A"
            if is_percent: return f"{val:.2f}%"
            if is_int: return str(int(val))
            return f"{val:.2f}"
...
--------------------------------------------------------------------------------
                     {_center('Current Run', 20)}|{_center('Previous Run', 24)}|{_center('All-Time Champion', 24)}
-------------------- -------------------- ------------------------ ------------------------
Version              {_ljust(c_ver, 20)}|{_ljust(p_ver, 24)}|{_ljust(champ_ver, 24)}
MAR Ratio:           {_rjust(c_mar, 20)}|{_rjust(p_mar, 24)}|{_rjust(champ_mar, 24)}
Max Drawdown:        {_rjust(c_mdd, 20)}|{_rjust(p_mdd, 24)}|{_rjust(champ_mdd, 24)}
Profit Factor:       {_rjust(c_pf, 20)}|{_rjust(p_pf, 24)}|{_rjust(champ_pf, 24)}
--------------------------------------------------------------------------------
"""
        return block
...
# =============================================================================
# 9. FRAMEWORK MEMORY & MAIN ORCHESTRATOR
# =============================================================================
def migrate_and_load_memory(memory_path: str) -> Dict:
    default_memory = {"champion_config": None, "historical_runs": []}
    jsonl_path = memory_path.replace(".json", ".jsonl")

    if os.path.exists(jsonl_path) and not os.path.exists(memory_path):
        logger.warning(f"Old '.jsonl' memory file found. Migrating to '.json'...")
        try:
            with open(jsonl_path, 'r') as f_in:
                history = [json.loads(line) for line in f_in if line.strip()]
            migrated_memory = {"champion_config": None, "historical_runs": history}
            best_run = max(history, key=lambda r: r.get("final_metrics", {}).get("calmar_ratio", 0), default=None)
            if best_run:
                migrated_memory["champion_config"] = best_run
                logger.info(f"Set initial champion from migrated history: {best_run.get('script_version')}")
            with open(memory_path, 'w') as f_out: json.dump(migrated_memory, f_out, indent=4)
            return migrated_memory
        except Exception as e:
            logger.error(f"Failed to migrate memory file: {e}"); return default_memory
...
def save_run_to_memory(memory_path: str, new_run_summary: Dict, current_memory: Dict):
    try:
        ...
        if current_champion is None or new_calmar > current_champion.get("final_metrics", {}).get("calmar_ratio", 0):
            ...
            logger.info(f"Current run's Calmar Ratio ({new_calmar:.2f}) did not beat the champion's ({current_champion.get('final_metrics', {}).get('calmar_ratio', 0):.2f}).")
...
def main():
    ...
    run_timestamp_str = start_time.strftime("%Y%m%d-%H%M%S")
    
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V106_Report_Compare",
        "run_timestamp": run_timestamp_str, # Add timestamp to config
        ...
        "FORWARD_TEST_GAP": "1D", "LOOKAHEAD_CANDLES": 100, "TREND_FILTER_THRESHOLD": 25.0, 
    ...
    memory_file = os.path.join(fallback_config['BASE_PATH'], "Results", "framework_memory.json")
    framework_history = migrate_and_load_memory(memory_file)
    current_config = gemini_analyzer.get_pre_flight_config(framework_history, fallback_config)
    current_config['run_timestamp'] = run_timestamp_str # Ensure timestamp is in the AI-driven config too
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        cycle_results_for_ai={
            "cycle":i+1, "objective_score":trainer.study.best_value if trainer.study else 0,
            "best_threshold":best_threshold, "cycle_pnl":cycle_pnl, "win_rate":cycle_win_rate,
            "num_trades":len(chunk_trades_df), "status": status, 
            "current_params":{k:v for k,v in config.model_dump().items() if k in ['LOOKAHEAD_CANDLES','OPTUNA_TRIALS','selected_features','MAX_DD_PER_CYCLE']},
        ...
        suggested_params = gemini_analyzer.analyze_cycle_and_suggest_changes(in_run_historical_cycles, all_feature_names)
    ...
    run_summary = {
        "script_version": initial_config.REPORT_LABEL,
        "timestamp": initial_config.run_timestamp,
        "final_metrics": gemini_analyzer._sanitize_dict(final_metrics),
        "initial_config": {k: v for k, v in initial_config.model_dump().items() if k in ['LOOKAHEAD_CANDLES', 'MAX_DD_PER_CYCLE', 'OPTUNA_TRIALS', 'selected_features']},
        "top_5_features": aggregated_shap.head(5).to_dict().get('SHAP_Importance', {}) if aggregated_shap is not None else {}
    }
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V106_Report_Compare.py