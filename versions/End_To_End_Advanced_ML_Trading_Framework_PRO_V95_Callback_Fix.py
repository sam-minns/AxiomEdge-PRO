# End_To_End_Advanced_ML_Trading_Framework_PRO_V95_Callback_Fix
#
# V95 Update:
# 1. CRITICAL FIX: Replaced the problematic `early_stopping_rounds` parameter with
#    the modern, robust `EarlyStopping` callback from XGBoost. This definitively
#    solves the recurring `TypeError` during optimization.
...
#
# V95.2 FIX: Reinstated the modern `EarlyStopping` callback. The previous errors
# indicate a severely outdated XGBoost library in the execution environment. This script
# uses the current, correct API. To resolve errors, the environment's library must be
# updated (`pip install --upgrade xgboost`).
...
from xgboost.callback import EarlyStopping # Restored for modern API usage
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        def objective(trial:optuna.Trial):
            ...
            try:
                ...
                # *** FIX APPLIED ***
                # ===================
                # Using the modern `EarlyStopping` callback. This is the correct, modern API.
                # This requires xgboost version >= 1.3.
                # If you get a `TypeError` on the 'callbacks' argument, please update your library:
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V95_Callback_Fix",
