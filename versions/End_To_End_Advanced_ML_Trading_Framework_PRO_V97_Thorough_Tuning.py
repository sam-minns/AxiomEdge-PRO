# End_To_End_Advanced_ML_Trading_Framework_PRO_V97_Thorough_Tuning
#
# V97 UPDATE (More Thorough Tuning):
# 1. ROBUSTNESS: Addressed the issue of walk-forward cycles completing "too quickly"
#    by making the hyperparameter search more rigorous. A suspiciously fast
#    optimization suggests that early stopping may be halting trials prematurely.
# 2. DEEPER OPTIMIZATION: The Optuna search space has been updated:
#    - `OPTUNA_TRIALS` increased from 20 to 35.
#    - `n_estimators` range increased to (200, 1500).
#    - `learning_rate` range lowered and narrowed to (0.005, 0.1) to encourage
#      more boosting rounds before early stopping can occur.
# 3. DIAGNOSTICS: Added a new log line to show the number of training samples
#    available for the meta-model in each cycle for better insight.
#
# V96 UPDATE (Final Compatibility Fix):
# 1. CRITICAL FIX: Resolved environment-specific errors by passing
#    `early_stopping_rounds` during the XGBClassifier's initialization, which is
#    the most compatible method for the scikit-learn API.
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        def objective(trial:optuna.Trial):
            ...
            # `early_stopping_rounds` is passed at initialization for scikit-learn API compatibility.
            # Search space is adjusted for a more thorough (slower) search.
            model_params={
                'objective':'binary:logistic','eval_metric':'logloss','booster':'gbtree',
                'tree_method':'hist', 'use_label_encoder':False, 'seed':42,
                'n_estimators':trial.suggest_int('n_estimators', 200, 1500, step=50), # Increased range
                'max_depth':trial.suggest_int('max_depth',3,8), # Slightly deeper trees
                'learning_rate':trial.suggest_float('learning_rate', 0.005, 0.1, log=True), # Lowered rate
            ...
            try:
                ...
                if trial.number == 0:
                    logger.info("    - Using early_stopping_rounds via model constructor for max compatibility.")
        ...
        try:
            ...
            study.optimize(objective,n_trials=self.config.OPTUNA_TRIALS,timeout=3600,show_progress_bar=False) # Increased timeout
            return study
        except Exception as e:
    ...
    def _train_final_model(self,best_params:Dict,X:pd.DataFrame,y:pd.Series)->Optional[Pipeline]:
        ...
        try:
            # IMPORTANT: Remove early stopping for the final model trained on all data.
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V97_Thorough_Tuning",
        ...
        "OPTUNA_TRIALS": 35, # Increased for more thorough search
        "TRAINING_WINDOW": '365D', "RETRAINING_FREQUENCY": '60D', "FORWARD_TEST_GAP": "1D",
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        # NEW (V97): Add diagnostic logging for training set size
        logger.info(f"  - Number of primary signals for meta-model training: {len(df_train_labeled)}")
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V97_Thorough_Tuning.py