# End_To_End_Advanced_ML_Trading_Framework_PRO_V148_Decorated.py
#
# V148 UPDATE (Progress Bar & FIX):
# 1. ADDED - TQDM Progress Bar: Integrated the `tqdm` library to display a
#    visual progress bar during the main walk-forward analysis loop.
...
# 3. FIXED - NameError: Corrected a critical `NameError` in the main loop that
#    prevented the script from running.
# 4. ENHANCED - Report Decoration: Overhauled the final text report with a
#    professional, boxed layout for significantly improved readability.
#
...
class PerformanceAnalyzer:
    ...
    def generate_text_report(self, m: Dict[str, Any], cycle_metrics: List[Dict], aggregated_shap: Optional[pd.DataFrame] = None, framework_memory: Optional[Dict] = None):
        ...
        for title, data in sections.items():
            ...
            report.append(_box_mid(WIDTH))
            for key, val in data:
                report.append(_box_text_kv(key, val, WIDTH))
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool):
    ...
    for period_start_date in tqdm(retraining_dates, desc="Walk-Forward Cycles"):
        ...
        # <<<<<<<<<<<<<<< FIX IS HERE >>>>>>>>>>>>>>>>>
        logger.info(f"\n--- Cycle Start: {period_start_date.date()} | Train: {train_start.date()}-{train_end.date()} | Test: {period_start_date.date()}-{test_end.date()} ---")
        ...
        if suggested_params:
            ...
            # =================================================================
            if is_continuous:
                logger.info("Daemon Mode: Pausing for 2.5 minutes to respect API rate limits...")
                time.sleep(150) # Use 150 seconds as a moderate delay
            else:
                logger.info("Single Run Mode: Pausing for 2.5 minutes between cycles...")
                time.sleep(150)
            # =================================================================
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V148_Decorated",
