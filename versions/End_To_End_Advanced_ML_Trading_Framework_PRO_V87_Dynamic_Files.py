# End_To_End_Advanced_ML_Trading_Framework_PRO_V87_Dynamic_Files
#
# V87 Update:
# 1. NEW - Dynamic File Loading: The script no longer uses a hardcoded list of
#    filenames in the configuration. It is now driven by a dynamic list created
#    in the main function, ensuring it processes the exact files provided.
# 2. All features and logic from V86 are retained. This script is complete and unabridged.
...
# =============================================================================
# 2. GEMINI AI ANALYZER (No changes from V86)
# =============================================================================
class GeminiAnalyzer:
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        try:
            response = requests.post(self.api_url, headers=self.headers, data=json.dumps(data)); response.raise_for_status()
            result = response.json(); return result["candidates"][0]["content"]["parts"][0]["text"]
        except requests.exceptions.RequestException as e: logger.error(f"Gemini API request failed: {e}"); return "{}"
        except (KeyError, IndexError) as e: logger.error(f"Failed to extract text from Gemini response: {e} - Response: {response.text}"); return "{}"
    def _extract_json_from_response(self, response_text: str) -> Dict:
        ...
        except (json.JSONDecodeError, AttributeError) as e: logger.error(f"Could not parse JSON from response: {e}\nResponse text: {response_text}"); return {}
    def analyze_cycle_and_suggest_changes(self, cycle_results: Dict) -> Dict:
        ...
        prompt = ("You are an expert trading model analyst. Given the following walk-forward trading cycle results, suggest new integer values for 'LOOKAHEAD_CANDLES' "
            "(between 50-200), 'OPTUNA_TRIALS' (between 15-30), and a new float value for 'TREND_FILTER_THRESHOLD' (between 20.0-40.0). "
            "Focus on improving the Profit Factor and reducing the Drawdown. Respond ONLY with a valid JSON object containing these keys.\n\n"
        ...
        logger.info(f"    - Parsed Suggestions: {suggestions}"); return suggestions
...
def main():
    ...
    try:
        ...
        initial_config=ConfigModel(**current_config);logger.info(f"-> Starting with configuration: {initial_config.REPORT_LABEL}")
        data_by_tf=DataLoader(initial_config).load_and_parse_data()
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        trainer=ModelTrainer(config);training_result=trainer.train(df_train_labeled)
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V87_Dynamic_Files.py