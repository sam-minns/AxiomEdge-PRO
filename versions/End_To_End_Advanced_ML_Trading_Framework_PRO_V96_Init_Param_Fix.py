# End_To_End_Advanced_ML_Trading_Framework_PRO_V96_Init_Param_Fix
#
# V96 UPDATE (Final Compatibility Fix):
# 1. CRITICAL FIX: Resolved the `unexpected keyword argument 'early_stopping_rounds'`
#    error by adopting the logic from the known working V93 script. The
#    `early_stopping_rounds` parameter is now correctly passed during the
#    `XGBClassifier`'s initialization, not to the `.fit()` method. This is the
#    standard, most compatible way to use early stopping with the scikit-learn API.
#
# V95.5 UPDATE (Forced Legacy API):
# 1. Reverted to using `early_stopping_rounds` to address environment conflicts.
#
# V95.4 UPDATE (Critical Fix & Refinements):
# 1. Corrected the `train_test_split` call to preserve chronological order.
# 2. Upgraded Gemini API endpoint and improved API key handling.
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        def objective(trial:optuna.Trial):
            ...
            # CRITICAL FIX (V96): `early_stopping_rounds` must be passed at initialization,
            # not during the .fit() call, to work with the scikit-learn API wrapper.
            model_params={
                ...
                'early_stopping_rounds': 50 # Parameter is now here
            }
            try:
                scaler=RobustScaler()
                ...
                # Initialize the classifier with all parameters, including for early stopping
                model=xgb.XGBClassifier(**model_params)
                
                if trial.number == 0:
                    logger.info("    - Using early_stopping_rounds via model constructor for max compatibility.")

                # The .fit call is now cleaner and more robust.
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V96_Init_Param_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V96_Init_Param_Fix.py