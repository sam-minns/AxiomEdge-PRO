# End_To_End_Advanced_ML_Trading_Framework_PRO_V155_FIXED.py
#
# V155 UPDATE (AttributeError Fix):
# 1. FIXED - AttributeError: Implemented the missing `label_meta_outcomes` method
#    in the FeatureEngineer class. This is essential for the meta-labeling strategy,
#    which was previously causing a crash. The method now correctly generates
#    binary labels (1 for correct, 0 for incorrect) for the secondary model.
#
# V154 UPDATE (Syntax Fix):
# 1. FIXED - SyntaxError: Corrected an 'invalid syntax' error in the
#    PerformanceAnalyzer class by replacing Unicode box-drawing characters
#    (e.g., '╔', '═') with standard ASCII equivalents ('+', '-') to ensure
#    compatibility with the Python parser.
...
class FeatureEngineer:
    ...
    def _calculate_htf_features(self,df:pd.DataFrame,p:str,s:int,a:int)->pd.DataFrame:
        # FIXED: Standardize timeframe identifier to uppercase
        tf_id = p.upper()
        ...
        for symbol,group in df.groupby('Symbol'):
            ...
            # FIXED: Use the standardized uppercase identifier for column names
    ...
    def _calculate_base_tf_native(self, g:pd.DataFrame)->pd.DataFrame:
        ...
        # FIXED: Restored missing calculations from previous version
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # FIXED: Use standardized uppercase identifiers for interaction features
        if medium_tf:
    ...
    def _label_meta_group(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        """Helper to generate binary labels for a single instrument group."""
        ...
        for i in range(len(group) - lookahead):
            ...
            if signal == 0: continue # No primary signal, so no meta-label
            ...
            if signal > 0:  # Primary model suggests LONG
                tp_long, sl_long = prices[i] + tp_dist, prices[i] - sl_dist
                ...
                if len(time_to_tp) > 0 and (len(time_to_sl) == 0 or time_to_tp[0] < time_to_sl[0]):
                    outcomes[i] = 1  # Correct prediction: TP hit before SL

            elif signal < 0:  # Primary model suggests SHORT
    ...
    def label_meta_outcomes(self, df: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        """
        Labels data for the secondary (meta) model. The target is binary:
        1 if the primary model's signal was correct (profitable), 0 otherwise.
        """
    ...
    def generate_text_report(self, m: Dict[str, Any], cycle_metrics: List[Dict], aggregated_shap: Optional[pd.DataFrame] = None, framework_memory: Optional[Dict] = None):
        ...
        sections = {
            "II. EXECUTIVE SUMMARY": [
                ...
                (f"Win Rate:", f"{m.get('win_rate', 0):>15.2%}"),
            ...
            "IV. RISK & DRAWDOWN ANALYSIS": [
                (f"Max Drawdown:", f"{m.get('max_drawdown_pct', 0):>15.2f}% (${m.get('max_drawdown_abs', 0):,.2f})"),
            ...
            "V. TRADE-LEVEL STATISTICS": [
                (f"Total Trades:", f"{m.get('total_trades', 0):>15}"),
                (f"Average Win:", f"${m.get('avg_win_amount', 0):>15,.2f}"),
                (f"Average Loss:", f"${m.get('avg_loss_amount', 0):>15,.2f}"),
                (f"Payoff Ratio:", f"{m.get('payoff_ratio', 0):>15.2f}")
            ]
        }
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool):
    ...
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        ...
        if df_train_labeled.empty or df_train_labeled['target'].abs().sum() == 0 : 
        ...
        if suggested_params:
            ...
            if is_continuous:
                logger.info("Daemon Mode: Pausing for 2.5 minutes to respect API rate limits..."); time.sleep(150)
            else:
                logger.info("Single Run Mode: Pausing for 2.5 minutes between cycles..."); time.sleep(150)
    ...
    if not aggregated_trades.empty:
        ...
        final_metrics = pa.generate_full_report(aggregated_trades, aggregated_equity_curve, in_run_historical_cycles, aggregated_shap, framework_history)
