# End_To_End_Advanced_ML_Trading_Framework_PRO_V175_APIDelay_FIX.py
#
# V175 UPDATE (API Delay Standardization):
# 1. FIXED: The conditional API timer logic has been removed. The delay between
#    API calls is now a constant 300 seconds (5 minutes) for all run types,
#    ensuring consistent behavior and rate-limiting compliance.
#
# V174 UPDATE (Portfolio & Risk Management Engine):
# 1. IMPLEMENTED: AI-Managed Concurrent Trading (`MAX_CONCURRENT_TRADES`).
# 2. IMPLEMENTED: AI-Managed Partial Profit-Taking (`PARTIAL_PROFIT_TRIGGER_R`, `PARTIAL_PROFIT_TAKE_PCT`).
# 3. IMPLEMENTED: Auto-Breakeven Stop on partial profit.
...
class PerformanceAnalyzer:
    ...
    def _calculate_metrics(self,trades_df:pd.DataFrame,equity_curve:pd.Series)->Dict[str,Any]:
        ...
        avg_full_loss = abs(final_exits_df[final_exits_df['PNL'] < 0]['PNL'].mean())
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V175_APIDelay_FIX",
    ...
    # V175 Change: Set a fixed 5-minute (300s) delay between API calls for all run types.
