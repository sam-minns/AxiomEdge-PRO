# End_To_End_Advanced_ML_Trading_Framework_PRO_V103_Cross_Version_Learning
#
# V103 UPDATE (Cross-Version Learning & Memory):
# 1. NEW - FRAMEWORK MEMORY: Implemented the user's suggestion for cross-version
#    learning. A new persistent file, `framework_memory.jsonl`, is created in the
#    Results folder. After each run, it logs key outcomes: the script version,
#    final performance metrics, the best cycle's configuration, and top SHAP features.
# 2. NEW - PRE-FLIGHT ANALYSIS: At startup, the script now reads its entire history
#    from the memory file and sends it to the Gemini AI with a new strategic prompt.
# 3. AI-DRIVEN INITIALIZATION: The AI is asked to act as a lead strategist, analyze
#    what has worked best across ALL previous versions, and recommend the optimal
#    starting configuration for the current run. This allows the framework to
#    evolve and learn from the successes and failures of its past iterations.
#
# V102 UPDATE (Circuit Breaker Forensics):
# 1. Added a "damage report" when the circuit breaker trips to provide the AI with
#    critical context on the mode of failure.
...
def setup_logging() -> logging.Logger:
    ...
    if not logger.handlers:
        ...
        # Create a specific log file for each version run inside its results folder
        # This will be handled after the config is set up.
...
class GeminiAnalyzer:
    def __init__(self):
        api_key = os.getenv("GEMINI_API_KEY", "AIzaSyAxoYnuzd4d5EbjeKS8bJoXtq_0AW-WVdME")
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        try:
            response = requests.post(self.api_url, headers=self.headers, data=json.dumps(data), timeout=120)
    ...
    def get_pre_flight_config(self, memory: List[Dict], fallback_config: Dict) -> Dict:
        """Analyzes the entire framework history to recommend a starting config."""
        ...
        prompt = (
            ...
            "Your task is to analyze the entire run history provided below. Identify which architectural changes, feature sets, and risk profiles have historically led to the best **risk-adjusted returns** (i.e., high Profit Factor, high Calmar/Sharpe Ratio, and low Maximum Drawdown). Pure profit is not the goal; stability and consistent risk-adjusted performance are.\n\n"
            "Analyze the following:\n"
            "- **Winning Configurations**: Which `selected_features` and `LOOKAHEAD_CANDLES` values appear most often in runs with high Profit Factors and low drawdowns?\n"
            "- **Losing Patterns**: Are there any features or parameter ranges that consistently lead to poor performance or 'Circuit Breaker' events?\n"
            "- **Evolution**: How has performance changed over time? Did a change in V99 lead to a better outcome than V98?\n\n"
            ...
            "Respond ONLY with a valid JSON object containing the following keys to configure the next run: `LOOKAHEAD_CANDLES` (int), `MAX_DD_PER_CYCLE` (float), `OPTUNA_TRIALS` (int), and `selected_features` (a list of strings).\n\n"
            f"FRAMEWORK MEMORY (HISTORY OF ALL PREVIOUS RUNS):\n{json.dumps(memory, indent=2)}"
        )
        ...
        if suggestions:
            # Validate and merge suggestions with fallback
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        ...
        # In-run analysis only needs the most recent cycle data
...
class ConfigModel(BaseModel):
    ...
    def __init__(self, **data: Any):
        ...
        self.LOG_FILE_PATH=os.path.join(result_folder_path,f"{self.REPORT_LABEL}_run.log")
        self.MEMORY_FILE_PATH=os.path.join(self.BASE_PATH,"Results","framework_memory.jsonl")

# ... The rest of the classes (DataLoader, FeatureEngineer, ModelTrainer, Backtester, PerformanceAnalyzer) are identical to V102 ...
# ... To save space, they are omitted here but are included in the full script below ...
...
# =============================================================================
def load_framework_memory(memory_path: str) -> List[Dict]:
    """Loads the historical records of all previous framework runs."""
...
def save_run_to_memory(memory_path: str, run_summary: Dict):
    """Appends the summary of the current run to the memory file."""
    with open(memory_path, 'a') as f:
        f.write(json.dumps(run_summary) + '\n')
    logger.info(f"-> Run summary appended to framework memory: {memory_path}")
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V103_Memory",
    ...
    # --- Pre-Flight Check with AI ---
    gemini_analyzer=GeminiAnalyzer()
    memory_file = os.path.join(fallback_config['BASE_PATH'], "Results", "framework_memory.jsonl")
    framework_history = load_framework_memory(memory_file)
    current_config = gemini_analyzer.get_pre_flight_config(framework_history, fallback_config)
    
    # --- Setup Version-Specific Logging ---
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        cycle_results_for_ai={
            ...
            "current_params":{k:v for k,v in config.dict().items() if k in ['LOOKAHEAD_CANDLES','OPTUNA_TRIALS','selected_features','MAX_DD_PER_CYCLE']},
    ...
    final_metrics = reporter.generate_full_report(final_trades_df,pd.Series(full_equity_curve),cycle_metrics,aggregated_shap)
    ...
    run_summary = {
        ...
        "initial_config": {k: v for k, v in initial_config.dict().items() if k in ['LOOKAHEAD_CANDLES', 'MAX_DD_PER_CYCLE', 'OPTUNA_TRIALS', 'selected_features']},
        "top_5_features": aggregated_shap.head(5).to_dict().get('SHAP_Importance', {}) if aggregated_shap is not None else {}
    }
    save_run_to_memory(initial_config.MEMORY_FILE_PATH, run_summary)
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V103_Cross_Version_Learning.py