# End_To_End_Advanced_ML_Trading_Framework_PRO_V71_Consolidated
#
# V71 Update:
# 1. Consolidated Best Features: This version reverts the simplifications of V70.
#    - Restored the multi-timeframe (D1, H1, M15) data loading from V47.
#    - Reinstated the full, sophisticated FeatureEngineer from V47, including
#      multi-timeframe context, Bollinger Bands, Stochastics, and other advanced features.
#    - Re-implemented the robust dynamic triple-barrier labeling from V47 to ensure
#      balanced class distribution for model training.
# 2. Incorporated Dynamic Confidence: Integrated the dynamic confidence logic from V55's
#    backtester, which adjusts the required prediction confidence based on the market regime (trending vs. ranging).
# 3. Enhanced Documentation: Added explicit comments emphasizing that this Python backtest
#    is a high-level simulation. It is a powerful tool for rapid prototyping, but final
#    validation should always be performed in the MQL5 Strategy Tester to account for
#    tick-level granularity and real-world broker conditions.

# --- FRAMEWORK DISCLAIMER ---
# This script is a high-level, bar-based simulation for ML strategy development.
# It does not replicate the tick-by-tick fidelity of the MQL5 Strategy Tester.
# Factors like intra-bar price movement, precise slippage, margin calls, and
# broker-specific execution are simplified.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V71_Consolidated",
...
# =============================================================================
# 3. DATA LOADER (Restored from V47)
# =============================================================================
class DataLoader:
...
# =============================================================================
# 4. FEATURE ENGINEERING (Restored from V47)
# =============================================================================
class FeatureEngineer:
    ...
    def _calculate_adx(self, g: pd.DataFrame, period: int) -> pd.DataFrame:
        ...
        df['DX'] = (abs(df['di_plus'] - df['di_minus']) / (abs(df['di_plus'] + df['di_minus'])).replace(0, 1e-9)) * 100
        df['ADX'] = df['DX'].ewm(alpha=alpha, adjust=False).mean()
        g['ADX'] = df['ADX'].shift(1) # Lagged to prevent lookahead
        return g
    ...
    def _calculate_bollinger_bands(self, g: pd.DataFrame, period: int) -> pd.DataFrame:
        ...
        g['bb_upper'] = (middle_band + (std_dev * 2)).shift(1)
        g['bb_middle'] = middle_band.shift(1)
        g['bb_lower'] = (middle_band - (std_dev * 2)).shift(1)
        g['bb_width'] = ((g['bb_upper'] - g['bb_lower']) / g['bb_middle'].replace(0, np.nan)).shift(1)
        return g
    ...
    def _calculate_stochastic(self, g: pd.DataFrame, period: int) -> pd.DataFrame:
        ...
        g['stoch_k'] = fast_k.rolling(window=3).mean().shift(1)
        g['stoch_d'] = g['stoch_k'].rolling(window=3).mean().shift(1)
        return g
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        df_m15_base = data_by_tf['M15'].copy().groupby('Symbol', group_keys=False).apply(self._calculate_m15_native).reset_index()
        ...
        df_merged = pd.merge_asof(df_merged.sort_values('Timestamp'), df_d1.sort_values('Timestamp'), on='Timestamp', by='Symbol', direction='backward')
        df_merged.set_index('Timestamp', inplace=True)
        df_merged = df_merged.sort_index()

        df_merged['hour'] = df_merged.index.hour
        df_merged['day_of_week'] = df_merged.index.dayofweek
        df_merged['adx_x_h1_trend'] = df_merged['ADX'] * df_merged['H1_ctx_Trend']
        df_merged['atr_x_d1_trend'] = df_merged['ATR'] * df_merged['D1_ctx_Trend']
        df_merged['market_regime'] = np.where(df_merged['ADX'] > self.config.TREND_FILTER_THRESHOLD, 1, 0) # 1 for Trend, 0 for Range
    ...
    def _apply_lagged_features(self, g: pd.DataFrame, p: str, s: int, a: int) -> pd.DataFrame:
        g[f'{p}_SMA'] = g['Close'].rolling(s).mean().shift(1)
        g[f'{p}_ATR'] = g['High'].sub(g['Low']).rolling(a).mean().shift(1)
        g[f'{p}_Trend'] = np.sign(g['Close'].shift(1) - g[f'{p}_SMA'])
        return g

    def _calculate_htf_features(self, df: pd.DataFrame, p: str, s: int, a: int) -> pd.DataFrame:
        df_out = df.groupby('Symbol', group_keys=False).apply(self._apply_lagged_features, p, s, a).reset_index()
    ...
    def _calculate_m15_native(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        g['ATR'] = g['High'].sub(g['Low']).rolling(lookback).mean().shift(1)
        ...
        g['RSI'] = (100 - (100 / (1 + (gain / loss.replace(0, 1e-9))))).shift(1)
        
        g = self._calculate_adx(g, lookback)
        g = self._calculate_bollinger_bands(g, self.config.BOLLINGER_PERIOD)
        g = self._calculate_stochastic(g, self.config.STOCHASTIC_PERIOD)
        for col in ['ATR', 'RSI', 'bb_width', 'stoch_k']: g[f'{col}_rank'] = g[col].rolling(rank_period).rank(pct=True).shift(1)

        sma_50 = g['Close'].rolling(window=sma_period).mean()
        g['dist_from_sma50'] = ((g['Close'] - sma_50) / g['ATR'].replace(0, np.nan)).shift(1)
        ...
        g['consecutive_candles'] = g['consecutive_candles'].shift(1)
    ...
    def _label_with_dynamic_barriers(self, df: pd.DataFrame, initial_sl_atr: float, initial_tp_atr: float, lookahead: int, min_class_ratio: float, max_iterations: int) -> pd.DataFrame:
        ...
        for i in range(max_iterations):
            ...
            # Use group_keys=False to avoid adding extra index level
            labeled_df = df.groupby('Symbol', group_keys=False).apply(self._label_group, sl_atr=current_sl, tp_atr=current_tp, lookahead=lookahead)
    ...
    def _label_with_fixed_barriers(self, df: pd.DataFrame, sl_atr: float, tp_atr: float, lookahead: int) -> pd.DataFrame:
        ...
        return df.groupby('Symbol', group_keys=False).apply(self._label_group, sl_atr=sl_atr, tp_atr=tp_atr, lookahead=lookahead)
    ...
    def _label_group(self, group: pd.DataFrame, sl_atr: float, tp_atr: float, lookahead: int) -> pd.DataFrame:
        ...
        for i in range(len(group) - lookahead):
            ...
            tp_long = entry_price + current_atr * tp_atr
            sl_long = entry_price - current_atr * sl_atr
            
            tp_short = entry_price - current_atr * tp_atr
            sl_short = entry_price + current_atr * sl_atr

            future_lows = lows[i+1 : i+1+lookahead]
            future_highs = highs[i+1 : i+1+lookahead]

            # Find first time each barrier is hit
            time_to_tp_long = np.where(future_highs >= tp_long)[0]
            time_to_sl_long = np.where(future_lows <= sl_long)[0]
            
            time_to_tp_short = np.where(future_lows <= tp_short)[0]
            time_to_sl_short = np.where(future_highs >= sl_short)[0]

            # Get the index of the first touch, or infinity if never touched
            ...
            if first_tp_long < first_sl_long: # Long trade won
                outcomes[i] = 1
            elif first_sl_long < first_tp_long: # Long trade lost
                outcomes[i] = -1
            
            # Check for short trade possibility only if long didn't trigger first
            if min(first_tp_long, first_sl_long) > min(first_tp_short, first_sl_short):
                if first_tp_short < first_sl_short: # Short trade won
                    outcomes[i] = -1
                elif first_sl_short < first_tp_short: # Short trade lost
                    outcomes[i] = 1
...
# =============================================================================
# 5. MODEL TRAINER (Restored from V47)
# =============================================================================
class ModelTrainer:
    FEATURES = [
        'ATR', 'RSI', 'ADX', 'ATR_rank', 'RSI_rank', 'hour', 'day_of_week', 
        'adx_x_h1_trend', 'atr_x_d1_trend', 'H1_ctx_Trend', 'D1_ctx_Trend', 
    ...
    def _train_final_model(self, best_params: Dict, X: pd.DataFrame, y: pd.Series) -> Optional[Pipeline]:
        ...
        try:
            ...
            # The scaler must be fitted on the training data before transforming the test data
            scaler = final_pipeline.named_steps['scaler']
            X_test_final_scaled = scaler.fit(X_train_final).transform(X_test_final)

            fit_params = {
                'model__eval_set': [(X_test_final_scaled, y_test_final.values)],
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: pd.Index):
        ...
        try:
            ...
            # For multi-class, shap_values is a list of arrays. We average the mean absolute SHAP values across all classes.
...
# =============================================================================
# 6. BACKTESTER (with Dynamic Confidence from V55)
# =============================================================================
class Backtester:
    def __init__(self, config: ConfigModel):
        self.config = config
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, model: Pipeline, initial_equity: float) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        for candle in candles:
            ...
            if not open_position:
                ...
                # Dynamic Confidence Threshold (from V55)
                # Lower threshold for high-momentum (trending) markets, higher for ranging
                ...
                if direction != 0 and confidence >= min_confidence:
                    ...
                    tier_name = 'standard' # Default tier
                    if confidence >= self.config.CONFIDENCE_TIERS['ultra_high']['min']:
                        tier_name = 'ultra_high'
                    elif confidence >= self.config.CONFIDENCE_TIERS['high']['min']:
                        tier_name = 'high'
                        
                    tier = self.config.CONFIDENCE_TIERS[tier_name]
                    risk_amt = equity * self.config.BASE_RISK_PER_TRADE_PCT * tier['risk_mult']
                    
                    # Note: Spread and Slippage are simplified as exit cost adjustments
                    ...
                    # Ensure TP distance is still positive after accounting for costs
                    ...
                    open_position = {'symbol': candle['Symbol'], 'direction': direction, 'sl': sl_price, 'tp': tp_price, 'risk_amt': risk_amt, 'rr': tier['rr'], 'confidence': confidence}
...
class PerformanceAnalyzer:
    ...
    def plot_shap_summary(self, shap_summary: pd.DataFrame):
        ...
        shap_summary.head(15).sort_values(by='SHAP_Importance').plot(kind='barh', legend=False, color='mediumseagreen')
    ...
    def _calculate_metrics(self, trades_df: pd.DataFrame, equity_curve: pd.Series) -> Dict[str, Any]:
        ...
        # Ensure 'ExecTime' is timezone-naive before calculation
        ...
        m['sharpe_ratio'] = (trades_df['PNL'].mean() / pnl_std) * np.sqrt(252 * (24 * 4)) if pnl_std > 0 else 0 # For M15 data
...
def main():
    ...
    for i, period_start_date in enumerate(retraining_dates):
        ...
        # Use .loc for robust time-series slicing
    ...
    # Ensure all_trades is a single DataFrame before passing to the reporter
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V71_Consolidated.py