# End_To_End_Advanced_ML_Trading_Framework_PRO_V95.3_Dynamic_Callback
#
...
#
# V95 Update:
# 1. CRITICAL FIX: Replaced the problematic `early_stopping_rounds` parameter with
#    the modern, robust `EarlyStopping` callback from XGBoost.
# 2. SIMPLIFICATION: Removed the unnecessary `_find_best_threshold` function.
#    The meta-model now uses a fixed, high-confidence threshold (0.65) for clarity
#    and stability, streamlining the training process.
# 3. REFINEMENT: Cleaned up the main configuration dictionary and refined the
#    primary signal generation logic to better suit the meta-labeling architecture.
...
class GeminiAnalyzer:
    def __init__(self):
        ...
        if not api_key or "YOUR" in api_key or "PASTE" in api_key:
            logger.warning("GEMINI_API_KEY not found. AI analysis will be skipped. Please paste your key to enable it.")
...
class ModelTrainer:
    ...
    def train(self,df_train:pd.DataFrame, feature_list: List[str])->Optional[Tuple[Pipeline,float]]:
        ...
        try:
            # Stratify if possible to maintain target distribution in val set
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V95.3_Dynamic_Callback.py