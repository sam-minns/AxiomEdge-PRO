# End_To_End_Advanced_ML_Trading_Framework_PRO_V169_Robust_AI_Input_FIX.py
#
# V169 UPDATE (Final AI Input Fix & Prompt Engineering):
# 1. FIXED - The sanitization function now correctly handles raw numbers (e.g., 5)
#    from the AI by assuming the unit is 'D' for days, aligning with the
#    AI's likely conservative intent and preventing an incorrect fallback to 90D.
# 2. IMPROVED - Prompts sent to the AI now contain stricter, more explicit
#    instructions to respond with a specific string format (e.g., "60D") to
#    dramatically reduce the chance of receiving an ambiguous value.
#
...
#    parse day (D), week (W), and month (M) based frequencies.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, daily_dd_report: Dict) -> Dict:
        ...
        if historical_results and historical_results[-1].get("Status") == "Circuit Breaker":
            prompt_details = (
                ...
                "4.  Consider other risk-reducing changes: **reduce `LOOKAHEAD_CANDLES`**, **reduce `OPTUNA_TRIALS`**, or **simplify the model by reducing `selected_features`**.\n"
            )
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration for this new strategy. Start with a conservative `RETRAINING_FREQUENCY` like '90D' and a low `MAX_DD_PER_CYCLE` like 0.15. **Ensure `RETRAINING_FREQUENCY` is a string with a unit (e.g., '90D').**\n\n"
