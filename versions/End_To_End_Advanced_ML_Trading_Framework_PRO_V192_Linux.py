# End_To_End_Advanced_ML_Trading_Framework_PRO_V192_Linux.py
#
...
#     pre-defines a "Contingency Path B" (a specific alternative strategy) and a
#     "Contingency Path C" (a specific parameter tweak) that are tailored to
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], framework_history: Dict, available_features: List[str], strategy_details: Dict, cycle_status: str, strategic_forecast: Optional[Dict] = None) -> Dict:
        ...
        if cycle_status in ["TRAINING_FAILURE", "PROBATION"] and strategic_forecast:
            prompt_details = (
                ...
                "1.  **EXECUTE CONTINGENCY B**: Immediately switch to the new strategy you planned for. Your response should contain the new `strategy_name` and its parameters.\n"
                "2.  **EXECUTE CONTINGENCY C**: Retry the current strategy but apply the parameter tweaks you planned. Your response should contain the new parameters for the current strategy.\n"
                "3.  **DEVIATE (NEW PATH)**: The failure was unexpected, and your original contingency plans are no longer optimal. Propose a completely new path (e.g., change features, simplify the model further, etc.)."
            )
