# End_To_End_Advanced_ML_Trading_Framework_PRO_V166_Daily_DD_Monitor.py
#
# V166 UPDATE (Daily DD Monitor & AI Cycle Control):
# 1. IMPLEMENTED - The backtester now tracks and reports the maximum drawdown for each
#    individual day within a testing cycle.
# 2. IMPLEMENTED - A new section in the final report, "HIGH DAILY DRAWDOWN EVENTS",
#    highlights any day where the daily drawdown exceeded 15%.
# 3. UPDATED - This new daily DD data is summarized and sent to the AI, which is
#    instructed to view it as a key measure of intra-day risk.
# 4. IMPLEMENTED - The AI can now dynamically adjust the 'RETRAINING_FREQUENCY'
#    (the cycle length), allowing it to adapt the model's retraining cadence.
#
# V165 UPDATE (API Retry Logic):
# 1. IMPLEMENTED - Added a robust retry mechanism for the Gemini API call.
#    If a connection fails, it will wait and retry after 5s, then 15s, then 30s
#    before stopping.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, daily_dd_report: Dict) -> Dict:
        if not self.api_key_valid: return {}
        ...
        base_prompt_analysis = "Analyze the recent cycle history and the daily drawdown report. High daily drawdowns, even without a full cycle circuit breaker, indicate excessive risk. If a 'Circuit Breaker' was tripped, be even more conservative."
        ...
        base_suggestions = (
            ...
            "- `RETRAINING_FREQUENCY`: How often to retrain (e.g., '60D', '90D', '120D'). Shorter means more adaptive.\n"
        )
        ...
        if historical_results and historical_results[-1].get("Status") == "Circuit Breaker":
            prompt_details = (
                ...
                "3.  Consider other risk-reducing changes: **shorten `RETRAINING_FREQUENCY`** for faster adaptation, **reduce `LOOKAHEAD_CANDLES`**, **reduce `OPTUNA_TRIALS`**, or **simplify the model by reducing `selected_features`**.\n"
            )
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration for this new strategy. Start with a conservative `RETRAINING_FREQUENCY` like '90D' and a low `MAX_DD_PER_CYCLE` like 0.15.\n\n"
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, is_meta_model: bool) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        for candle in candles:
            ...
            if not circuit_breaker_tripped:
                # Update peak equity for both cycle and day
            ...
            if symbol in open_positions:
                ...
                if exit_price:
                    equity += pnl
                    day_peak_equity = max(day_peak_equity, equity) # Update daily peak immediately after trade
            ...
            if circuit_breaker_tripped:
                day_peak_equity = max(day_peak_equity, equity) # Still track equity for DD reporting
                continue
        ...
        # Finalize the very last day of the cycle
        finalize_day_metrics(current_day, equity)
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    logger.info(f"Walk-forward analysis will run for {total_cycles} cycles with a retraining frequency of {config.RETRAINING_FREQUENCY}.")
    ...
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        ...
        # Update retraining frequency for the next iteration if AI changes it
        retrain_freq = pd.to_timedelta(config.RETRAINING_FREQUENCY)
        ...
        if 'selected_features' in config.model_dump():
            original_features = config.selected_features
            sanitized_list = [f for f in original_features if f in all_available_features]
            if len(sanitized_list) != len(original_features):
                logger.warning(f"  - Sanitizing feature list. Removed {len(original_features) - len(sanitized_list)} invalid features suggested by AI.")
                config.selected_features = sanitized_list
        ...
        if breaker_tripped:
            ...
            if failed_cycles_in_a_row >= 2:
                ...
                if intervention_suggestion and 'selected_features' in intervention_suggestion:
                    original_count = len(intervention_suggestion['selected_features'])
                    sanitized_features = [f for f in intervention_suggestion['selected_features'] if f in all_available_features]
                    if len(sanitized_features) != original_count:
                         logger.warning(f"  - AI (intervention) suggested invalid features. Sanitized list from {original_count} to {len(sanitized_features)} features.")
                    intervention_suggestion['selected_features'] = sanitized_features
        ...
        suggested_params = api_timer.call(gemini_analyzer.analyze_cycle_and_suggest_changes, in_run_historical_cycles, all_available_features, strategy_details, daily_dd_report)
        if suggested_params:
            logger.info(f"  - AI suggests updating params: {suggested_params}")

            if 'selected_features' in suggested_params:
                original_count = len(suggested_params['selected_features'])
                sanitized_features = [f for f in suggested_params['selected_features'] if f in all_available_features]
                if len(sanitized_features) != original_count:
                     logger.warning(f"  - AI suggested invalid features. Sanitized list from {original_count} to {len(sanitized_features)} features.")
                suggested_params['selected_features'] = sanitized_features
