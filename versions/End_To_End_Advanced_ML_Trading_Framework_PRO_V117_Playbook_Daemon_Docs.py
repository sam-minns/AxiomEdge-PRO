# End_To_End_Advanced_ML_Trading_Framework_PRO_V117_Playbook_Daemon_Docs
#
# V117 UPDATE (Daemon Docs):
# 1. DOCS - DAEMON CONTROLS: Added detailed comments to the daemon control
#    variables (`CONTINUOUS_RUN_HOURS` and `MAX_RUNS`) to clarify how the
#    timing works and to confirm that a run, once started, always completes.
#
# V116 UPDATE (Daemon Interrupt):
# 1. NEW - INTERRUPTIBLE COUNTDOWN: After a run completes in daemon mode, a
#    10-second countdown now allows the user to gracefully stop the script
#    with Ctrl+C before the next run begins.
...
# Looks for a .env file in the script's directory and loads it.
...
def setup_logging() -> logging.Logger:
    ...
    # Prevent duplicate handlers if re-running in a notebook
...
class GeminiAnalyzer:
    def __init__(self):
        ...
        # Check if the key from .env is invalid or missing
        if not api_key or "YOUR" in api_key or "PASTE" in api_key:
            ...
            try:
                ...
                self.api_key_valid = False
                api_key = None # Ensure api_key is None
        else:
        ...
        if self.api_key_valid:
            ...
            # Set dummy values if not valid to prevent other errors
            self.api_url = ""
            self.headers = {}
...
def run_single_instance(fallback_config: Dict, framework_history: Dict):
    ...
    strategy_playbook = {
        ...
        "Momentum": {
            "description": "Capitalizes on short-term price momentum.",
            "features": list(set(ModelTrainer.MOMENTUM_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (30, 90), "dd_range": (0.18, 0.30)
        },
        "RangeBound": {
            "description": "Trades within established ranges, using oscillators and trend-absence (low ADX).",
            "features": list(set(ModelTrainer.RANGE_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (20, 60), "dd_range": (0.10, 0.20)
        },
        "Seasonality": {
            "description": "Leverages recurring seasonal patterns or calendar effects.",
            "features": list(set(ModelTrainer.SEASONALITY_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (50, 120), "dd_range": (0.15, 0.28)
        },
        "PriceAction": {
            "description": "Trades based on the statistical outcomes of historical candlestick formations.",
            "features": list(set(ModelTrainer.PRICE_ACTION_FEATURES + ModelTrainer.SESSION_FEATURES)),
            "lookahead_range": (20, 80), "dd_range": (0.10, 0.25)
        }
    }
    ...
    # Pre-flight check to determine starting strategy and parameters
    ai_config_suggestion = gemini_analyzer.get_pre_flight_config(framework_history, strategy_playbook, current_config, exploration_rate=0.25)
    current_config.update(ai_config_suggestion)

    # Ensure selected_features from AI are valid
    chosen_strategy_features = strategy_playbook.get(current_config['strategy_name'], {}).get('features', fallback_config['selected_features'])
    ...
    if not current_config['selected_features']:
        current_config['selected_features'] = chosen_strategy_features[:5] # Fallback to first 5
        logger.warning("AI suggested features not valid for the chosen strategy. Falling back to a default subset.")
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        cycle_results_for_ai={
            "cycle":i+1, "strategy_name": config.strategy_name, "objective_score":trainer.study.best_value if trainer.study else 0,
        ...
        if suggested_params:
            ...
            logger.info("  - Respecting API rate limits (5-minute delay between cycles)..."); time.sleep(300)
...
def main():
    ...
    # Set to > 0 to run continuously for a set duration.
    # The timer starts when the script launches and measures real-world time,
    # including all processing and inter-cycle wait times (e.g., the 5-minute delays).
    # A run, once started, will ALWAYS complete. The time limit is only checked
    # AFTER a full run finishes. If the total elapsed time has exceeded the limit,
    # the script will stop.
    CONTINUOUS_RUN_HOURS = 0
    # Set to > 1 to run for a specific number of iterations. This provides a
    # more predictable way to control the number of complete end-to-end runs.
    MAX_RUNS = 3
    # --- End Controls ---

    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V117_Playbook_Daemon_Docs",
        "strategy_name": "TrendFollower", # Default strategy if memory is empty
        ...
        "selected_features": [] # Will be populated by playbook
    }
    ...
    while True:
        ...
        # Load memory on each iteration to get the latest champion
        ...
        # A single run of the entire backtest and learning process. The report is generated inside this function.
        run_single_instance(fallback_config, framework_history)
        ...
        # Check daemon exit conditions before the countdown
        ...
        # NEW: Interruptible countdown timer
        try:
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V117_Playbook_Daemon_Docs.py