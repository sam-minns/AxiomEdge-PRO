# End_To_End_Advanced_ML_Trading_Framework_PRO_V188_MicroDynamics.py
#
# V188 UPDATE (Micro-Dynamic Feature Suite):
# 1.  ADDED: Indicator Dynamics. The framework now calculates the recent slope of
#     key oscillators (RSI, Momentum) to measure the speed and direction of
#     their change, providing a leading signal.
# 2.  ADDED: Relative Performance. A new feature constantly measures each asset's
#     performance against the average of all other assets in the dataset,
#     helping to identify true market leaders and laggards in real-time.
# 3.  ADDED: Candle Microstructure Analysis. The script now analyzes the internal
#     structure of each candle, calculating features like the ratio of wick size
#     to body size to provide insight into market indecision and turning points.
# 4.  ENHANCED: The initial AI prompt has been updated to explicitly mention these
#     new categories of "dynamic" and "microstructure" features, encouraging the
#     AI to experiment with them when building and refining strategies.
#
# V187 UPDATE (Tiered Risk Management Integration):
# 1.  ADDED: Tiered Risk Management System.
# 2.  ADDED: AI-Driven Risk Profile Selection.
# 3.  ADDED: `USE_TIERED_RISK` boolean flag.
# 4.  REFINED: Backtester logic updated for the tiered system.
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        # V188 - Updated feature selection prompt
        dynamic_feature_prompt = (
            "**FEATURE SELECTION GUIDANCE:**\n"
            "When selecting features, consider the new **'micro-dynamic'** features. These can provide deeper market insight:\n"
            "- **Indicator Dynamics**: `RSI_slope`, `momentum_10_slope` (captures the rate of change).\n"
            "- **Relative Performance**: `relative_performance` (identifies market leaders/laggards).\n"
            "- **Candle Microstructure**: `wick_to_body_ratio` (shows market indecision).\n"
            "Experiment with these to find a sophisticated edge.\n"
        )
...
class FeatureEngineer:
    ...
    def _calculate_candle_microstructure(self, g: pd.DataFrame) -> pd.DataFrame:
        """V188: Calculates features based on the candle's internal structure."""
    ...
    def _calculate_indicator_dynamics(self, g: pd.DataFrame, period: int = 5) -> pd.DataFrame:
        """V188: Calculates the recent slope of indicators."""
        # Define a slope calculation function for rolling apply
        def get_slope(series):
            # Check if the series is valid for slope calculation
        ...
        # Calculate slope for RSI and momentum
    ...
    def _calculate_base_tf_native(self, g:pd.DataFrame)->pd.DataFrame:
        ...
        g_out = self._calculate_candle_microstructure(g_out) # V188
        g_out = self._calculate_indicator_dynamics(g_out)   # V188
    ...
    def _calculate_relative_performance(self, df: pd.DataFrame) -> pd.DataFrame:
        """V188: Calculates relative performance against the market average."""
        if 'pct_change' not in df.columns:
            logger.warning("  - 'pct_change' not found, cannot calculate relative performance.")
            return df
        # Calculate the mean pct_change across all assets for each timestamp
        # This requires the dataframe index to be unique per timestamp, which it is at this stage.
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # V188: Calculate relative performance
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V188_MicroDynamics",
