# End_To_End_Advanced_ML_Trading_Framework_PRO_V95.5_Legacy_API_Fix
#
# V95.5 UPDATE (Forced Legacy API for Robustness):
# 1. ROBUSTNESS: Reverted the early stopping mechanism to exclusively use the legacy
#    `early_stopping_rounds` parameter in the `model.fit()` call.
# 2. RATIONALE: This change is a direct response to persistent, environment-specific
#    TypeErrors where the `callbacks` argument was rejected despite a modern
#    XGBoost version being reported. Using `early_stopping_rounds` is a more
#    universally compatible approach that guarantees functionality across all
#    environments, sidestepping the installation-specific issue.
# 3. CLEANUP: Removed the now-unnecessary dynamic version checking, `EarlyStopping`
#    callback import, and the `packaging` library dependency to simplify the code.
#
# V95.4 UPDATE (Critical Fix & Refinements):
# 1. CRITICAL FIX: Corrected the data splitting logic (`train_test_split`) by
#    removing the `stratify` parameter, which is incompatible with `shuffle=False`
#    and essential for maintaining chronological order in time-series data.
# 2. API UPGRADE: Updated the Gemini API endpoint to use 'gemini-2.0-flash'.
# 3. ROBUSTNESS: Improved API key handling by prioritizing `os.getenv`.
...
# Note: EarlyStopping and version are no longer needed.
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        def objective(trial:optuna.Trial):
            ...
            try:
                ...
                # ================================================================
                # *** FORCED LEGACY API FOR ROBUSTNESS (V95.5) ***
                # ================================================================
                # Exclusively using `early_stopping_rounds` to ensure maximum
                # compatibility across all environments and avoid the
                # persistent `callbacks` TypeError.
                if trial.number == 0:
                    logger.info("    - Using legacy `early_stopping_rounds` for maximum compatibility.")
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V95_Legacy_API_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V95.5_Legacy_API_Fix.py