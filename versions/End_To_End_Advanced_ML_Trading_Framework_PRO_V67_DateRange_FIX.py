# End_To_End_Advanced_ML_Trading_Framework_PRO_V67_DateRange_FIX
#
# V67 Update:
# 1. Fixed TypeError when creating the retraining_dates range.
# 2. Modified the call to df_featured.index.max() to correctly handle the
#    tuple output and select only the timestamp component, ensuring a valid
#    end date is passed to the date_range function.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V67_DateRange_FIX",
...
class FeatureEngineer:
    ...
    def _label_group(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        hit_times = pd.DataFrame(index=group.index)
        
        # Long targets
        long_tp_hit = (group['High'].rolling(lookahead).max().shift(-lookahead) >= (group['Close'] + tp_barrier))
        long_sl_hit = (group['Low'].rolling(lookahead).min().shift(-lookahead) <= (group['Close'] - sl_barrier))
        
        # Short targets
        short_tp_hit = (group['Low'].rolling(lookahead).min().shift(-lookahead) <= (group['Close'] - tp_barrier))
        short_sl_hit = (group['High'].rolling(lookahead).max().shift(-lookahead) >= (group['Close'] + sl_barrier))
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V62_AutoBNN_Fix.py