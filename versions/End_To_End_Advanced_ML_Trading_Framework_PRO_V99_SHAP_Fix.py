# End_To_End_Advanced_ML_Trading_Framework_PRO_V99_SHAP_Fix
#
# V99 UPDATE (SHAP Fix for Multi-Class):
# 1. CRITICAL FIX: Resolved the `ValueError` during SHAP summary generation. The
#    error was caused by an incorrect aggregation of SHAP values for the multi-class
#    (Buy/Sell/Hold) model.
# 2. ROBUST CALCULATION: The logic in `_generate_shap_summary` has been updated to
#    correctly handle the list of arrays returned by the SHAP explainer for
#    multi-class problems. It now properly averages the feature importances across
#    all classes to produce a single, reliable importance score per feature.
#
# V98 UPDATE (Major Architecture Overhaul):
# 1. Reverted to the complex V93 architecture with Multi-Timeframe data, a 3-class
#    model, risk-adjusted optimization, and full SHAP reporting to ensure a
#    thorough and meaningful backtest.
...
class ModelTrainer:
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: pd.Index):
        ...
        try:
            ...
            # CRITICAL FIX V99: The method for aggregating multi-class SHAP values is corrected here.
            explainer = shap.TreeExplainer(model)
            shap_values = explainer.shap_values(X_sample)

            # For multi-class, shap_values is a list of arrays (one per class).
            # We calculate the mean absolute value for each feature across all classes.
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V99_SHAP_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V99_SHAP_Fix.py