# End_To_End_Advanced_ML_Trading_Framework_PRO_V179_Backtester_FIX.py
#
# V179 UPDATE (Critical Backtester Fix):
# 1. FIXED: A critical bug in the backtester where price data from one instrument
#    was incorrectly used to evaluate trades on other instruments. The loop has
#    been restructured to ensure each candle's data is only used for its
#    corresponding symbol, preventing data cross-contamination and restoring
#    accurate PNL calculation.
# 2. INTEGRATED: Newer features like partial profit-taking and logging controls
#    have been correctly integrated into the robust, symbol-specific backtesting
#    loop.
# 3. IMPROVED: The `PerformanceAnalyzer` correctly distinguishes between total trade
#    events (including partial closes) and final, completed trades, ensuring
#    clearer and more accurate reporting.
#
# V178 UPDATE (Robustness & Logging Control):
# 1. FIXED: A critical ValueError when the AI suggested month-based retraining
#    frequencies (e.g., '2M'). The script now correctly handles calendar
#    offsets, preventing crashes.
# 2. IMPLEMENTED: Added a global switch `LOG_ANOMALY_SKIPS` (default: False)
#    to control the logging of skipped trades due to anomaly detection.
# 3. IMPLEMENTED: Added a global switch `LOG_PARTIAL_PROFITS` (default: True)
#    to control the logging of partial profit-taking events.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    # V178 FIX: Use the frequency string for date_range and create an offset for calculations
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        # V178 FIX: Update frequency check logic
        retrain_freq_str = _sanitize_frequency_string(config.RETRAINING_FREQUENCY)
        ...
        # V178 FIX: Use offset for calendar math
        retrain_offset = pd.tseries.frequencies.to_offset(retrain_freq_str)
        ...
        if not trades.empty:
            ...
            # V178 FIX: Calculate MC days from offset
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V179_Backtester_FIX",
