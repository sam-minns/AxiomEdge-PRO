# End_To_End_Advanced_ML_Trading_Framework_PRO_V75_Mergefix
#
# V75 Update:
# 1. FIXED - Critical ValueError: Corrected the `ValueError: right keys must be sorted`
#    by explicitly sorting the higher-timeframe (H1, D1) DataFrames by timestamp
#    before passing them to the `pd.merge_asof` function.
# 2. All features and logic from V74 (Inter-Market, Session-Based, KeyError Hotfix) are retained.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V75_Mergefix",
    ...
    def load_and_parse_data(self) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        for filename in self.config.DATA_FILENAMES:
            ...
            if not os.path.exists(file_path):
                logger.warning(f"  - File not found, skipping: {file_path}"); continue
            try:
        ...
        for tf, dfs in data_by_tf.items():
            if dfs:
                ...
                logger.warning(f"  - No data found for {tf} timeframe."); processed_dfs[tf] = pd.DataFrame()
        if not processed_dfs or any(df.empty for df in processed_dfs.values()):
            logger.critical("  - Data loading failed. No data to process."); return None
...
class FeatureEngineer:
    ...
    def _calculate_stochastic(self, g: pd.DataFrame, period: int) -> pd.DataFrame:
        ...
        g['stoch_d'] = g['stoch_k'].rolling(window=3).mean(); return g
    ...
    def _calculate_cross_pair_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_out = df.copy()
        df_unique = df_out.reset_index().drop_duplicates(subset=['Timestamp', 'Symbol']).set_index('Timestamp')
        ...
        df_out = pd.merge(df_out.reset_index(), rel_strength_unpivoted, on=['Timestamp', 'Symbol'], how='left')
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if any(df.empty for df in data_by_tf.values()):
            logger.critical("  - One or more timeframes have no data."); return pd.DataFrame()
        ...
        # --- FIX: Sort right-hand dataframes by Timestamp before merging ---
        df_h1_sorted = df_h1.sort_values('Timestamp')
        df_d1_sorted = df_d1.sort_values('Timestamp')
    ...
    def _calculate_htf_features(self, df: pd.DataFrame, p: str, s: int, a: int) -> pd.DataFrame:
        ...
        for symbol, group in df.groupby('Symbol'):
            g = group.copy()
            sma = g['Close'].rolling(s, min_periods=s).mean(); atr = (g['High'] - g['Low']).rolling(a, min_periods=a).mean()
            trend = np.sign(g['Close'] - sma)
            temp_df = pd.DataFrame(index=g.index)
            temp_df[f'{p}_ctx_{p}_SMA'] = sma; temp_df[f'{p}_ctx_{p}_ATR'] = atr; temp_df[f'{p}_ctx_{p}_Trend'] = trend
    ...
    def _calculate_m15_native(self, g: pd.DataFrame) -> pd.DataFrame:
        logger.info(f"    - Calculating native M15 features for {g['Symbol'].iloc[0]}..."); g_out = g.copy()
        lookback, rank_period, sma_period = 14, 100, 50
        g_out['ATR'] = (g['High'] - g['Low']).rolling(lookback).mean()
        delta = g['Close'].diff(); gain = delta.where(delta > 0, 0).ewm(com=lookback - 1, adjust=False).mean()
    ...
    def label_outcomes(self, df: pd.DataFrame, sl_atr: float, tp_atr: float, lookahead: int) -> pd.DataFrame:
        ...
        labeled_dfs = [self._label_with_dynamic_barriers(group, sl_atr, tp_atr, lookahead) for _, group in df.groupby('Symbol')]
...
class ModelTrainer:
    ...
    def __init__(self, config: ConfigModel):
        self.config = config; self.shap_summary: Optional[pd.DataFrame] = None; self.class_weights: Optional[Dict[int, float]] = None
    def train(self, df_train: pd.DataFrame) -> Optional[Pipeline]:
        ...
        if min_ratio < self.config.MIN_CLASS_RATIO or class_counts.min() < self.config.MIN_CLASS_SAMPLES:
            logger.info(f"    - Classes are imbalanced. Applying RandomOverSampler."); X, y = RandomOverSampler(random_state=42).fit_resample(X, y)
    ...
    def _optimize_hyperparameters(self, X_train, y_train, X_val, y_val) -> Optional[optuna.study.Study]:
        ...
        try:
            study = optuna.create_study(direction='maximize'); study.optimize(objective, n_trials=self.config.OPTUNA_TRIALS, timeout=1200, show_progress_bar=True); return study
...
if __name__ == '__main__':
    ...
    # Paste the full main() function here from V74
    main()

# End_To_End_Advanced_ML_Trading_Framework_PRO_V75_Mergefix.py