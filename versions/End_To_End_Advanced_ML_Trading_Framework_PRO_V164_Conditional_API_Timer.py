# End_To_End_Advanced_ML_Trading_Framework_PRO_V164_Conditional_API_Timer.py
#
# V164 UPDATE (Conditional API Timer):
# 1. UPDATED - The 5-minute dynamic API timer is now conditional. It is disabled
#    (0-second delay) for default single runs and is only enabled for
#    multi-run (MAX_RUNS > 1) or continuous daemon modes.
#
# V163 UPDATE (Dynamic API Timer & Bugfix):
# 1. UPDATED - Implemented a new APITimer class that ensures a minimum of 5 minutes
#    elapses between the start of each API request by dynamically calculating and
#    waiting for the remaining time.
# 2. FIXED - ValueError: Corrected a crash caused by the AI suggesting non-numeric
#    features (e.g., 'DATE'). The script now sanitizes the AI's feature list
#    against available numeric features before use.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        ...
        # --- Sanitize AI-provided feature list BEFORE training ---
        ...
        if breaker_tripped:
            ...
            if failed_cycles_in_a_row >= 2:
                ...
                # Sanitize AI features for intervention
        ...
        suggested_params = api_timer.call(gemini_analyzer.analyze_cycle_and_suggest_changes, in_run_historical_cycles, all_available_features, strategy_details)
        if suggested_params:
            logger.info(f"  - AI suggests updating params: {suggested_params}")

            # Sanitize AI features for cycle update
