# End_To_End_Advanced_ML_Trading_Framework_PRO_V196_Linux.py
#
# V196 UPDATE (MQL5 Realism & Backtesting Integrity):
#   1. FIXED (Lookahead Bias): Overhauled the Backtester's trade execution logic. It now correctly uses the Open price of the current bar for entry and calculates all trade parameters (ATR, SL, TP) using data from the *previous* bar, eliminating lookahead bias and aligning with MQL5's execution model.
#   2. ENHANCED (Realistic Cost Modeling): Spread is now applied directly to the entry price to accurately simulate crossing the bid-ask spread (e.g., buying at the higher 'ask'). This provides a more realistic P&L calculation from the trade's inception.
#   3. ADDED (Broker & Margin Simulation): The framework now simulates critical broker constraints. New configuration parameters were added: `LEVERAGE` (defaulting to a realistic 30), `CONTRACT_SIZE`, `MIN_LOT_SIZE`, and `LOT_STEP`. The backtester now calculates and validates lot sizes and checks for sufficient free margin before initiating a trade.
#   4. ENHANCED (Conservative Bar Simulation): The exit logic was confirmed to be conservative, prioritizing stop-loss checks over take-profit within the same bar to provide a more robust performance estimate. Added comments to clarify this "worst-case" approach.
#
# V195 UPDATE (Optimization & AI Resilience):
#   1. IMPROVED (Hyperparameter Optimization): Reworked the custom_objective function to be more robust. The Sortino ratio calculation was corrected to prevent unfairly negative scores, ensuring the model's quality is evaluated more accurately and preventing unnecessary training failures. The objective weights were also rebalanced to prioritize classification accuracy (F1-score) while still rewarding a proxy for profitability.
#   2. ENHANCED (AI Failure Analysis): The AI prompt for handling model training failures (TRAINING_FAILURE state) has been significantly improved. It now provides clearer, hierarchical instructions, guiding the AI to prioritize feature set simplification as the primary recovery strategy, leading to more logical and effective adaptation.
#   3. ADDED (Regime Diagnosis): The framework now performs a quantitative analysis of market data(ADX, trending percentage) at startup to diagnose the current market regime (e.g., "Strong Trending", "Ranging").
#   4. ADDED (Regime Champions): The framework now maintains a regime_champions.json file. It stores the best-performing strategy configuration for each diagnosed regime from past runs.
#   5. ENHANCED (Initial Setup Prompt): The AI is now informed of the current market regime and any existing champion for it. It can choose to deploy the proven champion for safety or explore a new strategy if the context warrants it.
#   6. ADDED (Dynamic "Anchor" Intervention): When a strategy is quarantined, the AI can now choose to revert to the run's own most successful configuration as a safe "anchor" point, rather than always exploring something new.
#   5. FIXED (Persistence & Startup): Corrected bugs related to the new regime champion feature, ensuring the file is properly used at startup and updated at the end of a run.
...
class GeminiAnalyzer:
    def __init__(self):
        ...
        # --- FIX: Restored interactive `input()` from V193 to be more user-friendly on desktop.
        # It's wrapped in a try/except to prevent crashes in non-interactive environments (e.g., Docker).
    ...
    # --- FIX: Updated signature to match the V195 logic.
    ...
    def analyze_cycle_and_suggest_changes(
        ...
        cycle_status: str,
        shap_feedback: Dict[str, List[str]], # NEW: SHAP value feedback
        exit_analysis: Dict[str, int],      # NEW: Exit reason analysis
        volatility_analysis: Dict[str, float],# NEW: Volatility performance analysis
        strategic_forecast: Optional[Dict] = None
    ) -> Dict:
        if not self.api_key_valid: return {}

        base_prompt_intro = "You are an expert trading model analyst. Your primary goal is to create a STABLE and PROFITABLE strategy by making intelligent, data-driven changes, using the granular analytical feedback provided."
        ...
        # --- NEW: Build a dynamic prompt based on the new analytics ---
        analytical_insights_prompt = [
            "--- GRANULAR PERFORMANCE ANALYSIS ---"
        ]
        # 1. SHAP Feedback
        if shap_feedback.get("top_features"):
            analytical_insights_prompt.append(
                f"- **FEATURE IMPORTANCE (SHAP)**: The most predictive features in the last successful model were: `{shap_feedback['top_features']}`. The least predictive were: `{shap_feedback['bottom_features']}`. Your new feature set should heavily consider keeping the top features and removing the bottom ones to reduce noise."
            )
        # 2. Exit Reason Analysis
        if exit_analysis:
            sl_hits = exit_analysis.get("Stop Loss", 0)
            tp_hits = exit_analysis.get("Take Profit", 0)
            total_exits = sl_hits + tp_hits
            if total_exits > 0:
                sl_ratio = sl_hits / total_exits
                if sl_ratio > 0.7:
                    analytical_insights_prompt.append(
                        f"- **EXIT ANALYSIS**: CRITICAL! The strategy is hitting its Stop Loss {sl_ratio:.0%} of the time. This suggests the risk parameters or entry logic are poorly calibrated for the current market. You should propose changes to make the strategy less sensitive to short-term volatility, possibly by adjusting `CONFIDENCE_TIERS` or selecting more stable features."
                    )
        # 3. Volatility Performance Analysis
        if volatility_analysis:
            high_vol_pnl = volatility_analysis.get('High', 0.0)
            if high_vol_pnl < 0:
                analytical_insights_prompt.append(
                    f"- **VOLATILITY ANALYSIS**: The strategy lost ${abs(high_vol_pnl):.2f} during high volatility periods. It is not handling volatility spikes well. Propose changes to either A) avoid these periods by lowering `MAX_VOLATILITY_RANK` or B) change the feature set to one that can better navigate volatile conditions."
                )

        if len(analytical_insights_prompt) == 1:
            analytical_insights_prompt.append("- No specific performance anomalies detected in the last cycle.")

        analytical_insights_str = "\n".join(analytical_insights_prompt)
        # --- END NEW ---

        # --- IMPROVEMENT: Split logic for TRAINING_FAILURE and PROBATION for clearer AI instructions ---
        if cycle_status == "TRAINING_FAILURE":
            failure_intro = (
                "**CRITICAL: MODEL TRAINING FAILURE!**\n"
                "The model failed the quality gate, meaning its predictive power on validation data was too low. "
                "This is often caused by a feature set that is too noisy or a strategy that is not suited for the current market training window."
            )
            task_guidance = (
                "**YOUR PRIMARY GOAL:** Propose a change that INCREASES model simplicity and signal quality.\n"
                "1.  **FEATURE SET:** Your first instinct should be to **reduce the number of features**. Look at the SHAP feedback if available. Create a smaller, more potent feature set (3-5 features) using only the top-performing ones.\n"
                "2.  **STRATEGY:** If feature reduction has been tried, consider if the fundamental strategy (e.g., 'RangeBound') is wrong for the data. Consult your pre-run forecast if you have one.\n"
                "3.  **PARAMETERS:** Avoid increasing `OPTUNA_TRIALS` as a solution. Instead, focus on a better feature set."
            )
            if strategic_forecast:
                prompt_details = (
                    f"{failure_intro}\n\n"
                    f"{analytical_insights_str}\n\n"
                    f"**YOUR PRE-RUN FORECAST & GAME PLAN:**\n"
                    f"- **Anticipated Risk**: `{strategic_forecast.get('pre_mortem_analysis', 'N/A')}`\n"
                    f"- **Contingency B (New Strategy)**: `{strategic_forecast.get('contingency_path_b', {}).get('strategy_name', 'N/A')}`.\n"
                    f"- **Contingency C (Tweak Params)**: `{json.dumps(strategic_forecast.get('contingency_path_c', {}))}`.\n\n"
                    f"**YOUR TASK: Decide your next move.**\n{task_guidance}"
                )
            else:
                 prompt_details = (
                    f"{failure_intro}\n\n"
                    f"{analytical_insights_str}\n\n"
                    f"**YOUR TASK: Propose a fix.**\n{task_guidance}"
                 )
        elif cycle_status == "PROBATION":
            prompt_details = (
                "**STRATEGY ON PROBATION**\n"
                "The previous cycle hit its drawdown limit. Your **only goal** is to increase stability and **REDUCE RISK**. Profitability is secondary.\n\n"
                f"{analytical_insights_str}\n\n"
                "**YOUR TASK (Safety First - Drastic Risk Reduction):**\n"
                "1.  **You MUST suggest changes that lower risk based on the analysis above.** This is not optional.\n"
                "2.  **Lower risk by:** reducing `MAX_DD_PER_CYCLE` (must be between 0.05-0.15), and reducing `MAX_CONCURRENT_TRADES`.\n"
            ...
            prompt_details = (
                ...
                "Review the recent cycle performance and the new granular analysis to suggest a new configuration. Prioritize **SIMPLICITY** and **ROBUSTNESS**.\n\n"
                f"{analytical_insights_str}\n\n"
                "**YOUR TASK:**\n"
                "Suggest a new configuration. Use the analysis to make targeted changes.\n"
                "- Is the `selected_features` list too large? Does the SHAP feedback suggest a smaller, more focused set?\n"
                "- Does the Exit Analysis suggest your risk parameters (`CONFIDENCE_TIERS`) are wrong?\n"
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        # --- IMPROVEMENT: Corrected objective function to be more robust and provide better scores ---
        def custom_objective(trial:optuna.Trial):
            ...
            try:
                ...
                # --- FIX: Calculate Sortino Ratio more robustly ---
                if mean_pnl > 0:
                    downside_returns = pnl[pnl < 0]
                    # Use Root Mean Squared of negative PNLs for the denominator for stability
                    downside_std = np.sqrt((downside_returns**2).mean())
                    sortino = (mean_pnl / downside_std) if downside_std > 0 else mean_pnl
                else:
                    # If the mean PNL is not positive, the risk-adjusted return is zero for our objective.
                    sortino = 0

                # Objective: 60% F1-score (classification quality) and 40% Sortino (risk-adjusted profit proxy)
                objective_score = (0.6 * f1) + (0.4 * sortino)

                # --- Stability Check (Penalize models that fail on hold-out stability set) ---
                ...
                if stability_pnl.sum() < 0:
                    # Apply a significant penalty if the model is unprofitable on the stability set
...
class Backtester:
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, is_meta_model: bool) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        # --- V196 CHANGE: Convert dataframe to list of dicts for indexed access ---
        ...
        # --- V196 CHANGE: Loop starts from index 1 to allow safe access to `prev_candle` ---
        for i in range(1, len(candles)):
            current_candle = candles[i]
            prev_candle = candles[i-1] # Data from previous candle is used for decisions
            ...
            if symbol in open_positions:
                ...
                # --- V196 ENHANCEMENT: Conservative exit logic (SL check first) ---
                # This logic ensures that if a bar's High/Low hits both SL and TP,
                # the stop-loss is triggered, providing a more robust, worst-case simulation.
                ...
                elif pos['direction'] == -1: # Short position
            ...
            if not circuit_breaker_tripped and symbol not in open_positions and len(open_positions) < max_concurrent_trades:
                ...
                if prev_candle.get('anomaly_score') == -1:
                    if LOG_ANOMALY_SKIPS and random.random() < 0.1:
                         logger.info(f"  - Skipping trade check for {symbol} due to anomaly detection at {current_candle['Timestamp']}")
                    continue
                ...
                # --- V196 CHANGE: Model predictions are on the current candle, which is correct as they are based on features from prev_candle ---
                if self.is_meta_model:
                ...
                if direction != 0:
                    # --- V196 CHANGE: Use PREVIOUS candle's ATR to avoid lookahead bias ---
                    ...
                    # --- V196 CHANGE: Lot size calculation and margin check ---
                    lots = risk_amt / (sl_dist * self.config.CONTRACT_SIZE)
                    # Adhere to broker lot rules
                    ...
                    if free_margin < margin_required: continue

                    # --- V196 CHANGE: Enter at Open of CURRENT candle and apply spread ---
                    entry_price_raw = current_candle['Open']
                    spread_in_price = atr * self.config.SPREAD_PCTG_OF_ATR
                    entry_price = entry_price_raw + (spread_in_price * direction)
                    ...
                    open_positions[symbol]={
                        ...
                        'lot_size': lots, 'margin_used': margin_required # Store broker info
                    }
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict, diagnosed_regime: str) -> Optional[Dict]:
    ...
    is_new_overall_champion = (current_champion is None or
    ...
    if is_new_overall_champion:
        ...
        logger.info(f"Current run's MAR Ratio ({new_mar:.2f}) did not beat overall champion's ({champ_mar_val:.2f}).")
    ...
    # --- FIX: START: Add logic for Regime Champion persistence ---
    regime_champions = {}
    ...
    is_new_regime_champion = (current_regime_champion is None or
                             (new_mar is not None and new_mar > current_regime_champion.get("final_metrics", {}).get("mar_ratio", -np.inf)))

    if is_new_regime_champion:
        regime_champions[diagnosed_regime] = new_run_summary
        current_regime_mar = current_regime_champion.get("final_metrics", {}).get("mar_ratio", -np.inf) if current_regime_champion else -np.inf
        logger.info(f"NEW REGIME CHAMPION for '{diagnosed_regime}'! MAR Ratio ({new_mar:.2f}) beats previous ({current_regime_mar:.2f}).")
        try:
        ...
        logger.info(f"Run did not become the new champion for the '{diagnosed_regime}' regime.")
    # --- FIX: END ---
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    diagnosed_regime = "Ranging"
    try:
        if 'asset_statistics' in data_summary and data_summary['asset_statistics']:
            avg_adx = np.mean([stats['avg_adx'] for stats in data_summary['asset_statistics'].values()])
            avg_trending_pct = np.mean([float(stats['trending_pct'].strip('%')) for stats in data_summary['asset_statistics'].values()])
            if avg_adx > 25 and avg_trending_pct > 60: diagnosed_regime = "Strong Trending"
             ...
             logger.warning("Could not calculate quantitative regime metrics. Defaulting to 'Ranging'.")
    except (ValueError, TypeError) as e:
        logger.warning(f"Error calculating regime metrics: {e}. Defaulting to 'Ranging'.")

    regime_champions = {}
    regime_champions_path = temp_config.REGIME_CHAMPIONS_FILE_PATH
    if os.path.exists(regime_champions_path):
        try:
            with open(regime_champions_path, 'r') as f: regime_champions = json.load(f)
            logger.info(f"Successfully loaded regime champions from: {regime_champions_path}")
        except (json.JSONDecodeError, IOError) as e:
            logger.warning(f"Could not read or parse regime champions file at {regime_champions_path}: {e}")
    else:
        logger.info(f"Regime champions file not found at {regime_champions_path}. This file will be created if a new champion is found.")
    ...
    if not ai_setup:
        logger.critical("AI-driven setup failed. Using fallback configuration and exiting."); return
    ...
    if isinstance(ai_setup.get("nickname"), str) and ai_setup.get("nickname"):
        ...
        try:
            with open(bootstrap_config_for_path.NICKNAME_LEDGER_PATH, 'w') as f: json.dump(nickname_ledger, f, indent=4)
    ...
    if not config.selected_features and config.strategy_name in playbook:
        ...
        if config.selected_features:
            ...
            logger.critical(f"FATAL: AI did not provide features and no defaults exist for '{config.strategy_name}'."); return
    ...
    if retraining_dates.empty:
        logger.critical(f"Cannot proceed with backtest: No valid retraining dates could be determined. The data period is too short for the TRAINING_WINDOW '{config.TRAINING_WINDOW}'.")
        ...
        final_metrics = pa.generate_full_report(None, pd.Series([config.INITIAL_CAPITAL]), [], None, framework_history, None)
        run_summary.update({"run_end_ts": datetime.now().strftime("%Y%m%d-%H%M%S"), "final_metrics": final_metrics, "cycle_details": [{"Status": "ABORTED_NO_TEST_PERIOD"}]})
        save_run_to_memory(config, run_summary, framework_history, diagnosed_regime)
        logger.removeHandler(file_handler); file_handler.close(); return
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        train_end, test_end = period_start_date - forward_gap, period_start_date + retrain_offset
        ...
        if not check_label_quality(df_train_labeled, config.LABEL_MIN_EVENT_PCT):
            ...
            if not train_result or best_objective_score < MODEL_QUALITY_THRESHOLD:
                ...
                cycle_retry_count = 0
                last_successful_pipeline, last_successful_threshold, last_successful_features, last_successful_is_gnn = pipeline, threshold, feature_list_to_use, trainer.is_gnn_model
        ...
        if cycle_retry_count > 0:
            if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
                ...
                if last_successful_pipeline:
                    logger.warning("--> FALLING FORWARD: Using the model from the previous successful cycle to trade this period.")
                    pipeline, threshold, trainer = last_successful_pipeline, last_successful_threshold, ModelTrainer(config)
                    ...
                    cycle_retry_count, cycle_num = 0, cycle_num + 1
                    continue
            else:
                logger.info("  - Re-engaging AI for new parameters to retry this cycle...")
                # Pass empty dicts for now as no trades were made to generate analytics
                ai_suggestions = api_timer.call(gemini_analyzer.analyze_cycle_and_suggest_changes, in_run_historical_cycles, framework_history, all_available_features, config.model_dump(), "TRAINING_FAILURE", {}, {}, {})
                if ai_suggestions:
        ...
        if not trades.empty:
            ...
            temp_pa = PerformanceAnalyzer(config)
            cycle_pa_metrics = temp_pa._calculate_metrics(aggregated_trades, aggregated_equity_curve)
            run_configs_and_metrics.append({"final_params": config.model_dump(mode='json'), "final_metrics": cycle_pa_metrics})
        ...
        if breaker_tripped:
            if probationary_strategy == config.strategy_name:
                ...
                if consecutive_failures_on_probation >= 2:
                    ...
                    if intervention_suggestion and intervention_suggestion.get("action") == "revert" and personal_best_config:
                        logger.warning(f"  - AI chose to REVERT to this run's personal best strategy: {personal_best_config.get('final_params',{}).get('strategy_name')}")
                        config = ConfigModel(**personal_best_config['final_params'])
                    elif intervention_suggestion and intervention_suggestion.get("strategy_name") in playbook:
                        intervention_suggestion = _sanitize_ai_suggestions(intervention_suggestion)
                        logger.info(f"  - Strategic Intervention successful. AI chose to EXPLORE strategy: {intervention_suggestion['strategy_name']}")
                        config = ConfigModel(**{**config.model_dump(mode='json'), **intervention_suggestion})
                    else:
                        logger.error("  - Strategic intervention FAILED. Halting run to prevent further losses."); break
        ...
        # --- NEW: Granular Analytics for AI ---
        shap_feedback, exit_analysis, volatility_analysis = {}, {}, {}
        if not aggregated_shap.empty:
            shap_feedback['top_features'] = aggregated_shap.head(5).index.tolist()
            shap_feedback['bottom_features'] = aggregated_shap.tail(5).index.tolist()
        if not trades.empty:
            exit_analysis = trades[trades['ExitReason'].isin(['Stop Loss', 'Take Profit'])]['ExitReason'].value_counts().to_dict()
            trades_with_features = pd.merge_asof(trades.sort_values('ExecTime'), df_test.reset_index().sort_values('Timestamp'), left_on='ExecTime', right_on='Timestamp', by='Symbol')
            if 'market_volatility_index' in trades_with_features.columns:
                trades_with_features['vol_quantile'] = pd.cut(trades_with_features['market_volatility_index'], bins=[0, 0.2, 0.8, 1.0], labels=['Low', 'Medium', 'High'])
                volatility_analysis = trades_with_features.groupby('vol_quantile')['PNL'].sum().to_dict()
        # --- END NEW ---

        cycle_status_for_ai = "PROBATION" if probationary_strategy else "COMPLETED"
        suggested_params = api_timer.call(gemini_analyzer.analyze_cycle_and_suggest_changes, in_run_historical_cycles, framework_history, all_available_features, config.model_dump(), cycle_status_for_ai, shap_feedback, exit_analysis, volatility_analysis)
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V196_MQL5-Aligned",
