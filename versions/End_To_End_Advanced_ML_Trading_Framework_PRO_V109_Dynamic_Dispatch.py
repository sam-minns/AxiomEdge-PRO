# End_To_End_Advanced_ML_Trading_Framework_PRO_V109_Dynamic_Dispatch
#
# V109 UPDATE (Dynamic Strategy Dispatch):
# 1. NEW - DYNAMIC STRATEGY SELECTION: Implemented the user's suggestion for a fully
#    adaptive, hybrid system. The single pre-flight check is replaced. Now, at the
#    start of EACH walk-forward cycle, the AI analyzes the characteristics of the
#    upcoming training data.
# 2. NEW - MARKET FORECASTER AI: A new AI prompt asks the model to act as a market
#    regime classifier, selecting the most appropriate strategy (e.g., TrendFollower,
#    MeanReversion) from the playbook for the specific conditions of the next cycle.
# 3. ENHANCED REPORTING: The final report's 'Per-Cycle Performance' table now
#    includes a 'Strategy' column, making it easy to see which strategies were
#    deployed and how they performed under different market conditions.
#
# V108 UPDATE (Bug Fix):
# 1. Corrected a `NameError` that prevented the script from running.
...
class GeminiAnalyzer:
    ...
    def choose_strategy_for_cycle(self, train_data_summary: Dict, playbook: Dict, fallback_strategy: str) -> str:
        """Acts as a market forecaster to choose the best strategy for the upcoming cycle."""
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "Analyze the recent cycle history. If a 'Circuit Breaker' was tripped, you MUST analyze the `breaker_context` to understand the failure mode and make targeted suggestions.\n\n"
...
# ... (The rest of the script is largely the same, but with key modifications in PerformanceAnalyzer and main(). Full script provided below.)
...
class PerformanceAnalyzer:
    ...
    def plot_shap_summary(self,shap_summary:pd.DataFrame):
        ...
        plt.title(f"{self.config.REPORT_LABEL} ({self.config.strategy_name}) - Aggregated Feature Importance",fontsize=16,weight='bold')
    ...
    def _get_comparison_block(self, metrics: Dict, memory: Dict) -> str:
        ...
        previous_run = historical_runs[-1] if historical_runs else None # Get the actual last run
...
def main():
    # FIX V109: Initialize start_time here
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V109_Final",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V109_Final