# End_To_End_Advanced_ML_Trading_Framework_PRO_V69_Champion_Final
#
# V69 Update:
# 1. Implemented user-provided fix for the date_range TypeError by using
#    .get_level_values('Timestamp').max() to correctly handle the MultiIndex.
# 2. This script represents the final, stable, and best-performing model based on
#    the V55 architecture (Dynamic Confidence) and V47 feature set.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V69_Champion_Final",
...
class FeatureEngineer:
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # Add market regime based on ADX after it's calculated
        df_featured['market_regime'] = np.where(df_featured['ADX'] > self.config.TREND_FILTER_THRESHOLD, 1, 0)
        
        df_featured.set_index(['Symbol', 'Timestamp'], inplace=True)
    ...
    def _calculate_features(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        dm_plus = ((g['High'] - g['High'].shift()) > (g['Low'].shift() - g['Low'])).astype(int) * (g['High'] - g['High'].shift()).clip(lower=0)
        dm_minus = ((g['Low'].shift() - g['Low']) > (g['High'] - g['High'].shift())).astype(int) * (g['Low'].shift() - g['Low']).clip(lower=0)
        
        atr_adx = tr.ewm(alpha=1/14, adjust=False).mean()
        di_plus = 100 * (dm_plus.ewm(alpha=1/14, adjust=False).mean() / atr_adx)
        di_minus = 100 * (dm_minus.ewm(alpha=1/14, adjust=False).mean() / atr_adx)
    ...
    def _label_group(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        # Vectorized hit time calculation
        highs = group['High'].rolling(window=lookahead, min_periods=1).max().shift(-lookahead)
        lows = group['Low'].rolling(window=lookahead, min_periods=1).min().shift(-lookahead)
        
        long_tp_hit = (highs >= (group['Close'] + tp_barrier))
        long_sl_hit = (lows <= (group['Close'] - sl_barrier))
        
        short_tp_hit = (lows <= (group['Close'] - tp_barrier))
        short_sl_hit = (highs >= (group['Close'] + sl_barrier))
        
        group.loc[long_tp_hit & ~long_sl_hit, 'target'] = 1
        group.loc[short_tp_hit & ~short_sl_hit, 'target'] = -1
...
# =============================================================================
class ModelTrainer:
    FEATURES = ['hour', 'day_of_week', 'ATR', 'RSI', 'market_regime', 'ADX']
...
def main():
    ...
    # FIX: Correctly get the maximum timestamp from the MultiIndex
    max_date = df_featured.index.get_level_values('Timestamp').max()
    ...
    # Reset index to work with standard columns in the loop
    ...
    for i, period_start_date in enumerate(retraining_dates):
        ...
        df_train_raw = df_featured[(df_featured['Timestamp'] >= train_start) & (df_featured['Timestamp'] <= train_end)]
        df_test_chunk = df_featured[(df_featured['Timestamp'] >= period_start_date) & (df_featured['Timestamp'] <= test_end)]
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V69_Champion_Final.py