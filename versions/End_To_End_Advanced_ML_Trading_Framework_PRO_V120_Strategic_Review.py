# End_To_End_Advanced_ML_Trading_Framework_PRO_V120_Strategic_Review
#
...
# 3. NEW - CONTEXT-AWARE AI: This health report is passed to the AI, which is
#    instructed to penalize failing strategies and favor robust ones.
# 4. NEW - DEEPER MEMORY: Cycle-level details are now saved to the framework's
#    memory to enable these long-term historical calculations.
#
# V119 UPDATE (Hybrid Strategy Synthesis Engine):
# 1. NEW - DYNAMIC PLAYBOOK: The strategy playbook is now externalized to a
#    'strategy_playbook.json' file, seeded on the first run.
# 2. NEW - HYBRID STRATEGY CREATION: In daemon mode, the AI can now analyze its
#    history and synthesize new hybrid strategies, saving them to the playbook.
    ...
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, health_report: Dict, fallback_config: Dict, exploration_rate: float) -> Dict:
        ...
        if is_exploration:
            ...
            prompt = (
                ...
                "2. **CRITICAL: Review Strategic Health**: Review the `STRATEGIC HEALTH ANALYSIS`. This shows long-term problems. Heavily penalize strategies with high `ChronicFailureRate` or `CircuitBreakerFrequency`. A strategy with a good recent run but poor health is a risky choice.\n"
                "3. **Select a Strategy**: Choose the `strategy_name` from the playbook that you believe has the highest risk-adjusted potential for the *next* run, considering both recent performance and long-term health.\n"
                "4. **Define Parameters**: Define the initial parameters for the chosen strategy (`LOOKAHEAD_CANDLES`, `MAX_DD_PER_CYCLE`, `selected_features`, etc.).\n\n"
...
def check_and_create_hybrid(history: Dict, playbook: Dict, analyzer: GeminiAnalyzer, playbook_path: str) -> Optional[Dict]:
    ...
    successful_strategies = set(run["strategy_name"] for run in historical_runs if run.get("final_metrics", {}).get("calmar_ratio", 0) > 0.5)
...
def perform_strategic_review(history: Dict) -> Dict:
    ...
    strategy_names = set(run['strategy_name'] for run in historical_runs)

    for name in strategy_names:
        # Get the last 5 runs for this specific strategy
        strategy_runs = [run for run in historical_runs if run.get('strategy_name') == name][-5:]
        if not strategy_runs:
            continue

        # Chronic Failure Rate
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V120_Strategic_Review",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V120_Strategic_Review.py