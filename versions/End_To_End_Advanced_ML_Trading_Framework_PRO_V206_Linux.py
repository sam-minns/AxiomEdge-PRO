# End_To_End_Advanced_ML_Trading_Framework_PRO_V206_Linux.py
#
# V206 UPDATE (Realism, Robustness & Optimization):
#   1. ADDED (Simulated Latency): The Backtester now separates signal generation (candle N-1) from
#      trade execution (candle N), realistically modeling the delay in live environments and
#      invalidating strategies that rely on "free-look" intra-bar price action.
#   2. ADDED (Dynamic Spreads): The cost model now uses a symbol-specific dictionary of typical
#      spreads based on common broker data. Spreads automatically widen during high-volatility
#      periods (market_volatility_index > 0.8), better simulating real costs.
#   3. ADDED (Variable Slippage): Slippage is no longer a fixed percentage of ATR. It's now a
#      randomized, volatility-scaled factor, modeling the unpredictable nature of execution fills
#      during different market conditions.
#   4. ADDED (Pessimistic Exits): The backtester now conservatively assumes the Stop Loss is hit first 
#      if a single candle's range covers both the SL and TP levels.
#   5. IMPROVED (Exponential Complexity Penalty): The Optuna objective function's penalty for model
#      complexity (`max_depth`, `n_estimators`) is now exponential, making it much harder for the
#      AI to choose overfit models and strongly favoring simpler, more robust solutions.
#   6. IMPROVED (Optimization Reliability): The hyperparameter optimization process now uses 3-fold
#      cross-validation (KFold) for more reliable and generalized model parameter selection.
#   7. FIXED (Robust Imports): The script now gracefully handles the absence of the `optuna-integration`
#      package, preventing crashes. It conditionally uses the `XGBoostPruningCallback` to speed up
#      optimization only if the package is available.
#
# --- SCRIPT VERSION ---
VERSION = "206"
# ---------------------
...
class ConfigModel(BaseModel):
    ...
    # V206 UPDATE: Replaced static cost model with a more realistic, dynamic one.
    ...
    # --- Deprecated in V206, kept for backward compatibility if realism is off ---
...
class FeatureEngineer:
    ...
    def _calculate_hoffman_features(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        g['is_hoffman_irb_bullish'] = (is_strong_uptrend & (((g['Close'] - g['Low']) / candle_range) < 0.45) & (((g['Open'] - g['Low']) / candle_range) < 0.45)).astype(int)
        g['is_hoffman_irb_bearish'] = (is_strong_downtrend & (((g['High'] - g['Close']) / candle_range) < 0.45) & (((g['High'] - g['Open']) / candle_range) < 0.45)).astype(int)
        return g
...
def initialize_playbook(playbook_path: str) -> Dict:
    DEFAULT_PLAYBOOK = {
        ...
        "VolatilityExpansionBreakout": {
            ...
            "ideal_regime": ["Low Volatility", "High Volatility"],
            "ideal_macro_env": ["Event-Driven", "Neutral"],
        ...
        "RSIDivergenceReversal": {
            ...
            "ideal_regime": ["Ranging", "Weak Trending"],
            "ideal_macro_env": ["Neutral"],
            "asset_class_suitability": ["Any"]
        },
        "EMAScalpingFractal": {
