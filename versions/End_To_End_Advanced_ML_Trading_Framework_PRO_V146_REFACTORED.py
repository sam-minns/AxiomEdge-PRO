# End_To_End_Advanced_ML_Trading_Framework_PRO_V146_REFACTORED.py
#
# V146 UPDATE (Refactoring & Output Cleanup):
# 1. CLEANER OUTPUT: Removed verbose file path logging during initialization.
#    The framework now presents a clean, single "Run Initialized" message.
# 2. CODE MODULARITY: Refactored large functions in GeminiAnalyzer, DataLoader,
#    and PerformanceAnalyzer into smaller, more manageable private methods. This
#    improves readability and maintainability.
# 3. READABILITY: Improved comments, clarified variable names, and standardized
#    string formatting for a more professional and understandable codebase.
# 4. ENHANCED ERROR HANDLING: Implemented more specific error handling for
#    file I/O and API calls to provide clearer diagnostics.
#
# V145 UPDATE (JSON Serialization Fix):
# 1. CRASH FIX - TypeError: Fixed a fatal TypeError that occurred when serializing
#    the configuration to JSON. Replaced the deprecated `.dict()` method with
#    `.model_dump(mode='json')`.
#
# V144 UPDATE (Nickname Fix):
# 1. ATTRIBUTE FIX: Restored the `generate_nickname` method to the GeminiAnalyzer class.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict]):
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V146_REFACTORED",
    ...
    while True:
        ...
        try:
            run_single_instance(fallback_config, framework_history, playbook, nickname_ledger, directives)
        except Exception as e:
