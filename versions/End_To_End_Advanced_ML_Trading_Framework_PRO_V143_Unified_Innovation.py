# End_To_End_Advanced_ML_Trading_Framework_PRO_V143_Unified_Innovation.py
#
...
#    to obey any commands from the new directives file, fully integrating the long-term
#    stagnation logic.
# 3. UPGRADE - Context-Aware Tuning: The intra-cycle AI tuning prompt is now dynamic.
#    - If a GNN model is active, it asks the AI to tune GNN hyperparameters.
#    - If a Meta-Labeling model is active, it asks the AI to tune volatility filter parameters.
#    - This prevents the AI from making irrelevant suggestions and leverages its context window.
# 4. ARCHITECTURE RE-INTEGRATION - META-LABELING: The powerful meta-labeling architecture
...
#    been re-integrated as a standard part of the backtester, governed by AI-tunable parameters.
#
# V142 UPDATE (Quarantine Fix):
# 1. NEW FEATURE - Strategy Quarantine: The framework now quarantines strategies for 3
#    cycles after they are abandoned via a strategic intervention.
# 2. ENHANCED PROMPT - Quarantine Constraint: The intervention prompt now forbids the
#    AI from selecting any strategy that is currently in quarantine.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict, nickname_ledger: Dict, directives: List[Dict]):
    ...
    logger.info(f"--- Initial Config: {json.dumps(config.dict(), indent=2)} ---")
    ...
    while current_train_start + train_window < end_date:
        ...
        cycle_mar = (cycle_pnl / last_equity) / (equity_curve.pct_change().std() * np.sqrt(252)) if equity_curve.pct_change().std() > 0 else 0
        ...
        if breaker_tripped:
            ...
            if failed_cycles_in_a_row >= 2:
                ...
                if intervention_suggestion and intervention_suggestion.get("strategy_name") in playbook:
                    ...
                    config = ConfigModel(**{**config.dict(), **intervention_suggestion})
                    failed_cycles_in_a_row = 0
                else:
        ...
        if suggested_params:
            ...
            config = ConfigModel(**{**config.dict(), **suggested_params})
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V143_Unified_Innovation",
