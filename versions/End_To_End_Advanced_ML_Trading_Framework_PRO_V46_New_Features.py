# End_To_End_Advanced_ML_Trading_Framework_PRO_V46_New_Features
#
# V46 Update:
# 1. New Features: Added `dist_from_sma50` (ATR-normalized distance from 50-period SMA)
#    and `consecutive_candles` to provide more market context.
# 2. Faster Adaptation: Changed `RETRAINING_FREQUENCY` from 90D to 60D.
# 3. Increased Sensitivity: Lowered `MIN_CONFIDENCE_TO_TRADE` from 0.60 to 0.55.
...
class ConfigModel(BaseModel):
    ...
    MIN_CONFIDENCE_TO_TRADE: confloat(ge=0, le=1)
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V46_New_Features",
...
class FeatureEngineer:
    ...
    def _calculate_m15_native(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- ADD NEW FEATURES ---
        # 1. Distance from SMA50 (normalized by ATR)
        sma_50 = g['Close'].rolling(window=sma_period).mean()
        g['dist_from_sma50'] = ((g['Close'] - sma_50) / g['ATR']).shift(1)
...
class ModelTrainer:
    FEATURES = [
        ...
        'market_regime', 'dist_from_sma50', 'consecutive_candles' # Added new features
    ]
    ...
    def _train_final_model(self, best_params: Dict, X: pd.DataFrame, y: pd.Series) -> Optional[Pipeline]:
        ...
        try:
            ...
            fit_params = {
                'model__eval_set': [(X_test_final, y_test_final)],
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, model: Pipeline, initial_equity: float) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        for candle in candles:
            ...
            if not open_position:
                ...
                if direction != 0 and confidence >= self.config.MIN_CONFIDENCE_TO_TRADE:
                    atr = candle['ATR']
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V46_New_Features.py