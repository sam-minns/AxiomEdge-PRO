# End_To_End_Advanced_ML_Trading_Framework_PRO_V151_Clean_XGB_Params.py
#
# V151 UPDATE (Clean XGB Params):
# 1. FIXED - Deprecation Warning: Removed the unnecessary `use_label_encoder`
#    parameter from all XGBoost model configurations. This parameter is
#    deprecated in modern XGBoost versions and was causing harmless but
#    verbose warnings in the log output.
#
# V150 UPDATE (Parallel Optuna):
# 1. OPTIMIZED - Parallel Hyperparameter Tuning: Modified the Optuna study to
#    run trials in parallel by adding `n_jobs=-1`. This leverages all available
#    CPU cores to significantly speed up the hyperparameter search phase
#    without sacrificing the quality of the optimization.
...
class ModelTrainer:
    ...
    def _find_best_threshold(self, best_params, X_train, y_train, X_val, y_val, num_classes) -> float:
        ...
        temp_params = {'objective':objective,'booster':'gbtree','tree_method':'hist',**best_params} # <--- MODIFIED: Removed 'use_label_encoder'
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        def custom_objective(trial:optuna.Trial):
            param={'objective':objective,'eval_metric':eval_metric,'booster':'gbtree','tree_method':'hist','seed':42, # <--- MODIFIED: Removed 'use_label_encoder'
        ...
        try:
            ...
            study.optimize(custom_objective,n_trials=self.config.OPTUNA_TRIALS,timeout=3600, n_jobs=-1)
            return study
        except Exception as e:
    ...
    def _train_final_model(self,best_params:Dict,X:pd.DataFrame,y:pd.Series, feature_names: List[str], num_classes: int)->Optional[Pipeline]:
        ...
        try:
            ...
            objective = 'multi:softprob' if num_classes > 2 else 'binary:logistic'
            final_params={'objective':objective,'booster':'gbtree','tree_method':'hist','seed':42,**best_params} # <--- MODIFIED: Removed 'use_label_encoder'
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool):
    ...
    # Loop over the retraining dates with a simple counter
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        logger.info(f"\n--- Starting Cycle [{cycle_num}/{total_cycles}] ---")
        ...
        if df_train_raw.empty or df_test.empty:
            logger.warning(f"  - No data for cycle [{cycle_num}/{total_cycles}]. Skipping.")
            continue
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V151_Clean_XGB_Params",
