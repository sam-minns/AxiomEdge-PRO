# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.4_Slicing_Fix
#
# V94.4 Update:
# 1. BUG FIX: Resolved a fatal KeyError that occurred when a training period
#    start date fell on a weekend or holiday. Switched from label-based
#    slicing to a more robust boolean mask for all time series slicing.
#
# V94.3 Update:
# 1. BUG FIX: Resolved an AttributeError in the primary signal generation.
#
# V94.2 Update:
# 1. BUG FIX: Streamlined the data pipeline to only use M15 data.
#
# V94.1 Update:
...
class FeatureEngineer:
    ...
    def label_meta_outcomes(self, df: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        for i in range(len(signal_df)):
            ...
            try:
                entry_loc = df.index.get_loc(entry_idx_val)
            except KeyError:
                continue
            
            future_candles = df.iloc[entry_loc + 1 : entry_loc + 1 + lookahead]
            if future_candles.empty: continue

            entry_price = df.iloc[entry_loc]['Close']
            signal = np.sign(signal_df.iloc[i]['primary_model_signal'])
            sl_dist = df.iloc[entry_loc]['ATR'] * 1.5
            tp_dist = df.iloc[entry_loc]['ATR'] * 2.0
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V94_Slicing_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.4_Slicing_Fix.py