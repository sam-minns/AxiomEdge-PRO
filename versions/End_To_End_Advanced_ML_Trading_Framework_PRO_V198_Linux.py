# End_To_End_Advanced_ML_Trading_Framework_PRO_V198_Linux.py
#
# V198 UPDATE (Deeper AI Analysis & Context):
#   1. ADDED (Model Quality Feedback Loop): The `BestObjectiveScore` from the hyperparameter optimization is now saved with each cycle's results. This provides the AI with a crucial piece of metadata linking a model's internal quality score to its real-world PNL.
#   2. ENHANCED (AI Analysis Prompt): The core AI analysis prompt (`analyze_cycle_and_suggest_changes`) has been significantly upgraded. It now explicitly instructs the AI to analyze the relationship between the `BestObjectiveScore` and the `PNL`, allowing it to identify complex patterns like high-profit cycles emerging from low-confidence models, which it can then flag as unreliable or volatile.
#
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(
        ...
        cycle_status: str,
        shap_feedback: Dict[str, List[str]],
        exit_analysis: Dict[str, int],
        ...
        # --- V198 ENHANCEMENT: Build a more sophisticated analytical prompt ---
        analytical_insights_prompt = ["--- GRANULAR PERFORMANCE ANALYSIS ---"]

        # 1. Model Quality vs. Performance Analysis
        last_cycle = historical_results[-1] if historical_results else None
        if last_cycle and 'BestObjectiveScore' in last_cycle:
            score = last_cycle['BestObjectiveScore']
            pnl = last_cycle['PNL']
            if score < 0.15 and pnl > 0:
                analytical_insights_prompt.append(
                    f"- **MODEL UNRELIABILITY WARNING**: The last cycle's model was low quality (Score: {score:.3f}) but produced a positive PNL (${pnl:,.2f}). This suggests the profit may have been due to luck or random market noise, not a true edge. **Be cautious.** Do not aggressively increase risk based on this result. Consider reducing features to find a more stable model."
                )
            elif score < 0.05:
                 analytical_insights_prompt.append(
                    f"- **MODEL QUALITY FAILURE**: The last model failed its quality gate (Score: {score:.3f}). The features are likely too noisy or irrelevant. Your top priority is to propose a simpler, more robust feature set."
                )

        # 2. SHAP Feedback
        if shap_feedback.get("top_features"):
            analytical_insights_prompt.append(
                f"- **FEATURE IMPORTANCE (SHAP)**: The most predictive features were: `{shap_feedback['top_features']}`. The least predictive were: `{shap_feedback['bottom_features']}`. Your new feature set should heavily consider keeping the top features and removing the bottom ones to reduce noise."
            )
        ...
        if exit_analysis:
            ...
            if total_exits > 0:
                ...
                if sl_ratio > 0.7:
                    analytical_insights_prompt.append(
                        f"- **EXIT ANALYSIS**: CRITICAL! The strategy is hitting its Stop Loss {sl_ratio:.0%} of the time. This suggests the risk parameters or entry logic are poorly calibrated. Propose changes to make the strategy less sensitive to volatility, possibly by adjusting `CONFIDENCE_TIERS` or selecting more stable features."
                    )
        # 4. Volatility Performance Analysis
        if volatility_analysis:
            high_vol_pnl = volatility_analysis.get('High', 0.0)
            if high_vol_pnl < 0:
                analytical_insights_prompt.append(
                    f"- **VOLATILITY ANALYSIS**: The strategy lost ${abs(high_vol_pnl):.2f} during high volatility periods. Propose changes to either A) avoid these periods by lowering `MAX_VOLATILITY_RANK` or B) change the feature set to one that can better navigate volatile conditions."
                )
        ...
        # --- Logic for different cycle statuses ---
        if cycle_status == "TRAINING_FAILURE":
            task_guidance = (
                "**CRITICAL: MODEL TRAINING FAILURE!**\n"
                "The model failed the quality gate. Propose a change that INCREASES model simplicity and signal quality. Your first instinct should be to **reduce the number of features**. Look at the SHAP feedback. Create a smaller, more potent feature set (3-5 features)."
            )
        elif cycle_status == "PROBATION":
             task_guidance = (
                "**STRATEGY ON PROBATION**\n"
                "The previous cycle hit its drawdown limit. Your **only goal** is to **REDUCE RISK**. Profitability is secondary. You MUST suggest changes that lower risk, like reducing `MAX_DD_PER_CYCLE`, reducing `MAX_CONCURRENT_TRADES`, or switching to a lower `RISK_PROFILE`."
             )
        else: # Standard cycle
            task_guidance = (
                ...
                "- Use the `Model Quality vs. Performance` analysis to guide your risk appetite. A lucky win from a bad model is a warning sign.\n"
                f"- {context_feature_mandate}\n"
            )

        prompt_details = f"{task_guidance}\n\n{analytical_insights_str}"

        recent_history = framework_history.get("historical_runs", [])[-5:]
        data_context = (
            # V198 CHANGE: Cycle history now includes the objective score for better AI context
            f"**1. CURRENT RUN - CYCLE RESULTS (Score = Model Quality, PNL = Outcome):**\n{json.dumps(self._sanitize_dict(historical_results), indent=2)}\n\n"
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        best_objective_score = -1.0 # Initialize score for the cycle
        ...
        if cycle_retry_count > 0:
            if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
                ...
                if last_successful_pipeline:
                    ...
                    # V198 CHANGE: Add the objective score to the cycle result, even on failure
                    cycle_result = {"StartDate": period_start_date.date().isoformat(), "EndDate": test_end.date().isoformat(), "NumTrades": 0, "PNL": 0.00, "Status": "TRAINING_FAILURE_LIMIT", "BestObjectiveScore": round(best_objective_score, 4)}
        ...
        # V198 CHANGE: Add the objective score to the cycle result
        cycle_result = {
            "StartDate": period_start_date.date().isoformat(),
            "EndDate": test_end.date().isoformat(),
            "NumTrades": len(trades),
            "PNL": round(cycle_pnl, 2),
            "Status": "Circuit Breaker" if breaker_tripped else cycle_status_msg,
            "BestObjectiveScore": round(best_objective_score, 4)
        }
