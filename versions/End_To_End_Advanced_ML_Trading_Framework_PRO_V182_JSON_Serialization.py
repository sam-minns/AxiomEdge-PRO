# End_To_End_Advanced_ML_Trading_Framework_PRO_V182_JSON_Serialization.py
#
...
# 2. ADDED: A recursive sanitization function that traverses the results
#    dictionary before saving. It converts non-standard types (like numpy.int64,
#    pandas.Timestamp) into their standard Python equivalents (int, float, str),
#    ensuring compatibility with the `json` library.
# 3. IMPROVED: The robustness of saving historical run data and champion models,
#    preventing crashes at the end of a long run.
#
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict) -> Optional[Dict]:
    try:
        # --- FIX: Sanitize the dictionary before saving ---
    ...
    # Note: new_calmar can be None if the run had no trades
    ...
    if is_new_champion:
        ...
        champ_calmar_val = current_champion.get("final_metrics", {}).get("mar_ratio", 0) if current_champion else -np.inf
        logger.info(f"Current run's MAR Ratio ({new_calmar:.2f}) did not beat champion's ({champ_calmar_val:.2f}).")

    try:
        # --- FIX: Sanitize the champion data as well ---
        if champion_to_save:
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V182_JSON_Serialization",
