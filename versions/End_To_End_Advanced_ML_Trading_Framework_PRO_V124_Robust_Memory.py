# End_To_End_Advanced_ML_Trading_Framework_PRO_V124_Robust_Memory
#
# V124 UPDATE (Robust Memory & JSON Fix):
# 1. NEW - ROBUST MEMORY FORMAT: Re-architected memory storage. The 'champion'
#    is now saved to its own file, and historical runs are appended to a '.jsonl'
#    (JSON Lines) file, preventing a single failed write from corrupting the
#    entire history.
# 2. FIX - JSON PARSING: The memory loading function now reads the history line by
#    line. It will intelligently skip any malformed or incomplete lines, log a
#    warning, and continue, preventing crashes from corrupted data.
# 3. FIX - TYPEERROR: Corrected the root cause of the JSON corruption by ensuring
#    all date objects are converted to strings before being saved.
#
# V122 UPDATE (Critical Bug Fix):
# 1. FIX - NAMEERROR: Resolved a NameError crash caused by a missing 'import random'.
...
def load_memory(champion_path: str, history_path: str) -> Dict:
    ...
    # Load historical runs line-by-line for robustness
    historical_runs = []
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict):
    ...
    # 1. Append the new run to the history file (.jsonl)
    try:
    ...
    # 2. Determine and save the new champion
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V124_Robust_Memory",
    ...
    # Create a temporary config just to get the file paths
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V124_Robust_Memory.py