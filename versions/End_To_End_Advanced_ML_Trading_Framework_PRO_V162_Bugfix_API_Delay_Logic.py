# End_To_End_Advanced_ML_Trading_Framework_PRO_V162_Bugfix_API_Delay_Logic.py
#
# V162 UPDATE (Bugfix & API Delay Logic):
# 1. FIXED - pydantic.ValidationError: Corrected a crash on `nickname` by reordering
#    the configuration update logic, ensuring a valid string is always present
#    before Pydantic validation.
# 2. UPDATED - Implemented new API call delay logic: 30s for default single runs,
#    60s for multi-runs (MAX > 1), and 120s for continuous daemon mode.
#
# V161 UPDATE (Concise Logging):
# 1. FIXED - Refactored the initial setup logging to eliminate duplicate printing of the
#    AI's analysis notes, making the console output more concise.
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        def dynamic_progress_callback(study: optuna.study.Study, trial: optuna.trial.FrozenTrial):
            ...
            # FIX: Use `trial.number + 1` as it is the correct attribute for the trial index.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool, api_delay_seconds: int):
    ...
    # --- NICKNAME BUG FIX ---
    # First, update the master config dict with all suggestions from the AI.
    # This is the "unsafe" step that might introduce `nickname: None`.
    current_config_dict.update(ai_setup)

    # Second, check if the AI generated a NEW nickname. The AI is prompted to return
    # a string for a new name, or null (None) if one already exists.
    if isinstance(ai_setup.get("nickname"), str) and ai_setup.get("nickname"):
        ...
        # Use a temporary config model just to get the path to save the ledger
    ...
    # Third, GUARANTEE the nickname is a valid string before Pydantic validation.
    # This overwrites the potentially problematic `None` from the AI update with
    # the correct one from the (now updated) ledger, or a default string.
    ...
    # Now, the current_config_dict is safe to validate and will not raise an error.
    ...
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        ...
        if breaker_tripped:
            ...
            if failed_cycles_in_a_row >= 2:
                ...
                logger.info(f"Pausing for {api_delay_seconds} seconds to respect API call frequency after intervention attempt...")
        ...
        # Pause after every cycle's API call to respect API rate limits.
        logger.info(f"Pausing for {api_delay_seconds} seconds between cycles...")
        time.sleep(api_delay_seconds)
...
def main():
    ...
    if CONTINUOUS_RUN_HOURS > 0:
        api_delay_seconds = 120
    elif MAX_RUNS > 1:
        api_delay_seconds = 60
    else: # Default for single run (MAX_RUNS = 1)
    ...
    while True:
        ...
        try:
            run_single_instance(fallback_config, framework_history, playbook, nickname_ledger, directives, is_continuous, api_delay_seconds)
        except Exception as e:
