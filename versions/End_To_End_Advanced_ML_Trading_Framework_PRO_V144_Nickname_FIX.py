# End_To_End_Advanced_ML_Trading_Framework_PRO_V144_Nickname_FIX.py
#
# V144 UPDATE (Nickname & Serialization Fix):
# 1. ATTRIBUTE FIX: Restored the `generate_nickname` method to the GeminiAnalyzer
#    class. This method was present in V142 but was accidentally omitted in the
#    V143 update, preventing a potential AttributeError.
# 2. SERIALIZATION FIX: Fixed a `TypeError` that occurred when logging or saving the
#    configuration. Pydantic's `dict()` method was replaced with `model_dump(mode='json')`
#    to correctly convert Path objects to strings before JSON serialization.
#
# V143 UPDATE (Unified Innovation & Context-Aware AI):
# 1. UNIFIED DIRECTIVES: The long-term Stagnation Review now writes actionable
#    `FORCE_EXPLORATION` directives to a persistent `framework_directives.json` file
#    when it detects long-term strategy stagnation.
# 2. ENHANCED - Pre-Flight Check: The pre-flight AI prompt now loads and is required
#    to obey any commands from the new directives file.
# 3. UPGRADE - Context-Aware Tuning: The intra-cycle AI tuning prompt is now dynamic
#    to prevent irrelevant suggestions.
# 4. ARCHITECTURE RE-INTEGRATION - META-LABELING: The meta-labeling architecture
#    has been revived and added to the playbook as a new selectable strategy.
# 5. ARCHITECTURE RE-INTEGRATION - VOLATILITY FILTER: The proactive volatility filter has
#    been re-integrated into the backtester, governed by AI-tunable parameters.
...
class GeminiAnalyzer:
    ...
    def _extract_json_from_response(self, response_text: str) -> Dict:
        try:
            ...
            suggestions = json.loads(match.group(1).strip()) if match else {}
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict) -> Dict:
        ...
        if is_gnn_strategy:
            prompt = (f"{base_prompt_intro} You are managing a **GNN_Market_Structure** model. Its features are learned automatically, so **DO NOT suggest `selected_features`**.\n\n"
                      f"{base_prompt_analysis}\n\n"
            ...
            prompt = (f"{base_prompt_intro} You are managing a **Meta-Labeling** architecture. Your goal is to tune the secondary (XGBoost) model and the volatility filter.\n\n"
                      f"{base_prompt_analysis}\n\n"
                      "**Suggest a NEW configuration for:**\n"
                      "- `MAX_DD_PER_CYCLE`: Risk appetite (float between 0.05 and 0.4).\n"
            ...
            if last_cycle.get("status") == "Circuit Breaker":
                ...
                prompt = (
                    ...
                    "2.  **DO NOT increase `MAX_DD_PER_CYCLE`.** CRITICAL: The new value MUST be a float between 0.05 and 0.9.\n"
                    ...
                    "Respond ONLY with a valid JSON object containing your new, safer parameters.\n\n"
                    f"**FAILED CYCLE CONTEXT:**\n{json.dumps(self._sanitize_dict(last_cycle), indent=2)}\n\n"
                    f"**AVAILABLE FEATURES FOR THIS STRATEGY:**\n{available_features}"
                )
            else:
                prompt = (f"{base_prompt_intro} You are managing a standard model. Your goal is to tune its features and parameters.\n\n"
                          f"{base_prompt_analysis}\n\n"
                          "**Suggest a NEW configuration for:**\n"
                          "- `MAX_DD_PER_CYCLE`: Risk appetite (float between 0.05 and 0.4).\n"
...
class FeatureEngineer:
    ...
    def _label_group_meta(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        if not group.index.is_unique:
            logger.error(f"Index is not unique for group {group['Symbol'].iloc[0]}. Cannot proceed with labeling.")
            return pd.DataFrame()
        ...
        for i in range(len(signal_df)):
            ...
            if first_tp_hit and (not first_sl_hit or first_tp_hit < first_sl_hit):
                outcomes[i] = 1 # Meta-label is 1 if the primary signal was profitable
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        def custom_objective(trial:optuna.Trial):
            ...
            try:
                ...
                if num_classes > 2: pnl_map = {0: -1, 1: 0, 2: 1}
                else: pnl_map = {0: -1, 1: 1} # For meta-labeling, 0 is bad, 1 is good
        ...
        for candle in candles:
            ...
            if symbol not in open_positions:
                ...
                if self.is_meta_model:
                    # Meta model: only trade if prob > threshold and primary signal exists
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    try:
        ...
        updated = False
        for key, value in DEFAULT_PLAYBOOK.items():
            if key not in playbook:
                logger.info(f"Adding new strategy '{key}' to existing playbook.")
                playbook[key] = value
                updated = True
        if updated:
             with open(playbook_path, 'w') as f:
...
def perform_strategic_review(history: Dict, directives_path: str) -> Tuple[Dict, List[Dict]]:
    ...
    if len(recent_runs) >= 3:
        last_three_strats = [r.get('strategy_name') for r in recent_runs]
        if len(set(last_three_strats)) == 1 and last_three_strats[0] is not None:
            stagnant_strat_name = last_three_strats[0]
            calmar_values = [r.get("final_metrics", {}).get("calmar_ratio", 0) for r in recent_runs]
            is_not_improving = calmar_values[2] <= calmar_values[1] and calmar_values[1] <= calmar_values[0]
            ...
            if is_not_improving:
                ...
                stagnation_directive = {
                    ...
                    "reason": f"Stagnation: No improvement over last 3 runs (Calmars: {[round(c, 2) for c in calmar_values]})."
                }
    ...
    # Always clear old directives and write new ones, even if empty
    try:
        with open(directives_path, 'w') as f:
            json.dump(directives, f, indent=4)
        if directives:
            logger.info(f"--- STRATEGIC REVIEW: Directives saved to {directives_path}")
        else:
            logger.info(f"--- STRATEGIC REVIEW: No new directives generated. Cleared old directives file.")
    except IOError as e:
...
def determine_timeframe_roles(detected_tfs: List[str]) -> Dict[str, Optional[str]]:
    ...
    if not sorted_tfs: raise ValueError("No timeframes were detected from the provided data files.")
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict, nickname_ledger: Dict, directives: List[Dict]):
    ...
    version_label = script_name.replace(".py", "") if ".py" in script_name else script_name
    current_config_dict['REPORT_LABEL'] = version_label
    current_config_dict['nickname'] = nickname_ledger.get(version_label, "Run")
    ...
    logger.info(f"--- Run Nickname: {config.nickname} ---")
    logger.info(f"--- Strategy: {config.strategy_name} ---")
    logger.info(f"--- Initial Config: {config.model_dump_json(indent=2)} ---")

    data_loader = DataLoader(config)
    all_files = [f for f in os.listdir(config.BASE_PATH) if f.endswith(('.csv', '.txt')) and re.match(r'^[A-Z0-9]+_[A-Z0-9]+', f)]
    if not all_files:
        logger.critical("No data files found in the base path. Exiting.")
        return
    ...
    train_window = pd.to_timedelta(config.TRAINING_WINDOW)
    retrain_freq = pd.to_timedelta(config.RETRAINING_FREQUENCY)
    forward_gap = pd.to_timedelta(config.FORWARD_TEST_GAP)

    current_train_start = start_date
    aggregated_trades = pd.DataFrame()
    aggregated_equity_curve = pd.Series([config.INITIAL_CAPITAL])
    ...
    run_summary = {
        "script_version": config.REPORT_LABEL,
        "nickname": config.nickname,
        "strategy_name": config.strategy_name,
        "run_start_ts": config.run_timestamp,
        "initial_params": config.model_dump(mode='json')
    }

    while current_train_start + train_window < end_date:
        cycle_start_time = time.time()
        train_end = current_train_start + train_window
        test_start = train_end + forward_gap
        test_end = test_start + retrain_freq
        if test_end > end_date: test_end = end_date

        df_train_raw = full_df.loc[current_train_start:train_end].copy()
        df_test = full_df.loc[test_start:test_end].copy()
        ...
        if trainer.is_gnn_model:
            test_embeddings = trainer._get_gnn_embeddings_for_test(df_test)
            X_test = test_embeddings
        else:
            X_test = df_test[feature_list_to_use].copy().fillna(0)
        ...
        if is_meta_model and probs.size > 0:
            df_test['prob_0'] = probs[:, 0]
            df_test['prob_1'] = probs[:, 1]
        elif not is_meta_model and probs.size > 0:
            df_test['prob_short'] = probs[:, 0]
            df_test['prob_hold'] = probs[:, 1]
        ...
        cycle_mar = (cycle_pnl / last_equity) / (equity_curve.pct_change().std() * np.sqrt(252)) if not equity_curve.empty and equity_curve.pct_change().std() > 0 else 0

        cycle_result = {"StartDate": test_start.date().isoformat(), "EndDate": test_end.date().isoformat(),
                        "NumTrades": len(trades), "PNL": round(cycle_pnl, 2), "MAR_Ratio": round(cycle_mar, 2),
                        "Status": "Circuit Breaker" if breaker_tripped else "Completed"}
        ...
        if not trades.empty:
            ...
            new_equity_points = equity_curve.iloc[1:]
            aggregated_equity_curve = pd.concat([aggregated_equity_curve, new_equity_points], ignore_index=True)
        ...
        if breaker_tripped:
            ...
            if failed_cycles_in_a_row >= 2:
                ...
                if intervention_suggestion and intervention_suggestion.get("strategy_name") in playbook:
                    ...
                    config = ConfigModel(**{**config.model_dump(), **intervention_suggestion})
                    failed_cycles_in_a_row = 0
                else:
                    logger.error("  - Strategic intervention FAILED to select a valid new strategy. Continuing with current strategy and reduced risk.")
                    config.MAX_DD_PER_CYCLE = max(0.05, config.MAX_DD_PER_CYCLE * 0.75)
            else:
                logger.warning("  - First circuit breaker event. AI will be prompted for safer parameters.")
        else:
            failed_cycles_in_a_row = 0
        ...
        if suggested_params:
            ...
            config = ConfigModel(**{**config.model_dump(), **suggested_params})
    ...
    run_summary.update({
        ...
        "cycle_details": in_run_historical_cycles,
        "top_5_features": aggregated_shap.head(5).index.tolist() if not aggregated_shap.empty else [],
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V144_Nickname_FIX",
    ...
    while True:
        ...
        directives = []
        if is_continuous:
            _, directives = perform_strategic_review(framework_history, bootstrap_config.DIRECTIVES_FILE_PATH)

        # Always load any pre-existing directives, which might have been generated by the strategic review
        if os.path.exists(bootstrap_config.DIRECTIVES_FILE_PATH):
            try:
                with open(bootstrap_config.DIRECTIVES_FILE_PATH, 'r') as f:
                    directives = json.load(f)
                if directives:
                    logger.info(f"Loaded {len(directives)} directive(s) for this run.")
            except (json.JSONDecodeError, IOError):
                logger.error("Could not load directives file.")

        try:
            run_single_instance(fallback_config, framework_history, is_continuous, playbook, nickname_ledger, directives)
        except Exception as e:
