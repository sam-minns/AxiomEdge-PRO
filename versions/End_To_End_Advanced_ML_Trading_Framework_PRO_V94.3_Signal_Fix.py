# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.3_Signal_Fix
#
# V94.3 Update:
# 1. BUG FIX: Resolved a fatal AttributeError by ensuring the primary signal
#    generation uses a pandas Series, allowing the .diff() method to correctly
#    identify trade signal crossovers.
#
# V94.2 Update:
# 1. BUG FIX: Resolved a "Data loading failed" error by streamlining the data
#    pipeline to only use M15 data.
#
# V94.1 Update:
...
class FeatureEngineer:
    ...
    def _calculate_m15_native(self,g:pd.DataFrame)->pd.DataFrame:
        ...
        # CORRECTED: Convert numpy array to pandas Series before calling .diff()
    ...
    def label_meta_outcomes(self, df: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        for i in range(len(signal_df)):
            ...
            # Use .get_loc for precise index lookup
            try:
                entry_loc = df.index.get_loc(entry_idx_val)
            except KeyError:
                continue # Skip if the index is not found in the main dataframe
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V94_Signal_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.3_Signal_Fix.py