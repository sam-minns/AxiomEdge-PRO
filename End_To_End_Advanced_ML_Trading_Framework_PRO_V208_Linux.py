...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict, diagnosed_regime: str, regime_champions: Dict, correlation_summary_for_ai: str, macro_context: Dict) -> Dict:
        ...
        task_prompt = (
            ...
            "   - **Action:** Use Google Search to find the typical trading costs for these assets on a standard retail **ECN or Raw Spread** account.\n"
            "   - **Action:** In your JSON response, populate the `COMMISSION_PER_LOT` with a realistic round-turn value (e.g., 3.5 for $3.50 per lot) and update the `SPREAD_CONFIG` dictionary with typical 'normal_pips' for each detected asset.\n\n"
            "**STEP 2: STRATEGY SELECTION (Grounded Search & Context Synthesis)**\n"
            "   - **Synthesize Internal & External Context:** Analyze the `MACROECONOMIC CONTEXT`, `MARKET DATA SUMMARY`, and `ASSET CORRELATION SUMMARY`.\n"
