# End_To_End_Advanced_ML_Trading_Framework_PRO_V202_Linux.py
#
# V202 UPDATE (Community-Inspired Feature Enhancements):
#   1. ADDED (Indicator Acceleration): Integrated second-derivative features (e.g., `RSI_slope_acceleration`) to measure the rate of change in momentum, providing a leading signal for trend exhaustion.
#   2. ADDED (Return Autocorrelation): Added a rolling autocorrelation feature (`returns_autocorr_10`) to explicitly measure market memory and serial correlation, helping to distinguish trending from random-walk phases.
#   3. ADDED (Linear Regression Trend Context): Implemented a rolling linear regression slope (`[TF]_ctx_LinRegSlope`) as a more robust alternative/addition to SMA-based trend analysis for higher timeframe context.
#
...
try:
    import torch
    import torch.nn as nn # --- MODIFIED: Added nn alias
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict, diagnosed_regime: str, regime_champions: Dict) -> Dict:
        ...
        logger.info("-> Performing Initial AI Analysis & Setup (Grounded Search Enabled)...")
        ...
        if regime_champion:
            ...
            task_prompt = (
                "**YOUR TASK: Perform a grounded analysis to choose your starting strategy.**\n\n"
                f"The internal data summary suggests the current market regime is **'{diagnosed_regime}'**. We have a proven champion for this exact regime: `{champ_strat}` (MAR Ratio: {champ_mar:.2f}).\n\n"
                "**1. Grounded Verification (EXTERNAL SEARCH)**: Before deciding, **use Google Search to verify if the champion is still optimal.** Answer the following:\n"
                f"    - What are the major economic events (e.g., FOMC, NFP, CPI) scheduled for the primary assets ({asset_list}) in the next 1-2 weeks?\n"
                "    - What is the current market sentiment (e.g., 'risk-on', 'risk-off') for the broader financial markets?\n\n"
                "**2. Synthesize and Decide**:\n\n"
                f"   - **OPTION A: DEPLOY THE REGIME CHAMPION**: If your external search confirms the market conditions are still suitable for the `{champ_strat}` strategy, deploy it. To select it, respond with: `{{\"action\": \"deploy_champion\"}}`\n\n"
                "   - **OPTION B: EXPLORE A NEW STRATEGY**: If your search reveals imminent high-impact news or a sentiment shift that contradicts the champion strategy's logic (e.g., deploying a ranging strategy before a major CPI release), you **MUST** select a more appropriate strategy from the `STRATEGY PLAYBOOK`. Justify your choice in `analysis_notes` and provide the full configuration (`strategy_name`, `selected_features`, etc.) in the JSON response."
            )
        else:
            logger.info(f"  - No existing champion found for '{diagnosed_regime}'. AI will perform a full grounded analysis.")
            task_prompt = (
                "**YOUR TASK: Perform a grounded, multi-factor market analysis to select and configure the optimal starting strategy.**\n\n"
                f"1.  **Technical Diagnosis**: The internal data summary suggests the current market regime is **'{diagnosed_regime}'**.\n"
                "2.  **Grounded Verification (EXTERNAL SEARCH)**: **Use Google Search to find answers to the following:**\n"
                f"    - What are the major economic events or news announcements (e.g., FOMC meetings, NFP releases, CPI data) scheduled for the primary assets ({asset_list}) in the next 1-2 weeks?\n"
                "    - What is the current market sentiment (e.g., 'risk-on', 'risk-off', 'uncertain') for the broader financial markets?\n"
                "    - Are there any new academic papers or trading articles on sites like 'Quantpedia', 'ArXiv q-fin', or 'Seeking Alpha' discussing novel alpha signals or feature engineering techniques relevant to the current market conditions?\n\n"
                "3.  **Synthesize and Decide**: Based on your synthesis of BOTH the internal technical data AND the external grounded search, select the best strategy from the `STRATEGY PLAYBOOK`.\n"
                "    - If high-impact news is imminent, a breakout strategy like `VolatilityExpansionBreakout` might be better than a `MeanReversion` strategy, even if the recent technicals look range-bound. Justify your decision in `analysis_notes`.\n"
                "    - If your search uncovers a new, promising feature (e.g., a new type of volatility index), you may add it to the `selected_features` list for the chosen strategy, but you must explain why in the notes.\n\n"
        ...
        if suggestions.get("action") == "deploy_champion" and regime_champion:
            ...
            champion_params['analysis_notes'] = f"Deploying proven champion for '{diagnosed_regime}' regime after grounded verification."
    ...
    def analyze_cycle_and_suggest_changes(
        ...
        # V202 Grounded Search Enhancement: Add external context analysis point
        analysis_points = [
            ...
            "**6. External Context Post-Mortem (GROUNDED SEARCH):** The previous cycle ran from **{cycle_start_date}** to **{cycle_end_date}**. **Use Google Search to determine if any major market-moving events or narrative shifts occurred during this specific period.** Did a central bank pivot unexpectedly? Was there a geopolitical event that changed market sentiment? Synthesize this external context with the model's performance. For example, if a trend-following strategy suddenly failed, explicitly state whether it was because an external event (e.g., a key inflation report) caused the underlying trend to reverse."
        ]
...
class FeatureEngineer:
    ...
    # --- NEW: Markov Feature Calculation ---
    def _calculate_markov_features(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        # Determine candle color: 1 for green (Close > Open), -1 for red (Close < Open)
        candle_color = np.sign(g['Close'] - g['Open']).fillna(0)
        
        # Calculate streaks
        # Group by consecutive blocks of same-colored candles
        blocks = (candle_color != candle_color.shift()).cumsum()
        # For each block, calculate the cumulative sum (streak length)
    ...
    def _calculate_htf_features(self,df:pd.DataFrame,p:str,s:int,a:int)->pd.DataFrame:
        ...
        def get_rolling_slope(series, window):
            # Check if there's enough non-NaN data to compute a slope
            if series.notna().sum() < 2: return np.nan
            # Drop NaN values for polyfit
            series_clean = series.dropna()
            # If after dropping NaNs, we still have enough points, calculate the slope
        ...
        for symbol,group in df.groupby('Symbol'):
            ...
            # V202 Addition: Linear Regression Slope for HTF context
            ...
            temp_df[f'{tf_id}_ctx_LinRegSlope'] = lin_reg_slope # Add new feature
    ...
    def _calculate_base_tf_native(self, g:pd.DataFrame)->pd.DataFrame:
        ...
        g_out['SMA_30_weekly'] = g_out['Close'].rolling(window=30*5).mean() # Approximate
        if 'RealVolume' in g_out.columns:
            g_out['volume'] = g_out['RealVolume']
            # Simplified proxy for relative strength vs market
            if g_out.index.nlevels > 1: # Check if multi-index (like Date, Symbol)
                 g_out['relative_strength'] = (g_out['pct_change'] - g_out.groupby(level=0)['pct_change'].transform('mean'))
            else: # Fallback for single index
        ...
        if 'RealVolume' in g_out.columns and g_out['RealVolume'].sum() > 0:
            # Calculate VWAP components, grouping by date to handle the daily reset
            tpv = ((g_out['High'] + g_out['Low'] + g_out['Close']) / 3) * g_out['RealVolume']
            
            # Use a transform with cumsum for efficiency on grouped data
            ...
            # Forward-fill VWAP to handle periods with no volume at the start of a day
            g_out['VWAP'] = g_out['VWAP'].ffill()

            # --- Engineer VWAP-based Features ---
            # Normalized distance from VWAP
            g_out['price_to_vwap'] = (g_out['Close'] - g_out['VWAP']) / g_out['ATR'].replace(0, np.nan)
            
            # Sign indicating if price is above or below VWAP
            g_out['price_vs_vwap_sign'] = np.sign(g_out['Close'] - g_out['VWAP'])
            
            # Slope of the VWAP itself to gauge intraday momentum
            g_out['vwap_slope'] = g_out['VWAP'].diff()
        else:
            # If no volume data, create NaN columns to prevent errors downstream
            g_out['VWAP'] = np.nan
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # Drop rows where critical features for ANY strategy might be NaN, after shifting
        # This is a broader drop to ensure all playbook strategies have data
...
# --- NEW: Transformer Model Definition ---
class TimeSeriesTransformer(nn.Module if GNN_AVAILABLE else object): # Inherits from object if torch not present
    def __init__(
        self,
        feature_size=9,
        num_layers=2,
...
class ModelTrainer:
    ...
    def __init__(self,config:ConfigModel):
        ...
        # --- MODIFIED: Added flags for different model types ---
    ...
    def train(self, df_train: pd.DataFrame, feature_list: List[str], strategy_details: Dict) -> Optional[Tuple[Pipeline, float]]:
        ...
        # --- MODIFIED: Set flags based on strategy details ---
        ...
        # --- NEW: Transformer Training Path ---
        if self.is_transformer_model:
            if not GNN_AVAILABLE: # Re-use GNN check for PyTorch availability
            ...
            # For Transformer, the target is the future price, not a class
            # We will create a custom dataset for this inside the trainer
            # Let's assume a simple next-step price prediction for now
            ...
            X_seq, y_seq = [], []
            seq_len = 30 # A common sequence length for transformers
            ...
            for epoch in range(20): # Simplified training loop for demonstration
            ...
            # For Transformer, we don't use a threshold but the model itself
            # The "pipeline" will be just the PyTorch model
            return model, 0.0 # Return 0.0 as a dummy threshold
        ...
        else: # This is the default XGBoost path
            if not feature_list:
...
class Backtester:
    def __init__(self,config:ConfigModel):
        self.config=config
        # --- MODIFIED: Added model type flag ---
        self.is_meta_model = False
        self.is_transformer_model = False
        # --- END MODIFIED ---
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, strategy_details: Dict) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        # --- MODIFIED: Set model flags from strategy_details ---
        self.is_meta_model = strategy_details.get("requires_meta_labeling", False)
        self.is_transformer_model = strategy_details.get("requires_transformer", False)
        # --- END MODIFIED ---
        ...
        for i in range(1, len(candles)):
            ...
            if not circuit_breaker_tripped and symbol not in open_positions and len(open_positions) < max_concurrent_trades:
                ...
                # --- NEW: Signal generation for Transformer model ---
                if self.is_transformer_model:
                    predicted_price = current_candle.get('predicted_price', 0)
                    current_price = current_candle.get('Close', 0)
                    if predicted_price > 0 and current_price > 0:
                        # Simple directional logic
                        if predicted_price > current_price * 1.0005: # Price must increase by at least 0.05%
                            direction = 1
                        elif predicted_price < current_price * 0.9995: # Price must decrease by at least 0.05%
                            direction = -1
                        # Confidence based on magnitude of predicted change
                ...
                else: # Default XGBoost classification
        ...
        finalize_day_metrics(current_day, equity)
        # --- MODIFIED: Return signature remains the same ---
...
def initialize_playbook(playbook_path: str) -> Dict:
    DEFAULT_PLAYBOOK = {
        ...
        "ConfluenceTrend": {
            ...
            "features": ["DAILY_ctx_Trend", "H1_ctx_Trend", "RSI_slope", "stoch_k", "ADX", "momentum_10"],
        ...
        "BollingerMeanReversion": {
            ...
            "features": ["ADX", "RSI", "stoch_k", "bollinger_bandwidth", "hour", "day_of_week", "anomaly_score", "wick_to_body_ratio"],
            "lookahead_range": [20, 60],
            "dd_range": [0.1, 0.2]
        },
        ...
        "RegimeClarityFilter": {
            ...
            "features": ["ADX", "market_volatility_index", "bollinger_bandwidth", "close_fracdiff", "ATR", "DAILY_ctx_ATR", "relative_performance"],
            "lookahead_range": [30, 90],
            "dd_range": [0.05, 0.1]
        },
        "LinearRegressionMeanReversion": {
            "description": "[RANGING] A mean-reversion strategy that enters when the price crosses the linear regression channel, filtered by a long-term trend.",
            "features": ["linear_regression", "SMA_200"],
            "lookahead_range": [30, 90],
            "dd_range": [0.15, 0.3]
        },
        "DonchianBreakout": {
            "description": "[BREAKOUT] A classic breakout strategy that buys when the price breaks above the upper Donchian Channel and sells when it breaks below the lower channel.",
            "features": ["donchian_channel"],
            "lookahead_range": [50, 150],
            "dd_range": [0.2, 0.4]
        },
        "RSIPullback": {
            ...
            "lookahead_range": [40, 120],
            "dd_range": [0.15, 0.3]
        },
        "StanWeinsteinBreakout": {
            "description": "[BREAKOUT] A long-term trend-following strategy that enters on breakouts from consolidation, confirmed by volume and relative strength.",
            "features": ["SMA_30_weekly", "volume", "relative_strength"],
            "lookahead_range": [100, 200],
            "dd_range": [0.2, 0.4]
        },
        "MarkovReversion": {
            "description": "[RANGING] A mean-reversion strategy that uses candle streak length (Markov property) and return autocorrelation to time entries against short-term momentum exhaustion.",
            "features": ["ADX", "RSI", "bollinger_bandwidth", "markov_streak", "hour", "returns_autocorr_10"],
            "lookahead_range": [30, 80],
            "dd_range": [0.1, 0.25]
        },
        "TransformerForecaster": {
            "description": "[SPECIALIZED] Uses a Transformer deep learning model to directly forecast the next price movement, leveraging indicator dynamics and trend context.",
        ...
        "PanicFade": {
            "description": "[CRISIS/EVENT-DRIVEN] A counter-trend strategy designed to fade extreme, news-driven price spikes or drops. It enters when price action shows signs of exhaustion after a massive, high-velocity move, using candle wicks and anomaly scores as signals.",
            "features": ["anomaly_score", "ATR", "wick_to_body_ratio", "candle_body_size_vs_atr", "RSI", "market_volatility_index"],
            "lookahead_range": [20, 50],
            "dd_range": [0.25, 0.45]
        },
        "NakedPriceAction": {
            "description": "[PRICE ACTION] A pure price action strategy that ignores most indicators. It trades based on engulfing candles and dojis (via wick ratios) but qualifies entries with a robust, long-term linear regression trend context.",
            "features": ["is_engulfing", "is_doji", "wick_to_body_ratio", "DAILY_ctx_LinRegSlope", "hour", "day_of_week"],
            "lookahead_range": [40, 100],
            "dd_range": [0.15, 0.3]
        },
        "MomentumIgnition": {
            "description": "[TRENDING/BREAKOUT] This strategy aims to enter a trend just as it accelerates. It uses the new 'acceleration' features to find points where momentum is increasing rapidly, confirmed by the underlying trend direction.",
            "features": ["RSI_slope_acceleration", "momentum_10_slope_acceleration", "ADX", "DAILY_ctx_Trend", "H1_ctx_LinRegSlope", "volume", "market_volatility_index"],
            "lookahead_range": [50, 130],
            "dd_range": [0.2, 0.35]
        },
        "VWAPReversion": {
            "description": "[RANGING/INTRADAY] A classic mean-reversion strategy that enters when the price deviates significantly from the daily VWAP, expecting a return to the intraday mean. Best suited for non-trending days.",
            "features": ["price_to_vwap", "ADX", "RSI", "bollinger_bandwidth", "hour", "DAILY_ctx_Trend"],
            "lookahead_range": [20, 60],
            "dd_range": [0.1, 0.2]
        },
        "VWAPMomentum": {
            "description": "[TRENDING/INTRADAY] A trend-following strategy that uses VWAP as a dynamic filter. It enters on pullbacks to a rising VWAP in a general uptrend, or sells rallies to a falling VWAP in a downtrend.",
            "features": ["vwap_slope", "price_vs_vwap_sign", "DAILY_ctx_LinRegSlope", "ADX", "momentum_10_slope_acceleration", "volume"],
            "lookahead_range": [40, 120],
            "dd_range": [0.15, 0.3]
        },
        "HeikinAshiTrend": {
            "description": "[TRENDING/PRICE ACTION] A robust trend-following strategy that uses clean Heikin-Ashi candle signals to enter and ride trends, filtering out market noise. It qualifies entries with a long-term trend context.",
            ...
            if not train_result or (not trainer.is_transformer_model and best_objective_score < MODEL_QUALITY_THRESHOLD):
                ...
                if trainer.is_transformer_model:
                    ...
                    if X_test_seq:
                        ...
                        df_test['predicted_price'] = np.nan
...
def main():
    ...
    # 1. Define the 'Results' directory path
    results_dir = os.path.join(bootstrap_config.BASE_PATH, "Results")
    
    # 2. Create the 'Results' directory if it doesn't exist. This prevents an
    #    IOError if the script is run in a new location.
    os.makedirs(results_dir, exist_ok=True)
    
    # 3. Construct the full path to the playbook JSON file.
    playbook_file_path = os.path.join(results_dir, "strategy_playbook.json")

    # 4. Call initialize_playbook with the correct, full file path.
