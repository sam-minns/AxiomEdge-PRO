# End_To_End_Advanced_ML_Trading_Framework_PRO_V49_MACD_Feature
#
# V49 Update:
# 1. Added MACD features (`macd`, `macd_signal`, `macd_hist`) to provide the model
#    with more sophisticated momentum information.
# 2. Reverted the take-profit target in the labeling function back to `tp_atr=3.0`,
#    restoring the more successful high risk-reward profile from previous tests.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V49_MACD_Feature",
...
class FeatureEngineer:
    ...
    def _calculate_m15_native(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        g['macd'] = (exp12 - exp26).shift(1)
        g['macd_signal'] = g['macd'].ewm(span=9, adjust=False).mean().shift(1)
        g['macd_hist'] = (g['macd'] - g['macd_signal']).shift(1)
...
class ModelTrainer:
    FEATURES = [
        ...
        'macd', 'macd_signal', 'macd_hist' # Added MACD features
    ]
    ...
    def _train_final_model(self, best_params: Dict, X: pd.DataFrame, y: pd.Series) -> Optional[Pipeline]:
        ...
        try:
            ...
            fit_params = {
                'model__eval_set': [(X_test_final.values, y_test_final.values)],
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V49_MACD_Feature.py