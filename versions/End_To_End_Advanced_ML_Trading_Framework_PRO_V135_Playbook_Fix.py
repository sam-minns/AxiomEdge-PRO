# End_To_End_Advanced_ML_Trading_Framework_PRO_V135_Playbook_Fix.py
#
# V135 UPDATE (Default Playbook Fix):
# 1. CRITICAL FIX - AttributeError: Corrected the `initialize_playbook` function by
#    defining the feature-set lists locally. This resolves the crash that occurred
#    when the script attempted to create a new, default `strategy_playbook.json`.
# 2. IMPROVEMENT - RESTORED DEFAULT STRATEGIES: The default playbook now includes
#    the full set of original strategies for a more comprehensive starting point.
#
# V134 UPDATE (Dynamic Timeframe Handling):
# 1. NEW - TIMEFRAME DETECTION: The framework now dynamically detects all timeframes
#    present in the data folder by reading the CSV filenames.
# 2. NEW - DYNAMIC ROLE ASSIGNMENT: The script automatically sorts and assigns
#    'base', 'medium', and 'high' roles to the detected timeframes.
#
...
# 1. NEW - AI PARAMETER SANITIZER: The framework now sanitizes AI suggestions.
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    # Define feature lists locally to avoid class attribute errors
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V135_Playbook_Fix",
