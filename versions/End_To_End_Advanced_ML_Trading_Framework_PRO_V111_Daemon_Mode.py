# End_To_End_Advanced_ML_Trading_Framework_PRO_V111_Daemon_Mode
#
# V111 UPDATE (Continuous Operation & Exploration Mode):
# 1. NEW - DAEMON MODE: The script can now run in a continuous loop. Set `CONTINUOUS_RUN_HOURS`
#    or `MAX_RUNS` at the top of `main()` to have the framework automatically execute
#    runs back-to-back, with a 5-minute pause between each to respect API rate limits.
# 2. NEW - EXPLORATION MODE: To solve the "stuck strategy" problem, the AI now has an
#    `exploration_rate` (e.g., 25%). On a random chance, it will be forced into
#    "Exploration Mode," where it must test a non-champion strategy from the playbook
#    to gather new data and prevent overfitting to a single strategy type.
# 3. BUG FIX: Corrected a `NameError` for the `start_time` variable.
#
# V110 UPDATE (Method Fix):
# 1. Fixed a critical `AttributeError` by implementing the missing `get_pre_flight_config` method.
...
class GeminiAnalyzer:
    ...
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, fallback_config: Dict, exploration_rate: float) -> Dict:
        ...
        if is_exploration:
            ...
            if not non_champion_strategies: non_champion_strategies = list(playbook.keys()) # Fallback if only one strategy exists
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V111_Daemon_Mode.py