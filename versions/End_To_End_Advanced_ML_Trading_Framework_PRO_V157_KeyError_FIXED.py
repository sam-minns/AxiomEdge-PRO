# End_To_End_Advanced_ML_Trading_Framework_PRO_V157_KeyError_FIXED.py
#
# V157 UPDATE (KeyError Fix):
# 1. FIXED - KeyError: Corrected a crash by ensuring the 'DIRECTIVES_FILE_PATH' key is
#    defined in the fallback_config dictionary within the main function before it is
#    passed to other functions.
#
# V156 UPDATE (API Optimization & Data-Driven Setup):
# 1. ENHANCED - Combined Initial API Call: Merged the "Nickname Generation" and "Strategy
#    Selection" into a single, efficient API call at the start of a run.
# 2. ENHANCED - AI-Powered Data Analysis: The script now pre-analyzes all market data
#    and sends a summary (volatility, trendiness, correlations) to the AI. The AI uses
#    this context to make a more informed, data-driven initial strategy selection.
# 3. REFACTORED - API Call Interval: The delay between API calls within the walk-forward
#    loop has been increased to 5 minutes (300 seconds) as per user requirements.
...
class GeminiAnalyzer:
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        # Ensure the entire context is sent
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        """
        Performs the initial AI analysis, combining nickname generation, data analysis,
        ...
        # Part 1: Nickname generation instruction
        ...
        # Part 2: Directive and Health Report strings
        ...
        # Part 3: Filter playbook if GNN is not available
        ...
        prompt = (
            "You are a Master Trading Strategist and a Creative Analyst responsible for configuring a trading framework for its next run.\n\n"
            "**YOUR THREE TASKS:**\n"
            f"{nickname_prompt_part}"
            "2.  **Analyze Market Data**: Review the `MARKET DATA SUMMARY` provided below. Briefly comment on the general market conditions (e.g., 'highly correlated and trending', 'ranging with low volatility') in the `analysis_notes` key of your JSON response.\n"
            "3.  **Select Strategy & Configure**: Based on your data analysis, the framework's historical performance, and any critical directives, select the optimal `strategy_name` from the playbook. Then, provide a complete and robust starting configuration for all required parameters (e.g., `selected_features`, `MAX_DD_PER_CYCLE`, `LOOKAHEAD_CANDLES`). Prioritize strategies that are well-suited to the market conditions you identified.\n\n"
        ...
        if suggestions and "strategy_name" in suggestions:
            logger.info(f"  - Initial AI Analysis Complete. Notes: {suggestions.get('analysis_notes', 'N/A')}")
            return suggestions
        else:
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    DEFAULT_PLAYBOOK = {
        "VolatilityBreakout": {"description": "Enters on explosive breakouts from low-volatility consolidations.", "features": ["ATR", "bollinger_bandwidth", "ADX", "hour", "day_of_week"], "lookahead_range": [60, 120], "dd_range": [0.20, 0.35]},
        "RangeBound": {"description": "Trades reversals in a sideways channel, filtered by low ADX.", "features": ["ADX", "RSI", "stoch_k", "bollinger_bandwidth", "hour"], "lookahead_range": [20, 60], "dd_range": [0.10, 0.20]},
        "TrendPullback": {"description": "Enters on pullbacks (RSI) in the direction of an established long-term trend.", "features": ["D1_ctx_Trend", "ADX", "RSI", "H1_ctx_Trend", "momentum_10", "hour"], "lookahead_range": [50, 150], "dd_range": [0.15, 0.30]},
        ...
        "Meta_Labeling_Filter": {"description": "Uses a secondary ML filter to improve a simple primary model's signal quality.", "features": ["ADX", "RSI", "ATR", "bollinger_bandwidth", "H1_ctx_Trend", "D1_ctx_Trend"], "lookahead_range": [50, 100], "dd_range": [0.10, 0.25], "requires_meta_labeling": True},
    }
...
def load_nickname_ledger(ledger_path: str) -> Dict:
    """Loads the nickname ledger from file without making an API call."""
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool):
    ...
    # --- Start of refactored setup ---
    current_config_dict = fallback_config.copy()
    current_config_dict['run_timestamp'] = run_timestamp_str
    
    # Step 1: Load data and create features to build the data summary
    data_loader = DataLoader(ConfigModel(**current_config_dict))
    ...
    fe = FeatureEngineer(ConfigModel(**current_config_dict), tf_roles)
    ...
    # Step 2: Create Data Summary for AI Analysis
    data_summary = {}
    ...
    # Step 3: Perform single, consolidated API call for initial setup
    ...
    ai_setup = gemini_analyzer.get_initial_run_setup(version_label, nickname_ledger, framework_history, playbook, health_report, directives, data_summary)
    
    if not ai_setup:
        logger.critical("AI-driven setup failed. Using fallback configuration and exiting.")
        return

    # Step 4: Process AI response and finalize config
    # Update nickname ledger if a new one was generated
    if ai_setup.get("nickname"):
        ...
        try:
            with open(bootstrap_config.NICKNAME_LEDGER_PATH, 'w') as f: json.dump(nickname_ledger, f, indent=4)
            logger.info(f"  - Saved new nickname '{ai_setup['nickname']}' to ledger.")
    ...
    retraining_dates = pd.date_range(start=test_start_date, end=end_date, freq=retrain_freq)
    ...
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        ...
        if suggested_params:
            ...
            logger.info("Pausing for 5 minutes to respect API call frequency...");
            time.sleep(300)
...
if __name__ == '__main__':
    ...
    # Make sure to include the full, unchanged class definitions for ConfigModel, 
    # DataLoader, FeatureEngineer, ModelTrainer, Backtester, and PerformanceAnalyzer
    # in the final script before running. They have been omitted from this block for brevity.
    main()
