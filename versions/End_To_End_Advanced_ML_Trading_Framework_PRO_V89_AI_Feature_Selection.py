# End_To_End_Advanced_ML_Trading_Framework_PRO_V89_AI_Feature_Selection
#
# V89 Update:
# 1. NEW - AI Historical Context: The AI-tuning loop now maintains a full history
#    of all previous cycle results and sends it to Gemini, allowing the AI to make
#    more intelligent, context-aware suggestions over time.
# 2. NEW - AI-Driven Feature Selection: The model no longer uses a hardcoded list
#    of features. It now asks Gemini to analyze SHAP results and select a subset
#    of features to use for the next training cycle.
# 3. This script is complete and unabridged, with all prior bug fixes included.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "3. From the 'available_features' list, select a SUBSET of features you believe will perform best for the next cycle. Include the most important features from the SHAP results, but also consider experimenting by adding or removing others.\n\n"
...
class FeatureEngineer:
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # Drop rows with NaNs in any of the potential features before returning
...
def main():
    ...
    current_config={"BASE_PATH":os.getcwd(),"REPORT_LABEL":"ML_Framework_V88_Call_Fix","FORWARD_TEST_START_DATE":"2024-01-01","INITIAL_CAPITAL":100000.0,"CONFIDENCE_TIERS":{'ultra_high':{'min':0.80,'risk_mult':1.2,'size_mult':1.5,'rr':3.0},'high':{'min':0.70,'risk_mult':1.0,'size_mult':1.0,'rr':2.5},'standard':{'min':0.60,'risk_mult':0.8,'size_mult':0.5,'rr':2.0}},"BASE_RISK_PER_TRADE_PCT":0.01,"SPREAD_PCTG_OF_ATR":0.05,"SLIPPAGE_PCTG_OF_ATR":0.02,"OPTUNA_TRIALS":25,"TRAINING_WINDOW":'365D',"RETRAINING_FREQUENCY":'60D',"LOOKAHEAD_CANDLES":100,"MIN_CLASS_SAMPLES":50,"MIN_CLASS_RATIO":0.05,"TREND_FILTER_THRESHOLD":25.0,"BOLLINGER_PERIOD":20,"STOCHASTIC_PERIOD":14,"CALCULATE_SHAP_VALUES":False}
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        trainer=ModelTrainer(config);training_result=trainer.train(df_train_labeled, feature_list=current_config.get('selected_features', ModelTrainer.BASE_FEATURES))
        ...
        backtester=Backtester(config);chunk_trades_df,chunk_equity=backtester.run_backtest_chunk(df_test_chunk,model,current_config.get('selected_features', ModelTrainer.BASE_FEATURES),best_threshold,initial_equity=full_equity_curve[-1])
        ...
        cycle_results_for_ai={"cycle":i+1,"f1_score":trainer.study.best_value if trainer.study else 0,"best_threshold":best_threshold,"cycle_pnl":cycle_pnl,"win_rate":cycle_win_rate,"num_trades":len(chunk_trades_df),"current_params":{k:v for k,v in current_config.items() if k in ['LOOKAHEAD_CANDLES','TREND_FILTER_THRESHOLD','OPTUNA_TRIALS']}, "shap_summary": trainer.shap_summary.head(10).to_dict() if trainer.shap_summary is not None else {}}
        historical_cycle_results.append(cycle_results_for_ai)
        available_features = list(df_featured.columns.drop(['target', 'Close', 'Open', 'High', 'Low', 'RealVolume', 'Symbol'], errors='ignore'))
        suggested_params = gemini_analyzer.analyze_cycle_and_suggest_changes(historical_cycle_results, available_features)
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V88_Call_Fix.py