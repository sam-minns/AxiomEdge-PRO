# End_To_End_Advanced_ML_Trading_Framework_PRO_V92_Forensic_AI
#
# V92 Update:
# 1. NEW - Forensic AI Prompting: The Gemini prompt is now significantly more advanced,
#    tasking the AI with analyzing cycle history to find hidden patterns and event
#    triggers, moving beyond simple parameter suggestion.
# 2. NEW - Model Stability Gate: A second validation step is added within each
#    Optuna trial. Models that are not stable on a small, held-out "stability set"
#    are penalized, preventing fragile models from being selected.
# 3. FIXED - Resilient Reporting: The performance report is now always generated,
#    even if all cycles fail. The cycle breakdown will correctly show a
#    "Circuit Breaker" status, providing better data for failure analysis.
# 4. ADDED - API Rate Limiting: A 5-second delay is added after each API call to
#    ensure rate limits are respected, making the system more robust.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        logger.info("  - Sending cumulative cycle history to Gemini for forensic analysis...")
        prompt = (
            ...
            "The historical results show that the models are consistently unstable and fail immediately on out-of-sample data, even with risk management. Your task is to find out WHY and suggest changes to break this failure loop.\n\n"
            "Analyze the ENTIRE history of previous cycles for hidden patterns, correlations, or event triggers that might explain the failures. For example: "
            "  - Does a high `TREND_FILTER_THRESHOLD` correlate with the model failing? "
            "  - Is there a relationship between the `objective_score` in training and the out-of-sample PnL? "
            "  - Are there specific features that seem to cause instability when selected? "
            "  - Is the model learning a simple anomaly that does not persist?\n\n"
            ...
            "3.  **Feature Selection**: Select a SUBSET of features from the `available_features` list. Be creative. Perhaps a simpler model with fewer features would be more robust. Consider dropping features that have been present in many failing cycles.\n\n"
...
class PerformanceAnalyzer:
    ...
    def generate_full_report(self,trades_df:Optional[pd.DataFrame],equity_curve:Optional[pd.Series],cycle_metrics:List[Dict],aggregated_shap:Optional[pd.DataFrame]=None):
        ...
        # MODIFIED: Always generate report, but plots only if there's data.
    ...
    # ... (plot functions and _calculate_metrics are unchanged) ...
    ...
    def generate_text_report(self,m:Dict[str,Any],cycle_metrics:List[Dict],aggregated_shap:Optional[pd.DataFrame]=None):
        # ... (text report formatting is unchanged) ...
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V92_Forensic_AI",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V92_Forensic_AI.py