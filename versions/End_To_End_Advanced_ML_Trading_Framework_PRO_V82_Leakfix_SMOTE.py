# End_To_End_Advanced_ML_Trading_Framework_PRO_V82_Leakfix_SMOTE
#
# V82 Update:
# 1. CRITICAL FIX - Data Leak Patched: Fixed a major data leak in the cross-pair
#    feature calculation. Returns are now correctly shifted BEFORE being used to
#    calculate relative strength and correlation, preventing the model from seeing
#    current-candle information. This resolves the unrealistic F1 score of 1.0.
# 2. ENHANCED - Imbalance Handling: Upgraded the sampling technique from RandomOverSampler
#    to the more advanced SMOTE (Synthetic Minority Over-sampling TEchnique) as per
#    your strategic roadmap, to help the model generalize better.
# 3. This script is complete, unabridged, and contains all prior bug fixes.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V82_Leakfix_SMOTE",
    ...
    "LOOKAHEAD_CANDLES": 100, "MIN_CLASS_SAMPLES": 50, "MIN_CLASS_RATIO": 0.05, # Lowered ratio threshold slightly as SMOTE is more robust
...
class FeatureEngineer:
    ...
    def _calculate_cross_pair_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- FIX: Shift returns to prevent data leakage ---
        returns=df_pivot.pct_change(fill_method=None).shift(1).fillna(0)
...
class ModelTrainer:
    ...
    def train(self,df_train:pd.DataFrame)->Optional[Pipeline]:
        ...
        if min_ratio<self.config.MIN_CLASS_RATIO or class_counts.min()<self.config.MIN_CLASS_SAMPLES:
            logger.info(f"    - Classes are imbalanced. Applying SMOTE.");X,y=SMOTE(random_state=42).fit_resample(X,y)
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V82_Leakfix_SMOTE.py