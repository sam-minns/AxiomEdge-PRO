# End_To_End_Advanced_ML_Trading_Framework_PRO_V102_Breaker_Analysis
#
# V102 UPDATE (Circuit Breaker Forensics):
# 1. CRITICAL ANALYSIS: Added a "damage report" when the circuit breaker trips.
#    The framework now captures the number of trades, net PnL, and the results of
#    the last 5 trades leading to the drawdown failure.
# 2. ENHANCED AI PROMPT: This new forensic context is passed to the Gemini AI. The
#    prompt is updated to explicitly instruct the AI to analyze the *mode of failure*
#    (e.g., "death by a thousand cuts" vs. a few catastrophic losses) to make more
#    intelligent and targeted suggestions for the next cycle.
#
# V101 UPDATE (Final Fix & Configuration Change):
# 1. Fixed the critical SHAP calculation error by implementing the robust logic
#    from the user's V93 template.
# 2. Updated the retraining frequency to 90 days as requested.
...
class GeminiAnalyzer:
    ...
    def _summarize_historical_results(self, historical_results: List[Dict]) -> List[Dict]:
        ...
        for result in recent_history:
            ...
            if result.get("breaker_context"): # NEW: Include breaker context if it exists
                summary["breaker_context"] = result["breaker_context"]
        ...
        for candle in candles:
            if not circuit_breaker_tripped:
                ...
                if equity > 0 and chunk_peak_equity > 0 and (chunk_peak_equity - equity) / chunk_peak_equity > self.config.MAX_DD_PER_CYCLE:
                    ...
                    # NEW (V102): Capture the context of the failure
                    ...
                    open_positions = {} # Close all open positions
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V102_Breaker_Analysis",
        ...
        "OPTUNA_TRIALS": 30, "TRAINING_WINDOW": '365D', 
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        chunk_trades_df,chunk_equity,breaker_tripped,breaker_context = backtester.run_backtest_chunk(df_test_chunk,model,current_config['selected_features'],best_threshold,initial_equity=full_equity_curve[-1])
        ...
        cycle_results_for_ai={
            ...
            "current_params":{k:v for k,v in current_config.items() if k in ['LOOKAHEAD_CANDLES','TREND_FILTER_THRESHOLD','OPTUNA_TRIALS','selected_features','MAX_DD_PER_CYCLE']},
            "shap_summary": trainer.shap_summary.head(10).to_dict() if trainer.shap_summary is not None else {},
            "breaker_context": breaker_context # NEW (V102)
        }
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V102_Breaker_Analysis.py