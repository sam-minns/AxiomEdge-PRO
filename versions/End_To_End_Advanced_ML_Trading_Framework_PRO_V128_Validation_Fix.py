# End_To_End_Advanced_ML_Trading_Framework_PRO_V128_Validation_Fix
#
# V128 UPDATE (Recursive Parsing and Validation):
# 1. FIX - ROBUST AI PARSING: Replaced the AI response handling with a recursive
#    "flattening" function. It now correctly finds parameters regardless of how
#    the AI nests them (e.g., inside 'initial_config' or 'parameters').
# 2. NEW - HYBRID STRATEGY VALIDATION: Added a validation step to ensure that a
#    newly synthesized hybrid strategy has a correctly formatted feature list
#    before it is saved, preventing playbook corruption.
# 3. FIX - CHAMPION REPORTING: Corrected a logic bug where the "All-Time Champion"
#    on the report could lag by one run. It is now always current.
#
# V127 UPDATE (Phase 2 Adaptive Actions):
# 1. NEW - STRATEGY QUARANTINE, FORCED EXPLORATION, and DYNAMIC RISK ADAPTATION
#    have been implemented to allow the AI to make more decisive, intelligent
...
class GeminiAnalyzer:
    ...
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], fallback_config: Dict, exploration_rate: float) -> Dict:
        ...
        if is_exploration:
            ...
            quarantined_strats = [item for sublist in quarantined_strats for item in sublist] # flatten list
            ...
            if not available_strategies: # If all are quarantined
            ...
            prompt = (
                ...
                "3. **CRITICAL: Check for Stagnation**: If a strategy has a `StagnationWarning`, it has been used for several runs without improving. You should strongly consider choosing a **different** strategy to force exploration and break the negative pattern.\n"
                "4. **Select a Strategy**: Choose the `strategy_name` from the playbook that you believe has the highest risk-adjusted potential for the *next* run, considering all the above data.\n"
    ...
    def create_hybrid_strategy(self, historical_runs: List[Dict], current_playbook: Dict) -> Optional[Dict]:
        ...
        prompt = (
            ...
            "3.  **Features:** Create a new feature list which MUST be an array of strings (e.g., `[\"feature1\", \"feature2\"]`).\n"
        ...
        if new_hybrid and isinstance(new_hybrid, dict) and len(new_hybrid) == 1:
            ...
            if not isinstance(hybrid_body.get('features'), list) or not all(isinstance(f, str) for f in hybrid_body['features']):
                logger.error(f"--- HYBRID SYNTHESIS: AI created a hybrid with an invalid 'features' list. It must be a list of strings. Discarding. \nContent: {hybrid_body.get('features')}")
                return None
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict):
    ...
    if is_continuous:
        ...
        # Future: directives = generate_adaptive_directives(health_report)
    ...
    # NEW: Robustly handle nested AI responses
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V128_Validation_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V128_Validation_Fix.py