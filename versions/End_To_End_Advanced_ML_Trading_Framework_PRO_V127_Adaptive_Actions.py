# End_To_End_Advanced_ML_Trading_Framework_PRO_V127_Adaptive_Actions
#
# V127 UPDATE (Phase 2 Adaptive Actions & Logic Fix):
# 1. NEW - STRATEGY QUARANTINE: The AI can now issue a "QUARANTINE" directive,
#    temporarily banning chronically failing strategies from being selected.
# 2. NEW - FORCED EXPLORATION: When a champion strategy stagnates, the AI is
#    instructed to ignore it and explore other options from the playbook.
# 3. NEW - DYNAMIC RISK ADAPTATION: The framework can now reduce its global risk
#    percentage automatically if multiple strategies begin to fail.
# 4. FIX - CHAMPION REPORTING: Corrected a logic bug where the "All-Time Champion"
#    on the report was one run behind. It is now always up-to-date.
#
# V126 UPDATE (Robust Parsing & Bug Fix):
# 1. FIX - TYPEERROR: Implemented a more comprehensive fix to robustly handle
#    and flatten nested JSON responses from the AI.
# 2. FIX - STAGNATION LOGIC: Corrected a bug in the Stagnation anaylsis.
...
class GeminiAnalyzer:
    ...
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], fallback_config: Dict, exploration_rate: float) -> Dict:
        ...
        # Process directives for the prompt
        ...
        if is_exploration:
            ...
            prompt = (
                ...
                "3. **Analyze Past Performance**: Review the `framework_memory` for recent performance and the current champion.\n"
                "4. **Select a Strategy**: Choose the `strategy_name` from the playbook that you believe has the highest risk-adjusted potential for the *next* run.\n"
                "5. **Define Parameters**: Define the initial parameters for the chosen strategy.\n\n"
    ...
    def create_hybrid_strategy(self, historical_runs: List[Dict], current_playbook: Dict) -> Optional[Dict]:
        ...
        prompt = (
            ...
            "3.  **Features:** Create a new feature list (5-7 features) by selecting the most powerful and complementary features from BOTH strategies.\n"
...
class ConfigModel(BaseModel):
    ...
    LOG_FILE_PATH: str = ""; CHAMPION_FILE_PATH: str = ""; HISTORY_FILE_PATH: str = ""; PLAYBOOK_FILE_PATH: str = ""; DIRECTIVES_FILE_PATH: str = ""
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict) -> Optional[Dict]:
    ...
    if current_champion is None or (new_calmar is not None and new_calmar > current_champion.get("final_metrics", {}).get("calmar_ratio", 0)):
        ...
        new_champion_obj = new_run_summary # This run is the new champion
        ...
        champion_to_save = current_champion # The old champion remains
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict):
    ...
    health_report = {}
    directives = [] # Will hold action items like QUARANTINE
    if is_continuous:
        health_report = perform_strategic_review(framework_history)
        # In a future version, a function here would convert the health_report into directives.
        # For now, health_report is passed directly to the AI for consideration.
    ...
    if 'initial_config' in ai_config_suggestion and isinstance(ai_config_suggestion.get('initial_config'), dict):
        nested_params = ai_config_suggestion.pop('initial_config')
    ...
    if not current_config['selected_features']:
        current_config['selected_features'] = chosen_strategy_features[:5] if chosen_strategy_features else []
        logger.warning("AI-suggested features were empty or not valid. Falling back to a default subset.")
    ...
    # Update the summary with the final metrics before exiting
    ...
    # Re-save the final, complete summary for this run
    try:
        with open(initial_config.HISTORY_FILE_PATH, 'r') as f:
            lines = f.readlines()
        # Find the placeholder entry (last line) and replace it with the full summary
        if lines:
...
def main():
    ...
    MAX_RUNS = 3 # Set > 1 to enable Daemon mode
    # --- End Controls ---

    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V127_Adaptive_Actions",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V127_Adaptive_Actions.py