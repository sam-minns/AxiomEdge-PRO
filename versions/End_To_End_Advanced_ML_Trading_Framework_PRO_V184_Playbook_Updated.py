# End_To_End_Advanced_ML_Trading_Framework_PRO_V184_Regime_Intel.py
#
# V184 UPDATE (Regime Intelligence & Backtest Integrity):
# 1.  CRITICAL FIX: Implemented a "Training Retry Limit" to prevent overfitting.
#     The framework now has a `MAX_TRAINING_RETRIES_PER_CYCLE` parameter (default: 3).
#     If a model fails the Quality Gate more than this many times in one cycle,
#     the cycle is abandoned with a PNL of $0, and the framework moves to the
#     next walk-forward period. This eliminates the "unlimited dice rolls" flaw.
# 2.  ADDED: "Pre-Cycle Regime Analysis" for dynamic strategy switching. Before each
#     cycle, the AI now analyzes a statistical summary of recent market data.
#     It can proactively switch to the most suitable strategy from the playbook
#     (e.g., from 'TrendPullback' to 'RangeBound') if the market regime has shifted.
#     This makes the system adaptive rather than reactive.
# 3.  ADDED: A new AI endpoint `propose_regime_based_strategy_switch` dedicated to
#     the pre-cycle analysis, with a highly targeted prompt for this specific task.
# 4.  ENHANCED: The `strategy_playbook.json` is now structured to support this
#     new logic, with strategies clearly defined for different market types
#     (Trending, Ranging, Breakout), giving the AI a richer set of tools.
#
# V183 UPDATE (Intelligent Strategy Lifecycle & Quality Gates):
# 1. ADDED: A "Model Quality Gate."
# 2. ADDED: A "Probation & Quarantine" system for strategy failures.
# 3. FIXED: The "Short-Cycle Trap."
# 4. IMPROVED: AI prompting now forcefully mandates multi-timeframe context features.
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V184_Regime_Intel",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V184_FIXED.py