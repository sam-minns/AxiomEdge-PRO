# End_To_End_Advanced_ML_Trading_Framework_PRO_V60_Hybrid_FIX
#
# V60 Update:
# 1. Fixed TypeError in AutoBNNTrainer by adding the missing `likelihood_model` and `seed` arguments.
# 2. Fixed ValueError in ModelTrainer by refactoring the training pipeline to prevent
#    the target variable from being processed twice.
# 3. Streamlined the FeatureEngineer and data loading to be more suitable for the hybrid approach.
...
class HybridModelTrainer:
    ...
    def _train_xgboost(self, X_aug: pd.DataFrame, y: pd.Series) -> Optional[Pipeline]:
        ...
        # Pass y directly without creating a new df with .assign
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, model: HybridModelTrainer, initial_equity: float) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        for candle in candles:
            ...
            if not open_position:
                ...
                # Using a fixed confidence for simplicity in this hybrid model
                min_confidence = 0.55
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V59c_Hybrid_GNN_AutoBNN.py