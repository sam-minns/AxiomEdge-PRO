# End_To_End_Advanced_ML_Trading_Framework_PRO_V121_Stagnation_Fix
#
# V121 UPDATE (Stagnation Detection & Error Fix):
# 1. FIX - KEYERROR: Resolved a KeyError that occurred when analyzing historical
#    runs from older script versions that were missing certain keys.
# 2. NEW - STAGNATION DETECTION: The Strategic Review now detects if the same
#    strategy has been used for 3+ consecutive runs without significant improvement
#    or with a high circuit breaker frequency.
# 3. NEW - AI DIRECTIVE FOR EXPLORATION: If a strategy is flagged as stagnating,
#    the AI is now explicitly instructed to penalize that choice and favor a
#    different strategy to force adaptation.
#
# V120 UPDATE (Strategic Health Analysis):
# 1. NEW - STRATEGIC REVIEW PHASE: In daemon mode, the framework now performs a
#    historical analysis of all strategies in the playbook before selecting one.
# 2. NEW - ADVANCED METRICS: The review calculates a "Health Report" including
#    Chronic Failure Rate and Circuit Breaker Frequency for each strategy.
...
def perform_strategic_review(history: Dict) -> Dict:
    ...
    if len(recent_runs) >= 3:
        ...
        if len(set(last_three_strats)) == 1 and last_three_strats[0] is not None:
            ...
            is_not_improving = calmar3 <= calmar2 <= calmar1
            high_breaker_freq = float(health_report.get(stagnant_strat_name, {}).get("CircuitBreakerFrequency", "0%").strip('%')) > 33.0
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V121_Stagnation_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V121_Stagnation_Fix.py