# End_To_End_Advanced_ML_Trading_Framework_PRO_V70_Final
#
# V70 Update:
# 1. Fixed TypeError by restoring the correct, full-featured `FeatureEngineer` class
#    from the V47/V55 baseline.
# 2. This version now correctly implements the dynamic triple-barrier labeling.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V70_Final",
...
class FeatureEngineer:
    ...
    def _label_group(self, group: pd.DataFrame, sl_atr: float, tp_atr: float, lookahead: int) -> pd.DataFrame:
        ...
        sl_barrier = atr * sl_atr
        tp_barrier = atr * tp_atr
...
class ModelTrainer:
    ...
    def train(self, df_train: pd.DataFrame) -> Optional[Pipeline]:
        ...
        logger.info(f"  - Starting model training for period {df_train.index.get_level_values('Timestamp').min().date()} to {df_train.index.get_level_values('Timestamp').max().date()}...")
...
def main():
    ...
    for i, period_start_date in enumerate(retraining_dates):
        ...
        test_end = retraining_dates[i + 1] if i + 1 < len(retraining_dates) else df_featured['Timestamp'].max()
