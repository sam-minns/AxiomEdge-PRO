# End_To_End_Advanced_ML_Trading_Framework_PRO_V76_AmbiguityFix
#
# V76 Update:
# 1. FIXED - Critical ValueError: Refactored the `_calculate_cross_pair_features` function
#    to resolve the `ValueError: 'Timestamp' is both an index level and a column label`.
#    The new logic ensures the 'Timestamp' is handled consistently as a column during
#    all merge operations and is set back to the index only once at the end.
# 2. All features and logic from V75 (Inter-Market, Session-Based) are retained.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V76_AmbiguityFix",
    ...
    "OPTUNA_TRIALS": 50, "TRAINING_WINDOW": '365D', "RETRAINING_FREQUENCY": '60D',
...
class DataLoader:
    ...
    def load_and_parse_data(self) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        for filename in self.config.DATA_FILENAMES:
            ...
            try:
                parts = filename.split('_'); symbol, tf_str = parts[0], parts[1]
...
class FeatureEngineer:
    ...
    def _calculate_cross_pair_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_pivot = df_out.pivot(index='Timestamp', columns='Symbol', values='Close').ffill().bfill()
        ...
        rel_strength_unpivoted = rel_strength_df.stack().reset_index(name='rel_strength')
        rel_strength_unpivoted.columns = ['Timestamp', 'Symbol', 'rel_strength']
        df_out = pd.merge(df_out, rel_strength_unpivoted, on=['Timestamp', 'Symbol'], how='left')
    ...
    def _label_with_dynamic_barriers(self, df:pd.DataFrame, sl_atr:float, tp_atr:float, lookahead:int) -> pd.DataFrame:
        ...
        for i in range(self.config.MAX_BARRIER_ADJUSTMENT_ITERATIONS):
            ...
            if current_ratio>best_balance: best_balance,best_df,best_params=labeled_df,current_ratio,(current_sl,current_tp)
...
class PerformanceAnalyzer:
    ...
    def generate_full_report(self,trades_df:Optional[pd.DataFrame],equity_curve:Optional[pd.Series],cycle_metrics:List[Dict],aggregated_shap:Optional[pd.DataFrame]=None):
        if trades_df is None or trades_df.empty or equity_curve is None or len(equity_curve)<2:
            logger.warning("-> No trades were generated. Skipping final report.");self.generate_text_report({},cycle_metrics,aggregated_shap);return
    ...
    def _calculate_metrics(self,trades_df:pd.DataFrame,equity_curve:pd.Series)->Dict[str,Any]:
        ...
        m['net_profit']=m['ending_capital']-m['initial_capital'];m['net_profit_pct']=m['net_profit']/m['initial_capital'] if m['initial_capital']>0 else 0
        ...
        m['avg_win']=wins['PNL'].mean() if len(wins)>0 else 0;m['avg_loss']=losses['PNL'].mean() if len(losses)>0 else 0
        ...
        m['mar_ratio']=m['cagr']/(m['max_drawdown_pct']/100) if m['max_drawdown_pct']>0 else np.inf;return m
...
if __name__ == '__main__':
    # As per your request, setting console output encoding for PowerShell
    print("[Console]::OutputEncoding = [System.Text.Encoding]::UTF8")
    main()

# End_To_End_Advanced_ML_Trading_Framework_PRO_V76_AmbiguityFix.py