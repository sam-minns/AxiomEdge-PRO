# End_To_End_Advanced_ML_Trading_Framework_PRO_V148_ProgressBar_FIX.py
#
# V148 UPDATE (Progress Bar):
# 1. ADDED - TQDM Progress Bar: Integrated the `tqdm` library to display a
#    visual progress bar during the main walk-forward analysis loop, improving
#    user experience by showing completion percentage.
# 2. REFACTORED - Cycle Loop: Converted the main walk-forward `while` loop to a
#    `for` loop that iterates over pre-calculated dates to enable the progress bar.
#
# V147 UPDATE (Run Controls Restored):
# 1. ADDED - Run Limit Configuration: Re-integrated the `CONTINUOUS_RUN_HOURS`
#    and `MAX_RUNS` variables into the `main()` function.
# 2. ADDED - Conditional Inter-Cycle Delay: Restored the logic to pause
#    between walk-forward cycles for API rate limit management.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool):
    ...
    # Pre-calculate the retraining dates to get a total for the progress bar
    ...
    logger.info(f"Walk-forward analysis will run for {len(retraining_dates)} cycles.")
    ...
    # Loop over the retraining dates with a tqdm progress bar
    for period_start_date in tqdm(retraining_dates, desc="Walk-Forward Cycles"):
        cycle_start_time = time.time()
        train_end = period_start_date - forward_gap
        train_start = train_end - train_window
        test_end = period_start_date + retrain_freq
        ...
        logger.info(f"\n--- Cycle Start: {test_start.date()} | Train: {train_start.date()}-{train_end.date()} | Test: {period_start_date.date()}-{test_end.date()} ---")
        ...
        if df_train_labeled.empty:
            logger.warning("  - No valid labels generated for this training cycle. Skipping."); continue
        ...
        if not train_result:
            logger.error("  - Model training failed for this cycle. Skipping."); continue
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V148_ProgressBar_FIX",
