# End_To_End_Advanced_ML_Trading_Framework_PRO_V197_Linux.py
#
# V197 UPDATE (Profitability & Robustness Enhancements):
#   1. ENHANCED (Optimization Objective): Overhauled the `custom_objective` function in the hyperparameter optimizer. It now directly optimizes for a risk-adjusted return (a Sharpe Ratio proxy) across both validation and stability datasets, guiding the AI more effectively toward profitable and stable models.
#   2. ADDED (Champion Guardrail): Implemented a crucial check in the `save_run_to_memory` function. A run can now only become the 'Overall' or 'Regime' champion if it has a non-negative MAR Ratio AND executes a minimum number of trades (default 10). This prevents non-trading or loss-making strategies from being promoted.
#   3. REFINED (Default Configuration): Adjusted the default `fallback_config` to be more robust. The `LABEL_MIN_RETURN_PCT` was increased from 0.001 to 0.004 to filter for more significant trading signals, and the `CONFIDENCE_TIERS` reward ratios were made more conservative and achievable to encourage realistic trade generation.
#   4. ADDED (Feature Diversification Guidance): Added comments to the code and analysis documentation to highlight the risk of feature dominance (e.g., `ATR`) and to encourage experimentation with more diverse feature sets to improve model robustness.
#   5. FIXED (Objective Stability): The new optimization objective is inherently more stable and provides a clearer, non-capped gradient for the optimizer to follow, allowing it to better distinguish between the quality of different losing trials.
#
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict, diagnosed_regime: str) -> Optional[Dict]:
    ...
    # --- V197 ENHANCEMENT: Champion Guardrail ---
    ...
    if is_new_overall_champion:
        ...
        if current_champion:
            champ_mar_val = current_champion.get("final_metrics", {}).get("mar_ratio", -np.inf)
    ...
    if new_trade_count >= MIN_TRADES_FOR_CHAMPION and new_mar >= 0:
         if current_regime_champion is None:
             is_new_regime_champion = True
         else:
             current_regime_mar = current_regime_champion.get("final_metrics", {}).get("mar_ratio", -np.inf)
             if new_mar is not None and new_mar > current_regime_mar:
                is_new_regime_champion = True
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V197_MQL5-Aligned",
