# End_To_End_Advanced_ML_Trading_Framework_PRO_V203_Linux.py
#
# V203 UPDATE (AI Grounding & Bug Fixes):
#   1. ADDED (Correlation-Aware AI Analysis): The AI now receives an asset correlation matrix to perform a more holistic, event-driven analysis. It can now identify potential impacts from news affecting correlated assets, not just the primary trading asset.
#   2. FIXED (Pandas SettingWithCopyWarning): Resolved a `SettingWithCopyWarning` during the backtest prediction phase by using `.loc` for safer DataFrame column assignment, ensuring more stable operation.
#
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict, diagnosed_regime: str, regime_champions: Dict, correlation_summary_for_ai: str) -> Dict:
        if not self.api_key_valid:
        ...
        task_prompt = (
            ...
            "**1. Identify Correlated Cluster:**\n"
            "   - Analyze the `ASSET CORRELATION SUMMARY` provided in the context below. Identify the entire group of assets that are strongly correlated (positively or negatively) with the primary assets being traded ({asset_list}).\n\n"
            ...
            "   - **A) IMMINENT MAJOR EVENT (Direct or Correlated):** If your search reveals a high-impact event for ANY currency in the cluster within the next 24-48 hours, you MUST select a strategy designed for high volatility or pre-event risk management (e.g., `VolatilityExpansionBreakout`, `PanicFade`). Justify your choice by stating which currency the event belongs to and how its correlation might affect the primary assets.\n\n"
            "   - **B) CLEAR CALENDAR:** If the coast is clear for the entire correlated group, proceed with the standard technical analysis based on the `diagnosed_regime` to choose the best strategy from the playbook.\n\n"
    ...
    def analyze_cycle_and_suggest_changes(
        ...
        analysis_points = [
            ...
            "**6. External Context Post-Mortem (GROUNDED SEARCH):** The previous cycle ran from **{cycle_start_date}** to **{cycle_end_date}**. Use the provided `ASSET CORRELATION SUMMARY` and Google Search to analyze the cycle's performance.",
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    if pivot_df.shape[1] > 1:
        corr_matrix = pivot_df.corr()
        # To keep the prompt clean, we'll only show strong correlations
    ...
    ai_setup = api_timer.call(gemini_analyzer.get_initial_run_setup, version_label, nickname_ledger, framework_history, playbook, health_report, directives, data_summary, diagnosed_regime, regime_champions, correlation_summary_for_ai)
            ...
            if not train_result or (not trainer.is_transformer_model and best_objective_score < MODEL_QUALITY_THRESHOLD):
                ...
                if trainer.is_transformer_model:
                    ...
                    if not X_test.empty:
                        ...
                        # --- FIX: V203 - Use .loc to avoid SettingWithCopyWarning ---
