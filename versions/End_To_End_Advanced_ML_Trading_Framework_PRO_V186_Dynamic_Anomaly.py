# End_To_End_Advanced_ML_Trading_Framework_PRO_V186_Dynamic_Anomaly.py
#
# V186 UPDATE (Dynamic Anomaly Detection & Enhanced Features):
# 1.  ADDED: AI-Driven Anomaly Contamination Factor. Before each run, the AI
#     now analyzes recent market volatility (e.g., avg ATR, volatility rank)
#     and suggests a `contamination` factor for the Isolation Forest model.
#     This allows the anomaly filter to be stricter in volatile markets and
#     more lenient in calmer ones.
# 2.  ENHANCED: The `ANOMALY_FEATURES` list has been significantly upgraded.
#     It now includes direct price shocks (`pct_change`) and powerful ratio
#     features (`candle_body_size_vs_atr`, `atr_vs_daily_atr`) to give the
#     anomaly model a more nuanced, context-aware view of the market,
#     moving beyond simple absolute values.
# 3.  REFINED: Prompts to the Gemini AI have been consolidated and enriched
#     to ensure that each call is as context-rich as possible, enabling
#     more sophisticated, single-shot analysis for strategy selection and
#     parameter tuning.
#
# V185 UPDATE (API Resiliency):
# 1. ADDED: API Fallback a
#    The `_call_gemini` method now attempts to connect to the primary
#    'gemini-2.0-flash' model up to 3 times.
# 2. ADDED: If all primary attempts fail, it automatically switches to a backup
#    model, 'gemini-1.5-flash', and retries another 3 times. This enhances
#    the framework's robustness against primary API endpoint failures.
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        # V186 - New Anomaly Detection Configuration Prompt
        anomaly_prompt_part = (
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V186_Dynamic_Anomaly",
        "strategy_name": "Meta_Labeling_Filter", "INITIAL_CAPITAL": 100000.0,
        "CONFIDENCE_TIERS": {'ultra_high':{'min':0.85,'risk_mult':1.2,'rr':3.0},'high':{'min':0.75,'risk_mult':1.0,'rr':2.5},'standard':{'min':0.7,'risk_mult':0.8,'rr':2.0}},
