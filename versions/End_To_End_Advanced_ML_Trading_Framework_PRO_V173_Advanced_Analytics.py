# End_To_End_Advanced_ML_Trading_Framework_PRO_V173_Advanced_Analytics.py
#
# V173 UPDATE (Advanced Analytical Components):
# 1. IMPLEMENTED: Fractional Differentiation. The FeatureEngineer can now generate
#    stationary features while preserving market memory, creating more robust inputs
#    for the model (e.g., 'close_fracdiff').
# 2. IMPLEMENTED: Anomaly Detection Filter. An Isolation Forest model now runs within
#    the FeatureEngineer to generate an 'anomaly_score'. The backtester uses this
#    score to block new trades during market conditions deemed anomalous.
# 3. IMPLEMENTED: Monte Carlo Strategy Evaluator. After each cycle, a 5000-path
#    Monte Carlo simulation is run to assess tail risk. The 5% Value-at-Risk (VaR)
#    is calculated and passed to the AI to inform its next decision.
#
# V172 UPDATE (Critical Logging Fix & Forced Exploration):
# 1. FIXED - A critical bug where the file handler for logging was created but
#    never attached to the logger, preventing real-time log file updates.
# 2. IMPLEMENTED - A "Forced Exploration" mechanism to prevent the AI from getting
#    stuck in a loop of very short retraining cycles.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, daily_dd_report: Dict, mc_results: Dict) -> Dict:
        ...
        base_prompt_analysis = "Analyze the recent cycle history, the daily drawdown report, and the Monte Carlo VaR. High daily drawdowns or a large VaR, even without a full cycle circuit breaker, indicate excessive risk. If a 'Circuit Breaker' was tripped, be even more conservative."
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, is_meta_model: bool) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        for candle in candles:
            ...
            if candle.get('anomaly_score') == -1:
                if random.random() < 0.1: # Log anomaly skipping only 10% of the time to avoid spam
                    logger.info(f"  - Skipping trade check due to anomaly detection at {candle['Timestamp']}")
                continue
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    logger.addHandler(file_handler) # This was the missing line
