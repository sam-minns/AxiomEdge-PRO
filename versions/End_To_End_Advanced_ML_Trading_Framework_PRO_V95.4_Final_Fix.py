# End_To_End_Advanced_ML_Trading_Framework_PRO_V95.4_Final_Fix
#
# V95.4 UPDATE (Critical Fix & Refinements):
# 1. CRITICAL FIX: Corrected the data splitting logic in the hyperparameter optimization
#    routine. The `train_test_split` function was incorrectly called with both `shuffle=False`
#    and `stratify=y`, which is an invalid combination that raises a `ValueError`.
#    The `stratify` parameter has been removed to enforce chronological data splitting,
#    which is essential for preventing lookahead bias in a time-series context.
# 2. API UPGRADE: Updated the Gemini API endpoint to use the newer 'gemini-2.0-flash'
#    model as per standing instructions, ensuring access to the latest capabilities.
# 3. ROBUSTNESS: Improved API key handling by prioritizing the `os.getenv` method to securely
#    fetch the key from an environment variable, removing the hardcoded placeholder.
#
# V95.3 UPDATE (Dynamic API Fix):
# 1. ROBUSTNESS: Implemented a dynamic check for the XGBoost library version. The script now
#    automatically uses the modern `callbacks` API for XGBoost >= 1.3 and seamlessly
#    falls back to the legacy `early_stopping_rounds` for older versions.
#    This permanently resolves the `TypeError` across different environments.
# 2. DIAGNOSTICS: Added startup prints for the detected XGBoost version and Python
#    executable path to make environment debugging clear and simple.
# 3. CLARITY: Added logging within the training loop to explicitly state which
#    early stopping method is being used for the current run.
...
from packaging import version # Used for robust version comparison

# --- DYNAMIC API SELECTION & DIAGNOSTICS ---
import sys
import xgboost
...
class GeminiAnalyzer:
    def __init__(self):
        # Fetches key from environment variable first, then defaults to a placeholder.
        ...
        if not api_key or "YOUR" in api_key or "PASTE" in api_key:
            logger.warning("GEMINI_API_KEY not found as an environment variable. AI analysis will be skipped. Please set it to enable this feature.")
            self.api_key_valid = False
        else:
            self.api_key_valid = True
        # Updated to the newer Gemini 2.0 model endpoint as per user instruction.
...
class FeatureEngineer:
    ...
    def _label_group(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        if not group.index.is_unique:
            logger.error(f"Index is not unique for group. Cannot proceed with labeling.")
            return pd.DataFrame()
    ...
    def label_meta_outcomes(self, df: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        for symbol, group in df.groupby('Symbol'):
            logger.info(f"    - Labeling signals for {symbol}...")
            labeled_group = self._label_group(group, lookahead)
...
class ModelTrainer:
    ...
    def train(self,df_train:pd.DataFrame, feature_list: List[str])->Optional[Tuple[Pipeline,float]]:
        ...
        # CRITICAL FIX (V95.4): Removed `stratify=y` because it is incompatible with `shuffle=False`.
        # Preserving chronological order is essential for time-series data to prevent lookahead bias.
        ...
        best_threshold = 0.65 # Using a fixed, higher confidence threshold for the meta-model
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        def objective(trial:optuna.Trial):
            ...
            model_params={
                ...
                'tree_method':'hist', 'use_label_encoder':False, 'seed':42,
            ...
            try:
                ...
                # ================================================================
                # *** DYNAMIC API SELECTION FOR ROBUST EARLY STOPPING (V95.3) ***
                # ================================================================
                # Checks the installed XGBoost version and uses the appropriate API.
                # This ensures compatibility across different environments.
                
                use_modern_api = version.parse(xgboost.__version__) >= version.parse("1.3.0")
                
                if use_modern_api:
                    # Modern API for XGBoost >= 1.3
                    if trial.number == 0: # Log only on the first trial to avoid spam
                        logger.info("    - Using modern `callbacks` API for early stopping (XGBoost >= 1.3).")
                    early_stopping_callback = EarlyStopping(
                        rounds=50, metric_name='logloss', data_name='validation_0', save_best=True
                    )
                    ...
                    # Legacy API for XGBoost < 1.3
                    if trial.number == 0: # Log only on the first trial to avoid spam
                        logger.warning("    - Using legacy `early_stopping_rounds` API (XGBoost < 1.3). Consider upgrading XGBoost.")
                    model.fit(X_train_scaled, y_train,
                                eval_set=[(X_val_scaled, y_val)],
        ...
        try:
            ...
            study.optimize(objective,n_trials=self.config.OPTUNA_TRIALS,timeout=1800,show_progress_bar=False)
            return study
        except Exception as e:
    ...
    def _train_final_model(self,best_params:Dict,X:pd.DataFrame,y:pd.Series)->Optional[Pipeline]:
        ...
        try:
            # Note: We don't use early stopping here as we are training on the full training dataset.
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V95_Dynamic_Fix",
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        train_mask = (df_featured.index >= train_start) & (df_featured.index <= train_end)
        df_train_raw = df_featured.loc[train_mask]
        test_mask = (df_featured.index >= period_start_date) & (df_featured.index <= test_end)
        df_test_chunk = df_featured.loc[test_mask]

        if df_train_raw.empty or df_test_chunk.empty:
            logger.warning(f"  - Skipping cycle {i+1} due to empty data for the period.")
            continue
        ...
        if df_train_labeled.empty or df_train_labeled['target'].nunique()<2:
            logger.warning(f"  - Skipping cycle {i+1}: Not enough data or classes after labeling.")
        ...
        if training_result is None:
            logger.warning(f"  - Skipping cycle {i+1}: Model training failed.")
...
if __name__ == '__main__':
    # Ensure UTF-8 encoding for Windows to prevent character errors with logs/reports
    if os.name == 'nt': os.system("chcp 65001 > $null")
    main()

# End_To_End_Advanced_ML_Trading_Framework_PRO_V95.4_Final_Fix.py