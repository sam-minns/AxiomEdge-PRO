# End_To_End_Advanced_ML_Trading_Framework_PRO_V90.2_SHAP_OPT
#
# V90.2 Update:
# 1. PERFORMANCE FIX: Implemented subsampling in the `_generate_shap_summary` function.
#    SHAP values are now calculated on a random sample of 2000 data points instead
#    of the entire training set, reducing calculation time from minutes to seconds.
#
# V90.1 Update:
# 1. BUG FIX: Corrected a shape mismatch error in the `_generate_shap_summary` function.
#
# V90 Update:
# 1. MAJOR FEATURE EXPANSION: The feature engineering module has been significantly upgraded.
...
class GeminiAnalyzer:
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        try:
            ...
            # Ensure the response structure is valid before accessing parts
    ...
    def _extract_json_from_response(self, response_text: str) -> Dict:
        try:
            ...
            if match:
                ...
                # If no backticks, find the first valid JSON object
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        logger.info("  - Sending cumulative cycle history to Gemini for analysis...")
        prompt = (
            "You are an expert trading model analyst. You are conducting a walk-forward analysis. "
            "Based on the ENTIRE history of previous cycles, the SHAP feature importance from the most RECENT cycle, and the list of ALL available features, "
            "Your goal is to develop a robust forex trading strategy through backtesting. Focus on improving both the F1 score and MAR ratio of the strategy. Ultimately, your aim is to identify and exploit a real, actionable market edge—meaning the strategy should not only perform well statistically but also be profitable and sustainable under live market conditions. Consider technical indicators, risk management, and market dynamics as you design, test, and refine the approach. Provide clear reasoning for each improvement and highlight how changes contribute to the strategy’s edge. "
            "Suggest a new configuration for the NEXT cycle.\n\n"
        ...
        logger.info(f"    - Gemini Suggestion (Raw): {response_text}")
...
class ConfigModel(BaseModel):
    ...
    TRAINING_WINDOW: str; RETRAINING_FREQUENCY: str; LOOKAHEAD_CANDLES: conint(gt=0); MODEL_SAVE_PATH: str = ""
    PLOT_SAVE_PATH: str = ""; REPORT_SAVE_PATH: str = ""; SHAP_PLOT_PATH: str = ""; MIN_CLASS_SAMPLES: conint(gt=10) = 50
    MIN_CLASS_RATIO: confloat(gt=0.01, lt=1) = 0.05; TREND_FILTER_THRESHOLD: confloat(gt=0) = 25.0
    BOLLINGER_PERIOD: conint(gt=0) = 20; STOCHASTIC_PERIOD: conint(gt=0) = 14; CALCULATE_SHAP_VALUES: bool = True
    selected_features: List[str] # Now part of the config
...
class FeatureEngineer:
    ...
    def _calculate_m15_native(self,g:pd.DataFrame)->pd.DataFrame:
        ...
        g_out['macd_signal'] = g_out['macd'].ewm(span=9, adjust=False).mean()
        g_out['macd_histogram'] = g_out['macd'] - g_out['macd_signal']
        
        # On-Balance Volume (OBV)
        obv = (np.sign(g_out['Close'].diff()) * g_out['RealVolume']).fillna(0).cumsum()
        g_out['obv'] = obv
        ...
        g_out['price_above_sma200'] = (g_out['Close'] > g_out['sma_200']).astype(int)
        g_out['dist_from_sma50']=(g_out['Close']-g_out['sma_50'])/g_out['ATR'].replace(0,np.nan)
        
        # More Oscillators
        rsi_14 = g_out['RSI']
        stoch_rsi_k = (rsi_14 - rsi_14.rolling(14).min()) / (rsi_14.rolling(14).max() - rsi_14.rolling(14).min())
        g_out['stoch_rsi'] = stoch_rsi_k.rolling(3).mean() * 100
        typical_price = (g_out['High'] + g_out['Low'] + g_out['Close']) / 3
        ...
        # Volume-based and Momentum
        g_out['adx_trend_strength'] = g_out['ADX']
        g_out['volume'] = g_out['RealVolume']
        g_out['volume_sma20'] = g_out['RealVolume'].rolling(20).mean()
        g_out['volume_sma50'] = g_out['RealVolume'].rolling(50).mean()
        g_out['price_rate_of_change'] = g_out['Close'].pct_change(periods=14)
        g_out['momentum_10'] = g_out['Close'].diff(10)
        g_out['momentum_20'] = g_out['Close'].diff(20)

        # Pivot Points, S/R, Fibonacci (Daily)
        g_with_date = g_out.copy()
        g_with_date['date'] = g_with_date.index.normalize()
        ...
        daily_hlc['support_level'] = daily_hlc['prev_low']; daily_hlc['resistance_level'] = daily_hlc['prev_high']
        daily_range = daily_hlc['prev_high'] - daily_hlc['prev_low']
        daily_hlc['fibonacci_38_2'] = daily_hlc['prev_high'] - daily_range * 0.382
        daily_hlc['fibonacci_50'] = daily_hlc['prev_high'] - daily_range * 0.500
        daily_hlc['fibonacci_61_8'] = daily_hlc['prev_high'] - daily_range * 0.618
        ...
        ha_open = pd.Series(index=g.index, dtype=float); ha_open.iloc[0] = (g['Open'].iloc[0] + g['Close'].iloc[0]) / 2
        ...
        g_out['candle_body_size'] = abs(g_out['Close'] - g_out['Open'])
        g_out['candle_upper_shadow'] = g_out['High'] - pd.concat([g_out['Open'], g_out['Close']], axis=1).max(axis=1)
        ...
        for f in ['divergence_rsi', 'divergence_macd', 'divergence_stoch']: g_out[f] = 0
        
        # Session Volatility
        g_out['session_range'] = 0.0; g_out['session_volatility'] = 0.0
        g_out['session_high'] = 0.0; g_out['session_low'] = 0.0
        ...
        g_out['market_volatility_index'] = g_out['ATR'].rolling(50).rank(pct=True)
        g_out['trend_strength_index'] = g_out['adx_trend_strength'].rolling(50).rank(pct=True)
        g_out['regime_shift_flag'] = (g_out['market_volatility_index'].diff().abs() > 0.2).astype(int)
        
        # --- Final list of features to be shifted ---
        feature_cols = ['ATR', 'RSI', 'ADX', 'bb_width', 'stoch_k', 'stoch_d', 'dist_from_sma50', 'consecutive_candles', 'month', 'week', 'hour', 'day_of_week',
                        'is_london', 'is_ny', 'is_tokyo', 'is_london_ny_overlap', 'london_opening_transition', 'london_closing_transition',
                        'ny_opening_transition', 'ny_closing_transition', 'tokyo_opening_transition', 'tokyo_closing_transition', 'rsi_x_adx', 'contextual_atr',
                        'macd', 'macd_signal', 'macd_histogram', 'obv', 'ema_20', 'ema_50', 'ema_100', 'ema_200', 'sma_20', 'sma_50', 'sma_100', 'sma_200',
                        'price_above_sma200', 'bollinger_upper', 'bollinger_lower', 'bollinger_bandwidth', 'stoch_rsi', 'cci', 'willr',
                        'adx_trend_strength', 'volume', 'volume_sma20', 'volume_sma50', 'price_rate_of_change', 'momentum_10', 'momentum_20',
                        'support_level', 'resistance_level', 'fibonacci_38_2', 'fibonacci_50', 'fibonacci_61_8', 'pivot_point', 'pivot_r1', 'pivot_r2', 'pivot_s1', 'pivot_s2',
                        'aroon_up', 'aroon_down', 'aroon_oscillator', 'heikin_ashi_close', 'heikin_ashi_open', 'heikin_ashi_high', 'heikin_ashi_low',
                        'ichimoku_tenkan_sen', 'ichimoku_kijun_sen', 'ichimoku_senkou_span_a', 'ichimoku_senkou_span_b', 'ichimoku_chikou_span',
                        'price_action_high', 'price_action_low', 'price_action_close', 'price_action_open', 'candle_body_size', 'candle_upper_shadow', 'candle_lower_shadow',
                        'doji_detected', 'engulfing_detected', 'hammer_detected', 'divergence_rsi', 'divergence_macd', 'divergence_stoch',
                        'market_volatility_index', 'trend_strength_index', 'regime_shift_flag', 'session_volatility', 'session_range', 'session_high', 'session_low']
...
class ModelTrainer:
    BASE_FEATURES = [
        ...
        'hammer_detected', 'divergence_rsi', 'divergence_macd', 'divergence_stoch', 'market_volatility_index',
        'trend_strength_index', 'regime_shift_flag', 'session_volatility', 'session_range', 'session_high', 'session_low'
    ]
    ...
    def train(self,df_train:pd.DataFrame, feature_list: List[str])->Optional[Tuple[Pipeline,float]]:
        ...
        if min_ratio<self.config.MIN_CLASS_RATIO or class_counts.min()<self.config.MIN_CLASS_SAMPLES:logger.info(f"    - Classes are imbalanced. Applying SMOTE.");X,y=SMOTE(random_state=42).fit_resample(X,y)
    ...
    def _find_best_threshold(self, best_params, X_train, y_train, X_val, y_val) -> float:
        ...
        for threshold in np.arange(0.3, 0.7, 0.01):
            ...
            preds = np.where(max_probs > threshold, preds, 1) # Default to 'hold' if below threshold
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        try:study=optuna.create_study(direction='maximize');study.optimize(objective,n_trials=self.config.OPTUNA_TRIALS,timeout=1200,show_progress_bar=False);return study
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: pd.Index):
        ...
        try:
            ...
            # To speed up SHAP, run it on a representative sample of the data, not the whole dataset.
            # A sample of 2000 is usually sufficient for a robust global summary.
            ...
            shap_explanation = explainer(X_sample) # Explain the smaller sample
            
            # For multi-class, shap_explanation.abs.mean(0).values gives an array of shape (n_features, n_classes)
            # e.g., (30, 3). We need to average across the classes (axis=1) to get a single importance score per feature.
            mean_abs_shap_values = shap_explanation.abs.mean(0).values

            if mean_abs_shap_values.ndim == 2: # It will be 2D for multi-class, e.g. (30,3)
                overall_importance = mean_abs_shap_values.mean(axis=1)
            else: # It will be 1D for binary/regression
                overall_importance = mean_abs_shap_values
...
class Backtester:
    def __init__(self,config:ConfigModel):self.config=config
    def run_backtest_chunk(self,df_chunk_in:pd.DataFrame,model:Pipeline,feature_list:List[str],confidence_threshold:float,initial_equity:float)->Tuple[pd.DataFrame,pd.Series]:
...
def main():
    ...
    # This dictionary holds the parameters for the first cycle. Gemini will adjust them later.
    current_config={
        "BASE_PATH": os.getcwd(),
        "REPORT_LABEL": "ML_Framework_V90_SHAP_OPT",
        ...
        "OPTUNA_TRIALS": 20, # Initial number of trials
        "TRAINING_WINDOW": '365D',
        "RETRAINING_FREQUENCY": '60D',
        "LOOKAHEAD_CANDLES": 100, # Initial lookahead period
        "MIN_CLASS_SAMPLES": 50,
        "MIN_CLASS_RATIO": 0.05,
        "TREND_FILTER_THRESHOLD": 25.0, # Initial ADX threshold
        "BOLLINGER_PERIOD": 20,
        "STOCHASTIC_PERIOD": 14,
        "CALCULATE_SHAP_VALUES": True,
        "selected_features": ModelTrainer.BASE_FEATURES[:30] # Start with a subset
    }
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        logger.info(f"  - Using Config: LOOKAHEAD={config.LOOKAHEAD_CANDLES}, TREND_THRESH={config.TREND_FILTER_THRESHOLD}, OPTUNA_TRIALS={config.OPTUNA_TRIALS}")
        ...
        backtester=Backtester(config);chunk_trades_df,chunk_equity=backtester.run_backtest_chunk(df_test_chunk,model,current_config['selected_features'],best_threshold,initial_equity=full_equity_curve[-1])
        
        cycle_pnl=0;cycle_win_rate="N/A"
        if not chunk_trades_df.empty:
            ...
            cycle_metrics.append({'Cycle':i+1,'StartDate':period_start_date.date(),'NumTrades':len(chunk_trades_df),'WinRate':cycle_win_rate,'CyclePnL':f"${pnl:,.2f}",'Status':"Success"})
        ...
        cycle_results_for_ai={"cycle":i+1,"f1_score":trainer.study.best_value if trainer.study else 0,"best_threshold":best_threshold,"cycle_pnl":cycle_pnl,"win_rate":cycle_win_rate,"num_trades":len(chunk_trades_df),"current_params":{k:v for k,v in current_config.items() if k in ['LOOKAHEAD_CANDLES','TREND_FILTER_THRESHOLD','OPTUNA_TRIALS','selected_features']}, "shap_summary": trainer.shap_summary.head(10).to_dict() if trainer.shap_summary is not None else {}}
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V90.2_SHAP_OPT.py