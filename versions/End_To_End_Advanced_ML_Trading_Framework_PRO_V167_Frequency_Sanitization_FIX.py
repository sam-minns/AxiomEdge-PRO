# End_To_End_Advanced_ML_Trading_Framework_PRO_V167_Frequency_Sanitization_FIX.py
#
# V167 UPDATE (Critical Fix):
# 1. FIXED - A 'ValueError: only leading negative signs are allowed' crash that
#    occurred if the AI suggested a malformed string for RETRAINING_FREQUENCY.
# 2. IMPLEMENTED - A sanitization function that cleans the AI's frequency
#    suggestion into a valid format (e.g., "90D") before use, preventing the crash.
#
...
def _sanitize_frequency_string(freq_str: Any, default: str = '90D') -> str:
    """Sanitizes a string to be a valid pandas frequency, e.g., '90D'."""
    ...
    if match:
        days = match.group(1)
        sanitized_freq = f"{days}D"
        if sanitized_freq != freq_str:
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    while cycle_num <= len(retraining_dates):
        period_start_date = retraining_dates[cycle_num-1]
        ...
        # Regenerate the date range for subsequent cycles if frequency changed
        new_retrain_freq = pd.to_timedelta(_sanitize_frequency_string(config.RETRAINING_FREQUENCY))
        if new_retrain_freq != retrain_freq:
            logger.info(f"Retraining frequency changed to {config.RETRAINING_FREQUENCY}. Recalculating walk-forward cycles.")
            retraining_dates = pd.date_range(start=test_end + forward_gap, end=end_date, freq=new_retrain_freq)
            total_cycles = cycle_num + len(retraining_dates)
            cycle_num = 0 # Reset to start new cycle list
