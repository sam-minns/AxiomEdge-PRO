# End_To_End_Advanced_ML_Trading_Framework_PRO_V152_Optuna_Progress.py
#
# V152 UPDATE (Optuna Progress):
# 1. ENHANCED - Optuna Progress Indicator: Added a callback to the Optuna
#    study to log the progress after each trial completes. The log now shows
#    the trial number, its score, and the best score so far, providing
#    real-time feedback during the hyperparameter search.
#
...
#    run trials in parallel by adding `n_jobs=-1`.
...
class ModelTrainer:
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        # --- MODIFIED: Added callback for progress logging ---
        def log_trial_progress(study, trial):
            # Check if a best trial exists before accessing its value
            best_value = study.best_value if study.best_trial else "N/A"
            # Format the best_value to 4 decimal places if it's a float
        ...
        try:
            ...
            # --- MODIFIED: Added callbacks list ---
            study.optimize(custom_objective,
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V152_Optuna_Progress",
