# End_To_End_Advanced_ML_Trading_Framework_PRO_V107_Strategy_Playbook
#
# V107 UPDATE (Strategy Playbook & Master Strategist AI):
# 1. NEW - STRATEGY PLAYBOOK: Implemented the user's suggestion to move beyond a
#    monolithic strategy. The framework now contains a 'playbook' of distinct trading
#    archetypes (TrendFollower, MeanReversion, VolatilityBreakout), each with its
#    own philosophy, feature set, and parameter ranges.
# 2. NEW - MASTER STRATEGIST AI: The AI's role has been elevated.
#    - Pre-Flight: The AI now first selects the master strategy from the playbook
#      that it believes will perform best for the entire run, based on history.
#    - In-Cycle: The AI's suggestions are now constrained by the philosophy of the
#      chosen strategy (e.g., it will only tune trend features for a TrendFollower).
# 3. ENHANCED MEMORY: The framework memory now logs which strategy was used for each
#    run, allowing the AI to learn the long-term effectiveness of each approach.
...
# ... (The rest of the script from V106 is largely unchanged, but with key modifications in main(). Full script provided below.)
...
class FeatureEngineer:
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        feature_cols = [f for f in all_features if f in df_final.columns]
...
-------------------- -------------------- -------------------------------- --------------------------------
Version              {_ljust(c_ver, 20)}|{_ljust(p_ver, 32)}|{_ljust(champ_ver, 32)}
...
def main():
    ...
    strategy_playbook = {
        "TrendFollower": {
            ...
            "features": ['ADX', 'adx_x_h1_trend', 'atr_x_d1_trend', 'H1_ctx_Trend', 'D1_ctx_Trend', 'H1_ctx_SMA', 'D1_ctx_SMA', 'market_regime'],
            "lookahead_range": (150, 250), "dd_range": (0.25, 0.40)
        },
        "MeanReversion": {
            "description": "Aims for short-term reversals, using oscillators and volatility bands.",
            "features": ['RSI', 'stoch_k', 'stoch_d', 'bollinger_bandwidth', 'hour'],
            "lookahead_range": (40, 80), "dd_range": (0.15, 0.25)
        },
        "VolatilityBreakout": {
            "description": "Trades breakouts during high volatility sessions.",
            "features": ['ATR', 'bollinger_bandwidth', 'hour', 'day_of_week'],
            "lookahead_range": (60, 120), "dd_range": (0.20, 0.35)
        }
    }
    
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V107_Playbook",
        "run_timestamp": run_timestamp_str, "strategy_name": "TrendFollower", # Default strategy
    ...
    ai_config_suggestion = gemini_analyzer.get_pre_flight_config(framework_history, playbook, fallback_config)
    
    # Update current_config based on AI's chosen strategy
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        # In-cycle AI tunes the currently selected strategy
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V107_Strategy_Playbook.py