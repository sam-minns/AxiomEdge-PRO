# End_To_End_Advanced_ML_Trading_Framework_PRO_V176_Feature_Fallback_FIX.py
#
# V176 UPDATE (Robustness & Configuration Fixes):
# 1. FIXED: A critical bug where a run would fail if the AI selected a strategy
#    but omitted the 'selected_features' list. The framework now automatically
#    falls back to the default feature list from the playbook for the chosen strategy.
# 2. FIXED: The API Timer was not being initialized correctly for single runs.
#    It now correctly enforces the 300-second delay in all scenarios.
# 3. ENHANCED: The initial AI prompt was improved to better guide the AI in
#    providing a complete configuration.
#
# V175 UPDATE (API Delay Standardization):
# 1. FIXED: The conditional API timer logic was removed and set to a constant 300s.
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        prompt = (
            ...
            "2.  **Analyze Market Data**: Review the `MARKET DATA SUMMARY` provided below. Briefly comment on the general market conditions in the `analysis_notes` key.\n"
            ...
            "- `selected_features`: **CRITICAL: You MUST provide a NON-EMPTY list of feature names.** Use the `features` list from the chosen playbook strategy as your starting point.\n"
            "- `RETRAINING_FREQUENCY`: **MUST be a string** with a number and unit (e.g., '90D', '8W').\n"
            "- `MAX_CONCURRENT_TRADES`: An integer from 1 to 10. Higher for volatile/uncorrelated markets.\n"
            "- `PARTIAL_PROFIT_TRIGGER_R`: A float (e.g., 1.5), the R-multiple to trigger a partial close.\n"
            "- `PARTIAL_PROFIT_TAKE_PCT`: A float (e.g., 0.5), for the percentage of the position to close.\n\n"
            "**OUTPUT FORMAT**: Respond ONLY with a single, valid JSON object containing all required keys: `nickname`, `analysis_notes`, `strategy_name`, and the full set of configuration parameters for the chosen strategy.\n\n"
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    # V176 FIX: Implement the feature list fallback
    if not config.selected_features and config.strategy_name in playbook:
        ...
        if default_features:
            ...
            return
    elif not config.selected_features:
        logger.critical(f"FATAL: AI did not provide features and strategy '{config.strategy_name}' is not in playbook.")
        return
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V176_Feature_Fallback_FIX",
    ...
    # Set a fixed 5-minute (300s) delay between API calls for all run types.
