# End_To_End_Advanced_ML_Trading_Framework_PRO_V91_Risk_Managed
#
# V91 Update:
# 1. NEW - Risk Management Circuit Breaker: Added a `MAX_DD_PER_CYCLE` parameter
#    to halt trading within a cycle if the drawdown becomes too severe, preventing
#    catastrophic losses as seen in previous reports.
# 2. NEW - Risk-Adjusted Objective Function: Optuna's objective now optimizes a blend
#    of F1-score and Sortino ratio, forcing the model to find stabler, more
#    profitable parameters instead of just chasing accuracy.
# 3. ENHANCED - AI Prompt: The Gemini prompt is now more sophisticated, asking the AI
#    to suggest risk parameters and focus on creating a robust model.
# 4. REFINED - Data Pipeline: Removed SMOTE in favor of XGBoost's internal
#    `sample_weight` handling for a cleaner, more robust approach to class imbalance.
# 5. CLEANED - Feature Space: Removed non-functional placeholder features to improve
#    the efficiency and focus of the AI-driven feature selection.
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str]) -> Dict:
        ...
        prompt = (
            "You are an expert trading model analyst overseeing a walk-forward analysis. Your primary goal is to create a STABLE and PROFITABLE strategy. "
            "The previous runs showed high returns but suffered from catastrophic drawdowns and a low profit factor, indicating the model was unstable and over-leveraged. "
            "Your suggestions for the NEXT cycle must prioritize risk management and model robustness over raw F1-score.\n\n"
            "Analyze the ENTIRE history of previous cycles and the SHAP feature importance from the most RECENT cycle to suggest a new configuration.\n\n"
            "1.  **Risk Management**: Suggest a new float value for `MAX_DD_PER_CYCLE` (between 0.15 and 0.40). This is a circuit breaker to prevent huge losses in a single cycle. A lower value is safer.\n"
            ...
            "3.  **Feature Selection**: From the `available_features` list, select a SUBSET of features you believe will create a STABLE and GENERALIZABLE model. Do not just pick the top SHAP features; consider feature interactions and avoid overfitting to time-based anomalies.\n\n"
...
class ConfigModel(BaseModel):
    ...
    MAX_DD_PER_CYCLE: confloat(gt=0.05, lt=1.0) = 0.25 # NEW: Risk management parameter
...
# =============================================================================
class DataLoader: # No changes needed here
...
# =============================================================================
class ModelTrainer:
    # UPDATED: Removed placeholder features (divergence_*, session_*)
    BASE_FEATURES = [
    ...
    def train(self,df_train:pd.DataFrame, feature_list: List[str])->Optional[Tuple[Pipeline,float]]:
        ...
        # REMOVED SMOTE: Relying on XGBoost's sample_weight for class imbalance.
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val)->Optional[optuna.study.Study]:
        ...
        def custom_objective(trial:optuna.Trial):
            ...
            try:
                ...
                # Calculate Sortino on validation set predictions
                # Map predictions back to PNL: 1 for win, -1 for loss, 0 for hold
                pnl_map = {-1: 0, 0: -1, 1: -1, 2: 1} # short=-1(0), hold=0(1), long=1(2)
                pnl = pd.Series(preds).map(lambda p: pnl_map[p] if p != 1 else 0)
                
                downside_returns = pnl[pnl < 0]
                downside_std = downside_returns.std()
                sortino = (pnl.mean() / downside_std) if downside_std > 0 else 0
                
                # Combine F1 and Sortino into a single objective. The weights can be tuned.
                # We prioritize stability (Sortino) slightly more than raw accuracy (F1).
                objective_score = (0.4 * f1) + (0.6 * sortino)
                return objective_score

            except Exception as e:logger.warning(f"Trial {trial.number} failed with error: {e}");return -1.0
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: pd.Index):
        ...
        try:
            # PERFORMANCE OPTIMIZATION: Use a random sample for SHAP calculation
            ...
            if mean_abs_shap_values.ndim == 2:
...
class Backtester:
    ...
    def run_backtest_chunk(self,df_chunk_in:pd.DataFrame,model:Pipeline,feature_list:List[str],confidence_threshold:float,initial_equity:float)->Tuple[pd.DataFrame,pd.Series]:
        ...
        # NEW: Risk management for the current cycle
        chunk_peak_equity = initial_equity
        circuit_breaker_tripped = False

        candles=df_chunk.reset_index().to_dict('records')
        for candle in candles:
            # Update peak equity and check for circuit breaker
            if not circuit_breaker_tripped:
                if equity > chunk_peak_equity:
                    chunk_peak_equity = equity
                if (chunk_peak_equity - equity) / chunk_peak_equity > self.config.MAX_DD_PER_CYCLE:
                    ...
                    open_positions = {} # Close all open trades
            ...
            # Do not open new trades if circuit breaker is tripped
...
class PerformanceAnalyzer:
    ...
    def generate_text_report(self,m:Dict[str,Any],cycle_metrics:List[Dict],aggregated_shap:Optional[pd.DataFrame]=None):
        ...
        report=f"""...""" # Report generation is unchanged
        logger.info(report)
        try:
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V91_Risk_Managed",
        "FORWARD_TEST_START_DATE": "2024-01-01", "INITIAL_CAPITAL": 100000.0,
        "CONFIDENCE_TIERS": {
            'ultra_high': {'min': 0.80, 'risk_mult': 1.2, 'rr': 3.0},
            'high':       {'min': 0.70, 'risk_mult': 1.0, 'rr': 2.5},
            'standard':   {'min': 0.60, 'risk_mult': 0.8, 'rr': 2.0}
        },
        "BASE_RISK_PER_TRADE_PCT": 0.01, "SPREAD_PCTG_OF_ATR": 0.05, "SLIPPAGE_PCTG_OF_ATR": 0.02,
        "OPTUNA_TRIALS": 20, "TRAINING_WINDOW": '365D', "RETRAINING_FREQUENCY": '60D',
        ...
        "CALCULATE_SHAP_VALUES": True, "MAX_DD_PER_CYCLE": 0.3, # Initial value for the new risk parameter
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        cycle_results_for_ai={"cycle":i+1,"objective_score":trainer.study.best_value if trainer.study else 0,"best_threshold":best_threshold,"cycle_pnl":cycle_pnl,"win_rate":cycle_win_rate,"num_trades":len(chunk_trades_df),"current_params":{k:v for k,v in current_config.items() if k in ['LOOKAHEAD_CANDLES','TREND_FILTER_THRESHOLD','OPTUNA_TRIALS','selected_features','MAX_DD_PER_CYCLE']}, "shap_summary": trainer.shap_summary.head(10).to_dict() if trainer.shap_summary is not None else {}}
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V91_Risk_Managed.py