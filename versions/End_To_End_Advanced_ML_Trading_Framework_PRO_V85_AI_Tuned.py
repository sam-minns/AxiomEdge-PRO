# End_To_End_Advanced_ML_Trading_Framework_PRO_V85_AI_Tuned
#
# V85 Update:
# 1. NEW - Gemini API Integration via Requests: Re-implemented the GeminiAnalyzer to use
#    the `requests` library as per the new specification, removing the `google-generativeai` SDK.
# 2. NEW - Robust JSON Parsing: Included a helper function to reliably extract and parse
#    JSON from Gemini's text responses, handling markdown code blocks.
# 3. NEW - Dynamic AI-Tuning Loop: The main loop now dynamically updates its configuration
#    for each cycle based on JSON suggestions received from the Gemini API.
# 4. FIXED - Full Metrics Reporting: The PerformanceAnalyzer now includes the comprehensive
#    set of requested backtest metrics, including the MAR ratio.
# 5. FIXED - File Organization: All output files are now saved into a structured
#    `Results/{Report_Label}/` directory.
# 6. This is a complete, unabridged script with all prior bug fixes included.

# --- SETUP INSTRUCTIONS ---
# 1. Install required libraries: pip install requests python-dotenv pandas numpy matplotlib shap xgboost optuna scikit-learn imbalanced-learn pydantic
# 2. Create a file named .env in this directory.
# 3. Add your API key to the .env file: GEMINI_API_KEY="YOUR_API_KEY_HERE"
...
class GeminiAnalyzer:
    ...
    def analyze_cycle_and_suggest_changes(self, cycle_results: Dict) -> Dict:
        logger.info("  - Sending cycle results to Gemini for analysis...")
        
        # Using a single, more direct prompt for efficiency
        prompt = (
            "You are an expert trading model analyst. Given the following walk-forward trading cycle results, "
            "suggest new integer values for 'LOOKAHEAD_CANDLES' (between 50-200), 'OPTUNA_TRIALS' (between 15-30), "
            "and a new float value for 'TREND_FILTER_THRESHOLD' (between 20.0-40.0). "
            "Focus on improving the Profit Factor and reducing the Drawdown. "
...
class ConfigModel(BaseModel):
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        result_folder_path = os.path.join(self.BASE_PATH, "Results", self.REPORT_LABEL)
...
# (These classes are complete and unabridged)
# =============================================================================
class DataLoader:
...
class FeatureEngineer:
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        df_final=df_with_cross_feats.copy()
        df_final.replace([np.inf,-np.inf],np.nan,inplace=True)
        df_final.dropna(subset=[f for f in all_features if f in df_final.columns],inplace=True)
    ...
    def _calculate_m15_native(self,g:pd.DataFrame)->pd.DataFrame:
        ...
        regime_score=(g_out['ADX']-g_out['ADX'].min())/(g_out['ADX'].max()-g_out['ADX'].min())
        g_out['contextual_atr'] = g_out['ATR'] * regime_score
        feature_cols=['ATR','RSI','ADX','bb_width','stoch_k','stoch_d','dist_from_sma50','consecutive_candles','month','week','is_london','is_ny','is_tokyo','is_london_ny_overlap','london_opening_transition','london_closing_transition','ny_opening_transition','ny_closing_transition','tokyo_opening_transition','tokyo_closing_transition','rsi_x_adx','contextual_atr']
...
# =============================================================================
class ModelTrainer:
    BASE_FEATURES=['ATR','RSI','ADX','ATR_rank','RSI_rank','hour','day_of_week','adx_x_h1_trend','atr_x_d1_trend','H1_ctx_H1_Trend','D1_ctx_D1_Trend','bb_width','stoch_k','stoch_d','bb_width_rank','stoch_k_rank','market_regime','dist_from_sma50','consecutive_candles','is_london','is_ny','is_tokyo','is_london_ny_overlap','candles_into_london','candles_into_ny','candles_into_tokyo','rel_strength','month','week','london_opening_transition','london_closing_transition','ny_opening_transition','ny_closing_transition','tokyo_opening_transition','tokyo_closing_transition','rsi_x_adx','contextual_atr']
    def __init__(self,config:ConfigModel):self.config=config;self.shap_summary:Optional[pd.DataFrame]=None;self.class_weights:Optional[Dict[int,float]]=None;self.best_threshold=0.5
    def train(self,df_train:pd.DataFrame)->Optional[Tuple[Pipeline,float]]:
        ...
        self.best_threshold = self._find_best_threshold(study.best_params, X_train, y_train, X_val, y_val)
    ...
    def _find_best_threshold(self, best_params, X_train, y_train, X_val, y_val) -> float:
        ...
        temp_pipeline = Pipeline([('scaler', RobustScaler()), ('model', xgb.XGBClassifier(**{'objective':'multi:softprob','num_class':3,'booster':'gbtree','tree_method':'hist','use_label_encoder':False,**best_params}))])
        temp_pipeline.fit(X_train, y_train)
        probs = temp_pipeline.predict_proba(X_val)
        best_f1, best_thresh = 0, 0.5
        for threshold in np.arange(0.3, 0.7, 0.01):
            preds = np.argmax(probs, axis=1)
            preds = np.where(np.max(probs, axis=1) > threshold, preds, 1)
...
# =============================================================================
# 6. BACKTESTER & 7. PERFORMANCE ANALYZER & 8. MAIN ORCHESTRATOR
# =============================================================================
class Backtester:
    def __init__(self,config:ConfigModel):self.config=config
    def run_backtest_chunk(self,df_chunk_in:pd.DataFrame,model:Pipeline,confidence_threshold:float,initial_equity:float)->Tuple[pd.DataFrame,pd.Series]:
...
def main():
    ...
    current_config={"BASE_PATH":os.getcwd(),"DATA_FILENAMES":["AUDUSD_Daily_202001060000_202506020000.csv","AUDUSD_H1_202001060000_202506021800.csv","AUDUSD_M15_202105170000_202506021830.csv","EURUSD_Daily_202001060000_202506020000.csv","EURUSD_H1_202001060000_202506021800.csv","EURUSD_M15_202106020100_202506021830.csv","GBPUSD_Daily_202001060000_202506020000.csv","GBPUSD_H1_202001060000_202506021800.csv","GBPUSD_M15_202106020015_202506021830.csv","USDCAD_Daily_202001060000_202506020000.csv","USDCAD_H1_202001060000_202506021800.csv","USDCAD_M15_202105170000_202506021830.csv"],"REPORT_LABEL":"ML_Framework_V85_AI_Tuned","FORWARD_TEST_START_DATE":"2024-01-01","INITIAL_CAPITAL":100000.0,"CONFIDENCE_TIERS":{'ultra_high':{'min':0.80,'risk_mult':1.2,'size_mult':1.5,'rr':3.0},'high':{'min':0.70,'risk_mult':1.0,'size_mult':1.0,'rr':2.5},'standard':{'min':0.60,'risk_mult':0.8,'size_mult':0.5,'rr':2.0}},"BASE_RISK_PER_TRADE_PCT":0.01,"SPREAD_PCTG_OF_ATR":0.05,"SLIPPAGE_PCTG_OF_ATR":0.02,"OPTUNA_TRIALS":25,"TRAINING_WINDOW":'365D',"RETRAINING_FREQUENCY":'60D',"LOOKAHEAD_CANDLES":100,"MIN_CLASS_SAMPLES":50,"MIN_CLASS_RATIO":0.05,"TREND_FILTER_THRESHOLD":25.0,"BOLLINGER_PERIOD":20,"STOCHASTIC_PERIOD":14,"CALCULATE_SHAP_VALUES":False}
    try:
        gemini_analyzer = GeminiAnalyzer();config=ConfigModel(**current_config);logger.info(f"-> Configuration loaded for report: {config.REPORT_LABEL}")
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        training_result = trainer.train(df_train_labeled)
        ...
        backtester=Backtester(config);chunk_trades_df,chunk_equity=backtester.run_backtest_chunk(df_test_chunk,model,best_threshold,initial_equity=full_equity_curve[-1])
        ...
        cycle_results_for_ai={"cycle":i+1,"f1_score":trainer.study.best_value if trainer.study else 0,"best_threshold":best_threshold,"cycle_pnl":cycle_pnl,"win_rate":cycle_win_rate,"num_trades":len(chunk_trades_df),"current_params":{"LOOKAHEAD_CANDLES":config.LOOKAHEAD_CANDLES,"TREND_FILTER_THRESHOLD":config.TREND_FILTER_THRESHOLD,"OPTUNA_TRIALS":config.OPTUNA_TRIALS}}
        suggested_params = gemini_analyzer.analyze_cycle_and_suggest_changes(cycle_results_for_ai)
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V85_AI_Tuned.py