# End_To_End_Advanced_ML_Trading_Framework_PRO_V200_Linux.py
#
...
#   1. ADDED (Trade Excursion Analysis - MAE/MFE): The Backtester now calculates the Maximum Adverse Excursion (MAE) and Maximum Favorable Excursion (MFE) for every trade, allowing the AI to analyze *how* a trade won or lost.
#   2. ADDED (Feature Importance History - SHAP Drift): The framework now tracks the history of SHAP feature importance across all cycles, prompting the AI to analyze this for "feature drift" as a sign of regime change.
#   3. ADDED (Hyperparameter History - Optuna Trials): A summary of the best and worst performing hyperparameter sets from all Optuna trials across the entire run is now passed to the AI for long-term memory.
#   4. ENHANCED (AI Analysis & Prompts): The core AI analysis prompt was overhauled to leverage all the new data points (MAE/MFE, SHAP Drift, Optuna History, and Model Quality Scores).
#   5. ENHANCED (Training Failure Logic): Logic for repeated training failures was made more robust, triggering a strategic intervention to question the strategy itself.
