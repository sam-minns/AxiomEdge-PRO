# End_To_End_Advanced_ML_Trading_Framework_PRO_V113_Playbook_FIX
#
# V113 UPDATE (Critical Fixes):
# 1. FIX - DOTENV LOAD: Correctly implemented `load_dotenv()` to ensure the .env file
#    with the API key is found and loaded at runtime.
# 2. FIX - NAMEERROR: Resolved a critical `NameError` where `strategy_playbook` was
#    incorrectly referenced as `playbook`, causing a crash during the walk-forward cycle.
#
# V112 UPDATE (Expanded Playbook & Daemon Control):
# 1. NEW - EXPANDED STRATEGY PLAYBOOK: The playbook has been greatly expanded based
#    on user suggestions. New feature categories (Momentum, Seasonality, etc.) and
#    corresponding feature engineering have been added to create a more diverse
#    set of strategic archetypes for the AI to choose from.
# 2. FIX - DAEMON MODE CONTROLS: The modifiable variables `CONTINUOUS_RUN_HOURS` and
#    `MAX_RUNS` have been added to the top of the `main` function, allowing for
#    easy activation and control of the continuous run mode.
...
class GeminiAnalyzer:
    def __init__(self):
        api_key = os.getenv("GEMINI_API_KEY") # No default key, relies on .env
...
def main():
    # --- Daemon Mode Controls (V112) ---
    # Set to > 0 to run for a specific duration (in hours). Script will stop after this time.
    CONTINUOUS_RUN_HOURS = 1
    # Set to > 1 to run for a specific number of iterations.
    MAX_RUNS = 1
    # --- End Controls ---

    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V113_Playbook_FIX",
    ...
    while True:
        ...
        # A single run of the entire backtest and learning process
        ...
        logger.info(f"Run {run_count} complete. Waiting 5 minutes for the next run...")
        time.sleep(300)
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V113_Playbook_FIX.py