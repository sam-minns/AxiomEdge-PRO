# End_To_End_Advanced_ML_Trading_Framework_PRO_V72_Fixed
#
# V72 Update:
# 1. FIXED - KeyError: Corrected a column naming mismatch for higher-timeframe (HTF)
#    features (e.g., 'H1_ctx_Trend' is now correctly referenced as 'H1_ctx_H1_Trend').
#    This resolves the crash seen in V71.
# 2. FIXED - DeprecationWarning: Updated all `groupby().apply()` calls with
#    `include_groups=False` to adhere to the latest pandas standards and remove warnings.
# 3. Code remains consolidated with the best features of V47 and V55.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V72_Fixed",
    ...
    def load_and_parse_data(self) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        for tf, dfs in data_by_tf.items():
            if dfs:
                ...
                # Group by symbol before cleaning index to handle multiple symbols correctly
                all_symbols_df = []
                for symbol in symbols_found:
                    symbol_df = combined[combined['Symbol'] == symbol]
                    symbol_df = symbol_df[~symbol_df.index.duplicated(keep='first')].sort_index()
                    all_symbols_df.append(symbol_df)
...
class FeatureEngineer:
    ...
    def _calculate_adx(self, g: pd.DataFrame, period: int) -> pd.DataFrame:
        ...
        df['di_plus'] = (df['dm_plus_smooth'] / df['ATR_adx'].replace(0, 1e-9)) * 100
        df['di_minus'] = (df['dm_minus_smooth'] / df['ATR_adx'].replace(0, 1e-9)) * 100
        df['DX'] = (abs(df['di_plus'] - df['di_minus']) / (abs(df['di_plus'] + df['di_minus'])).replace(0, 1e-9)) * 100
        df['ADX'] = df['DX'].ewm(alpha=alpha, adjust=False).mean()
        g['ADX'] = df['ADX'].shift(1)
        return g
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        df_merged.set_index(['Timestamp', 'Symbol'], inplace=True)
        df_merged = df_merged.sort_index()

        df_merged['hour'] = df_merged.index.get_level_values('Timestamp').hour
        df_merged['day_of_week'] = df_merged.index.get_level_values('Timestamp').dayofweek
    ...
    def _calculate_htf_features(self, df: pd.DataFrame, p: str, s: int, a: int) -> pd.DataFrame:
        df_out = df.groupby('Symbol').apply(self._apply_lagged_features, p=p, s=s, a=a, include_groups=False).reset_index()
    ...
    def _calculate_m15_native(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        g_shifted = g.shift(1) # Shift once at the start
        g['ATR'] = g_shifted['High'].sub(g_shifted['Low']).rolling(lookback).mean()
        
        delta = g_shifted['Close'].diff()
        ...
        g = self._calculate_adx(g, lookback) # ADX has internal shift
        
        bb_shifted = self._calculate_bollinger_bands(g, self.config.BOLLINGER_PERIOD)
        g[['bb_upper', 'bb_middle', 'bb_lower', 'bb_width']] = bb_shifted[['bb_upper', 'bb_middle', 'bb_lower', 'bb_width']]
        
        stoch_shifted = self._calculate_stochastic(g, self.config.STOCHASTIC_PERIOD)
        g[['stoch_k', 'stoch_d']] = stoch_shifted[['stoch_k', 'stoch_d']]

        for col in ['ATR', 'RSI', 'bb_width', 'stoch_k']: g[f'{col}_rank'] = g[col].rolling(rank_period).rank(pct=True)

        sma_50 = g_shifted['Close'].rolling(window=sma_period).mean()
        g['dist_from_sma50'] = ((g_shifted['Close'] - sma_50) / g['ATR'].replace(0, np.nan))

        direction = np.sign(g_shifted['Close'].diff())
    ...
    def _label_with_dynamic_barriers(self, df: pd.DataFrame, initial_sl_atr: float, initial_tp_atr: float, lookahead: int, min_class_ratio: float, max_iterations: int) -> pd.DataFrame:
        ...
        for i in range(max_iterations):
            ...
            labeled_df = df.groupby('Symbol').apply(self._label_group, sl_atr=current_sl, tp_atr=current_tp, lookahead=lookahead, include_groups=False)
    ...
    def _label_group(self, group: pd.DataFrame, sl_atr: float, tp_atr: float, lookahead: int) -> pd.DataFrame:
        ...
        for i in range(len(group) - lookahead):
            ...
            tp_long, sl_long = entry_price + current_atr * tp_atr, entry_price - current_atr * sl_atr
            tp_short, sl_short = entry_price - current_atr * tp_atr, entry_price + current_atr * sl_atr
            ...
            elif first_sl_long < first_tp_long: outcomes[i] = -1
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V72_Fixed.py