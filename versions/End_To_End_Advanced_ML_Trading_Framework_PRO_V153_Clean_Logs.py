# End_To_End_Advanced_ML_Trading_Framework_PRO_V153_Clean_Logs_FIXED.py
#
# V153 UPDATE (Clean Logs):
# 1. ENHANCED - Improved Readability: Removed redundant logging statements for AI
#    parameter suggestions. Instead of printing the raw response, parsed
#    suggestions, and applied parameters, the log now shows a single, clean
#    confirmation message, making the output much easier to follow.
#
# V152 UPDATE (Optuna Progress):
# 1. ENHANCED - Optuna Progress Indicator: Added a callback to the Optuna
#    study to log the progress after each trial completes.
#
# V151 UPDATE (Clean XGB Params):
# 1. FIXED - Deprecation Warning: Removed the unnecessary `use_label_encoder`
#    parameter from all XGBoost model configurations.
...
class GeminiAnalyzer:
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to decode Gemini response JSON: {e} - Response: {response.text}"); return "{}"
    ...
    def _create_exploitation_prompt(self, memory: Dict, health_report_str: str, directive_str: str, playbook: Dict) -> str:
        ...
        return (
            ...
            "Your ultimate goal is to produce a high and STABLE risk-adjusted return (MAR Ratio).\n\n"
            "**INSTRUCTIONS:**\n"
            ...
            "3. **Select Strategy**: Choose the `strategy_name` from the playbook that you believe has the highest risk-adjusted potential for the *next* run.\n"
            "4. **Define Initial Parameters**: Define a safe starting configuration for the chosen strategy based on its playbook definition and historical performance.\n\n"
            "Respond ONLY with a valid JSON object containing all required startup parameters (`strategy_name`, `selected_features`, `LOOKAHEAD_CANDLES`, `OPTUNA_TRIALS`, `MAX_DD_PER_CYCLE`, etc.).\n\n"
            f"{directive_str}\n\n"
            f"{health_report_str}"
    ...
    def _create_exploration_prompt(self, playbook: Dict, directives: List[Dict], champion_strategy: Optional[str], exploration_rate: float) -> str:
        logger.info(f"--- ENTERING EXPLORATION MODE (Chance: {exploration_rate:.0%}) ---")
        quarantined_strats = [d['strategy'] for d in directives if d.get('action') == 'FORCE_EXPLORATION']
        ...
        chosen_strategy = random.choice(available_strategies) if available_strategies else "Meta_Labeling_Filter"
        ...
        return (
            ...
            "Respond ONLY with a valid JSON object containing all required startup parameters (`strategy_name`, `selected_features`, etc.).\n"
            f"STRATEGY PLAYBOOK:\n{json.dumps(playbook, indent=2)}\n\n"
        )
    ...
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], fallback_config: Dict, exploration_rate: float) -> Dict:
        ...
        if directives:
            ...
            for d in directives:
                if d.get('action') == 'FORCE_EXPLORATION':
                    directive_str += f"- The strategy '{d['strategy']}' is stagnating. **You MUST SELECT a DIFFERENT strategy** to force exploration. Penalize `{d['strategy']}` heavily in your decision.\n"
        ...
        if is_exploration:
            prompt = self._create_exploration_prompt(playbook, directives, champion_strategy, exploration_rate)
        else:
            prompt = self._create_exploitation_prompt(memory, health_report_str, directive_str, playbook)
        ...
        if suggestions and "strategy_name" in suggestions:
            ...
            logger.info(f"  - Pre-Flight Check complete. AI chose strategy '{final_config['strategy_name']}'.")
            return final_config
        else:
            logger.warning("  - Pre-Flight Check failed to select a strategy. Using fallback.")
            return fallback_config

    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict) -> Dict:
        if not self.api_key_valid: return {}
        strategy_name = strategy_details.get("description", "Unknown Strategy") # Using description for better logging
        
        # --- MODIFIED: Removed redundant logging ---
        # The main loop will now handle the summary log
        
        is_gnn_strategy = strategy_details.get("requires_gnn", False)
        is_meta_label_strategy = strategy_details.get("requires_meta_labeling", False)
        ...
        base_prompt_analysis = "Analyze the recent cycle history. If a 'Circuit Breaker' was tripped, be more conservative with risk parameters."
        json_response_format = "Respond ONLY with a valid JSON object containing these keys."

        if is_gnn_strategy:
            prompt_details = (
                "You are managing a **GNN_Market_Structure** model. Its features are learned automatically, so **DO NOT suggest `selected_features`**.\n\n"
                "**Suggest a NEW configuration for:**\n"
                "- `MAX_DD_PER_CYCLE`: Risk appetite (float between 0.05 and 0.4).\n"
        ...
        elif is_meta_label_strategy:
            prompt_details = (
                "You are managing a **Meta-Labeling** architecture. Your goal is to tune the secondary (XGBoost) model and the volatility filter.\n\n"
                "**Suggest a NEW configuration for:**\n"
                "- `MAX_DD_PER_CYCLE`: Risk appetite (float between 0.05 and 0.4).\n"
                "- `LOOKAHEAD_CANDLES` and `OPTUNA_TRIALS` (integers).\n"
                "- `MIN_VOLATILITY_RANK` & `MAX_VOLATILITY_RANK`: Volatility filter range (floats between 0.0 and 1.0).\n"
                "- `selected_features`: A subset of features for the secondary model from the available list.\n"
            )
            data_context = (
                f"**SUMMARIZED HISTORICAL CYCLE RESULTS:**\n{json.dumps(self._sanitize_dict(historical_results), indent=2)}\n\n"
                f"**AVAILABLE FEATURES FOR META-MODEL:**\n{available_features}"
            )
        else: 
            if historical_results and historical_results[-1].get("Status") == "Circuit Breaker":
                prompt_details = (
                    ...
                    "3.  Consider other risk-reducing changes: **reduce `LOOKAHEAD_CANDLES`**, **reduce `OPTUNA_TRIALS`**, or **simplify the model by reducing `selected_features`**.\n"
                )
                data_context = (
                    f"**FAILED CYCLE CONTEXT:**\n{json.dumps(self._sanitize_dict(historical_results[-1]), indent=2)}\n\n"
                    f"**AVAILABLE FEATURES FOR THIS STRATEGY:**\n{available_features}"
                )
            else:
                prompt_details = (
                    "You are managing a standard model. Your goal is to tune its features and parameters.\n\n"
                    "**Suggest a NEW configuration for:**\n"
                    "- `MAX_DD_PER_CYCLE`: Risk appetite (float between 0.05 and 0.4).\n"
                    "- `LOOKAHEAD_CANDLES`, `OPTUNA_TRIALS` (integers).\n"
                    "- `selected_features`: A subset of features from the available list.\n"
                )
                data_context = (
                    f"**SUMMARIZED HISTORICAL CYCLE RESULTS:**\n{json.dumps(self._sanitize_dict(historical_results), indent=2)}\n\n"
                    f"**AVAILABLE FEATURES FOR THIS STRATEGY:**\n{available_features}"
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration for this new strategy.\n\n"
...
class FeatureEngineer:
    ...
    def __init__(self, config: ConfigModel, timeframe_roles: Dict[str, str]): self.config = config; self.roles = timeframe_roles
    ...
    def _calculate_momentum(self, g:pd.DataFrame) -> pd.DataFrame:
        g['momentum_10'] = g['Close'].diff(10); g['momentum_20'] = g['Close'].diff(20); return g
    def _calculate_seasonality(self, g: pd.DataFrame) -> pd.DataFrame:
        g['month'] = g.index.month; g['week_of_year'] = g.index.isocalendar().week.astype(int); g['day_of_month'] = g.index.day; return g
    ...
    def _calculate_base_tf_native(self, g:pd.DataFrame)->pd.DataFrame:
        ...
        g_out['ATR']=(g_out['High']-g_out['Low']).rolling(lookback).mean();delta=g_out['Close'].diff();gain=delta.where(delta > 0,0).ewm(com=lookback-1,adjust=False).mean()
        loss=-delta.where(delta < 0,0).ewm(com=lookback-1,adjust=False).mean();g_out['RSI']=100-(100/(1+(gain/loss.replace(0,1e-9))))
        g_out=self._calculate_adx(g_out,lookback); g_out=self._calculate_bollinger_bands(g_out,self.config.BOLLINGER_PERIOD)
        g_out=self._calculate_stochastic(g_out,self.config.STOCHASTIC_PERIOD); g_out = self._calculate_momentum(g_out)
        g_out = self._calculate_seasonality(g_out); g_out = self._calculate_candlestick_patterns(g_out)
        ...
        g_out['market_volatility_index'] = g_out['ATR'].rolling(100).rank(pct=True); return g_out
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if base_tf not in data_by_tf: logger.critical(f"Base timeframe '{base_tf}' data is missing. Cannot proceed."); return pd.DataFrame()
        df_base_list = [self._calculate_base_tf_native(group) for _, group in data_by_tf[base_tf].groupby('Symbol')]
        df_base = pd.concat(df_base_list).reset_index(); df_merged = df_base
        ...
        if medium_tf: df_final[f'adx_x_{medium_tf}_trend'] = df_final['ADX'] * df_final.get(f'{medium_tf}_ctx_Trend', 0)
        if high_tf: df_final[f'atr_x_{high_tf}_trend'] = df_final['ATR'] * df_final.get(f'{high_tf}_ctx_Trend', 0)
        ...
        df_final.replace([np.inf,-np.inf],np.nan,inplace=True); df_final.dropna(inplace=True)
        logger.info(f"  - Merged data and created features. Final dataset shape: {df_final.shape}"); logger.info("[SUCCESS] Feature engineering complete."); return df_final
    ...
    def _label_group(self,group:pd.DataFrame,lookahead:int)->pd.DataFrame:
        ...
        for i in range(len(group)-lookahead):
            ...
            first_tp_long=time_to_tp_long[0] if len(time_to_tp_long)>0 else np.inf; first_sl_long=time_to_sl_long[0] if len(time_to_sl_long)>0 else np.inf
            ...
            first_tp_short=time_to_tp_short[0] if len(time_to_tp_short)>0 else np.inf; first_sl_short=time_to_sl_short[0] if len(time_to_sl_short)>0 else np.inf
    ...
    def _label_group_meta(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        signal_df = group[group['primary_model_signal'] != 0].copy()
        if signal_df.empty: return pd.DataFrame()
        outcomes = np.zeros(len(signal_df), dtype=int)
        if not group.index.is_unique: logger.error(f"Index is not unique for group {group['Symbol'].iloc[0]}. Cannot proceed with meta-labeling."); return pd.DataFrame()
        for i in range(len(signal_df)):
            entry_idx_val = signal_df.index[i]
            try: entry_loc = group.index.get_loc(entry_idx_val)
            except KeyError: continue
            future_candles = group.iloc[entry_loc + 1 : entry_loc + 1 + lookahead];
            if future_candles.empty: continue
            entry_price = group.iloc[entry_loc]['Close']; signal = np.sign(signal_df.iloc[i]['primary_model_signal'])
            sl_dist = group.iloc[entry_loc]['ATR'] * 1.5; tp_dist = group.iloc[entry_loc]['ATR'] * 2.0
            if pd.isna(sl_dist) or sl_dist <= 1e-9: continue
            sl = entry_price - sl_dist if signal == 1 else entry_price + sl_dist; tp = entry_price + tp_dist if signal == 1 else entry_price - tp_dist
            if signal == 1:
                tp_hits = future_candles.index[future_candles['High'] >= tp]; sl_hits = future_candles.index[future_candles['Low'] <= sl]
            else:
                tp_hits = future_candles.index[future_candles['Low'] <= tp]; sl_hits = future_candles.index[future_candles['High'] >= sl]
            first_tp_hit = tp_hits[0] if not tp_hits.empty else None; first_sl_hit = sl_hits[0] if not sl_hits.empty else None
            if first_tp_hit and (not first_sl_hit or first_tp_hit < first_sl_hit): outcomes[i] = 1
        signal_df['target'] = outcomes; return signal_df
    def label_meta_outcomes(self, df: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        labeled_dfs = [self._label_group_meta(group, lookahead) for _, group in df.groupby('Symbol')]
        labeled_df = pd.concat(labeled_dfs)
        logger.info(f"  - Generated {len(labeled_df)} meta-labels for training."); return labeled_df

# =============================================================================
# 6. MODELS & TRAINER
# =============================================================================
class GNNModel(torch.nn.Module if GNN_AVAILABLE else object):
    def __init__(self, in_channels, hidden_channels, out_channels): super(GNNModel, self).__init__(); self.conv1 = GCNConv(in_channels, hidden_channels); self.conv2 = GCNConv(hidden_channels, out_channels)
    def forward(self, data): x, edge_index = data.x, data.edge_index; x = self.conv1(x, edge_index); x = F.relu(x); x = F.dropout(x, p=0.5, training=self.training); x = self.conv2(x, edge_index); return x

class ModelTrainer:
    GNN_BASE_FEATURES = ['ATR', 'RSI', 'ADX', 'bollinger_bandwidth', 'stoch_k', 'momentum_10', 'hour', 'day_of_week']
    def __init__(self,config:ConfigModel): self.config=config; self.shap_summary:Optional[pd.DataFrame]=None; self.class_weights:Optional[Dict[int,float]]=None; self.best_threshold=0.5; self.study: Optional[optuna.study.Study] = None; self.is_gnn_model = False; self.is_meta_model = False; self.gnn_model: Optional[GNNModel] = None; self.gnn_scaler = MinMaxScaler(); self.asset_map: Dict[str, int] = {}
    def train(self, df_train: pd.DataFrame, feature_list: List[str], strategy_details: Dict) -> Optional[Tuple[Pipeline, float]]:
        logger.info(f"  - Starting model training using strategy: '{strategy_details.get('description', 'N/A')}'"); self.is_gnn_model = strategy_details.get("requires_gnn", False); self.is_meta_model = strategy_details.get("requires_meta_labeling", False)
        if self.is_gnn_model:
            if not GNN_AVAILABLE: logger.error("  - Skipping GNN model training: PyTorch/PyG libraries not found."); return None
            logger.info("  - GNN strategy detected. Generating graph embeddings as features..."); gnn_embeddings = self._train_gnn(df_train)
            if gnn_embeddings.empty: logger.error("  - GNN embedding generation failed. Aborting cycle."); return None
            X = gnn_embeddings; feature_list = list(X.columns); logger.info(f"  - Feature set replaced by {len(feature_list)} GNN embeddings.")
            y_map={-1:0,0:1,1:2};y=df_train['target'].map(y_map).astype(int); num_classes = 3
        elif self.is_meta_model:
            logger.info("  - Meta-Labeling strategy detected. Training secondary filter model."); X = df_train[feature_list].copy().fillna(0); y = df_train['target'].astype(int); num_classes = 2
        else: X = df_train[feature_list].copy().fillna(0); y_map={-1:0,0:1,1:2};y=df_train['target'].map(y_map).astype(int); num_classes = 3
        if len(y.unique()) < num_classes: logger.warning(f"  - Skipping cycle: Not enough classes ({len(y.unique())}) for {num_classes}-class model."); return None
        self.class_weights=dict(zip(np.unique(y),compute_class_weight(class_weight='balanced',classes=np.unique(y),y=y)))
        X_train_val, X_stability, y_train_val, y_stability = train_test_split(X, y, test_size=0.1, shuffle=False); X_train, X_val, y_train, y_val = train_test_split(X_train_val, y_train_val, test_size=0.2, shuffle=False)
    ...
    def _create_graph_data(self, df: pd.DataFrame) -> Tuple[Optional[Data], Dict[str, int]]:
        logger.info("    - Creating graph structure from asset correlations..."); pivot_df = df.pivot(columns='Symbol', values='Close').ffill().dropna(how='all', axis=1)
        if pivot_df.shape[1] < 2: logger.warning("    - Not enough assets to build a correlation graph. Skipping GNN."); return None, {}
        corr_matrix = pivot_df.corr(); assets = corr_matrix.index.tolist(); asset_map = {asset: i for i, asset in enumerate(assets)}; edge_list = []
        for i in range(len(assets)):
            for j in range(i + 1, len(assets)):
                if abs(corr_matrix.iloc[i, j]) > 0.3: edge_list.extend([[asset_map[assets[i]], asset_map[assets[j]]], [asset_map[assets[j]], asset_map[assets[i]]]])
        if not edge_list: logger.warning("    - No strong correlations found. Creating a fully connected graph as fallback."); edge_list = [[i, j] for i in range(len(assets)) for j in range(len(assets)) if i != j]
        edge_index = torch.tensor(edge_list, dtype=torch.long).t().contiguous(); feature_cols = [f for f in self.GNN_BASE_FEATURES if f in df.columns]
        node_features = df.groupby('Symbol')[feature_cols].mean().reindex(assets).fillna(0)
        node_features_scaled = pd.DataFrame(self.gnn_scaler.fit_transform(node_features), index=node_features.index); x = torch.tensor(node_features_scaled.values, dtype=torch.float)
    ...
    def _train_gnn(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        optimizer = Adam(self.gnn_model.parameters(), lr=0.01, weight_decay=5e-4); self.gnn_model.train()
        for epoch in range(self.config.GNN_EPOCHS): optimizer.zero_grad(); out = self.gnn_model(graph_data); loss = out.mean(); loss.backward(); optimizer.step()
        self.gnn_model.eval()
        with torch.no_grad(): embeddings = self.gnn_model(graph_data).numpy()
        ...
        full_embeddings = df['Symbol'].map(embedding_df.to_dict('index')).apply(pd.Series); full_embeddings.index = df.index; return full_embeddings
    def _get_gnn_embeddings_for_test(self, df_test: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [f for f in self.GNN_BASE_FEATURES if f in df_test.columns]; test_node_features = df_test.groupby('Symbol')[feature_cols].mean()
        aligned_features = test_node_features.reindex(self.asset_map.keys()).fillna(0)
        test_node_features_scaled = pd.DataFrame(self.gnn_scaler.transform(aligned_features), index=aligned_features.index); x = torch.tensor(test_node_features_scaled.values, dtype=torch.float)
        graph_data, _ = self._create_graph_data(df_test)
        if graph_data is None: return pd.DataFrame()
        graph_data.x = x; self.gnn_model.eval()
        with torch.no_grad(): embeddings = self.gnn_model(graph_data).numpy()
        embedding_df = pd.DataFrame(embeddings, index=self.asset_map.keys(), columns=[f"gnn_{i}" for i in range(self.config.GNN_EMBEDDING_DIM)])
        full_embeddings = df_test['Symbol'].map(embedding_df.to_dict('index')).apply(pd.Series); full_embeddings.index = df_test.index; return full_embeddings
    def _find_best_threshold(self, best_params, X_train, y_train, X_val, y_val, num_classes) -> float:
        logger.info("    - Tuning classification threshold for F1 score..."); objective = 'multi:softprob' if num_classes > 2 else 'binary:logistic'
        ...
        fit_params={'model__sample_weight':y_train.map(self.class_weights)}; temp_pipeline.fit(X_train, y_train, **fit_params); probs = temp_pipeline.predict_proba(X_val)
        best_f1, best_thresh = -1, 0.5
        for threshold in np.arange(0.3, 0.7, 0.01):
            if num_classes > 2: max_probs = np.max(probs, axis=1); preds = np.argmax(probs, axis=1); preds = np.where(max_probs > threshold, preds, 1)
            else: preds = (probs[:, 1] > threshold).astype(int)
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        def log_trial_progress(study, trial):
            ...
            best_value_str = f'{best_value:.4f}' if isinstance(best_value, float) else best_value
            logger.info(f"      > Optuna Trial [{trial.number + 1}/{self.config.OPTUNA_TRIALS}] Finished. Value: {trial.value:.4f} | Best Value: {best_value_str}")
        objective = 'multi:softprob' if num_classes > 2 else 'binary:logistic'; eval_metric = 'mlogloss' if num_classes > 2 else 'logloss'
        def custom_objective(trial:optuna.Trial):
            param={'objective':objective,'eval_metric':eval_metric,'booster':'gbtree','tree_method':'hist','seed':42,
                   'n_estimators':trial.suggest_int('n_estimators',200,1000,step=50), 'max_depth':trial.suggest_int('max_depth',3,8),
                   'learning_rate':trial.suggest_float('learning_rate',0.01,0.2,log=True), 'subsample':trial.suggest_float('subsample',0.6,1.0),
                   'colsample_bytree':trial.suggest_float('colsample_bytree',0.6,1.0), 'gamma':trial.suggest_float('gamma',0,5),
                   'reg_lambda':trial.suggest_float('reg_lambda',1e-8,5.0,log=True), 'alpha':trial.suggest_float('alpha',1e-8,5.0,log=True),
            ...
            try:
                ...
                pnl_map = {0: -1, 1: 0, 2: 1} if num_classes > 2 else {0: -1, 1: 1}; pnl = pd.Series(preds).map(pnl_map)
                downside_returns = pnl[pnl < 0]; downside_std = downside_returns.std(); sortino = (pnl.mean() / downside_std) if downside_std > 0 else 0
        ...
        try:
            ...
            study.optimize(custom_objective, n_trials=self.config.OPTUNA_TRIALS, timeout=3600, n_jobs=-1, callbacks=[log_trial_progress])
            return study
    ...
    def _train_final_model(self,best_params:Dict,X:pd.DataFrame,y:pd.Series, feature_names: List[str], num_classes: int)->Optional[Pipeline]:
        ...
        try:
            best_params.pop('early_stopping_rounds', None); objective = 'multi:softprob' if num_classes > 2 else 'binary:logistic'
            ...
            fit_params={'model__sample_weight':y.map(self.class_weights)}; final_pipeline.fit(X,y,**fit_params)
            if self.config.CALCULATE_SHAP_VALUES: self._generate_shap_summary(final_pipeline.named_steps['model'],final_pipeline.named_steps['scaler'].transform(X), feature_names, num_classes)
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: List[str], num_classes: int):
        ...
        try:
            if len(X_scaled) > 2000: logger.info(f"    - Subsampling data for SHAP from {len(X_scaled)} to 2000 rows."); np.random.seed(42); sample_indices = np.random.choice(X_scaled.shape[0], 2000, replace=False); X_sample = X_scaled[sample_indices]
            else: X_sample = X_scaled
            explainer = shap.TreeExplainer(model); shap_explanation = explainer(X_sample)
            if num_classes > 2: mean_abs_shap_per_class = shap_explanation.abs.mean(0).values; overall_importance = mean_abs_shap_per_class.mean(axis=1) if mean_abs_shap_per_class.ndim == 2 else mean_abs_shap_per_class
            else: overall_importance = np.abs(shap_explanation.values).mean(axis=0)
...
# =============================================================================
class Backtester:
    def __init__(self,config:ConfigModel): self.config=config; self.is_meta_model = False
    def run_backtest_chunk(self,df_chunk_in:pd.DataFrame, confidence_threshold:float, initial_equity:float, is_meta_model: bool) -> Tuple[pd.DataFrame,pd.Series,bool,Optional[Dict]]:
        if df_chunk_in.empty:return pd.DataFrame(),pd.Series([initial_equity]), False, None
        df_chunk=df_chunk_in.copy(); self.is_meta_model = is_meta_model
        trades,equity,equity_curve,open_positions=[],initial_equity,[initial_equity],{}; chunk_peak_equity = initial_equity; circuit_breaker_tripped = False; breaker_context = None
        ...
        for candle in candles:
            if not circuit_breaker_tripped:
                ...
                if equity > 0 and chunk_peak_equity > 0 and (chunk_peak_equity - equity) / chunk_peak_equity > self.config.MAX_DD_PER_CYCLE:
                    logger.warning(f"  - CIRCUIT BREAKER TRIPPED! Drawdown exceeded {self.config.MAX_DD_PER_CYCLE:.0%} for this cycle. Closing all positions."); circuit_breaker_tripped = True
            ...
            if symbol not in open_positions:
                ...
                if self.is_meta_model:
                    prob_take_trade = candle.get('prob_1', 0); primary_signal = candle.get('primary_model_signal', 0)
                    if prob_take_trade > confidence_threshold and primary_signal != 0: direction = int(np.sign(primary_signal)); confidence = prob_take_trade
                else:
                    if 'prob_short' in candle:
                        probs=np.array([candle['prob_short'],candle['prob_hold'],candle['prob_long']]); max_confidence=np.max(probs)
                        if max_confidence >= confidence_threshold: pred_class=np.argmax(probs); direction=1 if pred_class==2 else -1 if pred_class==0 else 0; confidence = max_confidence
                if direction != 0:
                    ...
                    tier=self.config.CONFIDENCE_TIERS[tier_name]; base_risk_amt = equity * self.config.BASE_RISK_PER_TRADE_PCT * tier['risk_mult']; risk_amt = min(base_risk_amt, self.config.RISK_CAP_PER_TRADE_USD)
...
class PerformanceAnalyzer:
    ...
    def generate_full_report(self,trades_df:Optional[pd.DataFrame],equity_curve:Optional[pd.Series],cycle_metrics:List[Dict],aggregated_shap:Optional[pd.DataFrame]=None, framework_memory:Optional[Dict]=None) -> Dict[str, Any]:
        ...
        logger.info(f"[SUCCESS] Final report generated and saved to: {self.config.REPORT_SAVE_PATH}"); return metrics
    def plot_equity_curve(self,equity_curve:pd.Series):
        ...
        plt.title(f"{self.config.nickname or self.config.REPORT_LABEL} - Walk-Forward Equity Curve",fontsize=16,weight='bold'); plt.xlabel("Trade Number",fontsize=12);plt.ylabel("Equity ($)",fontsize=12);plt.grid(True,which='both',linestyle=':');
        try: plt.savefig(self.config.PLOT_SAVE_PATH);plt.close(); logger.info(f"  - Equity curve plot saved to: {self.config.PLOT_SAVE_PATH}")
        except Exception as e: logger.error(f"  - Failed to save equity curve plot: {e}")
    def plot_shap_summary(self,shap_summary:pd.DataFrame):
        plt.style.use('seaborn-v0_8-darkgrid');plt.figure(figsize=(12,10)); shap_summary.head(20).sort_values(by='SHAP_Importance').plot(kind='barh',legend=False,color='mediumseagreen')
        title_str = f"{self.config.nickname or self.config.REPORT_LABEL} ({self.config.strategy_name}) - Aggregated Feature Importance"
        plt.title(title_str,fontsize=16,weight='bold'); plt.xlabel("Mean Absolute SHAP Value",fontsize=12);plt.ylabel("Feature",fontsize=12);plt.tight_layout();
        try: plt.savefig(self.config.SHAP_PLOT_PATH);plt.close(); logger.info(f"  - SHAP summary plot saved to: {self.config.SHAP_PLOT_PATH}")
        except Exception as e: logger.error(f"  - Failed to save SHAP plot: {e}")
    def _calculate_metrics(self,trades_df:pd.DataFrame,equity_curve:pd.Series)->Dict[str,Any]:
        m={};m['initial_capital']=self.config.INITIAL_CAPITAL;m['ending_capital']=equity_curve.iloc[-1]; m['total_net_profit']=m['ending_capital']-m['initial_capital'];m['net_profit_pct']=(m['total_net_profit']/m['initial_capital']) if m['initial_capital']>0 else 0
        returns=trades_df['PNL']/m['initial_capital'];wins=trades_df[trades_df['PNL']>0];losses=trades_df[trades_df['PNL']<0]; m['gross_profit']=wins['PNL'].sum();m['gross_loss']=abs(losses['PNL'].sum());m['profit_factor']=m['gross_profit']/m['gross_loss'] if m['gross_loss']>0 else np.inf
        m['total_trades']=len(trades_df);m['winning_trades']=len(wins);m['losing_trades']=len(losses); m['win_rate']=m['winning_trades']/m['total_trades'] if m['total_trades']>0 else 0
        m['avg_win_amount']=wins['PNL'].mean() if len(wins)>0 else 0;m['avg_loss_amount']=abs(losses['PNL'].mean()) if len(losses)>0 else 0; m['payoff_ratio']=m['avg_win_amount']/m['avg_loss_amount'] if m['avg_loss_amount']>0 else np.inf
        m['expected_payoff']=(m['win_rate']*m['avg_win_amount'])-((1-m['win_rate'])*m['avg_loss_amount']) if m['total_trades']>0 else 0
        running_max=equity_curve.cummax();drawdown_abs=running_max-equity_curve;m['max_drawdown_abs']=drawdown_abs.max() if not drawdown_abs.empty else 0; m['max_drawdown_pct']=((drawdown_abs/running_max).replace([np.inf,-np.inf],0).max())*100
        exec_times=pd.to_datetime(trades_df['ExecTime']).dt.tz_localize(None);years=((exec_times.max()-exec_times.min()).days/365.25) if not trades_df.empty else 1; years = max(years, 1/365.25)
        ...
        pnl_series = trades_df['PNL']; win_streaks = (pnl_series > 0).astype(int).groupby((pnl_series <= 0).cumsum()).cumsum(); loss_streaks = (pnl_series < 0).astype(int).groupby((pnl_series >= 0).cumsum()).cumsum()
        m['longest_win_streak'] = win_streaks.max(); m['longest_loss_streak'] = loss_streaks.max(); return m
    def _get_comparison_block(self, metrics: Dict, memory: Dict, ledger: Dict, width: int) -> str:
        champion = memory.get('champion_config'); historical_runs = memory.get('historical_runs', []); previous_run = historical_runs[-1] if historical_runs else None
        def get_data(source: Optional[Dict], key: str, is_percent: bool = False) -> str:
            if not source: return "N/A"
            val = source.get(key) if key in source else source.get("final_metrics", {}).get(key)
        ...
        def get_info(source: Optional[Dict], key: str) -> str:
            if not source: return "N/A"; return str(source.get(key, 'N/A'))
        def get_nickname(source: Optional[Dict]) -> str:
            if not source: return "N/A"; version = get_info(source, 'REPORT_LABEL') or get_info(source, 'script_version'); return ledger.get(version, "N/A")
        c_nick, p_nick, champ_nick = get_nickname(self.config.model_dump()), get_nickname(previous_run), get_nickname(champion)
        ...
        col_w = (width - 5) // 4; header = f"│ {'Metric'.ljust(col_w-1)}│{'Current Run'.center(col_w)}│{'Previous Run'.center(col_w)}│{'All-Time Champion'.center(col_w)}│"; sep = f"├{'─'*(col_w)}┼{'─'*(col_w)}┼{'─'*(col_w)}┼{'─'*(col_w)}┤"
        rows = [f"│ {'Run Nickname'.ljust(col_w-1)}│{c_nick.center(col_w)}│{p_nick.center(col_w)}│{champ_nick.center(col_w)}│", f"│ {'Strategy'.ljust(col_w-1)}│{c_strat.center(col_w)}│{p_strat.center(col_w)}│{champ_strat.center(col_w)}│", f"│ {'MAR Ratio'.ljust(col_w-1)}│{c_mar.center(col_w)}│{p_mar.center(col_w)}│{champ_mar.center(col_w)}│", f"│ {'Max Drawdown'.ljust(col_w-1)}│{c_mdd.center(col_w)}│{p_mdd.center(col_w)}│{champ_mdd.center(col_w)}│", f"│ {'Profit Factor'.ljust(col_w-1)}│{c_pf.center(col_w)}│{p_pf.center(col_w)}│{champ_pf.center(col_w)}│"]
    ...
    def generate_text_report(self, m: Dict[str, Any], cycle_metrics: List[Dict], aggregated_shap: Optional[pd.DataFrame] = None, framework_memory: Optional[Dict] = None):
        WIDTH = 90
        def _box_top(w): return f"╔{'═' * (w-2)}╗"; def _box_mid(w): return f"╠{'═' * (w-2)}╣"; def _box_bot(w): return f"╚{'═' * (w-2)}╝"
        def _box_line(text, w): padding = w - 4 - len(text); return f"║ {text}{' ' * padding} ║" if padding >= 0 else f"║ {text[:w-5]}... ║"
        def _box_title(title, w): return f"║ {title.center(w-4)} ║"
        def _box_text_kv(key, val, w): val_str = str(val); key_len = len(key); padding = w - 4 - key_len - len(val_str); return f"║ {key}{' ' * padding}{val_str} ║"
        ledger = {};
        ...
        report = [_box_top(WIDTH)]; report.append(_box_title('ADAPTIVE WALK-FORWARD PERFORMANCE REPORT', WIDTH)); report.append(_box_mid(WIDTH)); report.append(_box_line(f"Nickname: {self.config.nickname or 'N/A'} ({self.config.strategy_name})", WIDTH)); report.append(_box_line(f"Version: {self.config.REPORT_LABEL}", WIDTH)); report.append(_box_line(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", WIDTH))
        if framework_memory: report.append(_box_mid(WIDTH)); report.append(_box_title('I. PERFORMANCE vs. HISTORY', WIDTH)); report.append(_box_mid(WIDTH)); report.append(self._get_comparison_block(m, framework_memory, ledger, WIDTH))
        sections = {"II. EXECUTIVE SUMMARY": [(f"Initial Capital:", f"${m.get('initial_capital', 0):>15,.2f}"), (f"Ending Capital:", f"${m.get('ending_capital', 0):>15,.2f}"), (f"Total Net Profit:", f"${m.get('total_net_profit', 0):>15,.2f} ({m.get('net_profit_pct', 0):.2%})"), (f"Profit Factor:", f"{m.get('profit_factor', 0):>15.2f}"), (f"Win Rate:", f"{m.get('win_rate', 0):>15.2%}"), (f"Expected Payoff:", f"${m.get('expected_payoff', 0):>15.2f}")], "III. CORE PERFORMANCE METRICS": [(f"Annual Return (CAGR):", f"{m.get('cagr', 0):>15.2%}"), (f"Sharpe Ratio (annual):", f"{m.get('sharpe_ratio', 0):>15.2f}"), (f"Sortino Ratio (annual):", f"{m.get('sortino_ratio', 0):>15.2f}"), (f"Calmar Ratio / MAR:", f"{m.get('mar_ratio', 0):>15.2f}")], "IV. RISK & DRAWDOWN ANALYSIS": [(f"Max Drawdown:", f"{m.get('max_drawdown_pct', 0):>15.2f}% (${m.get('max_drawdown_abs', 0):,.2f})"), (f"Recovery Factor:", f"{m.get('recovery_factor', 0):>15.2f}"), (f"Longest Losing Streak:", f"{m.get('longest_loss_streak', 0):>15} trades")], "V. TRADE-LEVEL STATISTICS": [(f"Total Trades:", f"{m.get('total_trades', 0):>15}"), (f"Average Win:", f"${m.get('avg_win_amount', 0):>15,.2f}"), (f"Average Loss:", f"${m.get('avg_loss_amount', 0):>15,.2f}"), (f"Payoff Ratio:", f"{m.get('payoff_ratio', 0):>15.2f}")]}
        for title, data in sections.items():
            report.append(_box_mid(WIDTH)); report.append(_box_title(title, WIDTH)); report.append(_box_mid(WIDTH))
            for key, val in data: report.append(_box_text_kv(key, val, WIDTH))
        report.append(_box_mid(WIDTH)); report.append(_box_title('VI. WALK-FORWARD CYCLE BREAKDOWN', WIDTH)); report.append(_box_mid(WIDTH))
        ...
        report.append(_box_mid(WIDTH)); report.append(_box_title('VII. MODEL FEATURE IMPORTANCE (TOP 15)', WIDTH)); report.append(_box_mid(WIDTH))
        ...
        report.append(_box_bot(WIDTH)); final_report = "\n".join(report); logger.info("\n" + final_report)
        try:
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict) -> Optional[Dict]:
    ...
    else: champion_to_save = current_champion; champ_calmar_val = current_champion.get("final_metrics", {}).get("mar_ratio", 0); logger.info(f"Current run's MAR Ratio ({new_calmar:.2f}) did not beat champion's ({champ_calmar_val:.2f}).")
    try:
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    DEFAULT_PLAYBOOK = {
        "VolatilityBreakout": {
            "description": "Enters trades on explosive breakouts from low-volatility consolidations, confirmed by ATR and ADX.",
            "features": ["ATR", "bollinger_bandwidth", "ADX", "hour", "day_of_week"],
            "lookahead_range": [60, 120],
            "dd_range": [0.20, 0.35]
        },
        "RangeBound": {
            "description": "Trades reversals within a sideways channel, filtered by a low ADX to confirm a non-trending market.",
            "features": ["ADX", "RSI", "stoch_k", "bollinger_bandwidth", "hour"],
            "lookahead_range": [20, 60],
            "dd_range": [0.10, 0.20]
        },
        "TrendPullback": {
            "description": "A hybrid approach that enters on pullbacks (using RSI) in the direction of an established long-term trend.",
            "features": ["D1_ctx_Trend", "ADX", "RSI", "H1_ctx_Trend", "momentum_10", "hour"],
            "lookahead_range": [50, 150],
            "dd_range": [0.15, 0.30]
        },
        "GNN_Market_Structure": {
            "description": "Uses a Graph Neural Network (GNN) to model inter-asset correlations and generate advanced predictive features.",
            "features": [],
        ...
        "Meta_Labeling_Filter": {
            "description": "A two-stage model that uses a secondary ML filter to improve the signal quality of a simple primary model.",
            "features": ["ADX", "RSI", "ATR", "bollinger_bandwidth", "H1_ctx_Trend", "D1_ctx_Trend"],
            "lookahead_range": [50, 100],
            "dd_range": [0.10, 0.25],
            "requires_meta_labeling": True
        },
        "FilteredBreakout": {
            "description": "A hybrid that trades high-volatility breakouts (ATR, Bollinger) but only in the direction of the long-term daily trend.",
            "features": ["ATR", "bollinger_bandwidth", "D1_ctx_Trend", "ADX", "hour"],
            "lookahead_range": [60, 120],
            "dd_range": [0.20, 0.35]
        },
        "VolatilityTrendReversal": {
            "description": "Identifies trend reversals during periods of low volatility, confirmed by ADX and RSI.",
            "features": ["ATR", "ADX", "RSI", "bollinger_bandwidth", "hour"],
            "lookahead_range": [50, 100],
            "dd_range": [0.15, 0.25]
        },
        "RangeBoundTrendConfirm": {
            "description": "Filters range-bound signals (RSI, Stochastics) by confirming they align with the prevailing higher-timeframe trend.",
            "features": ["RSI", "stoch_k", "ADX", "H1_ctx_Trend", "hour"],
            "lookahead_range": [20, 60],
            "dd_range": [0.10, 0.20]
        },
        "MeanReversionWithTrendBias": {
            "description": "Applies mean reversion (RSI) but only takes trades in the direction of an established trend, confirmed by ADX.",
            "features": ["RSI", "ADX", "H1_ctx_Trend", "hour"],
            "lookahead_range": [20, 60],
            "dd_range": [0.10, 0.20]
        }
    }
    ...
    try:
        ...
        if updated: logger.info("Updating playbook with new default strategies..."); playbook.update({k: v for k, v in DEFAULT_PLAYBOOK.items() if k not in playbook});
...
def initialize_nickname_ledger(ledger_path: str, analyzer: GeminiAnalyzer, script_version: str) -> Dict:
    logger.info("-> Initializing Nickname Ledger..."); ledger = {}
    if os.path.exists(ledger_path):
        try:
            with open(ledger_path, 'r') as f: ledger = json.load(f)
            logger.info(f"  - Loaded existing nickname ledger from: {ledger_path}")
        except (json.JSONDecodeError, IOError) as e: logger.error(f"  - Could not read or parse nickname ledger file. Creating a new one. Error: {e}")
    if script_version not in ledger:
        logger.info(f"  - Script version '{script_version}' is new. Generating a codename..."); new_nickname = analyzer.generate_nickname(list(ledger.values())); ledger[script_version] = new_nickname; logger.info(f"  - Assigned new nickname: '{new_nickname}'")
        try:
            with open(ledger_path, 'w') as f: json.dump(ledger, f, indent=4)
            logger.info(f"  - Saved updated ledger to: {ledger_path}")
        except IOError as e: logger.error(f"  - Failed to save the new nickname to the ledger: {e}")
    else: logger.info(f"  - Found existing nickname for '{script_version}': '{ledger[script_version]}'")
    return ledger

def perform_strategic_review(history: Dict, directives_path: str) -> Tuple[Dict, List[Dict]]:
    logger.info("--- STRATEGIC REVIEW: Analyzing long-term strategy health..."); health_report = {}; directives = []; historical_runs = history.get("historical_runs", [])
    if len(historical_runs) < 3: logger.info("--- STRATEGIC REVIEW: Insufficient history for a full review."); return health_report, directives
    ...
    for name in strategy_names:
        ...
        failures = sum(1 for run in strategy_runs if run.get("final_metrics", {}).get("mar_ratio", 0) < 0.1); chronic_failure_rate = failures / len(strategy_runs)
        total_cycles = sum(len(run.get("cycle_details", [])) for run in strategy_runs); breaker_trips = sum(sum(1 for cycle in run.get("cycle_details", []) if cycle.get("Status") == "Circuit Breaker") for run in strategy_runs)
    ...
    if len(recent_runs) >= 3 and len(set(r.get('strategy_name') for r in recent_runs)) == 1:
        stagnant_strat_name = recent_runs[0].get('strategy_name'); calmar_values = [r.get("final_metrics", {}).get("mar_ratio", 0) for r in recent_runs]
        is_not_improving = calmar_values[2] <= calmar_values[1] <= calmar_values[0]
        if is_not_improving:
            ...
            directives.append(stagnation_directive); logger.warning(f"--- STRATEGIC REVIEW: Stagnation detected for '{stagnant_strat_name}'. Creating directive.")
    try:
...
def determine_timeframe_roles(detected_tfs: List[str]) -> Dict[str, Optional[str]]:
    ...
    tf_with_values = sorted([(tf, FeatureEngineer.TIMEFRAME_MAP.get(tf.upper(), 99999)) for tf in detected_tfs], key=lambda x: x[1]); sorted_tfs = [tf[0] for tf in tf_with_values]
    roles = {'base': sorted_tfs[0], 'medium': None, 'high': None}
    if len(sorted_tfs) == 2: roles['high'] = sorted_tfs[1]
    elif len(sorted_tfs) >= 3: roles['medium'] = sorted_tfs[1]; roles['high'] = sorted_tfs[2]
        if len(sorted_tfs) > 3: logger.warning(f"Detected {len(sorted_tfs)} timeframes. Using {roles['base']}, {roles['medium']}, and {roles['high']}.")
    logger.info(f"Dynamically determined timeframe roles: {roles}"); return roles

def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], is_continuous: bool):
    run_timestamp_str = datetime.now().strftime("%Y%m%d-%H%M%S"); gemini_analyzer = GeminiAnalyzer()
    current_config_dict = fallback_config.copy(); current_config_dict['run_timestamp'] = run_timestamp_str
    health_report, _ = perform_strategic_review(framework_history, fallback_config['DIRECTIVES_FILE_PATH'])
    ai_config_suggestion = gemini_analyzer.get_pre_flight_config(framework_history, playbook, health_report, directives, current_config_dict, exploration_rate=0.25)
    current_config_dict.update(ai_config_suggestion)
    script_name = os.path.basename(__file__) if '__file__' in locals() else fallback_config["REPORT_LABEL"]
    version_label = script_name.replace(".py", ""); current_config_dict['REPORT_LABEL'] = version_label; current_config_dict['nickname'] = nickname_ledger.get(version_label, f"Run-{run_timestamp_str}")
    config = ConfigModel(**current_config_dict)
    file_handler = RotatingFileHandler(config.LOG_FILE_PATH, maxBytes=5*1024*1024, backupCount=2); file_handler.setLevel(logging.DEBUG); file_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'); file_handler.setFormatter(file_formatter); logger.addHandler(file_handler)
    ...
    if not all_files: logger.critical("No data files found in the base path matching pattern. Exiting."); return
    data_by_tf, detected_timeframes = data_loader.load_and_parse_data(all_files)
    if not data_by_tf: return
    tf_roles = determine_timeframe_roles(detected_timeframes); fe = FeatureEngineer(config, tf_roles); full_df = fe.create_feature_stack(data_by_tf)
    ...
    train_window, retrain_freq, forward_gap = pd.to_timedelta(config.TRAINING_WINDOW), pd.to_timedelta(config.RETRAINING_FREQUENCY), pd.to_timedelta(config.FORWARD_TEST_GAP)
    test_start_date = start_date + train_window + forward_gap; retraining_dates = pd.date_range(start=test_start_date, end=end_date, freq=retrain_freq); total_cycles = len(retraining_dates)
    logger.info(f"Walk-forward analysis will run for {total_cycles} cycles.")
    aggregated_trades, aggregated_equity_curve = pd.DataFrame(), pd.Series([config.INITIAL_CAPITAL]); in_run_historical_cycles, aggregated_shap = [], pd.DataFrame()
    ...
    for cycle_num, period_start_date in enumerate(retraining_dates, 1):
        logger.info(f"\n--- Starting Cycle [{cycle_num}/{total_cycles}] ---"); cycle_start_time = time.time()
        train_end = period_start_date - forward_gap; train_start = train_end - train_window; test_end = period_start_date + retrain_freq
        ...
        if df_train_raw.empty or df_test.empty: logger.warning(f"  - No data for cycle [{cycle_num}/{total_cycles}]. Skipping."); continue
        logger.info(f"  - Dates | Train: {train_start.date()}-{train_end.date()} | Test: {period_start_date.date()}-{test_end.date()}")
        strategy_details = playbook.get(config.strategy_name, {}); is_meta_model = strategy_details.get("requires_meta_labeling", False)
        ...
        if df_train_labeled.empty: logger.warning("  - No valid labels generated for this training cycle. Skipping."); continue
        ...
        if not train_result: logger.error("  - Model training failed for this cycle. Skipping."); continue
        pipeline, threshold = train_result
        if trainer.shap_summary is not None: aggregated_shap = trainer.shap_summary if aggregated_shap.empty else aggregated_shap.add(trainer.shap_summary, fill_value=0)
        ...
        backtester = Backtester(config); trades, equity_curve, breaker_tripped, breaker_context = backtester.run_backtest_chunk(df_test, threshold, last_equity, is_meta_model)
        ...
        if not trades.empty: aggregated_trades = pd.concat([aggregated_trades, trades], ignore_index=True); aggregated_equity_curve = pd.concat([aggregated_equity_curve, equity_curve.iloc[1:]], ignore_index=True); last_equity = equity_curve.iloc[-1]
        if breaker_tripped:
            ...
            if failed_cycles_in_a_row >= 2:
                ...
                intervention_suggestion = gemini_analyzer.propose_strategic_intervention(in_run_historical_cycles[-2:], playbook, config.strategy_name, quarantine_list)
                if intervention_suggestion and intervention_suggestion.get("strategy_name") in playbook:
                    logger.info(f"  - Intervention successful. Switching to strategy: {intervention_suggestion['strategy_name']}"); config = ConfigModel(**{**config.model_dump(mode='json'), **intervention_suggestion}); failed_cycles_in_a_row = 0
                else: logger.error("  - Strategic intervention FAILED to select a valid new strategy. Reducing risk and continuing."); config.MAX_DD_PER_CYCLE = max(0.05, config.MAX_DD_PER_CYCLE * 0.75)
        else: failed_cycles_in_a_row = 0
        suggested_params = gemini_analyzer.analyze_cycle_and_suggest_changes(in_run_historical_cycles, all_available_features, strategy_details)
        if suggested_params:
            ...
            if is_continuous: logger.info("Daemon Mode: Pausing for 2.5 minutes to respect API rate limits..."); time.sleep(150)
            else: logger.info("Single Run Mode: Pausing for 2.5 minutes between cycles..."); time.sleep(150)
        logger.info(f"--- Cycle complete. PNL: ${cycle_pnl:,.2f} | Final Equity: ${last_equity:,.2f} | Time: {time.time() - cycle_start_time:.2f}s ---")
    final_metrics = {};
    if not aggregated_trades.empty: pa = PerformanceAnalyzer(config); final_metrics = pa.generate_full_report(aggregated_trades, aggregated_equity_curve, in_run_historical_cycles, aggregated_shap, framework_history)
...
def main():
    ...
    bootstrap_config = ConfigModel(**fallback_config, run_timestamp="init", nickname="init"); playbook = initialize_playbook(bootstrap_config.BASE_PATH); analyzer = GeminiAnalyzer()
    while True:
        run_count += 1
        if is_continuous: logger.info(f"\n{'='*30} STARTING DAEMON RUN {run_count} {'='*30}\n")
        else: logger.info(f"\n{'='*30} STARTING SINGLE RUN {'='*30}\n")
        script_name = os.path.basename(__file__) if '__file__' in locals() else fallback_config["REPORT_LABEL"]
        version_label = script_name.replace(".py", ""); nickname_ledger = initialize_nickname_ledger(bootstrap_config.NICKNAME_LEDGER_PATH, analyzer, version_label)
        ...
        try: run_single_instance(fallback_config, framework_history, playbook, nickname_ledger, directives, is_continuous)
        ...
        if not is_continuous: logger.info("Single run complete. Exiting."); break
        if MAX_RUNS > 1 and run_count >= MAX_RUNS: logger.info(f"Reached max run limit of {MAX_RUNS}. Exiting daemon mode."); break
        if CONTINUOUS_RUN_HOURS > 0 and (datetime.now() - script_start_time).total_seconds() / 3600 >= CONTINUOUS_RUN_HOURS: logger.info(f"Reached max runtime of {CONTINUOUS_RUN_HOURS} hours. Exiting daemon mode."); break
        try:
            sys.stdout.write("\n")
            for i in range(10, 0, -1): sys.stdout.write(f"\r>>> Run {run_count} complete. Press Ctrl+C to stop. Continuing in {i:2d} seconds..."); sys.stdout.flush(); time.sleep(1)
            sys.stdout.write("\n\n")
        except KeyboardInterrupt: logger.info("\n\nDaemon stopped by user. Exiting gracefully."); break
