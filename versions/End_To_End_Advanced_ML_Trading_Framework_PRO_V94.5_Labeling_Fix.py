# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.5_Labeling_Fix
#
# V94.5 Update:
# 1. BUG FIX: Resolved a fatal TypeError in the meta-labeling function. The
#    labeling process is now correctly performed on a per-symbol basis, which
#    prevents errors caused by duplicate timestamps in the combined dataframe.
#
# V94.4 Update:
# 1. BUG FIX: Resolved a KeyError from slicing on non-trading days.
#
# V94.3 Update:
# 1. BUG FIX: Resolved an AttributeError in the primary signal generation.
...
class FeatureEngineer:
    ...
    def _label_group(self, group: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        # This helper function processes a dataframe for a single symbol
        signal_df = group[group['primary_model_signal'] != 0].copy()
        outcomes = np.zeros(len(signal_df), dtype=int)
        
        # Ensure the group index is unique before using get_loc
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V94_Labeling_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.5_Labeling_Fix.py