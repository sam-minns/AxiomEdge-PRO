# End_To_End_Advanced_ML_Trading_Framework_PRO_V105_Champion_Mode
#
# V105 UPDATE (Champion Model & Evolving Baseline):
# 1. NEW - CHAMPION BENCHMARK: Implemented the user's suggestion for an evolving
#    baseline. The `framework_memory.json` file now stores both the full history and
#    a separate "champion_config" representing the best run ever achieved.
# 2. NEW - RISK-ADJUSTED PROMOTION: The "best" run is determined by the highest
#    Calmar Ratio (risk-adjusted return). After each run, if the new result has a
#    better Calmar Ratio, it replaces the old champion.
# 3. ENHANCED PRE-FLIGHT AI: The AI's initial analysis is now more strategic. It is
#    given both the full history and the current champion's configuration, with the
#    explicit goal of creating a new configuration that can beat the champion's
#    risk-adjusted performance.
#
# V104 UPDATE (API and Sanitization Fix):
# 1. Fixed the `400 Bad Request` API error with a robust data sanitization function.
# 2. Updated Pydantic's `.dict()` method to `.model_dump()`.
...
class GeminiAnalyzer:
    ...
    def get_pre_flight_config(self, memory: Dict, fallback_config: Dict) -> Dict:
        ...
        prompt = (
            ...
            "- **The Goal**: Propose a new configuration that intelligently modifies the champion's settings. You might try a different feature subset, adjust risk parameters, or change the lookahead window based on insights from the entire history.\n\n"
            "Respond ONLY with a valid JSON object containing the following keys to configure the next run: `LOOKAHEAD_CANDLES`, `MAX_DD_PER_CYCLE`, `OPTUNA_TRIALS`, and `selected_features`.\n\n"
...
# ... All other classes are unchanged from V102/V104 ...
...
# =============================================================================
def load_framework_memory(memory_path: str) -> Dict:
    """Loads the historical records and the champion configuration."""
    ...
    try:
        ...
        if "champion_config" not in memory or "historical_runs" not in memory:
            logger.warning("Memory file is malformed. Starting fresh.")
            return default_memory
        return memory
...
def save_run_to_memory(memory_path: str, new_run_summary: Dict, current_memory: Dict):
    """Appends the new run and updates the champion if the new run is better."""
    try:
        # Append current run to history
        current_memory["historical_runs"].append(new_run_summary)
        
        # Determine the new champion based on Calmar Ratio
        current_champion = current_memory.get("champion_config")
        new_calmar = new_run_summary.get("final_metrics", {}).get("calmar_ratio", 0)

        if current_champion is None:
            logger.info("No previous champion. This run becomes the new champion.")
            current_memory["champion_config"] = new_run_summary
        else:
            champion_calmar = current_champion.get("final_metrics", {}).get("calmar_ratio", 0)
            if new_calmar > champion_calmar:
                logger.info(f"NEW CHAMPION! Current run's Calmar Ratio ({new_calmar:.2f}) beats the previous champion's ({champion_calmar:.2f}).")
                current_memory["champion_config"] = new_run_summary
            else:
                logger.info(f"Current run's Calmar Ratio ({new_calmar:.2f}) did not beat the champion's ({champion_calmar:.2f}).")
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V105_Champion_Mode",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V105_Champion_Mode.py