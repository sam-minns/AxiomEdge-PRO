# End_To_End_Advanced_ML_Trading_Framework_PRO_V190_Linux.py
#
...
#     (`analyze_cycle_and_suggest_changes`) now receives the full long-term
#     framework history, not just the current run's cycle history. This allows it
...
#     and must justify its choice based on historical performance.
# 5.  ADDED (Dynamic Playbook): A new AI function, `propose_playbook_amendment`,
#     is triggered when a strategy is quarantined. The AI can now suggest permanently
#     retiring or reworking a chronically failing strategy in the `strategy_playbook.json` file.
...
#     It also filters out 'retired' strategies from the playbook.
#
# V189 UPDATE (Forced Simplicity & Decisive Pivots):
# 1.  REINFORCED: AI prompts for training failures have been completely overhauled.
# 2.  ADDED: The AI is now forced to choose one of two distinct recovery paths.
# 3.  ENHANCED: Prompts now have a stronger bias towards simplicity.
# 4.  ENHANCED: Prompts for "probation" strategies prioritize risk reduction.
# 5.  ADDED: AI now suggests the number of `OPTUNA_TRIALS`.
#
# V188 UPDATE (Micro-Dynamic Feature Suite):
# 1.  ADDED: Indicator Dynamics, Relative Performance, Candle Microstructure Analysis.
#
# Linux Conversion:
# 1.  Removed Windows-specific 'chcp' command.
# 2.  Verified all paths are handled using os.path.join().
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V190_LongTermMemory",
