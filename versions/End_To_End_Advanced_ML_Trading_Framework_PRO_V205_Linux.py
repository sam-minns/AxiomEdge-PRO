# End_To_End_Advanced_ML_Trading_Framework_PRO_V205_Linux.py
#
# V205 UPDATE (Realism & Robustness Fixes):
#   1. IMPROVED (Pessimistic Intra-Bar Exit Logic): The Backtester now resolves intra-bar ambiguity
#      by assuming the Stop Loss is always hit first if both SL and TP prices are breached within
#      the same candle. This eliminates a key source of lookahead bias and produces more conservative,
#      realistic results, better aligning with MQL5's tick-based simulation.
#   2. IMPROVED (Trade Cost Realism): The entry price calculation now includes a new `MIN_SPREAD_PIPS`
#      parameter, ensuring a minimum fixed cost is always applied in addition to variable slippage.
#      This prevents the AI from exploiting unrealistically low costs in low-volatility environments.
#   3. ADDED (Model Complexity Penalty): The Optuna hyperparameter optimization objective now includes
#      a penalty term for model complexity (based on `max_depth` and `n_estimators`). This encourages
#      the AI to favor simpler, more robust models and mitigates the risk of overfitting to training data.
#
# V204 UPDATE (Playbook Expansion & Feature Support):
#   1. ADDED (New Strategies): Integrated three new strategies into the default playbook:
#      - 'MACDTrendFollow': A classic trend-following strategy using MACD crossovers filtered by a long-term moving average.
#      - 'EMAScalpingFractal': A high-frequency scalping strategy using EMA alignment and Williams Fractal for entry signals.
#      - 'ClassicBollingerRSI': A traditional mean-reversion strategy with a specific exit rule at the middle Bollinger Band.
#   2. ADDED (New Indicators): The FeatureEngineer now calculates MACD, Williams Fractals, and additional SMAs (20, 100) to support the new strategies.
#   3. ADDED (Custom Exit Logic): The Backtester now supports custom exit logic defined in the playbook (e.g., 'middle_bollinger' exit for the new RSI strategy), allowing for more diverse strategy implementation.
#
# --- SCRIPT VERSION ---
VERSION = "205"
# ---------------------
...
try:
    ...
    GNN_AVAILABLE = False
    # --- MODIFIED: Added dummy classes/values for all imports ---
    # FIX: Define dummy objects with a .Module attribute to prevent errors
    # during class definition when PyTorch/PyG are not installed.
    class _dummy_module_container:
        Module = object
        def __init__(self):
            self.Module = object

    # Assign dummy containers to all required torch-related modules
...
# =============================================================================
# 2. CONFIGURATION & VALIDATION (V206 UPDATE)
# =============================================================================
class ConfigModel(BaseModel):
    ...
    # Replaced static cost model with a more realistic, dynamic one.
    USE_REALISTIC_EXECUTION: bool = True
    SIMULATE_LATENCY: bool = True
    EXECUTION_LATENCY_MS: conint(ge=50, le=500) = 150 # Avg latency in ms
    USE_VARIABLE_SLIPPAGE: bool = True
    SLIPPAGE_VOLATILITY_FACTOR: confloat(ge=0.0, le=5.0) = 1.5 # Multiplier for slippage calc
    SPREAD_CONFIG: Dict[str, Dict[str, float]] = Field(default_factory=lambda: {
        'default': {'normal_pips': 1.8, 'volatile_pips': 5.5}, # Default for any unlisted pair
    ...
    # --- Deprecated in V205, kept for backward compatibility if realism is off ---
    SPREAD_PCTG_OF_ATR: confloat(ge=0) = 0.0 
    SLIPPAGE_PCTG_OF_ATR: confloat(ge=0) = 0.0
    MIN_SPREAD_PIPS: confloat(ge=0) = 0.0
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict, diagnosed_regime: str, regime_champions: Dict, correlation_summary_for_ai: str, macro_context: Dict) -> Dict:
        ...
        task_prompt = (
            "**YOUR TASK: Perform a grounded, event-driven analysis using the provided inter-asset correlations to select the optimal starting strategy.**\n\n"
            ...
            "You must provide the full configuration (`strategy_name`, `selected_features`, etc.) in the JSON response."
        )
...
class FeatureEngineer:
    ...
    # --- MODIFIED: Added MACD_hist for anomaly detection ---
    ANOMALY_FEATURES = [
    ...
    # --- START: NEW HELPER METHODS FOR ADVANCED STRATEGIES ---

    def _calculate_rsi_divergence(self, g: pd.DataFrame, lookback: int = 14) -> pd.DataFrame:
        """Calculates a simplified RSI divergence signal."""
        ...
        g['rsi_bearish_divergence'] = (price_makes_higher_high & rsi_makes_lower_high)

        return g

    def _calculate_hoffman_features(self, g: pd.DataFrame) -> pd.DataFrame:
        """Calculates Hoffman Inventory Retracement Bar features."""
        ...
        # Bullish IRB: In an uptrend, candle body is in the lower 45%
        g['is_hoffman_irb_bullish'] = (
            is_strong_uptrend &
            (((g['Close'] - g['Low']) / candle_range) < 0.45) &
            (((g['Open'] - g['Low']) / candle_range) < 0.45)
        ).astype(int)

        # Bearish IRB: In a downtrend, candle body is in the upper 45%
        g['is_hoffman_irb_bearish'] = (
            is_strong_downtrend &
            (((g['High'] - g['Close']) / candle_range) < 0.45) &
            (((g['High'] - g['Open']) / candle_range) < 0.45)
        ).astype(int)
        return g

    def _calculate_ict_features(self, g: pd.DataFrame, swing_lookback: int = 10) -> pd.DataFrame:
        """Calculates simplified features for ICT concepts."""
        ...
        # 2. Change of Character (CHoCH) & Liquidity Grabs
        # Identify recent swing highs and lows
        swing_highs = g['High'].rolling(swing_lookback, center=True).max().shift(int(swing_lookback/2))
        swing_lows = g['Low'].rolling(swing_lookback, center=True).min().shift(int(swing_lookback/2))

        # Liquidity Grab (Stop Hunt): Wick above/below swing point, close back inside
        ...
        # Change of Character: A break of the most recent *significant* swing point
        # A simple proxy: has price crossed the last major swing high/low?
    ...
    def _calculate_bollinger_bands(self, g:pd.DataFrame, period:int) -> pd.DataFrame:
        ...
        # --- MODIFIED: Added middle band for exit logic strategies ---
    ...
    def _calculate_indicator_dynamics(self, g: pd.DataFrame, period: int = 5) -> pd.DataFrame:
        ...
        # V202 Addition: "Acceleration" features
    ...
    def _calculate_markov_features(self, g: pd.DataFrame) -> pd.DataFrame:
        """Calculates the current streak of consecutive up/down candles."""
    ...
    def _calculate_htf_features(self,df:pd.DataFrame,p:str,s:int,a:int)->pd.DataFrame:
        ...
        for symbol,group in df.groupby('Symbol'):
            ...
            lin_reg_slope = g['Close'].rolling(window=s, min_periods=s).apply(get_rolling_slope, raw=False, args=(s,))
    ...
    def _calculate_base_tf_native(self, g:pd.DataFrame)->pd.DataFrame:
        ...
        # --- Standard Indicators (RSI, ATR, ADX) ---
        ...
        if self.config.USE_PCA_REDUCTION:
            for p in self.config.RSI_PERIODS_FOR_PCA:
                ...
                rs_p = gain_p / loss_p.replace(0, 1e-9)
                g_out[f'rsi_{p}'] = 100 - (100 / (1 + rs_p))
        ...
        # --- General Purpose SMAs & Channels ---
        g_out['SMA_200'] = g_out['Close'].rolling(window=200).mean()
        g_out['SMA_50'] = g_out['Close'].rolling(window=50).mean()
        ...
        # --- Additional EMAs for Scalping & Trend Strategies ---
        ...
        # --- NEW: CALLS TO ADVANCED STRATEGY FEATURE CALCULATORS ---
        ...
        if 'RealVolume' in g_out.columns:
            ...
            if g_out.index.nlevels > 1:
                 ...
                 g_out['relative_strength'] = 0
        else:
            g_out['volume'] = 0
            g_out['relative_strength'] = 0
            
        if 'RealVolume' in g_out.columns and g_out['RealVolume'].sum() > 0:
            tpv = ((g_out['High'] + g_out['Low'] + g_out['Close']) / 3) * g_out['RealVolume']
            cum_volume = g_out.groupby(g_out.index.date)['RealVolume'].transform('cumsum')
            ...
            g_out['VWAP'] = np.nan
            g_out['price_to_vwap'] = np.nan
            g_out['price_vs_vwap_sign'] = np.nan
            g_out['vwap_slope'] = np.nan
    ...
    def _label_group(self,group:pd.DataFrame,lookahead:int)->pd.DataFrame:
        group = group.copy() # Create an explicit copy to prevent SettingWithCopyWarning
...
class ModelTrainer:
    ...
    def train(self, df_train: pd.DataFrame, feature_list: List[str], strategy_details: Dict) -> Optional[Tuple[Pipeline, float]]:
        ...
        self.study=self._optimize_hyperparameters(X_train_val, y_train_val, X_stability, y_stability, num_classes)
    ...
    def _optimize_hyperparameters(self,X_train_val, y_train_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        logger.info(f"    - Starting hyperparameter optimization with Pruning & 3-Fold CV ({self.config.OPTUNA_TRIALS} trials)...")

        def dynamic_progress_callback(study: optuna.study.Study, trial: optuna.trial.FrozenTrial):
            n_trials = self.config.OPTUNA_TRIALS
            trial_number = trial.number + 1
            n_pruned = study.get_trials(deepcopy=False, states=[optuna.trial.TrialState.PRUNED])
            progress_str = f"> Optuna: Trial {trial_number}/{n_trials} (Pruned: {len(n_pruned)}) | Best Score: {study.best_value:.4f}"
            sys.stdout.write(f"\r{progress_str.ljust(85)}")
            sys.stdout.flush()
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_val)):
                ...
                try:
                    ...
                    model.fit(X_train_scaled, y_train, eval_set=[(X_val_scaled, y_val)], 
                              ...
                              early_stopping_rounds=50, **fit_params)
                    ...
                    sharpe_proxy = (mean_return / std_return) if std_return > 1e-9 else (mean_return * 10)
                    
                    # Exponential complexity penalty to more aggressively punish overfitting.
                    depth_penalty = 1.05 ** (param['max_depth'] - 3) - 1
                    estimator_penalty = 0.0001 * (param['n_estimators'] - 200)
                    complexity_penalty = depth_penalty + estimator_penalty
                    
                    # Apply penalty as a divisor to Sharpe, making complex models much less attractive
                    final_objective_score = sharpe_proxy / (1 + max(0, complexity_penalty))
                    cv_scores.append(final_objective_score)

                except optuna.exceptions.TrialPruned:
                     raise
                except Exception as e:
                    sys.stdout.write("\n")
                    logger.warning(f"Trial {trial.number}, Fold {fold+1} failed with error: {e}")
                    cv_scores.append(-2.0)
...
class Backtester:
    ...
    # V205 UPDATE: New private method to handle complex cost calculations
    def _calculate_realistic_costs(self, candle: Dict) -> Tuple[float, float]:
        ...
        spread_info = self.config.SPREAD_CONFIG.get(symbol, self.config.SPREAD_CONFIG['default'])
        vol_rank = candle.get('market_volatility_index', 0.5)
        # Spread widens if volatility is in the top 20%
        spread_pips = spread_info['volatile_pips'] if vol_rank > 0.8 else spread_info['normal_pips']
        spread_cost = spread_pips * point_size

        # 2. Variable Slippage Calculation
        slippage_cost = 0
        if self.config.USE_VARIABLE_SLIPPAGE:
            atr = candle.get('ATR', 0)
            # Slippage is a random factor of ATR, scaled by the volatility factor
            random_factor = random.uniform(0.1, 1.0) * self.config.SLIPPAGE_VOLATILITY_FACTOR
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, strategy_details: Dict) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        # V205 UPDATE: Loop starts from 1 to allow looking at prev_candle for signals
        for i in range(1, len(candles)):
            current_candle = candles[i]
            prev_candle = candles[i-1] # Signal is generated on the previous candle
            ...
            for symbol, pos in open_positions.items():
                ...
                if pos['direction'] == 1:
                    if candle_low <= pos['sl'] and candle_high >= pos['tp']:
                        pnl, exit_price, exit_reason = -pos['risk_amt'], pos['sl'], "Stop Loss (Pessimistic)"
                        stop_hit_first = True
                else:
                    if candle_high >= pos['sl'] and candle_low <= pos['tp']:
                        pnl, exit_price, exit_reason = -pos['risk_amt'], pos['sl'], "Stop Loss (Pessimistic)"
                        stop_hit_first = True
                ...
                if stop_hit_first:
                    ...
                    trades.append({'ExecTime': current_candle['Timestamp'], 'Symbol': symbol, 'PNL': pnl, 'Equity': equity, 'Confidence': pos['confidence'], 'Direction': pos['direction'], 'ExitReason': exit_reason, 'MAE': round(mae, 5), 'MFE': round(mfe, 5)})
                ...
                if self.use_tp_ladder:
                    for level_idx, tp_level_price in enumerate(pos['tp_prices']):
                        if not pos['tp_levels_hit'][level_idx]:
                            if (pos['direction'] == 1 and candle_high >= tp_level_price) or (pos['direction'] == -1 and candle_low <= tp_level_price):
                                ...
                                day_peak_equity = max(day_peak_equity, equity)
                                equity_curve.append(equity)
                                trades.append({'ExecTime': current_candle['Timestamp'], 'Symbol': symbol, 'PNL': level_pnl, 'Equity': equity, 'Confidence': pos['confidence'], 'Direction': pos['direction'], 'ExitReason': f"TP Ladder {level_idx+1}"})
                    ...
                    if pos['risk_amt_remaining'] > 1e-6:
                        if (pos['direction'] == 1 and candle_low <= pos['sl']) or (pos['direction'] == -1 and candle_high >= pos['sl']):
                            ...
                            mae = abs(pos['mae_price'] - pos['entry_price'])
                            mfe = abs(pos['mfe_price'] - pos['entry_price'])
                            trades.append({'ExecTime': current_candle['Timestamp'], 'Symbol': symbol, 'PNL': sl_pnl, 'Equity': equity, 'Confidence': pos['confidence'], 'Direction': pos['direction'], 'ExitReason': "Stop Loss (Ladder)", 'MAE': round(mae, 5), 'MFE': round(mfe, 5)})
                            symbols_to_close.append(symbol)
                            if equity <= 0: continue
                    ...
                    if self.config.USE_PARTIAL_PROFIT and not pos['partial_profit_taken']:
                        ...
                        if (pos['direction'] == 1 and candle_high >= partial_tp_price) or (pos['direction'] == -1 and candle_low <= partial_tp_price):
                            partial_pnl = pos['risk_amt'] * self.config.PARTIAL_PROFIT_TRIGGER_R * self.config.PARTIAL_PROFIT_TAKE_PCT
                            equity += partial_pnl
                            day_peak_equity = max(day_peak_equity, equity)
                            equity_curve.append(equity)
                            trades.append({'ExecTime': current_candle['Timestamp'], 'Symbol': symbol, 'PNL': partial_pnl, 'Equity': equity, 'Confidence': pos['confidence'], 'Direction': pos['direction'], 'ExitReason': f"Partial TP ({self.config.PARTIAL_PROFIT_TAKE_PCT:.0%})"})
                            pos['risk_amt'] *= (1 - self.config.PARTIAL_PROFIT_TAKE_PCT)
                            pos['sl'] = pos['entry_price']
                            pos['partial_profit_taken'] = True

                    if pos['direction'] == 1:
                        if candle_low <= pos['sl']: pnl, exit_price, exit_reason = -pos['risk_amt'], pos['sl'], "Stop Loss"
                        elif candle_high >= pos['tp']: pnl, exit_price, exit_reason = pos['risk_amt'] * pos['rr'], pos['tp'], "Take Profit"
                    elif pos['direction'] == -1:
                        if candle_high >= pos['sl']: pnl, exit_price, exit_reason = -pos['risk_amt'], pos['sl'], "Stop Loss"
                        elif candle_low <= pos['tp']: pnl, exit_price, exit_reason = pos['risk_amt'] * pos['rr'], pos['tp'], "Take Profit"
            ...
            # V205 UPDATE: Signal generation and trade entry logic now considers latency
            ...
            if not circuit_breaker_tripped and symbol not in open_positions and len(open_positions) < max_concurrent_trades:
                ...
                # Signal is based on PREVIOUS candle's data
                if self.is_transformer_model:
                    predicted_price = prev_candle.get('predicted_price', 0)
                    current_price = prev_candle.get('Close', 0)
                    if predicted_price > 0 and current_price > 0:
                        if predicted_price > current_price * 1.0005: direction = 1
                        elif predicted_price < current_price * 0.9995: direction = -1
                        confidence = min(0.99, abs(predicted_price / current_price - 1) * 100)
                ...
                if direction != 0:
                    ...
                    # V206 UPDATE: Realistic execution at the NEXT candle's open
                    ...
                    else: # Fallback to old model if realism is disabled
                        point_size = 0.0001 if 'JPY' not in symbol else 0.01
                        spread_cost = self.config.MIN_SPREAD_PIPS * point_size
                        slippage_cost = atr * self.config.SLIPPAGE_PCTG_OF_ATR
                        total_cost = spread_cost + slippage_cost

                    entry_price = entry_price_base + (total_cost * direction)
                    sl_price, tp_price = entry_price - sl_dist * direction, entry_price + tp_dist * direction

                    open_positions[symbol]={'direction':direction, 'entry_price': entry_price, 'sl':sl_price,'tp':tp_price, 'risk_amt':risk_amt, 'rr':tier['rr'], 'confidence':confidence, 'sl_dist': sl_dist, 'partial_profit_taken': False, 'lot_size': lots, 'margin_used': margin_required, 'mfe_price': entry_price, 'mae_price': entry_price}
...
# Assuming 'logger' is configured elsewhere in your script
    ...
    for name, ticker in tickers.items():
        try:
            ...
            if not data.empty and len(data) > 5:
                ...
                # If 'Close' is a DataFrame (e.g., from multi-level columns), select the first column.
                ...
                # Ensure the extracted values are scalars before comparison
                ...
                macro_context[name] = {
                    ...
                    "1_week_change_pct": round(week_change_pct, 2)
                }
            else:
...
def initialize_playbook(playbook_path: str) -> Dict:
    # This default playbook now includes explicit metadata for the AI to make more
    # informed, data-driven initial strategy selections.
    DEFAULT_PLAYBOOK = {
        # --- TREND-FOLLOWING STRATEGIES ---
        "TrendPullback": {
            "description": "[TRENDING] Enters on pullbacks (RSI) in the direction of an established long-term trend.",
            "features": ["DAILY_ctx_Trend", "ADX", "RSI", "H1_ctx_Trend", "momentum_10_slope", "hour", "close_fracdiff", "relative_performance"],
            "lookahead_range": [50, 150], "dd_range": [0.15, 0.3],
            "ideal_regime": ["Strong Trending", "Weak Trending"],
            "ideal_macro_env": ["Risk-On", "Risk-Off"], # Works in clear directional environments
            "asset_class_suitability": ["Forex Majors", "Indices", "Commodities"]
        },
        "MACDTrendFollowing": {
            ...
            "lookahead_range": [50, 150], "dd_range": [0.15, 0.3],
            "ideal_regime": ["Strong Trending", "Weak Trending"],
            "ideal_macro_env": ["Risk-On", "Risk-Off", "Neutral"],
            "asset_class_suitability": ["Forex Majors", "Indices", "Equities"]
        },
        "EmaCrossoverRsiFilter": {
            ...
            "ideal_macro_env": ["Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Indices", "Equities", "Forex Majors"]
        },
        "HeikinAshiTrend": {
            ...
            "ideal_regime": ["Strong Trending", "Weak Trending"],
            "ideal_macro_env": ["Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Indices", "Commodities", "Forex Majors"]
        },
        "HoffmanTrendRetracement": {
            ...
            "ideal_regime": ["Strong Trending"],
            "ideal_macro_env": ["Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Equities", "Indices", "Forex Majors"]
        },
        "RSIPullback": {
        ...
        "VWAPMomentum": {
            ...
            "lookahead_range": [40, 120], "dd_range": [0.15, 0.3],
            "ideal_regime": ["Weak Trending", "Strong Trending"],
            "ideal_macro_env": ["Neutral", "Risk-On"],
            "asset_class_suitability": ["Indices", "Equities"]
        },
        ...
        "FilteredBreakout": {
            ...
            "ideal_regime": ["High Volatility", "Strong Trending"],
            "ideal_macro_env": ["Event-Driven", "Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Indices", "Commodities", "Forex Majors"]
        },
        "VolatilityExpansionBreakout": {
            ...
            "features": ["bollinger_bandwidth", "ATR", "market_volatility_index", "momentum_20", "DAILY_ctx_Trend", "H1_ctx_Trend", "RSI_slope"],
            "lookahead_range": [70, 140], "dd_range": [0.2, 0.4],
            "ideal_regime": ["Low Volatility", "High Volatility"], # Starts in low, profits in high
            "ideal_macro_env": ["Event-Driven", "Neutral"], # Pre-news consolidation
        ...
        "StanWeinsteinBreakout": {
            ...
            "asset_class_suitability": ["Equities", "Indices"]
        },
        "DonchianBreakout": {
            "description": "[BREAKOUT] A classic breakout strategy that buys/sells when the price breaks the Donchian Channel.",
            "features": ["donchian_channel", "ATR", "ADX"],
            "lookahead_range": [50, 150], "dd_range": [0.2, 0.4],
            "ideal_regime": ["Strong Trending", "High Volatility"],
            "ideal_macro_env": ["Event-Driven", "Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Commodities", "Indices"]
        },
        "MACDFractalBreakout": {
            ...
            "ideal_regime": ["Weak Trending", "Strong Trending", "High Volatility"],
            "ideal_macro_env": ["Neutral", "Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Any"]
        },
        ...
        "RangeBound": {
            ...
            "ideal_regime": ["Ranging", "Low Volatility"],
            "ideal_macro_env": ["Neutral"],
            "asset_class_suitability": ["Forex Crosses", "Forex Majors"]
        },
        "MeanReversionOscillator": {
            ...
            "features": ["RSI", "stoch_k", "ADX", "market_volatility_index", "close_fracdiff", "hour", "day_of_week", "wick_to_body_ratio"],
        ...
        "ClassicBollingerRSI": {
            "description": "[RANGING] A traditional mean-reversion strategy with a specific exit rule at the middle Bollinger Band.",
            "features": ["bollinger_lower", "bollinger_upper", "bollinger_middle", "RSI", "ADX"],
        ...
        "RSIDivergenceReversal": {
            "description": "[RANGING/REVERSAL] A counter-trend strategy that enters when price action diverges from the RSI.",
            "features": ["rsi_bullish_divergence", "rsi_bearish_divergence", "stoch_k", "stoch_d", "ADX"],
            "lookahead_range": [30, 90], "dd_range": [0.15, 0.3],
            "ideal_regime": ["Ranging", "Weak Trending"], # Catches trend exhaustion
            "ideal_macro_env": ["Neutral"],
            "asset_class_suitability": ["Any"]
        },
        ...
        "EMAScalpingFractal": {
            ...
            "features": ["EMA_20", "EMA_50", "EMA_100", "fractal_up", "fractal_down"],
            "lookahead_range": [15, 45], "dd_range": [0.1, 0.25],
            "ideal_regime": ["Weak Trending", "Strong Trending"],
            "ideal_macro_env": ["Neutral"], # Avoids high-impact news
            "asset_class_suitability": ["Forex Majors", "Indices"]
        },
        "EngulfingRsiScalp": {
        ...
        "PanicFade": {
            ...
            "ideal_regime": ["High Volatility"],
            "ideal_macro_env": ["Event-Driven", "Risk-Off"],
            "asset_class_suitability": ["Any"]
        },
        "ICTMarketStructure": {
            "description": "[PRICE ACTION/INSTITUTIONAL] A methodology focused on identifying liquidity zones and Fair Value Gaps (FVG).",
            "features": ["fvg_bullish_exists", "fvg_bearish_exists", "choch_up_signal", "choch_down_signal", "liquidity_grab_up", "liquidity_grab_down", "DAILY_ctx_Trend"],
            "lookahead_range": [40, 120], "dd_range": [0.2, 0.35],
            "ideal_regime": ["Strong Trending", "Weak Trending"],
            "ideal_macro_env": ["Neutral", "Risk-On", "Risk-Off"],
            "asset_class_suitability": ["Forex Majors", "Indices"]
        ...
        "GNN_Market_Structure": {
            ...
            "features": [], # Features are generated by the GNN itself
            "lookahead_range": [80, 150], "dd_range": [0.15, 0.3],
            "requires_gnn": True,
            "ideal_regime": ["Any"], # Its goal is to find relationships regardless of regime
            "ideal_macro_env": ["Any"],
            "asset_class_suitability": ["Forex Majors", "Indices"] # Requires multiple correlated assets
        },
        "TransformerForecaster": {
            "description": "[SPECIALIZED] Uses a Transformer deep learning model to directly forecast the next price movement.",
            "features": ["ATR", "RSI_slope_acceleration", "bollinger_bandwidth", "momentum_10", "DAILY_ctx_LinRegSlope", "H1_ctx_Trend", "hour"],
            "lookahead_range": [30, 90], "dd_range": [0.15, 0.35],
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    # <-- NEW: Fetch macro context data at the start of the run
    macro_context = get_macro_context_data()

    # --- NEW: Correlation Matrix for AI Context ---
    ...
    # <-- MODIFIED: Pass macro_context to the AI for initial setup
    # NOTE: You will also need to update the function definition in the GeminiAnalyzer class to accept this new argument.
    ...
    while cycle_num < len(retraining_dates):
        ...
        # <-- MODIFIED: Add .copy() to prevent SettingWithCopyWarning
        ...
        # <-- MODIFIED: This entire block handles failures and quarantines correctly.
        if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
            ...
            # <-- MODIFIED: Pass macro_context to the AI for cycle analysis
            # NOTE: You will also need to update the function definition in the GeminiAnalyzer class to accept this new argument.
...
def main():
    ...
    fallback_config={
        ...
        "MIN_SPREAD_PIPS": 0.5,
        # --- MODIFIED (V205): Reduced trials to account for 3-fold CV ---
        "OPTUNA_TRIALS": 30, # Was 50. Each trial is now 3x more robust.
        # --- END MODIFIED ---
