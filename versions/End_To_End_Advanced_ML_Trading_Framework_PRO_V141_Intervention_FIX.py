# End_To_End_Advanced_ML_Trading_Framework_PRO_V141_Intervention_FIX.py
#
# V141 UPDATE (Strategic Intervention FIX):
# 1. CRASH FIX - Constraint-Aware AI: Updated the "SAFETY FIRST" prompt to include the
#    valid range for MAX_DD_PER_CYCLE, preventing the AI from suggesting a value
#    that violates Pydantic model validation and causes a crash.
# 2. NEW LOGIC - Failure Pattern Detection: The main walk-forward loop now tracks
#    consecutive "Circuit Breaker" failures for a given strategy.
# 3. NEW AI FUNCTION - Strategic Intervention: Added a new `propose_strategic_intervention`
#    method to the GeminiAnalyzer. This is triggered after 2+ consecutive failures.
# 4. NEW PROMPT - Breaking the "Death Spiral": The new intervention prompt provides the
#    AI with the history of recent failures and instructs it to ABANDON the failing
#    strategy and select a DIFFERENT one from the playbook, preventing it from getting stuck.
#
# V140 UPDATE (GNN Hybrid Integration):
# 1. GNN MODULE INTEGRATED: Transplanted the Graph Neural Network logic directly into
#    the ModelTrainer, enabling a 'GNN Mode' for feature generation.
# 2. DYNAMIC GNN ACTIVATION: Added a 'GNN_Market_Structure' strategy to the playbook
#    with a `requires_gnn: true` flag to dynamically trigger the GNN pipeline.
...
class GeminiAnalyzer:
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str) -> Dict:
        """Called when a strategy has failed repeatedly. Prompts the AI to pick a new strategy."""
        if not self.api_key_valid: return {}
        ...
        prompt = (
            ...
            "2.  **Select a NEW STRATEGY:** You **MUST** choose a *different* strategy from the playbook that you believe is better suited "
            f"for the current market conditions. **DO NOT SELECT `{last_failed_strategy}` again.** Consider a completely different approach (e.g., if a trend strategy failed, consider mean-reversion or a GNN structural model).\n"
            "3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration (`MAX_DD_PER_CYCLE`, `LOOKAHEAD_CANDLES`, etc.) for this new strategy.\n\n"
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V141_Intervention_FIX",
