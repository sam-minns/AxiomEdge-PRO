# End_To_End_Advanced_ML_Trading_Framework_PRO_V65_Hybrid_Final
#
# V65 Update:
# 1. FINAL AUTOBNN FIX: Corrected the KeyError by using the full, correct likelihood
#    model name: "normal_likelihood_logistic_noise".
# 2. Refactored the FeatureEngineer and HybridModelTrainer for a cleaner data
#    flow and to better support the multi-model architecture.
# 3. Simplified the main loop and GNN data creation for robustness.
...
class AutoBNNTrainer:
    ...
    def _create_model(self, model_type: str, input_dim: int):
        ...
        return ab.estimators.AutoBnnMapEstimator(
            model_type,
            likelihood_model="normal_likelihood_logistic_noise", # FIX: Use full string key
            seed=key,
            **common_config
        )
...
class HybridModelTrainer:
    ...
    def predict_proba(self, X_test: pd.DataFrame) -> Optional[np.ndarray]:
        ...
        if self.autobnn_trainer and self.hybrid_config.get("autobnn_enabled"):
            ...
            if autobnn_probs is not None and xgb_probs is not None and autobnn_probs.shape == xgb_probs.shape:
...
class Backtester:
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, model: HybridModelTrainer, initial_equity: float) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if class_probs is None:
             logger.error("Model prediction failed, skipping backtest chunk.")
             return pd.DataFrame(), pd.Series([initial_equity])
