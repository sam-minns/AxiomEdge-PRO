# End_To_End_Advanced_ML_Trading_Framework_PRO_V98_Architecture_Revival
#
# V98 UPDATE (Major Architecture Overhaul):
# 1. ARCHITECTURE REVIVAL: The entire script has been reverted to the more complex
#    and powerful architecture based on the user-provided V93 template. This addresses
#    the core issues of cycles being "too quick" and reporting being incomplete.
# 2. MULTI-TIMEFRAME FEATURES: The data loader and feature engineer now process
#    Daily, H1, and M15 data to create a rich, context-aware feature set of 90+
#    indicators, as was done in the reference version.
# 3. ADVANCED MODELING: Re-implemented the 3-class (Buy/Sell/Hold) model structure
#    with regime-adjusted profit-taking and stop-loss targets.
# 4. THOROUGH OPTIMIZATION: Re-instated the advanced Optuna objective function that
#    balances F1-score with a Sortino ratio and a stability penalty, forcing a
#    much more meaningful and time-intensive hyperparameter search.
# 5. FULL SHAP REPORTING: Re-enabled the full SHAP value calculation, aggregation
#    across walk-forward cycles, and plotting. The final report now includes a
#    comprehensive feature importance section.
...
class ModelTrainer:
    ...
    def _find_best_threshold(self, best_params, X_train, y_train, X_val, y_val) -> float:
        ...
        temp_params.pop('early_stopping_rounds', None) # Remove for this quick training
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability)->Optional[optuna.study.Study]:
        ...
        def custom_objective(trial:optuna.Trial):
            ...
            try:
                ...
                if stability_pnl.sum() < 0: objective_score -= 0.5 # Penalty for instability
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: pd.Index):
        ...
        try:
            ...
            summary = pd.DataFrame(mean_abs_shap,index=feature_names,columns=['SHAP_Importance']).sort_values(by='SHAP_Importance',ascending=False)
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V98_Architecture_Revival",
    ...
    # Files needed for the V93 architecture
    files_to_process=[
...
if __name__ == '__main__':
    if os.name == 'nt':
        # Setting console to UTF-8 on Windows to prevent character errors
        os.system("chcp 65001 > nul")
    main()

# End_To_End_Advanced_ML_Trading_Framework_PRO_V98_Architecture_Revival.py