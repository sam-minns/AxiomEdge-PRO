# End_To_End_Advanced_ML_Trading_Framework_PRO_V194_Linux.py
#
# V194 UPDATE (Resilience & Strategic Memory):
# 1.  ADDED (Anchor to Personal Best): The framework now tracks the best-performing
#     configuration during a run. When a strategy is quarantined, the AI can
#     choose to revert to this proven "anchor" instead of exploring a new one.
# 2.  ADDED (Catastrophic Failure Handling): If a model's training score is
#     extremely poor (e.g., < -0.10), the framework now bypasses standard retries
#     and immediately attempts to "fall-forward" to the last good model,
#     prioritizing stability.
# 3.  ENHANCED (Playbook Review): The AI review process for amending the core
#     strategy playbook is now also triggered if a strategy consistently fails
#     to produce a trainable model, not just from backtest failures.
# 4.  FIXED (Context Integrity): Ensured the AI receives the correct, fresh
#     context when analyzing a training failure for a retry, preventing
#     irrelevant suggestions based on stale data from prior cycles.
#
...
class GeminiAnalyzer:
    def __init__(self):
        ...
        # Per user request, hardcoding the newer version
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        # Per user request, send as much context as possible
        if len(prompt) > 30000:
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str], dynamic_best_config: Optional[Dict] = None) -> Dict:
        ...
        # Construct the core of the prompt first
        base_prompt = (
        ...
        # Add the "Anchor to Personal Best" option if a best configuration is available
        if dynamic_best_config:
            best_strat_name = dynamic_best_config.get('final_params', {}).get('strategy_name', 'N/A')
            best_strat_mar = dynamic_best_config.get('mar_ratio', 'N/A')
            ...
            # Fallback to the original prompt if no anchor is available
            explore_only_prompt = (
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    CATASTROPHIC_FAILURE_THRESHOLD = -0.10 # [Rec 3] New threshold for catastrophic failures
    ...
    # [Rec 2] "Anchor to Personal Best" state variables
    dynamic_best_config_snapshot = None
    dynamic_best_mar_ratio = -np.inf
    run_peak_equity = config.INITIAL_CAPITAL
    run_max_drawdown_abs = 0
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        train_result, best_objective_score = None, -99 # Initialize to failure state
        ...
        # --- [Rec 3] Catastrophic Failure & Quality Gate Logic ---
        is_catastrophic_failure = best_objective_score < CATASTROPHIC_FAILURE_THRESHOLD
        is_normal_failure = not train_result or best_objective_score < MODEL_QUALITY_THRESHOLD
        
        if is_catastrophic_failure:
            logger.critical(f"!! CATASTROPHIC FAILURE !! Objective score ({best_objective_score:.3f}) is below severe threshold ({CATASTROPHIC_FAILURE_THRESHOLD}). Bypassing retries.")
            cycle_retry_count = config.MAX_TRAINING_RETRIES_PER_CYCLE + 1 # Force abandon/fall-forward
        elif is_normal_failure:
            cycle_retry_count += 1
            logger.critical(f"!! MODEL QUALITY GATE FAILED !! Score ({best_objective_score:.3f}) is below threshold ({MODEL_QUALITY_THRESHOLD}). Retry {cycle_retry_count}/{config.MAX_TRAINING_RETRIES_PER_CYCLE}.")
        ...
        # --- Training Failure & Retry Logic ---
        if cycle_retry_count > 0:
            if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
                logger.error(f"!! CYCLE ABANDONED !! Exceeded max training retries for '{config.strategy_name}'.")
                # [Rec 4] Trigger playbook amendment on exhausting retries
                logger.warning(f"Triggering playbook review for chronically failing strategy: {config.strategy_name}")
                ...
                # (Amendment logic to modify the playbook file is handled below where this is also called)

                if last_successful_pipeline: # Fall-Forward
                    logger.warning("--> FALLING FORWARD: Using the model from the previous successful cycle.")
                    ...
                    if not X_test.empty:
                        probs = last_successful_pipeline.predict_proba(X_test)
                        if is_meta_model and probs.shape[1] == 2: df_test[['prob_0', 'prob_1']] = probs
                        elif not is_meta_model and probs.shape[1] == 3: df_test[['prob_short', 'prob_hold', 'prob_long']] = probs
                    ...
                    logger.error("--> No prior successful model exists. Skipping this period with $0 PNL.")
                    in_run_historical_cycles.append({"StartDate": period_start_date.date().isoformat(), "EndDate": test_end.date().isoformat(), "NumTrades": 0, "PNL": 0.00, "Status": "SKIPPED_NO_MODEL"})
                    cycle_retry_count = 0; cycle_num += 1; continue
            else: # Standard retry logic
                logger.info("  - Re-engaging AI for new parameters to retry this cycle...")
                # [Rec 1] Bug Fix: Pass the current, failed config to the AI for accurate analysis.
                ...
                if ai_suggestions:
                    ...
                    logger.error("  - AI failed to provide suggestions. Retrying cycle with same parameters.")
        ...
        # [Rec 2] Anchor to Personal Best: Update best config if current performance is better
        if not aggregated_equity_curve.empty:
            run_peak_equity = max(run_peak_equity, aggregated_equity_curve.max())
            current_drawdown_abs = run_peak_equity - last_equity
            run_max_drawdown_abs = max(run_max_drawdown_abs, current_drawdown_abs)
            run_max_drawdown_pct = (run_max_drawdown_abs / run_peak_equity) if run_peak_equity > 0 else 0
            years_elapsed = (test_end - start_date).days / 365.25
            cagr = ((last_equity / config.INITIAL_CAPITAL) ** (1 / years_elapsed)) - 1 if years_elapsed > 0 else 0
            current_mar = cagr / run_max_drawdown_pct if run_max_drawdown_pct > 0 else cagr * 1000 # Handle zero DD case
            if current_mar > dynamic_best_mar_ratio:
                dynamic_best_mar_ratio = current_mar
                dynamic_best_config_snapshot = { "mar_ratio": current_mar, "final_params": config.model_dump(mode='json')}
                logger.info(f"  - New Personal Best Anchor Set! MAR Ratio: {current_mar:.2f} with strategy '{config.strategy_name}'")
        ...
        if breaker_tripped:
            if probationary_strategy == config.strategy_name:
                ...
                if consecutive_failures_on_probation >= 2:
                    ...
                    # [Rec 4] Expanded playbook review trigger
                    playbook_amendment = api_timer.call(gemini_analyzer.propose_playbook_amendment, config.strategy_name, framework_history, playbook)
                    if playbook_amendment and 'action' in playbook_amendment:
                        # (Amendment logic is the same as the original block)
                        ...
                        if action == 'retire':
                            if strat_name in playbook: playbook[strat_name]['retired'] = True
                        elif action == 'rework' and 'new_config' in playbook_amendment:
                            if strat_name in playbook: playbook[strat_name].update(playbook_amendment['new_config'])
                        try:
                    ...
                    # [Rec 2] Call intervention with the anchor config
                    intervention_suggestion = api_timer.call(gemini_analyzer.propose_strategic_intervention, in_run_historical_cycles[-2:], playbook, config.strategy_name, quarantine_list, dynamic_best_config_snapshot)
                    
                    if intervention_suggestion:
                        if intervention_suggestion.get("action") == "revert" and dynamic_best_config_snapshot:
                            logger.warning(f"!! AI CHOSE TO REVERT TO PERSONAL BEST ANCHOR (MAR: {dynamic_best_mar_ratio:.2f}) !!")
                            config = ConfigModel(**{**config.model_dump(mode='json'), **dynamic_best_config_snapshot['final_params']})
                            probationary_strategy, consecutive_failures_on_probation = None, 0
                        elif intervention_suggestion.get("strategy_name") in playbook:
                            ...
                            logger.error("  - Strategic intervention FAILED. AI response invalid. Halting run.")
                            break
                    else:
                        logger.error("  - Strategic intervention FAILED. No AI response. Halting run."); break
            else:
        ...
        cycle_num += 1
        cycle_retry_count = 0 # Reset for the next cycle period
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V194",
