# End_To_End_Advanced_ML_Trading_Framework_PRO_V109_NameError_Fix_2
#
# V109 UPDATE (Critical Bug Fix):
# 1. CRITICAL FIX: Corrected a second `NameError` that occurred inside the main
#    walk-forward loop. The script was again passing an undefined variable `playbook`
#    instead of `strategy_playbook` to the in-cycle AI analysis function. This has
#    been resolved, fully enabling the adaptive loop.
#
# V107/108 UPDATE (Strategy Playbook & Master Strategist AI):
# 1. Implemented a 'playbook' of distinct trading archetypes.
# 2. Elevated the AI to a master strategist that first selects a master strategy
#    from the playbook before tuning its parameters.
...
class PerformanceAnalyzer:
    ...
    def _get_comparison_block(self, metrics: Dict, memory: Dict) -> str:
        ...
        # Find the actual previous run to compare against, not just the one before the current.
        previous_run = next((run for run in reversed(historical_runs) if run.get("script_version") != self.config.REPORT_LABEL), None)
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V108_NameError_Fix",
    ...
    # FIX (V108): Use the correct variable name `strategy_playbook`
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V108_NameError_Fix.py