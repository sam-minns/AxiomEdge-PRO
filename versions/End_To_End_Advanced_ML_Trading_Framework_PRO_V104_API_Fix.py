# End_To_End_Advanced_ML_Trading_Framework_PRO_V104_API_Fix
#
# V104 UPDATE (API and Sanitization Fix):
# 1. CRITICAL API FIX: Resolved the `400 Client Error: Bad Request` from the Gemini
#    API. A new data sanitization function has been added to recursively clean the
#    history payload before it's sent, removing invalid data types (NaN, Infinity)
#    that were causing the API to reject the request. This restores the adaptive
#    learning loop.
# 2. Pydantic Update: Addressed the `PydanticDeprecatedSince20` warning by replacing
#    all instances of the `.dict()` method with the modern `.model_dump()`.
#
# V103 UPDATE (Cross-Version Learning & Memory):
# 1. Implemented a persistent `framework_memory.jsonl` to enable learning across
#    different script versions.
# 2. Added a "Pre-Flight Check" where the AI analyzes all past runs to recommend an
#    optimal starting configuration for the new run.
...
class GeminiAnalyzer:
    ...
    def _sanitize_value(self, value):
        """Converts numpy/inf/nan types to JSON-safe types."""
    ...
    def _sanitize_dict(self, data: Any) -> Any:
        """Recursively sanitizes a dictionary or list for JSON conversion."""
...
# ... (The rest of the classes are unchanged from V102, full script provided below)
...
class PerformanceAnalyzer:
    def __init__(self,config:ConfigModel):self.config=config
    def generate_full_report(self,trades_df:Optional[pd.DataFrame],equity_curve:Optional[pd.Series],cycle_metrics:List[Dict],aggregated_shap:Optional[pd.DataFrame]=None) -> Dict[str, Any]:
...
def load_framework_memory(memory_path: str) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Could not read or parse memory file at {memory_path}: {e}")
        return []
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V104_API_Fix",
    ...
    # Setup version-specific file logging
    # Remove existing handlers to avoid duplicate logging if re-running in a notebook
    ...
    for i,period_start_date in enumerate(retraining_dates):
        # Use a fresh ConfigModel instance for each cycle to reflect AI updates
    ...
    run_summary = {
        ...
        "final_metrics": gemini_analyzer._sanitize_dict(final_metrics), # Sanitize metrics
...
if __name__ == '__main__':
    ...
    main()
# End_To_End_Advanced_ML_Trading_Framework_PRO_V104_API_Fix.py