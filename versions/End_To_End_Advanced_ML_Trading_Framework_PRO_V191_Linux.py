# End_To_End_Advanced_ML_Trading_Framework_PRO_V191_Linux.py
#
# V191 UPDATE (Robustness & Label Quality):
# 1.  ADDED (Dynamic Labeling): The core labeling process (`label_outcomes`) now
#     includes a `LABEL_MIN_RETURN_PCT` check. This filters out noisy, low-return
#     price movements from being labeled as valid trades, directly addressing
#     the "Garbage In, Garbage Out" problem in choppy markets.
# 2.  ADDED (Label Sanity Check): A new pre-flight check, `check_label_quality`,
#     runs before each training cycle. It verifies that a minimum percentage of
#     trade labels were generated, preventing expensive, doomed-to-fail model
#     optimizations on un-trainable data.
# 3.  ADDED (Fall-Forward Mechanism): If all training retries for a cycle fail,
#     the framework will no longer skip the period. It now "falls forward,"
#     using the last successfully trained model to trade the current period,
#     ensuring a continuous equity curve and a more realistic backtest.
# 4.  ENHANCED (Model Quality Gate): The entire training process is now more
#     resilient. It fails faster on poor data and handles inevitable training
#     failures gracefully, leading to more reliable model performance and gate scores.
#
# V190 UPDATE (Long-Term Memory & Dynamic Playbook):
# 1.  FIXED (Bug): The main loop logic in daemon mode has been corrected to properly
#     respect the `MAX_RUNS` setting, especially for MAX_RUNS=1.
# 2.  FIXED (Bug): The final report's cycle breakdown now correctly formats the
#     'Circuit Breaker' context, replacing the raw dictionary with a clean summary.
# 3.  ENHANCED (AI Memory): The AI's mid-run analysis function
#     now receives the full long-term framework history, allowing it
#     to distinguish between acute and chronic strategy failures.
# 4.  ENHANCED (Failure Recovery): The rigid 'Path A/B' logic for training failures
#     has been replaced. The AI is now prompted to choose between changing features (A),
#     simplifying the model (B), or changing the training process itself (C).
# 5.  ADDED (Dynamic Playbook): A new AI function, `propose_playbook_amendment`,
#     is triggered when a strategy is quarantined, allowing the AI to suggest permanent
#     changes to the `strategy_playbook.json` file.
# 6.  ENHANCED (AI Context): The initial setup prompt now includes the `analysis_notes`
#     from the last three runs, giving the AI a memory of its own past reasoning.
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        feature_selection_guidance = (
            ...
            "Your most important task is selecting a **small, targeted feature set.** Using 20+ features has proven counterproductive.\n"
        ...
        prompt = (
            ...
            "3.  **Select Strategy & Configure**: Based on your analysis, select the optimal `strategy_name` from the playbook. Then, provide a complete starting configuration. This is your most important task.\n"
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], framework_history: Dict, available_features: List[str], strategy_details: Dict, cycle_status: str) -> Dict:
        ...
        if cycle_status == "TRAINING_FAILURE":
            prompt_details = (
                ...
                "The previous training attempt resulted in an unstable model. Minor tweaks are NOT the answer. You must make a **drastic and decisive pivot** to break this failure cycle.\n\n"
                "**YOUR TASK: Analyze the short-term and long-term history. Propose a recovery plan from the options below. Justify your choice based on the historical data in your `analysis_notes`.**\n\n"
                "**PATH A: DRASTICALLY CHANGE THE FEATURE SET**\n"
                "   - Propose a **completely different and much smaller feature set.**\n"
                "   - The current features are likely the problem. Select a new, targeted list of just **4-6 features** from the `AVAILABLE FEATURES` list.\n"
                f"   - {context_feature_mandate}\n\n"
                "**PATH B: DRASTICALLY SIMPLIFY THE MODEL COMPLEXITY**\n"
                "   - Propose a significant reduction in the model's learning capacity to combat overfitting.\n"
                "   - Suggest much **simpler hyperparameter search spaces** (e.g., reduce `OPTUNA_TRIALS`, reduce `max_depth` and `n_estimators`).\n\n"
                "**PATH C: FUNDAMENTALLY CHANGE THE TRAINING PROCESS**\n"
                "   - Propose a change to the core data labeling or training setup.\n"
                "   - This could involve adjusting `LOOKAHEAD_CANDLES` or the `anomaly_contamination_factor`.\n"
                "   - Use this path if you believe the features and model are okay, but the *problem definition* is wrong for the current market.\n"
            )
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        if cycle_retry_count > 0:
            if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
                ...
                ai_suggestions = api_timer.call(gemini_analyzer.analyze_cycle_and_suggest_changes, in_run_historical_cycles, framework_history, all_available_features, config.model_dump(), cycle_status="TRAINING_FAILURE")
                if ai_suggestions:
        ...
        suggested_params = api_timer.call(gemini_analyzer.analyze_cycle_and_suggest_changes, in_run_historical_cycles, framework_history, all_available_features, config.model_dump(), cycle_status=cycle_status_for_ai)
        if suggested_params:
