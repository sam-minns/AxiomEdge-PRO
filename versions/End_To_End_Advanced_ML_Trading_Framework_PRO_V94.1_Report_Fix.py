# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.1_Report_Fix
#
# V94.1 Update:
# 1. BUG FIX: Restored the quantitative performance report generation in the
#    `PerformanceAnalyzer`, which was accidentally removed in the last update.
#    The script now correctly outputs the full performance summary.
#
# V94 Update:
# 1. MAJOR ARCHITECTURAL CHANGE - Meta-Labeling: The framework now uses a
#    two-stage meta-labeling approach to filter low-quality signals.
# 2. NEW - Proactive Volatility Filter: A new risk layer to avoid trading
#    in overly chaotic or stagnant market conditions.
# 3. ENHANCED - AI Integration: The AI prompt is overhauled to manage the
#    new architecture and focus on breaking failure loops.
...
class FeatureEngineer:
    ...
    def _calculate_m15_native(self,g:pd.DataFrame)->pd.DataFrame:
        ...
        # Add other base features needed by the meta-model
        ...
        crossover = signal - np.roll(signal, 1)
        crossover[0] = 0 # No crossover on the first candle
        g_out['primary_model_signal'] = crossover / 2 # Results in 1 for bull cross, -1 for bear cross
    ...
    def label_meta_outcomes(self, df: pd.DataFrame, lookahead: int) -> pd.DataFrame:
        ...
        outcomes = np.zeros(len(signal_df), dtype=int) # Default outcome is 0 (loss/timeout)
        ...
        for i in range(len(signal_df)):
            ...
            signal = signal_df.iloc[i]['primary_model_signal']
...
class ModelTrainer:
    BASE_FEATURES = [
        'ATR', 'RSI', 'market_volatility_index', 'price_above_sma200', 'macd',
...
class PerformanceAnalyzer:
    ...
    def generate_text_report(self,m:Dict[str,Any],cycle_metrics:List[Dict]):
        # --- THIS IS THE RESTORED REPORT BLOCK ---
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V94_Report_Fix",
        "FORWARD_TEST_START_DATE": "2024-01-01", "INITIAL_CAPITAL": 100000.0,
        "CONFIDENCE_TIERS": {}, # No longer used
        ...
        "selected_features": ModelTrainer.BASE_FEATURES[:8] # Start with a simple model
    }
    ...
    try:
        ...
        # For meta-labeling, we only need the M15 data for features and signals
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V94.1_Report_Fix.py