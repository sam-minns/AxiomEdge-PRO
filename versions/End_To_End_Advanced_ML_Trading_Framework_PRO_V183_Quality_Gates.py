# End_To_End_Advanced_ML_Trading_Framework_PRO_V183_Quality_Gates.py
#
# V183 UPDATE (Intelligent Strategy Lifecycle & Quality Gates):
# 1. ADDED: A "Model Quality Gate." After training, if the model's objective
#    score is below a threshold, it's discarded, and the AI is forced to
#    re-configure for the same cycle, preventing flawed models from ever being backtested.
# 2. ADDED: A "Probation & Quarantine" system for strategy failures. A strategy
#    now goes on 'probation' after one failure, prompting the AI for conservative
#    changes. It's only 'quarantined' after a second consecutive failure,
#    triggering a strategic intervention. This stops frantic strategy-switching.
# 3. FIXED: The "Short-Cycle Trap." When forced exploration triggers a longer
#    retraining period, the AI is now blocked from immediately suggesting a short
#    cycle again, ensuring the long test period is completed.
# 4. IMPROVED: AI prompting now forcefully mandates the inclusion of multi-
#    timeframe context features (e.g., 'DAILY_ctx_Trend') to guide the AI
#    towards more robust and powerful models.
#
# V182 UPDATE (JSON Serialization Fix):
# 1. FIXED: A critical TypeError: 'Object of type int64 is not JSON serializable'
#    that occurred when saving run results containing NumPy numeric types.
...
class GeminiAnalyzer:
    def __init__(self):
        ...
        if self.api_key_valid:
            # Per user instructions, using the specified model version
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        prompt = (
            ...
            # --- FIX: More direct instruction in the main task ---
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, cycle_status: str) -> Dict:
        ...
        # --- FIX: Use the same strengthened mandate here ---
...
class FeatureEngineer:
    ...
    # Implementing the proposed change to add more context to the anomaly detector
...
class ModelTrainer:
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: List[str], num_classes: int):
        ...
        try:
            ...
            if num_classes > 2:
                # For multi-class, SHAP returns a list of arrays (one for each class)
                # We take the mean absolute value across all classes for an overall importance score
...
class PerformanceAnalyzer:
    ...
    def _calculate_metrics(self,trades_df:pd.DataFrame,equity_curve:pd.Series)->Dict[str,Any]:
        ...
        pnl_std=returns.std()
        m['sharpe_ratio']=(returns.mean()/pnl_std)*np.sqrt(252*24*4) if pnl_std>0 else 0 # Assuming 15min data
...
def _sanitize_ai_suggestions(suggestions: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Define which keys must be integers to prevent incorrect type casting
    ...
    for key, (lower, upper) in bounds.items():
        if key in sanitized and isinstance(sanitized.get(key), (int, float)):
            ...
            # --- FIX ---
            # Only round and cast to int if the key is known to be an integer field.
            # This prevents float fields like 'MAX_DD_PER_CYCLE' from being rounded to 0.
            if key in integer_keys:
                clamped_value = int(round(clamped_value))

            if original_value != clamped_value:
                # The log message will now correctly show clamping to 0.05 instead of 0
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        if is_short_cycle and not is_forcing_long_cycle:
        ...
        if short_cycle_streak >= 4:
            ...
            retrain_freq_str = '90D'
            is_forcing_long_cycle = True
            short_cycle_streak = 0
        ...
        if not train_result or best_objective_score < MODEL_QUALITY_THRESHOLD:
            logger.critical(f"!! MODEL QUALITY GATE FAILED !! Objective score ({best_objective_score:.3f}) is below threshold ({MODEL_QUALITY_THRESHOLD}). Model discarded.")
            ...
            continue # Retry the same cycle without incrementing cycle_num
        ...
        # --- PROBATION & QUARANTINE LOGIC (V183) ---
        ...
        if breaker_tripped:
            if probationary_strategy == config.strategy_name:
                ...
                if consecutive_failures_on_probation >= 2:
                    ...
                    if intervention_suggestion and intervention_suggestion.get("strategy_name") in playbook:
                        ...
                        probationary_strategy, consecutive_failures_on_probation = None, 0 # Reset probation state
                    else:
                        logger.error("  - Strategic intervention FAILED. Halting run to prevent further losses.")
                        break # Exit the main loop
                else: # Still on probation, but hasn't hit quarantine limit
                    cycle_status_for_ai = "PROBATION"
            else: # First failure, start probation
            ...
            probationary_strategy, consecutive_failures_on_probation = None, 0 # Reset on success
            cycle_status_for_ai = "COMPLETED"

        # --- AI Cycle-End Analysis ---
        ...
        # If a long cycle was forced, reset the flag for the next cycle
        if is_forcing_long_cycle:
            is_forcing_long_cycle = False
        
        cycle_num += 1 # Advance to the next cycle
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V183_Quality_Gates",
