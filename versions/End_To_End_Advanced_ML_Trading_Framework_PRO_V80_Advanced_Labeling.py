# End_To_End_Advanced_ML_Trading_Framework_PRO_V80_Advanced_Labeling
#
# V80 Update:
# 1. NEW - Advanced Labeling: Implemented "Volatility-Regime Adjusted Barriers". The
#    triple-barrier TP/SL levels are now dynamically scaled based on recent price
#    volatility, creating more intelligent and context-aware training labels.
# 2. FIXED - SettingWithCopyWarning: Resolved the pandas warning by ensuring the
#    backtester operates on an explicit copy of the data chunk.
# 3. All optimizations and features from V79 are retained. This script is complete and unabridged.
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V80_Advanced_Labeling",
...
class FeatureEngineer:
    ...
    def label_outcomes(self,df:pd.DataFrame,sl_atr:float,tp_atr:float,lookahead:int)->pd.DataFrame:
        logger.info("  - Generating trade labels with volatility-adjusted barriers...");
        labeled_dfs=[self._label_group(group,sl_atr,tp_atr,lookahead) for _,group in df.groupby('Symbol')]
    ...
    def _label_group(self,group:pd.DataFrame,sl_atr:float,tp_atr:float,lookahead:int)->pd.DataFrame:
        ...
        group['volatility'] = group['Close'].pct_change().rolling(20).std().bfill()
        min_vol, max_vol = group['volatility'].quantile(0.1), group['volatility'].quantile(0.9)
        group['volatility_scalar'] = 1 + np.clip((group['volatility'] - min_vol) / (max_vol - min_vol), 0, 1)
        outcomes=np.zeros(len(group));prices,lows,highs,atr_vals,vol_scalars=group['Close'].values,group['Low'].values,group['High'].values,group['ATR'].values,group['volatility_scalar'].values
        for i in range(len(group)-lookahead):
            atr,vol_scalar = atr_vals[i],vol_scalars[i]
            if pd.isna(atr) or atr<=1e-9:continue
            tp_long = prices[i] + (atr * tp_atr * vol_scalar)
            sl_long = prices[i] - (atr * sl_atr * vol_scalar)
...
class Backtester:
    ...
    def run_backtest_chunk(self,df_chunk_in:pd.DataFrame,model:Pipeline,initial_equity:float)->Tuple[pd.DataFrame,pd.Series]:
        ...
        df_chunk = df_chunk_in.copy() # Fix for SettingWithCopyWarning
        ...
        class_probs=model.predict_proba(X_test);df_chunk.loc[:,'prob_short'],df_chunk.loc[:,'prob_hold'],df_chunk.loc[:,'prob_long']=class_probs[:,0],class_probs[:,1],class_probs[:,2]
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V80_Advanced_Labeling.py