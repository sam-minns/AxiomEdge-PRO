# End_To_End_Advanced_ML_Trading_Framework_PRO_V158_FeatureName_FIXED.py
#
# V158 UPDATE (Feature Name Fix):
# 1. FIXED - KeyError: Corrected a crash caused by a mismatch between feature names in the
#    playbook (e.g., 'D1_ctx_Trend') and the dynamically generated feature names
#    ('DAILY_ctx_Trend'). The playbook has been updated to align with the code's output.
#
# V157 UPDATE (KeyError Fix):
# 1. FIXED - KeyError: Corrected a crash by ensuring the 'DIRECTIVES_FILE_PATH' key is
#    defined in the fallback_config dictionary within the main function.
#
# V156 UPDATE (API Optimization & Data-Driven Setup):
# 1. ENHANCED - Combined Initial API Call: Merged "Nickname Generation" and "Strategy
#    Selection" into a single, efficient API call.
# 2. ENHANCED - AI-Powered Data Analysis: The script now pre-analyzes all market data
#    and sends a summary to the AI for a more informed strategy selection.
# 3. REFACTORED - API Call Interval: The delay between API calls was increased to 5 minutes.
...
class ConfigModel(BaseModel):
    ...
    analysis_notes: str = "" # To capture AI notes
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    # Aligned playbook feature names with generated names (e.g., DAILY_ctx_Trend)
    DEFAULT_PLAYBOOK = {
