# End_To_End_Advanced_ML_Trading_Framework_PRO_V125_AI_Response_Fix
#
# V125 UPDATE (Robust AI Response Handling):
# 1. FIX - TYPEERROR: Resolved a TypeError crash by making the script robust to
#    variations in the AI's JSON output format. It now correctly handles and
#    "flattens" nested parameters returned by the AI.
#
# V124 UPDATE (Robust Memory & JSON Fix):
# 1. NEW - ROBUST MEMORY FORMAT: Re-architected memory to use a JSONL file for
#    history, preventing a single failed write from corrupting all past data.
# 2. FIX - JSON PARSING: The memory loading function now skips any malformed
#    lines in the history file, preventing crashes from corrupted data.
...
class GeminiAnalyzer:
    ...
    def get_pre_flight_config(self, memory: Dict, playbook: Dict, health_report: Dict, fallback_config: Dict, exploration_rate: float) -> Dict:
        ...
        if is_exploration:
            ...
            prompt = (
                ...
                "**INSTRUCTIONS:**\n"
                "1. **Analyze Past Performance**: Review the `framework_memory` for recent performance and the current champion.\n"
                "2. **CRITICAL: Review Strategic Health**: Review the `STRATEGIC HEALTH ANALYSIS`. This shows long-term problems. Heavily penalize strategies with high `ChronicFailureRate` or `CircuitBreakerFrequency`.\n"
                ...
                "5. **Define Parameters**: Define the initial parameters for the chosen strategy (`LOOKAHEAD_CANDLES`, `MAX_DD_PER_CYCLE`, `selected_features`, etc.).\n\n"
...
class ConfigModel(BaseModel):
    ...
    LOG_FILE_PATH: str = ""; CHAMPION_FILE_PATH: str = ""; HISTORY_FILE_PATH: str = ""; PLAYBOOK_FILE_PATH: str = ""
...
def save_run_to_memory(config: ConfigModel, new_run_summary: Dict, current_memory: Dict):
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict):
    ...
    if 'initial_config' in ai_config_suggestion and isinstance(ai_config_suggestion['initial_config'], dict):
        nested_params = ai_config_suggestion.pop('initial_config')
        current_config.update(nested_params)
    ...
    # Ensure selected_features is a list before trying to slice it
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V125_AI_Response_Fix",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V125_AI_Response_Fix.py