# End_To_End_Advanced_ML_Trading_Framework_PRO_V145_JSON_FIX.py
#
...
#    `.model_dump(mode='json')`, which correctly converts Path objects and other
#    complex types into JSON-compatible strings.
# 2. DEPRECATION FIX: The use of `.model_dump()` also resolves the
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool, playbook: Dict, nickname_ledger: Dict, directives: List[Dict]):
    ...
    logger.info(f"--- Initial Config: {json.dumps(config.model_dump(mode='json'), indent=2)} ---")
    ...
    while current_train_start + train_window < end_date:
        ...
        if not X_test.empty:
            ...
            probs = np.array([]) # Ensure probs is an array
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V145_JSON_FIX",
