# End_To_End_Advanced_ML_Trading_Framework_PRO_V101_Final_Fix
#
# V101 UPDATE (Final Fix & Configuration Change):
# 1. CRITICAL SHAP FIX: The SHAP calculation has been definitively fixed by replacing
#    the faulty logic with the exact, proven implementation from the user-provided
#    V93 template. This resolves the recurring `ValueError: Shape of passed values`
#    by correctly using the `shap.Explanation` object for multi-class models.
# 2. CONFIGURATION UPDATE: The framework's `RETRAINING_FREQUENCY` has been updated
#    from '60D' to '90D' as per the user's request.
#
# V100 UPDATE (Prompt Enhancement):
# 1. The AI prompt was enhanced to focus on finding hidden strengths and opportunities,
#    in addition to fixing failures.
...
class ModelTrainer:
    ...
    def _generate_shap_summary(self, model: xgb.XGBClassifier, X_scaled: np.ndarray, feature_names: pd.Index):
        ...
        try:
            ...
            # CRITICAL FIX (V101): Using the exact, robust logic from V93 template
            explainer = shap.TreeExplainer(model)
            shap_explanation = explainer(X_sample) # This is a shap.Explanation object
            
            # 1. Get mean absolute SHAP values per feature, per class -> shape (n_features, n_classes)
            mean_abs_shap_per_class = shap_explanation.abs.mean(0).values

            # 2. Average these values across all classes to get a single importance value per feature -> shape (n_features,)
            overall_importance = mean_abs_shap_per_class.mean(axis=1)
...
def main():
    ...
    current_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V101_Final_Fix",
        ...
        "RETRAINING_FREQUENCY": '90D', # Updated as per request
        "FORWARD_TEST_GAP": "1D",
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V101_Final_Fix.py