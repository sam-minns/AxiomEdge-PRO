# End_To_End_Advanced_ML_Trading_Framework_PRO_V174_Portfolio_Engine.py
#
# V174 UPDATE (Portfolio & Risk Management Engine):
# 1. IMPLEMENTED: AI-Managed Concurrent Trading. The backtester can now execute
#    multiple trades simultaneously across different assets. The AI sets the
#    `MAX_CONCURRENT_TRADES` parameter (1-10) to control market exposure.
# 2. IMPLEMENTED: AI-Managed Partial Profit-Taking. The AI can now enable and tune
#    a partial profit strategy using two new parameters:
#      - `PARTIAL_PROFIT_TRIGGER_R`: The R:R multiple at which to close a part of the trade.
#      - `PARTIAL_PROFIT_TAKE_PCT`: The percentage of the trade to close.
# 3. IMPLEMENTED: Auto-Breakeven Stop. When a partial profit is taken, the stop-loss
#    for the remainder of the position is automatically moved to the entry price.
#
# V173 UPDATE (Advanced Analytical Components):
# 1. IMPLEMENTED: Fractional Differentiation.
# 2. IMPLEMENTED: Anomaly Detection Filter (Isolation Forest).
# 3. IMPLEMENTED: Monte Carlo Strategy Evaluator.
...
def flush_loggers():
    ...
    for handler in logging.getLogger().handlers:
        handler.flush()
    # Ensure our specific logger's handlers are also flushed
...
class APITimer:
    ...
    def _wait_if_needed(self):
        if self.interval.total_seconds() <= 0:
            return  # Timer is disabled, so don't wait.
...
class GeminiAnalyzer:
    ...
    def _call_gemini(self, prompt: str) -> str:
        ...
        for attempt, delay in enumerate([0] + retry_delays):  # Initial attempt (delay=0) + retries
            if delay > 0:
            ...
            try:
                ...
                if "candidates" in result and result["candidates"] and "content" in result["candidates"][0] and "parts" in result["candidates"][0]["content"]:
                    ...
                    return "{}"  # Return empty on invalid structure, don't retry

            except requests.exceptions.RequestException as e:
                logger.error(f"Gemini API request failed on attempt {attempt + 1}: {e}")
                if attempt == len(retry_delays):  # This was the last attempt
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        prompt = (
            ...
            "3.  **Select Strategy & Configure**: Based on your data analysis, the framework's historical performance, and any critical directives, select the optimal `strategy_name` from the playbook. Then, provide a complete and robust starting configuration for all required parameters. This includes the new risk management parameters. **This is your most important task.**\n\n"
            "**PARAMETER RULES:**\n"
            "- `RETRAINING_FREQUENCY`: **MUST be a string** with a number and unit (e.g., '90D', '8W', '3M').\n"
            "- `selected_features`: **MUST be a NON-EMPTY list** of feature names.\n"
            "- `MAX_CONCURRENT_TRADES`: An integer from 1 to 10. Higher for volatile/uncorrelated markets.\n"
            "- `PARTIAL_PROFIT_TRIGGER_R`: A float (e.g., 1.5), representing the R-multiple to trigger a partial close.\n"
            "- `PARTIAL_PROFIT_TAKE_PCT`: A float (e.g., 0.5), for the percentage of the position to close.\n\n"
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, daily_dd_report: Dict, mc_results: Dict) -> Dict:
        if not self.api_key_valid: return {}
        strategy_name = strategy_details.get("description", "Unknown Strategy")
        ...
        # --- Daily DD Context ---
        high_dd_days = {day: data for day, data in daily_dd_report.items() if data.get('drawdown_pct', 0) > 15}
        daily_dd_context_str = "No significant daily drawdowns were observed."
        if high_dd_days:
            daily_dd_context_str = (
                f"**CRITICAL DAILY RISK ANALYSIS:** The last cycle experienced {len(high_dd_days)} day(s) with drawdowns over 15%, "
                "indicating high intra-day volatility risk. This must be addressed. "
                f"Details: {json.dumps(high_dd_days, indent=2)}"
            )
        
        # --- Monte Carlo Context ---
        mc_context_str = (
            "**MONTE CARLO RISK ANALYSIS:**\n"
            f"- 5% Value-at-Risk (VaR) vs Current Equity: ${mc_results.get('var_5pct'):,.2f} vs ${mc_results.get('current_equity'):,.2f}\n"
            f"- This VaR represents a potential {(mc_results.get('current_equity') - mc_results.get('var_5pct')) / mc_results.get('current_equity'):.1%} loss in a 1-in-20 adverse scenario.\n"
            "Analyze this tail risk. A large VaR relative to equity suggests the strategy is risky."
        )
        ...
        base_prompt_analysis = "Analyze the recent cycle history, the daily drawdown report, and the Monte Carlo VaR. High daily drawdowns or a large VaR, even without a full cycle circuit breaker, indicate excessive risk. If a 'Circuit Breaker' was tripped, be even more conservative. If trades are winning but reversing before the full target, consider introducing or adjusting partial profit-taking."
        json_response_format = "Respond ONLY with a valid JSON object containing your suggested keys."

        base_suggestions = (
            "- `MAX_DD_PER_CYCLE`: Risk appetite for the whole cycle (float between 0.05 and 0.4).\n"
            "- `RETRAINING_FREQUENCY`: How often to retrain. **CRITICAL: You MUST respond with a string containing a number followed by a unit 'D', 'W', or 'M'** (e.g., '60D', '7W', '2M').\n"
            "- `selected_features`: **CRITICAL: You MUST provide a NON-EMPTY list of feature names** from the available list.\n"
            "- `MAX_CONCURRENT_TRADES`: Number of simultaneous positions (integer 1-10).\n"
            "- `PARTIAL_PROFIT_TRIGGER_R`: R-multiple to take partial profit (float, e.g., 1.5).\n"
            "- `PARTIAL_PROFIT_TAKE_PCT`: Percent of position to close (float, e.g., 0.5 for 50%).\n"
        )
        ...
        if is_gnn_strategy:
            prompt_details = (
                ...
                "- `MAX_DD_PER_CYCLE`, `RETRAINING_FREQUENCY`, `MAX_CONCURRENT_TRADES`, `PARTIAL_PROFIT_TRIGGER_R`, `PARTIAL_PROFIT_TAKE_PCT` (see format rules above).\n"
        ...
        # Override with safety-first prompt if breaker was tripped
        if historical_results and historical_results[-1].get("Status") == "Circuit Breaker":
            prompt_details = (
                ...
                "2.  **Lower risk by:** reducing `MAX_DD_PER_CYCLE` (must be between 0.05-0.25), reducing `MAX_CONCURRENT_TRADES`, or introducing/lowering `PARTIAL_PROFIT_TRIGGER_R` to take profits earlier.\n"
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        prompt = (
            ...
            "3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration for this new strategy. Start with a conservative `RETRAINING_FREQUENCY` like '90D', a low `MAX_DD_PER_CYCLE` like 0.15, and a low `MAX_CONCURRENT_TRADES` like 2. Set a conservative partial profit take (e.g., `PARTIAL_PROFIT_TRIGGER_R`: 1.5). **Ensure `RETRAINING_FREQUENCY` is a string with a unit and `selected_features` is a NON-EMPTY list.**\n\n"
...
class FeatureEngineer:
    ...
    def _fractional_differentiation(self, series: pd.Series, d: float, thres: float = 1e-5) -> pd.Series:
        ...
        width = len(weights)
        if width > len(series): return pd.Series(index=series.index) # Not enough data
    ...
    def _calculate_base_tf_native(self, g:pd.DataFrame)->pd.DataFrame:
        ...
        # Add fractional differentiation feature
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # Generate anomaly scores after all other base features are created
    ...
    def train(self, df_train: pd.DataFrame, feature_list: List[str], strategy_details: Dict) -> Optional[Tuple[Pipeline, float]]:
        ...
        if self.is_gnn_model:
            ...
            if gnn_embeddings.empty:
                ...
                return None
            X = gnn_embeddings
            feature_list = list(X.columns) # Use GNN features now
        ...
        else: # Standard or Meta-Labeling model
            # --- FEATURE LIST VALIDATION ---
            if feature_list is None or not feature_list:
                logger.error(f"  - Training aborted for strategy '{strategy_details.get('description', 'N/A')}': The 'selected_features' list is empty. Cannot train a model with no features.")
                return None
            # --- END VALIDATION ---
        ...
        # Final check for empty splits which can cause crashes
    ...
    def _optimize_hyperparameters(self,X_train,y_train,X_val,y_val, X_stability, y_stability, num_classes)->Optional[optuna.study.Study]:
        ...
        def custom_objective(trial:optuna.Trial):
            ...
            try:
                ...
                # Need to print a newline if a trial fails to not mess up the progress bar
                sys.stdout.write("\n")
...
class Backtester:
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, is_meta_model: bool) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        for candle in candles:
            ...
            # --- Daily DD Logic: Check for day change ---
            ...
            # Iterate over a copy of keys since dict may be modified
            for symbol in list(open_positions.keys()):
                pos = open_positions[symbol]
                pnl, exit_price, exit_reason = 0, None, ""

                # V174: Check for Partial Profit Take
            ...
            if equity <= 0: break # Exit main loop if account blown
            ...
            if candle.get('anomaly_score') == -1:
                if random.random() < 0.1: logger.info(f"  - Skipping trade check due to anomaly detection at {candle['Timestamp']}")
                continue
            ...
            if symbol not in open_positions and len(open_positions) < self.config.MAX_CONCURRENT_TRADES:
                ...
                if direction != 0:
                    ...
                    open_positions[symbol]={
                        ...
                        'sl_dist': sl_dist, 'partial_profit_taken': False # V174 additions
                    }
...
class PerformanceAnalyzer:
    ...
    def _calculate_metrics(self,trades_df:pd.DataFrame,equity_curve:pd.Series)->Dict[str,Any]:
        ...
        m['total_trade_events']=len(trades_df) # This now includes partials
        # We need to identify unique trades to calculate win rate correctly
        # A simple way is to check for non-partial exits
        ...
        # Payoff ratio is more meaningful on full trade outcomes
        avg_full_win = final_exits_df[final_exits_df['PNL'] > 0]['PNL'].mean()
        avg_full_loss = abs(final_exits_df[final_exit_df['PNL'] < 0]['PNL'].mean())
        ...
        pnl_series = final_exits_df['PNL'] # Streaks based on final outcomes
...
def run_monte_carlo_simulation(price_series: pd.Series, n_simulations: int = 5000, n_days: int = 90) -> np.ndarray:
    ...
    price_paths[0] = price_series.iloc[-1]  # Start from the last known price
...
def _sanitize_frequency_string(freq_str: Any, default: str = '90D') -> str:
    ...
    if isinstance(freq_str, int):
        # AI provided a number, assume it means days
    ...
    # Try to use the string directly if it's a valid pandas offset alias
    try:
        pd.tseries.frequencies.to_offset(freq_str)
        logger.info(f"Using valid frequency alias from AI: '{freq_str}'")
        return freq_str
    except ValueError:
        # If direct conversion fails, try to extract number and D/W/M unit
        match = re.search(r'(\d+)\s*([DdWwMm])', freq_str, re.IGNORECASE)
        if match:
            num, unit = match.groups()
            sanitized_freq = f"{num}{unit.upper()}"
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    DEFAULT_PLAYBOOK = {
        ...
        "TrendPullback": {"description": "Enters on pullbacks (RSI) in the direction of an established long-term trend.", "features": ["DAILY_ctx_Trend", "ADX", "RSI", "H1_ctx_Trend", "momentum_10", "hour"], "lookahead_range": [50, 150], "dd_range": [0.15, 0.30]},
        ...
        "Meta_Labeling_Filter": {"description": "Uses a secondary ML filter to improve a simple primary model's signal quality.", "features": ["ADX", "RSI", "ATR", "bollinger_bandwidth", "H1_ctx_Trend", "DAILY_ctx_Trend"], "lookahead_range": [50, 100], "dd_range": [0.10, 0.25], "requires_meta_labeling": True},
        "FilteredBreakout": {"description": "A hybrid that trades high-volatility breakouts (ATR, Bollinger) but only in the direction of the long-term daily trend.", "features": ["ATR", "bollinger_bandwidth", "DAILY_ctx_Trend", "ADX", "hour"], "lookahead_range": [60, 120], "dd_range": [0.20, 0.35] },
    }
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    # Sanitize AI-provided frequency string
    ...
    retrain_freq = pd.to_timedelta(retrain_freq_str)
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        if short_cycle_streak >= 4:
            ...
            short_cycle_streak = 0 # Reset streak
        
        retrain_freq_str = _sanitize_frequency_string(config.RETRAINING_FREQUENCY)
        retrain_freq = pd.to_timedelta(retrain_freq_str)
        
        if retrain_freq < pd.Timedelta(days=30):
            short_cycle_streak += 1
        else:
            short_cycle_streak = 0
        # --- END FORCED EXPLORATION LOGIC ---
        ...
        if not trades.empty:
            ...
            # Run Monte Carlo simulation for the next period
            mc_days = (pd.to_timedelta(config.RETRAINING_FREQUENCY).days)
    ...
    if not aggregated_trades.empty:
        ...
        final_metrics = pa.generate_full_report(aggregated_trades, aggregated_equity_curve, in_run_historical_cycles, aggregated_shap, framework_memory, aggregated_daily_dd_reports)
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V174_Portfolio_Engine",
    ...
    # --- CONDITIONAL API TIMER LOGIC ---
    if MAX_RUNS > 1 or CONTINUOUS_RUN_HOURS > 0:
        api_interval_seconds = 300  # 5 minutes for multi-run or daemon mode
    else:
        api_interval_seconds = 0  # No delay for a single run
    # --- END ---
