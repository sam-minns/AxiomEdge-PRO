# End_To_End_Advanced_ML_Trading_Framework_PRO_V118_Playbook_Run_Modes
#
# V118 UPDATE (Single Run vs. Daemon Mode):
# 1. NEW - DUAL RUN MODES: The script now has two distinct operating modes.
#    - DEFAULT (Single Run): Runs once with a 5-second delay between cycles.
#    - DAEMON MODE: Activates if MAX_RUNS or CONTINUOUS_RUN_HOURS is increased.
#      Uses a 5-minute inter-cycle delay and features the interruptible countdown.
#
# V117 UPDATE (Daemon Docs):
# 1. DOCS - DAEMON CONTROLS: Added detailed comments to the daemon control
#    variables to clarify timing and that a run, once started, always completes.
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, is_continuous: bool):
    ...
    for i,period_start_date in enumerate(retraining_dates):
        ...
        if suggested_params:
            ...
            # Conditional delay based on run mode
            if is_continuous:
...
def main():
    ...
    # A run, once started, will ALWAYS complete. The timer is checked after a run finishes.
    CONTINUOUS_RUN_HOURS = 0
    # Set to > 1 to run for a specific number of iterations (end-to-end runs).
    MAX_RUNS = 2
    # --- End Controls ---

    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V118_Playbook_Run_Modes",
    ...
    while True:
        ...
        run_single_instance(fallback_config, framework_history, is_continuous)
        ...
        # Interruptible countdown timer for daemon mode
        try:
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V118_Playbook_Run_Modes.py