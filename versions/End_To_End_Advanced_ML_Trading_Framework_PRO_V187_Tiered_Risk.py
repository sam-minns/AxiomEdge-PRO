# End_To_End_Advanced_ML_Trading_Framework_PRO_V187_Tiered_Risk.py
#
# V187 UPDATE (Tiered Risk Management Integration):
# 1.  ADDED: Tiered Risk Management System, inspired by proprietary trading firm models.
#     The framework can now use a `TIERED_RISK_CONFIG` dictionary to dynamically
#     adjust risk percentage and max concurrent trades based on account equity.
# 2.  ADDED: AI-Driven Risk Profile Selection. The initial AI prompt now includes
#     a task to select a `RISK_PROFILE` ('Low', 'Medium', 'High'), allowing the
#     framework to adopt a more conservative or aggressive risk posture from the
#     outset based on market analysis.
# 3.  ADDED: `USE_TIERED_RISK` boolean flag in the configuration to easily
#     enable or disable this new risk model, providing strategic flexibility.
# 4.  REFINED: The Backtester logic has been updated to use the tiered system
#     for determining risk per trade and max open positions if enabled.
#
# V186 UPDATE (Dynamic Anomaly Detection & Enhanced Features):
# 1.  ADDED: AI-Driven Anomaly Contamination Factor.
# 2.  ENHANCED: The `ANOMALY_FEATURES` list was upgraded with ratio features.
# 3.  REFINED: AI prompts were consolidated for more context-rich, single-shot analysis.
...
class GeminiAnalyzer:
    ...
    def get_initial_run_setup(self, script_version: str, ledger: Dict, memory: Dict, playbook: Dict, health_report: Dict, directives: List[Dict], data_summary: Dict) -> Dict:
        ...
        anomaly_prompt_part = (
            "4.  **Configure Anomaly Detection**: Review the `MARKET DATA SUMMARY`, focusing on volatility metrics (`avg_atr`, `volatility_rank`). "
        ...
        # V187 - New Tiered Risk Configuration Prompt
        tiered_risk_prompt_part = (
            "5.  **Select Risk Profile**: Based on market conditions and your chosen strategy's aggressiveness, select a `RISK_PROFILE`. "
        ...
        prompt = (
            ...
            "3.  **Select Strategy & Configure**: Based on your analysis, select the optimal `strategy_name` from the playbook. Then, provide a complete starting configuration. This is your most important task. Your configuration MUST include a `selected_features` key.\n"
    ...
    def analyze_cycle_and_suggest_changes(self, historical_results: List[Dict], available_features: List[str], strategy_details: Dict, cycle_status: str) -> Dict:
        ...
        if cycle_status == "TRAINING_FAILURE":
            prompt_details = (
                ...
                "2.  **Lower risk by:** reducing `MAX_DD_PER_CYCLE` (must be between 0.05-0.20), reducing `MAX_CONCURRENT_TRADES` (if not using tiered risk).\n"
                "3.  Consider a shorter `RETRAINING_FREQUENCY` for faster adaptation.\n"
                "4.  If using tiered risk, consider suggesting a switch to a lower `RISK_PROFILE` (e.g., 'Medium' to 'Low').\n"
                f"5.  {context_feature_mandate}\n"
            )
        else: # Standard cycle
            prompt_details = (
                ...
                "- `MAX_DD_PER_CYCLE`, `RETRAINING_FREQUENCY`, `MAX_CONCURRENT_TRADES` (if not using tiered risk), `RISK_PROFILE`, `USE_PARTIAL_PROFIT`, etc.\n"
                f"- {context_feature_mandate}\n"
            )
    ...
    def propose_strategic_intervention(self, failure_history: List[Dict], playbook: Dict, last_failed_strategy: str, quarantine_list: List[str]) -> Dict:
        ...
        prompt = (
            ...
            f"3.  **Propose a Safe Starting Configuration:** Provide a reasonable and SAFE starting configuration for this new strategy. {context_feature_mandate} Start with conservative values: `RETRAINING_FREQUENCY`: '90D', `MAX_DD_PER_CYCLE`: 0.15 (float), `RISK_PROFILE`: 'Medium', and **`USE_PARTIAL_PROFIT`: false**.\n\n"
    ...
    def propose_regime_based_strategy_switch(self, regime_data_summary: Dict, playbook: Dict, current_strategy_name: str, quarantine_list: List[str]) -> Dict:
        ...
        # FIX: Explicitly check for a 'null' string response, which is a valid signal of no change.
        if response_text.strip().lower() == 'null':
            logger.info("  - AI analysis confirms current strategy is optimal for the upcoming regime. No changes made.")
            return {} # Return empty dict to signify no change, preventing the parsing error.

        # If not null, proceed with JSON extraction
        suggestions = self._extract_json_from_response(response_text)
        return suggestions
...
class FeatureEngineer:
    ...
    # V186 - Enhanced anomaly features
    ANOMALY_FEATURES = [
    ...
    def _get_anomaly_scores(self, df: pd.DataFrame, contamination: float) -> pd.Series:
        # Use the dynamically set contamination factor from the config
    ...
    def _calculate_momentum(self, g:pd.DataFrame) -> pd.DataFrame:
        ...
        g['pct_change'] = g['Close'].pct_change() # V186 addition
        return g
    ...
    def _calculate_candlestick_patterns(self, g: pd.DataFrame) -> pd.DataFrame:
        ...
        g['candle_body_size_vs_atr'] = g['candle_body_size'] / g['ATR'].replace(0, 1) # V186 addition
        return g
    ...
    def create_feature_stack(self, data_by_tf: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if high_tf:
            ...
            # V186 - Add new ratio feature for anomaly detection
...
class Backtester:
    ...
    def _get_tiered_risk_params(self, equity: float) -> Tuple[float, int]:
        ...
        # Ensure tiers are sorted by equity cap to handle the lookup correctly
        ...
        # Fallback to the highest tier if equity exceeds all defined caps
    ...
    def run_backtest_chunk(self, df_chunk_in: pd.DataFrame, confidence_threshold: float, initial_equity: float, is_meta_model: bool) -> Tuple[pd.DataFrame, pd.Series, bool, Optional[Dict], Dict]:
        ...
        for candle in candles:
            ...
            # V187 - Get dynamic risk parameters
            ...
            if not circuit_breaker_tripped and symbol not in open_positions and len(open_positions) < max_concurrent_trades:
                ...
                if direction != 0:
                    ...
                    # Use the dynamic base_risk_pct from tiered config if enabled
...
def _sanitize_ai_suggestions(suggestions: Dict[str, Any]) -> Dict[str, Any]:
    ...
    bounds = {
        ...
        'anomaly_contamination_factor': (0.001, 0.1) #V186
    }
...
def initialize_playbook(base_path: str) -> Dict:
    ...
    "FilteredBreakout": {
        ...
        "features": [
            "ATR", "bollinger_bandwidth", "DAILY_ctx_Trend", "ADX", "hour", "anomaly_score"
        ],
        ...
        "RangeBound": {
            ...
            "features": [
                "ADX", "RSI", "stoch_k", "stoch_d", "bollinger_bandwidth", "hour", "is_doji"
            ],
        ...
        "TrendPullback": {
            ...
            "features": [
                "DAILY_ctx_Trend", "ADX", "RSI", "H1_ctx_Trend", "momentum_10", "hour", "close_fracdiff"
            ],
        ...
        "ConfluenceTrend": {
            ...
            "features": [
                "DAILY_ctx_Trend", "H1_ctx_Trend", "RSI", "stoch_k", "ADX", "momentum_10"
            ],
        ...
        "MeanReversionOscillator": {
            ...
            "features": [
                "RSI", "stoch_k", "ADX", "market_volatility_index", "close_fracdiff", "hour", "day_of_week"
            ],
            "lookahead_range": [20, 60],
            "dd_range": [0.15, 0.25]
        },
        ...
        "Meta_Labeling_Filter": {
            ...
            "features": [
                "ADX", "RSI", "ATR", "bollinger_bandwidth", "H1_ctx_Trend", "DAILY_ctx_Trend", "momentum_20"
            ],
        ...
        "BollingerMeanReversion": {
            ...
            "features": [
                "ADX", "RSI", "stoch_k", "bollinger_bandwidth", "hour", "day_of_week", "anomaly_score"
            ],
            "lookahead_range": [20, 60],
            "dd_range": [0.1, 0.2]
        },
        "VolatilityExpansionBreakout": {
            ...
            "features": [
                "bollinger_bandwidth", "ATR", "market_volatility_index", "momentum_20", "DAILY_ctx_Trend", "H1_ctx_Trend"
            ],
        ...
        "RegimeClarityFilter": {
            ...
            "features": [
                "ADX", "market_volatility_index", "bollinger_bandwidth", "close_fracdiff", "ATR", "DAILY_ctx_ATR"
            ],
...
def run_single_instance(fallback_config: Dict, framework_history: Dict, playbook: Dict, nickname_ledger: Dict, directives: List[Dict], api_interval_seconds: int):
    ...
    cycle_retry_count = 0 # V184 ADDITION
    ...
    while cycle_num < len(original_retraining_dates):
        ...
        # --- PRE-CYCLE REGIME ANALYSIS (V184) ---
        ...
        if not df_regime.empty:
            ...
            if regime_switch_suggestion and "strategy_name" in regime_switch_suggestion:
                ...
                # Update config with the new strategy and its defaults from the playbook
        ...
        if not train_result or best_objective_score < MODEL_QUALITY_THRESHOLD:
            ...
            if cycle_retry_count > config.MAX_TRAINING_RETRIES_PER_CYCLE:
                ...
                cycle_num += 1 # Move to next cycle
        ...
        # If training was successful, reset the retry counter
        cycle_retry_count = 0
...
def main():
    ...
    fallback_config={
        "BASE_PATH": os.getcwd(), "REPORT_LABEL": "ML_Framework_V187_Tiered_Risk",
        ...
        "RISK_PROFILE": "Medium",
        "TIERED_RISK_CONFIG": {
            # Equity Cap: {Risk Profile: {risk_pct_of_equity, num_pairs}}
            ...
            9e9:   {'Low': {'risk_pct': 0.005, 'pairs': 6}, 'Medium': {'risk_pct': 0.0075,'pairs': 6}, 'High': {'risk_pct': 0.01, 'pairs': 6}} # Represents 'Any Amount'
        }
    }
