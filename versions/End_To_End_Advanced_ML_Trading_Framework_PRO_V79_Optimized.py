# End_To_End_Advanced_ML_Trading_Framework_PRO_V79_Optimized
#
# V79 Update:
# 1. OPTIMIZED - XGBoost Engine: Set the `tree_method` to 'hist', which is significantly
#    faster for the larger datasets we are now using.
# 2. OPTIMIZED - Hyperparameter Search: Reduced default Optuna trials from 50 to 25
#    to speed up each walk-forward cycle.
# 3. OPTIMIZED - Optional SHAP: Added a `CALCULATE_SHAP_VALUES` flag to the config.
#    Set this to False for rapid testing and True for a final, detailed analysis run.
# 4. This script is complete, unabridged, and contains all prior bug fixes.
...
class ConfigModel(BaseModel):
    ...
    MIN_CLASS_RATIO: confloat(gt=0.01, lt=1) = 0.1; DYNAMIC_BARRIER_ADJUSTMENT: bool = True
    MAX_BARRIER_ADJUSTMENT_ITERATIONS: conint(gt=0) = 5; TREND_FILTER_THRESHOLD: confloat(gt=0) = 25.0
    BOLLINGER_PERIOD: conint(gt=0) = 20; STOCHASTIC_PERIOD: conint(gt=0) = 14
    CALCULATE_SHAP_VALUES: bool = True # New optimization flag
...
raw_config = {
    ...
    "REPORT_LABEL": "ML_Framework_V79_Optimized",
    ...
    "OPTUNA_TRIALS": 25, "TRAINING_WINDOW": '365D', "RETRAINING_FREQUENCY": '60D', # Reduced trials
    "LOOKAHEAD_CANDLES": 100, "MIN_CLASS_SAMPLES": 50, "MIN_CLASS_RATIO": 0.1,
    "DYNAMIC_BARRIER_ADJUSTMENT": True, "MAX_BARRIER_ADJUSTMENT_ITERATIONS": 5,
    "TREND_FILTER_THRESHOLD": 25.0, "BOLLINGER_PERIOD": 20, "STOCHASTIC_PERIOD": 14,
    "CALCULATE_SHAP_VALUES": False # Set to False for speed, True for final detailed report
}

# =============================================================================
# 3. DATA LOADER & 4. FEATURE ENGINEERING (Identical to V78, no changes needed)
# =============================================================================
class DataLoader:
...
class FeatureEngineer:
    ...
    def label_outcomes(self,df:pd.DataFrame,sl_atr:float,tp_atr:float,lookahead:int)->pd.DataFrame:
        logger.info("  - Generating trade labels...");labeled_dfs=[self._label_with_dynamic_barriers(group,sl_atr,tp_atr,lookahead) for _,group in df.groupby('Symbol')];return pd.concat(labeled_dfs)
    def _label_with_dynamic_barriers(self,df:pd.DataFrame,sl_atr:float,tp_atr:float,lookahead:int)->pd.DataFrame:
        best_df,best_balance,best_params=df.copy(),0,(sl_atr,tp_atr);logger.info(f"    - Adjusting barriers for {df['Symbol'].iloc[0]}...")
        for i in range(self.config.MAX_BARRIER_ADJUSTMENT_ITERATIONS):
            current_sl,current_tp=sl_atr*(0.8+0.1*i),tp_atr*(0.8+0.1*i);labeled_df=self._label_group(df.copy(),sl_atr=current_sl,tp_atr=current_tp,lookahead=lookahead)
            ...
            if current_ratio > best_balance:best_balance,best_df,best_params=current_ratio,labeled_df,(current_sl,current_tp)
        logger.info(f"    - Best barriers for {df['Symbol'].iloc[0]}: SL={best_params[0]:.2f}, TP={best_params[1]:.2f} -> Ratio={best_balance:.2f}");return best_df
    def _label_group(self,group:pd.DataFrame,sl_atr:float,tp_atr:float,lookahead:int)->pd.DataFrame:
        ...
        outcomes=np.zeros(len(group));prices,lows,highs,atr_vals=group['Close'].values,group['Low'].values,group['High'].values,group['ATR'].values
        for i in range(len(group)-lookahead):
            atr=atr_vals[i]
            if pd.isna(atr) or atr<=1e-9:continue
            tp_long,sl_long=prices[i]+atr*tp_atr,prices[i]-atr*sl_atr;future_highs,future_lows=highs[i+1:i+1+lookahead],lows[i+1:i+1+lookahead]
...
class Backtester:
    ...
    def run_backtest_chunk(self,df_chunk_in:pd.DataFrame,model:Pipeline,initial_equity:float)->Tuple[pd.DataFrame,pd.Series]:
        ...
        corr_features=[col for col in df_chunk_in.columns if col.startswith('corr_')];all_features=ModelTrainer.BASE_FEATURES+corr_features
        feature_cols=[f for f in all_features if f in df_chunk_in.columns];X_test=df_chunk_in[feature_cols].copy().fillna(0)
        class_probs=model.predict_proba(X_test);df_chunk_in['prob_short'],df_chunk_in['prob_hold'],df_chunk_in['prob_long']=class_probs[:,0],class_probs[:,1],class_probs[:,2]
...
class PerformanceAnalyzer:
    ...
    def _calculate_metrics(self,trades_df:pd.DataFrame,equity_curve:pd.Series)->Dict[str,Any]:
        ...
        m['gross_profit']=wins['PNL'].sum(); m['gross_loss']=abs(losses['PNL'].sum())
        ...
        m['longest_win_streak']=longest_win_streak;m['longest_loss_streak']=longest_loss_streak
        return m
...
# End_To_End_Advanced_ML_Trading_Framework_PRO_V79_Optimized.py